...
/**
 * Clips polygonal geometry to a rectangle. This implementation is faster, more
 * robust, and less sensitive to invalid input than
 * {@link Geometry#intersection(Geometry)}.
 *
 * <p>
 * It can also enforce a supplied precision model on the computed result. The
 * inputs do not have to meet the precision model. This allows clipping using
 * integer coordinates in the output, for example.
 *
 * @author mdavis
 */
public class RectangleClipPolygon {
	...
	public Geometry clipCollection(Geometry geom) {
		if (isOutsideRectangle(geom))
			return null;
		// TODO: need to precision reduce
		if (isInsideRectangle(geom))
			return geom.copy();
	...
	private Polygon clipPolygon(Polygon poly) {
		if (isOutsideRectangle(poly))
			return null;
		// TODO: need to precision reduce
		if (isInsideRectangle(poly))
			return (Polygon) poly.copy();
	...
	private LinearRing clipRing(LinearRing ring) {
		if (isOutsideRectangle(ring))
			return null;
		// TODO: need to precision reduce
		if (isInsideRectangle(ring))
	...
	private Coordinate[] clipRingToBox(Coordinate[] ring) {
		...
		for (int edgeIndex = 0; edgeIndex < 4; edgeIndex++) {
			...
			/*
			 * // this is a further optimization to clip entire line // but not clear it
			 * makes much difference if (edgeIndex >= 1 && isInsideEdge(currentCoordsEnv,
			 * edgeIndex)) // all pts inside - skip clipping against this edge continue;
			 */
	 ...
	 *
	 * @param coords
	 *            the coordinates for the ring. Must be closed.
	 * @param edgeIndex
	 * @return the clipped points, or null if all were clipped
	 */
	...
	/**
	 * The clipped geometry may be invalid (due to coincident linework at clip
	 * edges, or due to precision reduction if performed). This method fixed the
	 * geometry topology to be valid.
	 *
	 * <p>
	 * Currently uses the buffer(0) trick. This should work in most cases (but need
	 * to verify this). But it may produce unexpected results if the input polygon
	...
	private Geometry fixTopology(Geometry geom) {
		...
		// TODO: ensure this meets required precision model (see
		// GeometryPrecisionReducer)
		return geom.buffer(0);
	}

	/**
	 * Due to the nature of the S-H algorithm, it should never happen that the
	 * computation of intersection line slope is infinite (i.e. encounters
	 * division-by-zero).
	 *
	 * @param a
	 * @param b
	...
	/*
	 * private boolean isInsideEdge(Envelope env, int edgeIndex) { switch
	 * (edgeIndex) { case ENV_BOTTOM: return env.getMinY() > clipEnvMinY; case
	 * ENV_RIGHT: return env.getMaxX() < clipEnvMaxX; case ENV_TOP: return
	 * env.getMaxY() < clipEnvMaxY; case ENV_LEFT: default: return env.getMinX() >
	 * clipEnvMinX; } }
	 *
	 * private boolean isOutsideEdge(Envelope env, int edgeIndex) { switch
	 * (edgeIndex) { case ENV_BOTTOM: return env.getMaxY() < clipEnvMinY; case
	 * ENV_RIGHT: return env.getMinX() > clipEnvMaxX; case ENV_TOP: return
	 * env.getMinY() > clipEnvMaxY; case ENV_LEFT: default: return env.getMaxX() <
	 * clipEnvMinX; } }
	 *
