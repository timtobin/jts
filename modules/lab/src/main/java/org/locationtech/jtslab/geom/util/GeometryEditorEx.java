...
/**
 * A class which supports creating new {@link Geometry}s which are modifications
 * of existing ones, maintaining the same type structure. Geometry objects are
 * intended to be treated as immutable. This class "modifies" Geometry instances
 * by traversing them, applying a user-defined {@link GeometryEditorOperation},
 * {@link CoordinateSequenceOperation} or {@link CoordinateOperation} and
 * creating new Geometry instances with the same structure but (possibly)
 * modified components.
 *
 * <p>
 ...
 *
 * <ul>
 * <li>the values of the coordinates may be changed. The editor does not check
 * whether changing coordinate values makes the result Geometry invalid
 * <li>the coordinate lists may be changed (e.g. by adding, deleting or
 * modifying coordinates). The modifed coordinate lists must be consistent with
 * their original parent component (e.g. a <tt>LinearRing</tt> must always have
 * at least 4 coordinates, and the first and last coordinate must be equal)
 * <li>components of the original geometry may be deleted (e.g. holes may be
 ...
 * <tt>Polygon</tt> cannot be collapsed into a <tt>LineString</tt>). If changing
 * the structure is required, use a {@link GeometryTransformer}.
 *
 * <p>
 * This class supports creating an edited Geometry using a different
 * <code>GeometryFactory</code> via the
 * {@link org.locationtech.jts.geom.util.GeometryEditor#GeometryEditor(GeometryFactory)}
 * constructor. Examples of situations where this is required is if the geometry
 * is transformed to a new SRID and/or a new PrecisionModel.
 *
 * <p>
 * <b>Usage Notes</b>
 *
 * <ul>
 * <li>The resulting Geometry is not checked for validity. If validity needs to
 * be enforced, the new Geometry's {@link Geometry#isValid} method should be
 * called.
 * <li>By default the UserData of the input geometry is not copied to the
 * result.
 * </ul>
 *
...
public class GeometryEditorEx {
	...
	/**
	 * The factory used to create the modified Geometry. If <tt>null</tt> the
	 ...
	 * {@link Geometry}s with the same {@link GeometryFactory} as the input
	 * Geometry.
	 */
	 ...
	 * {@link GeometryOperation} and the same {@link GeometryFactory} as the input
	 * Geometry.
	 *
	 * @param operation
	...
	/**
	 * Creates a GeometryEditor which edits geometries using a given
	 * {@link GeometryOperation} and the given {@link GeometryFactory}.
	 *
	 * @param operation
	 *            the edit operation to use
	 * @param targetFactory
	...
	/**
	 * Creates a new GeometryEditor object which will create edited
	 * {@link Geometry}s with the given {@link GeometryFactory}.
	 *
	 * @param targetFactory
	 *            the GeometryFactory to create edited Geometries with
	 */
	...
	/**
	 * Edit a {@link Geometry}. Clients can create subclasses of
	 * {@link GeometryEditorOperation} or {@link CoordinateOperation} to perform
	 * required modifications.
	 *
	 * @param geometry
	 *            the Geometry to edit
	 * @return a new {@link Geometry} which is the result of the editing (which may
	 *         be empty)
	 */
	...
	private GeometryCollection editGeometryCollection(GeometryCollection collection) {
		...
		GeometryCollection collectionForType = (GeometryCollection) operation.edit(collection, targetFactory);
		...
		if (collectionForType.getClass() == MultiPoint.class) {
			return targetFactory.createMultiPoint((Point[]) geometries.toArray(new Point[]{}));
		}
		if (collectionForType.getClass() == MultiLineString.class) {
			return targetFactory.createMultiLineString(geometries.toArray(new LineString[]{}));
		}
		if (collectionForType.getClass() == MultiPolygon.class) {
			return targetFactory.createMultiPolygon(geometries.toArray(new Polygon[]{}));
		}
		return targetFactory.createGeometryCollection(geometries.toArray(new Geometry[]{}));
	}
	...
	private Polygon editPolygon(Polygon polygon) {
		...
		return targetFactory.createPolygon(shell, (LinearRing[]) holes.toArray(new LinearRing[]{}));
	}

	/**
	 * Sets whether the User Data is copied to the edit result. Only the object
	 * reference is copied.
	 *
	...
	/**
	 * A {@link GeometryEditorOperation} which edits the coordinate list of a
	 * {@link Geometry}. Operates on Geometry subclasses which contains a single
	 * coordinate list.
	 */
	public abstract static class CoordinateOperation implements GeometryEditorOperation {
		 ...
		 *
		 * <p>
		 * If it is desired to preserve the immutability of Geometry instances, if the
		 * coordinates are changed a new array should be created and returned.
		 *
		 * @param coordinates
		...
		public final Geometry edit(Geometry geometry, GeometryFactory targetFactory) {
			if (geometry instanceof LinearRing) {
				return targetFactory.createLinearRing(edit(geometry.getCoordinates(), geometry));
			}

			if (geometry instanceof LineString) {
				return targetFactory.createLineString(edit(geometry.getCoordinates(), geometry));
			}
	 ...
	 * of a {@link Geometry}. Operates on Geometry subclasses which contains a
	 * single coordinate list.
	 */
	public abstract static class CoordinateSequenceOperation implements GeometryEditorOperation {
		/**
		...
		public final Geometry edit(Geometry geometry, GeometryFactory targetFactory) {
			if (geometry instanceof LinearRing ring) {
				return targetFactory.createLinearRing(edit(ring.getCoordinateSequence(), geometry, targetFactory));
			}

			if (geometry instanceof LineString string) {
				return targetFactory.createLineString(edit(string.getCoordinateSequence(), geometry, targetFactory));
			}

			if (geometry instanceof Point point) {
				return targetFactory.createPoint(edit(point.getCoordinateSequence(), geometry, targetFactory));
			}
	...
	public interface GeometryEditorOperation {
		/**
		 * Edits a Geometry by returning a new Geometry with a modification. The
		 * returned geometry may be:
		 *
		 * <ul>
		 * <li>the input geometry itself The returned Geometry might be the same as the
		 * Geometry passed in. It may be <code>null</code> if the geometry is to be
		 * deleted.
		 *
		 * @param geometry
		 *            the Geometry to modify
		 * @param targetFactory
		 *            the factory with which to construct the modified Geometry (may be
		 *            different to the factory of the input geometry)
	...
	/**
	 * A GeometryEditorOperation which does not modify the input geometry. This can
	 * be used for simple changes of GeometryFactory (including PrecisionModel and
	 * SRID).
	 *
	 * @author mbdavis
	 */
