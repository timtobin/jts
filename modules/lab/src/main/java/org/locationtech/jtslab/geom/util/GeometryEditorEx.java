 ...
 * This class "modifies" Geometry instances by traversing them, applying a user-defined {@link
 * GeometryEditorOperation}, {@link CoordinateSequenceOperation} or {@link CoordinateOperation} and
 * creating new Geometry instances with the same structure but (possibly) modified components.
 *
 * <p>Examples of the kinds of modifications which can be made are:
 *
 * <ul>
 ...
 *   <li>the coordinate lists may be changed (e.g. by adding, deleting or modifying coordinates).
 *       The modifed coordinate lists must be consistent with their original parent component (e.g.
 *       a <tt>LinearRing</tt> must always have at least 4 coordinates, and the first and last
 *       coordinate must be equal)
 *   <li>components of the original geometry may be deleted (e.g. holes may be removed from a
 *       Polygon, or LineStrings removed from a MultiLineString). Deletions will be propagated up
 *       the component tree appropriately.
 * </ul>
 *
 ...
 *
 * <p>This class supports creating an edited Geometry using a different <code>GeometryFactory</code>
 * via the {@link org.locationtech.jts.geom.util.GeometryEditor#GeometryEditor(GeometryFactory)}
 * constructor. Examples of situations where this is required is if the geometry is transformed to a
 * new SRID and/or a new PrecisionModel.
 *
 * <p><b>Usage Notes</b>
 *
 * <ul>
 *   <li>The resulting Geometry is not checked for validity. If validity needs to be enforced, the
 *       new Geometry's {@link Geometry#isValid} method should be called.
...
public class GeometryEditorEx {
  ...
  /**
   * Creates a GeometryEditor which edits geometries using a given {@link GeometryOperation} and the
   * same {@link GeometryFactory} as the input Geometry.
   *
  ...
  /**
   * Edit a {@link Geometry}. Clients can create subclasses of {@link GeometryEditorOperation} or
  ...
  private Geometry editInternal(Geometry geometry) {
    ...
    if (targetFactory == null) targetFactory = geometry.getFactory();
  ...
  private GeometryCollection editGeometryCollection(GeometryCollection collection) {
    ...
    GeometryCollection collectionForType =
        (GeometryCollection) operation.edit(collection, targetFactory);
  ...
  public interface GeometryEditorOperation {
     ...
     *
     * <ul>
     *   <li>the input geometry itself The returned Geometry might be the same as the Geometry
     *       passed in. It may be <code>null</code> if the geometry is to be deleted.
     *
     * @param geometry the Geometry to modify
     * @param targetFactory the factory with which to construct the modified Geometry (may be
  ...
  public abstract static class CoordinateOperation implements GeometryEditorOperation {
     ...
     *
     * <p>If it is desired to preserve the immutability of Geometry instances, if the coordinates
  ...
  public abstract static class CoordinateSequenceOperation implements GeometryEditorOperation {
    public final Geometry edit(Geometry geometry, GeometryFactory targetFactory) {
      if (geometry instanceof LinearRing ring) {
        return targetFactory.createLinearRing(
            edit(ring.getCoordinateSequence(), geometry, targetFactory));
      }

      if (geometry instanceof LineString string) {
        return targetFactory.createLineString(
            edit(string.getCoordinateSequence(), geometry, targetFactory));
      }

      if (geometry instanceof Point point) {
        return targetFactory.createPoint(
            edit(point.getCoordinateSequence(), geometry, targetFactory));
      }
     ...
     */
    public abstract CoordinateSequence edit(
        CoordinateSequence coordSeq, Geometry geometry, GeometryFactory targetFactory);
  }
}
