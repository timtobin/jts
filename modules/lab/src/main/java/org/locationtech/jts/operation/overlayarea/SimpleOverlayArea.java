...
/**
 * Computes the result area of an overlay usng the Overlay-Area, for simple
 * polygons. Simple polygons have no holes. No indexing is used. This is faster
 * than {@link OverlayArea} for polygons with low vertex count.
 *
 * @author mdavis
 */
public class SimpleOverlayArea {
	...
	private double areaForInteriorVertices(CoordinateSequence ring, boolean isCCW, CoordinateSequence ring2) {
		double area = 0;
		/**
		 * Compute rays originating at vertices inside the resultant (i.e. A vertices
		 * inside B, and B vertices inside A)
		 */
	...
	private double areaForIntersections(CoordinateSequence ringA, boolean isCCWA, CoordinateSequence ringB,
			boolean isCCWB) {
		...
		for (int i = 0; i < ringA.size() - 1; i++) {
			...
			for (int j = 0; j < ringB.size() - 1; j++) {
				...
				if (li.hasIntersection()) {
					 ...
					 * entering A => rays are IP-A0:L, IP-B1:R (where :L/R indicates result is to
					 * the Left or Right).
					 *
					 * <p>
