...
class AxisNode {
	...
	private static void addLine(Coordinate p0, Coordinate p1, GeometryFactory geomFact, List<LineString> lines) {
	...
	private void addInternalLines(Coordinate p, int skipEdge, List<LineString> lines, GeometryFactory geomFact) {
		if (skipEdge != 0)
			addLine(p0, p, geomFact, lines);
		if (skipEdge != 1)
			addLine(p1, p, geomFact, lines);
		if (skipEdge != 2)
			addLine(p2, p, geomFact, lines);
	}
	...
	/*
	 * //--- Using cc int point isn't as good as midpoint private void
	 * fillEdgePoints(int longEdge, Coordinate cc) { if (p0 == null) p0 =
	 * medialPoint(0, longEdge, cc); if (p1 == null) p1 = medialPoint(1, longEdge,
	 * cc); if (p2 == null) p2 = medialPoint(2, longEdge, cc); }
	 *
	 * private Coordinate medialPoint(int edge, int longEdge, Coordinate cc) { if
	 * (edge != longEdge) { return tri.midpoint(edge); } return intersection(
	 * tri.getEdgeStart(edge), tri.getEdgeEnd(edge), tri.getCoordinate(
	 * Tri.oppVertex(edge) ), cc); }
	 *
	 * private Coordinate intersection(Coordinate p00, Coordinate p01, Coordinate
	 * p10, Coordinate p11) { LineIntersector li = new RobustLineIntersector();
	 * li.computeIntersection(p00, p01, p10, p11); return li.getIntersection(0); }
	 */
...
/**
 * Constructs an approximation to the medial axis of a {@link Polygon} based on
...
public class DelaunayMedialAxis {
	...
	/*
	 * //-- Testing only public static Geometry axisPointSegment(Geometry pt,
	 * Geometry seg) { Coordinate p = pt.getCoordinate(); Coordinate[] pts =
	 * seg.getCoordinates(); Coordinate axisPt = medialAxisPoint(p, pts[0], pts[1]);
	 * return pt.getFactory().createPoint(axisPt); }
	 */

	private static Coordinate angleBisector(Tri tri, int v) {
		return Triangle.angleBisector(tri.getCoordinate(Tri.prev(v)), tri.getCoordinate(v),
	...
	/**
	 * Tests if a triangle and its adjacent tri form a "tube", where the opposite
	 * edges of the triangles are on the boundary.
	 *
	 * @param tri
	 *            the triangle to test
	 * @param eAdj
	 *            the edge adjacent to the next triangle
	...
	/**
	 * Computes the approximate point where the medial axis between a point and a
	 * line segment intersects the line between the point and the segment endpoint
	 *
	 * @param p
	 *            the point
	 * @param p0
	...
	/**
	 * Computes the approximate point where the medial axis between two line
	 * segments intersects the line between the ends of the segments.
	 *
	 * @param p00
	 *            the start vertex of segment 0
	 * @param p01
	 *            the end vertex of segment 0
	 * @param p10
	 *            the start vertex of segment 1
	 * @param p11
	 *            the end vertex of segment 1
	 * @return the approximate medial axis point
	 */
	private static Coordinate medialAxisPoint(Coordinate p00, Coordinate p01, Coordinate p10, Coordinate p11) {
		double endFrac0 = 0;
		double endFrac1 = 1;
		double eps = 0.0;
	...
	private List<LineString> constructLines(List<Tri> tris) {
		...
		/**
		 * Construct paths from cap tris (tris with two outer sides). These will end at
		 * either a node tri or a previously processed tri (if the triangulation has no
		 * nodes).
		 */
		int numCaps = 0;
		for (Tri tri : tris) {
		...
		// TODO: handle case with two caps and no nodes (by rebuilding line connecting
		// midpoints from
		// cap to cap)
		...
		/**
		 * If no caps and no nodes exist, triangulation is an O-shape. Pick any tri to
		 * start the path from. Path building terminates when start tri is encountered
		 * again
		 */
	...
	private LineString constructPath(AxisNode node) {
		...
		/**
		 * If next tri is a node as well, queue it. No path is constructed, since node
		 * internal lines connect
		 */
	 ...
	 *
	 * @param tri1
	 *            the first triangle in the tube
	 * @param tri2
	 *            the second triangle in the tube
	...
	private Coordinate exitPointWedge(Tri tri, int eExit) {
		/**
		 * Midpoint produces a straighter line in nearly-parallel corridors, but is more
		 * see-sawed elsewhere.
		 */
		return tri.midpoint(eExit);
		/*
		 * int eBdy = indexOfNonAdjacent(tri); Coordinate pt =
		 * tri.getCoordinate(Tri.oppVertex(eBdy)); Coordinate p0 =
		 * tri.getCoordinate(eBdy); Coordinate p1 = tri.getCoordinate(Tri.next(eBdy));
		 * if (Tri.next(eBdy) != eExit) { p0 = tri.getCoordinate(Tri.next(eBdy)); p1 =
		 * tri.getCoordinate(eBdy); } //return medialAxisPoint(pt, p0, p1);*
		 */
	}
	...
	private void extendPath(Tri tri, int edgeEntry, List<Coordinate> pts) {
		...
		isTube(tri, eAdj)) {
			/**
			 * This triangle and the next one form a "tube" so use both to construct the
			 * medial line.
			 */
