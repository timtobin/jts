 ...
 * approach. This provides a validation of the correctness of the
 * {@link OverlayArea} approach. It is not intended to replace the standard
...
public class GeometryArea {
	...
	public double areaRing(LinearRing ring) {
		...
		for (int i = 1; i < seq.size(); i++) {
			...
			int i1 = i;
			/**
			 * Sum the partial areas for the two opposing SegmentVectors representing the
			 * edge. If the ring is oriented CW then the interior is to the right of the
			 * vector, and the opposing vector is opposite.
			 */
			area += EdgeVector.area2Term(seq.getX(i0), seq.getY(i0), seq.getX(i1), seq.getY(i1), isCW)
