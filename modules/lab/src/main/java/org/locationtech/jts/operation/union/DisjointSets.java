...
/**
 * A data structure that represents a partition of a set into disjoint subsets,
 * and allows merging subsets. Set items are represented by integer indices
 * (which will typically be an index into an array of the objects actually being
 * partitioned). Initially each item is in its own subset. Client code can merge
 * subsets of items as required for the algorithm being performed (e.g. set
 * partitioning or clustering). The current partitioning can be computed at any
 * time, and subset items accessed using the {@link Subsets} accessor.
 *
 * <p>
 * See the Wikipedia article on
 * <a href='https://en.wikipedia.org/wiki/Disjoint-set_data_structure'>disjoint
 * set data structures</a>.
 *
 * @author Martin Davis
 */
public class DisjointSets {
	...
	/**
	 * Merges two subsets containing the given items. Note that the items do not
	 * have to be the roots of their respective subsets. If the items are in the
	 * same subset the partitioning does not change.
	 *
	 * @param i
	 *            an item index
	 * @param j
	...
	/**
	 * Gets a representation of the current partitioning. This creates a snapshot of
	 * the partitioning; the set can be merged further after this call.
	 *
	...
	/**
	 * A representation of a partition of a set of items into disjoint subsets. It
	 * provides accessors for the number of subsets, the size of each subset, and
	 * the items of each subset.
	 *
	 * <p>
	 * The item indices in each subset are sorted. This means that the item ordering
	 * is stable; that is, the items have the same order they did in the original
	 * set.
	 */
	public class Subsets {
		 ...
		 *            the subset index
		 * @param i
		 *            the index of the item in the subset
		 * @return the item
		 */
