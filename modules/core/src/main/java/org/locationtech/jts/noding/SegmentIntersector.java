 ...
 * {@link #processIntersections(SegmentString, int, SegmentString, int)} method
 * is called whenever the {@code Noder} detects that two {@code SegmentString}s
 * <i>might</i> intersect.
 *
 * <p>
 * This interface can be used either to find all intersections, or to simply
 * detect the presence of an intersection. If only detection is needed,
 * implementations may short-circuit further computation by returning
 * {@code true} from the {@link #isDone()} method.
 *
 * <p>
...
public interface SegmentIntersector {
	 ...
	 *
	 * <p>
	 * By default, this method returns {@code false}, indicating that all possible
	 * intersections will be processed. Override this method to return {@code true}
	 * if you want to short-circuit further processing (for example, once an
