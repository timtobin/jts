...
/**
 * Computes various kinds of common geometric shapes. Provides various ways of
 * specifying the location and extent and rotations of the generated shapes, as
 * well as number of line segments used to form them.
 *
 * <p>
...
public class GeometricShapeFactory {
	...
	/**
	 * Creates an elliptical arc, as a {@link LineString}. The arc is always created
	 * in a counter-clockwise direction. This can easily be reversed if required by
	 * using {#link LineString.reverse()}
	 *
	 * @param startAng
	...
	/**
	 * Creates an elliptical {@link Polygon}. If the supplied envelope is square the
	...
	/**
	 * Creates a supercircular {@link Polygon} of a given positive power.
	 *
	...
	protected Geometry rotate(Geometry geom) {
		if (rotationAngle != 0.0) {
			AffineTransformation trans = AffineTransformation.rotationInstance(rotationAngle, dim.getCentre().x,
					dim.getCentre().y);
			geom.apply(trans);
		}
		return geom;
	...
	/**
	 * Sets the location of the shape by specifying the base coordinate (which in
	 * most cases is the lower left point of the envelope containing the shape).
	 *
	 * @param base
	...
	/**
	 * Sets the location of the shape by specifying the centre of the shape's
	 * bounding box
	 *
	 * @param centre
	...
	/**
	 * Sets the total number of points in the created {@link Geometry}. The created
	 * geometry will have no more than this number of points, unless more are needed
	 * to create a valid geometry.
	 */
	...
	/**
	 * Sets the rotation angle to use for the shape. The rotation is applied
	 * relative to the centre of the shape.
	 *
	 * @param radians
	...
	protected static class Dimensions {
		...
		public Envelope getEnvelope() {
			...
			if (centre != null) {
				return new Envelope(centre.x - width / 2, centre.x + width / 2, centre.y - height / 2,
						centre.y + height / 2);
			}
