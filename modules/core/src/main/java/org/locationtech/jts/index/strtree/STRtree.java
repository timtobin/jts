 ...
 * spatial data.
 *
 * <p>The STR packed R-tree is simple to implement and maximizes space utilization; that is, as many
 * leaves as possible are filled to capacity. Overlap between nodes is far less than in a basic
 * R-tree. However, the index is semi-static; once the tree has been built (which happens
 * automatically upon the first query), items may not be added. Items may be removed from the tree
 * using {@link #remove(Envelope, Object)}.
 *
 * <p>Described in: P. Rigaux, Michel Scholl and Agnes Voisard. <i>Spatial Databases With
 ...
 * than one thread must be synchronized externally.
 *
 * <p>Querying a tree is thread-safe. The building phase is done synchronously, and querying is
 * stateless.
 *
 * @version 1.7
 */
public class STRtree extends AbstractSTRtree implements SpatialIndex, Serializable {
  ...
  /** */
  @Serial private static final long serialVersionUID = 259274702368956900L;
  ...
  private static final IntersectsOp intersectsOp =
      (aBounds, bBounds) -> ((Envelope) aBounds).intersects((Envelope) bBounds);
  ...
  protected List createParentBoundables(List childBoundables, int newLevel) {
    ...
    List[] verticalSlices =
        verticalSlices(sortedChildBoundables, (int) Math.ceil(Math.sqrt(minLeafCount)));
   ...
   *
   * <p>The minimum recommended capacity setting is 4.
   */
  ...
  /**
   * Constructs an STRtree with the given maximum number of child nodes that a node may have, and
   * the root that links to all other nodes
   *
   * <p>The minimum recommended capacity setting is 4.
   */
   ...
   * Branch-and-Bound tree traversal algorithm is used to provide an efficient search.
   *
   * <p>If the tree is empty, the return value is <code>null
   * </code.
   * If the tree contains only one item,
   * the return value is a pair containing that item.
   * <b>
   * If it is required to find only pairs of distinct items, the {@link ItemDistance} function must
   * be <b>anti-reflexive</b>.
   *
   * @param itemDist a distance metric applicable to the items in this tree
   * @return the pair of the nearest items or <code>null</code> if the tree is empty
   */
   ...
   *
   * <p>The query <tt>object</tt> does <b>not</b> have to be contained in the tree, but it does have
   * to be compatible with the <tt>itemDist</tt> distance metric.
   *
   ...
   * @return the nearest item in this tree or <code>null</code> if the tree is empty
   */
   ...
   * @return the pair of the nearest items, one from each tree or <code>null</code> if no pair of
   *     distinct items can be found
   */
  ...
  private Object[] nearestNeighbour(BoundablePair initBndPair) {
    ...
    while (!priQ.isEmpty() && distanceLowerBound > 0.0) {
      ...
      if (bndPair.isLeaves()) {
        ...
        /**
         * Otherwise, expand one side of the pair, and insert the expanded pairs into the queue. The
         * choice of which side to expand is determined heuristically.
         */
   ...
   * {@link ItemDistance} is used as the distance metric. A Branch-and-Bound tree traversal
   * algorithm is used to provide an efficient search.
   *
   * @param tree another tree
   * @param itemDist a distance metric applicable to the items in the trees
  ...
  private boolean isWithinDistance(BoundablePair initBndPair, double maxDistance) {
    ...
    while (!priQ.isEmpty()) {
       ...
       *
       * <p>NOTE: using Envelope MinMaxDistance would provide a tighter bound, but not much
   ...
   * following paper:
   *
   * <p>Roussopoulos, Nick, Stephen Kelley, and Frédéric Vincent. "Nearest neighbor queries." ACM
   * sigmod record. Vol. 24. No. 2. ACM, 1995.
   *
   * <p>The query {@code item} does <b>not</b> have to be contained in the tree, but it does have to
   * be compatible with the {@code itemDist} distance metric.
   *
   * <p>If the tree size is smaller than k fewer items will be returned. If the tree is empty an
