...
public class RelateComputer {
  ...
  public IntersectionMatrix computeIM() {
     ...
     * Isolated components are components that do not touch any other components in the graph. They
     * can be identified by the fact that they will contain labels containing ONLY a single element,
     * the one for their parent geometry. We only need to check components contained in the input
     * graphs, since isolated components will not have been replaced by new components formed by
     * intersections.
     */
  ...
  private void computeProperIntersectionIM(SegmentIntersector intersector, IntersectionMatrix im) {
     ...
     * <i>interior</i> intersection, then there is an Interior-Interior intersection too. Note that
     * it does not follow that the Interior of the Line intersects the Exterior of the Area, since
     * there may be another Area component which contains the rest of the Line.
     */
   ...
   * is the nominal dimension of the boundary unless the boundary is empty, in which case it is
   * {@link Dimension#FALSE}. For linear geometries the Boundary Node Rule determines whether the
   * boundary is empty.
   *
  ...
  private static int getBoundaryDim(Geometry geom, BoundaryNodeRule boundaryNodeRule) {
    /** If the geometry has a non-empty boundary the intersection is the nominal dimension. */
    if (BoundaryOp.hasBoundary(geom, boundaryNodeRule)) {
      /**
       * special case for lines, since Geometry.getBoundaryDimension is not aware of Boundary Node
       * Rule.
       */
