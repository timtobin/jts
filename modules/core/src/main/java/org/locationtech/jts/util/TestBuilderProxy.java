...
/**
 * A proxy to call TestBuilder functions. If the code is not being run in the
 * context of the TestBuilder, functions act as a no-op.
 *
 * <p>
 * It is recommended that functions only be inserted into code temporarily (i.e.
...
public class TestBuilderProxy {
	...
	/**
	 * Tests whether the proxy is active (i.e. the TestBuilder is available). This
	 * allows avoiding expensive geometry creation if not needed.
	 *
	...
	/**
	 * Shows a geometry as an indicator in the TestBuilder Edit panel. The geometry
	 * is only displayed until the next screen refresh. The TestBuilder also
	 * provides a menu option to capture indicators on a layer.
	 *
	 * @param geom
	...
	public static void showIndicator(Geometry geom) {
		...
		try {
			...
			// Fail silently to avoid unexpected output in production
			// Or perhaps should fail noisy, since at this point the function should be
			// working?
		}
	}
