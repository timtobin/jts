...
public class CoordinateSequences {
   ...
   */
  public static void copyCoord(
      CoordinateSequence src, int srcPos, CoordinateSequence dest, int destPos) {
   ...
   */
  public static CoordinateSequence ensureValidRing(
  ...
  private static CoordinateSequence createClosedRing(
  ...
  public static CoordinateSequence extend(
      CoordinateSequenceFactory fact, CoordinateSequence seq, int size) {
    CoordinateSequence newseq = fact.create(size, seq.getDimension());
    int n = seq.size();
    copy(seq, 0, newseq, 0, n);
    ...
    if (n > 0) {
      for (int i = n; i < size; i++) copy(seq, n - 1, newseq, i, 1);
    }
    return newseq;
  }
  ...
  /**
   * Shifts the positions of the coordinates until the coordinate at <code>firstCoordinateIndex
   * </code> is first.
   *
   * @param seq the coordinate sequence to rearrange
   ...
   * @param seq the coordinate sequence to rearrange
   * @param indexOfFirstCoordinate the index of the coordinate to make first
   * @param ensureRing makes sure that {@code} will be a closed ring upon exit
   */
  public static void scroll(
      CoordinateSequence seq, int indexOfFirstCoordinate, boolean ensureRing) {
