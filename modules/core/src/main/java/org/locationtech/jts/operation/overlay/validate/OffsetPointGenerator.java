 ...
 * {@link Geometry}. Can be used to generate probe points for determining whether a polygonal
 * overlay result is incorrect. The input geometry may have any orientation for its rings, but
 * {@link #setSidesToGenerate(boolean, boolean)} is only meaningful if the orientation is known.
 *
 * @author Martin Davis
 * @version 1.7
 */
public class OffsetPointGenerator {
  ...
  /**
   * Generates the two points which are offset from the midpoint of the segment <tt>(p0, p1)</tt> by
   * the <tt>offsetDistance</tt>.
   *
   * @param p0 the first point of the segment to offset from
   * @param p1 the second point of the segment to offset from
   */
  private void computeOffsetPoints(
