...
class OverlayEdgeRing {
	...
	private final List<OverlayEdgeRing> holes = new ArrayList<>(); // a list of EdgeRings which are holes in this
	// EdgeRing
	...
	private Coordinate[] computeRingPts(OverlayEdge start) {
		...
		do {
			if (edge.getEdgeRing() == this)
				throw new TopologyException("Edge visited twice during ring-building at " + edge.getCoordinate(),
						edge.getCoordinate());
	...
	/**
	 * Finds the innermost enclosing shell OverlayEdgeRing containing this
	 * OverlayEdgeRing, if any. The innermost enclosing ring is the <i>smallest</i>
