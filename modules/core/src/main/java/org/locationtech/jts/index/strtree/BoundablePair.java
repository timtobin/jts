...
/**
 * A pair of {@link Boundable}s, whose leaf items support a distance metric
 * between them. Used to compute the distance between the members, and to expand
 * a member relative to the other in order to produce new branches of the
 * Branch-and-Bound evaluation tree. Provides an ordering based on the distance
 * between the members, which allows building a priority queue by minimum
 * distance.
 *
 * @author Martin Davis
 */
class BoundablePair implements Comparable {
	 ...
	 *
	 * @param i
	 *            the index of the member to return (0 or 1)
