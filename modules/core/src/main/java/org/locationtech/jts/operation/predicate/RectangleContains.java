...
/**
 * Optimized implementation of the <tt>contains</tt> spatial predicate for cases
 * where the first {@link Geometry} is a rectangle. This class works for all
...
public class RectangleContains {
	...
	private boolean isContainedInBoundary(Geometry geom) {
		...
		if (geom instanceof LineString string)
			return isLineStringContainedInBoundary(string);
	...
	private boolean isLineSegmentContainedInBoundary(Coordinate p0, Coordinate p1) {
		 ...
		 * the other ordinate is not the same as a boundary ordinate
		 *
		 * <p>
	...
	/**
	 * Tests if a linestring is completely contained in the boundary of the target
	 * rectangle.
	 *
	 * @param line
