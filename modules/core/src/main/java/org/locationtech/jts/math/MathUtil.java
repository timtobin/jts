...
public class MathUtil {
  ...
  /**
   * Computes the length of the vector (x,y). This is the length of the hypotenuse of a right
   * triangle with sides of length x and y.
   *
   * <p>This function is faster than the standard {@link Math.hypot} function.
   *
  ...
  /**
   * Computes an index which wraps around a given maximum value. For values &gt;= 0, this is equals
   * to <tt>val % max</tt>. For values &lt; 0, this is equal to <tt>max - (-val) % max</tt>
   *
  ...
  /**
   * Generates a quasi-random sequence of numbers in the range [0,1]. They are produced by an
   * additive recurrence with 1/&phi; as the constant. This produces a low-discrepancy sequence
   * which is more evenly distribute than random numbers.
   *
   * <p>See <a
   * href='https://en.wikipedia.org/wiki/Low-discrepancy_sequence#Additive_recurrence'>Wikipedia:
   * Low-discrepancy Sequences - Additive Recurrence</a>.
   *
   * <p>The sequence is initialized by calling it with any positive fractional number; 0 works well
   * for most uses.
   *
  ...
  /**
   * Generates a quasi-random sequence of numbers in the range [0,1]. They are produced by an
   * additive recurrence with constant &alpha;.
   *
   * <pre>
   *     R(&alpha;) :  t<sub>n</sub> = { t<sub>0</sub> + n&alpha; },  n = 1,2,3,...
   * </pre>
   *
   * When &alpha; is irrational this produces a <a
   * href='https://en.wikipedia.org/wiki/Low-discrepancy_sequence#Additive_recurrence'>Low
   * discrepancy sequence</a> which is more evenly distributed than random numbers.
   *
   * <p>The sequence is initialized by calling it with any positive fractional number. 0 works well
   * for most uses.
   *
   ...
   *
   * <p>One use is to randomize points inserted into a {@link KDtree}.
   *
