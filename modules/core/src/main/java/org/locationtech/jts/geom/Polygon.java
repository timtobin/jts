 ...
 * by {@link LinearRing}s. The boundary rings of the polygon may have any
 ...
 * <li>the linear rings for the shell and holes are valid (i.e. are closed and
 * do not self-intersect)
 * <li>holes touch the shell or another hole at at most one point (which implies
 * that the rings of the shell and holes must not cross)
 * <li>the interior of the polygon is connected, or equivalently no sequence of
...
public class Polygon extends Geometry implements Polygonal {
	...
	/**
	 * The exterior boundary, or <code>null</code> if this <code>Polygon</code> is
	 * empty.
	 */
