...
/**
 * A prepared version for {@link Polygonal} geometries. This class supports both {@link Polygon}s
 * and {@link MultiPolygon}s.
 *
 * <p>This class does <b>not</b> support MultiPolygons which are non-valid (e.g. with overlapping
 * elements).
 *
...
public class PreparedPolygon extends BasicPreparedGeometry {
  ...
  public synchronized FastSegmentSetIntersectionFinder getIntersectionFinder() {
    /**
     * MD - Another option would be to use a simple scan for segment testing for small geometries.
     * However, testing indicates that there is no particular advantage to this approach.
     */
    if (segIntFinder == null)
      segIntFinder =
          new FastSegmentSetIntersectionFinder(
              SegmentStringUtil.extractSegmentStrings(getGeometry()));
    return segIntFinder;
  }

  public synchronized PointOnGeometryLocator getPointLocator() {
    if (pia == null) pia = new IndexedPointInAreaLocator(getGeometry());
