 ...
 * processors (notably the Intel Pentium architecture) perform floating point
 ...
 *
 * <p>
 * For example, the following code example constructs three DD instances: two to
...
public final class DD implements Serializable, Comparable, Cloneable {
	...
	/**
	 * A value representing the result of an operation which does not return a valid
	 * number.
	 */
	...
	/**
	 * Determines the decimal magnitude of a number. The magnitude is the exponent
	 * of the greatest power of 10 which is less than or equal to the number.
	 *
	 * @param x
	...
	private static int magnitude(double x) {
		...
		/**
		 * Since log computation is inexact, there may be an off-by-one error in the
		 * computed magnitude. Following tests that magnitude is correct, and adjusts it
		 * if not
		 */
	 ...
	 * The format accepted is similar to the standard Java real number syntax. It is
	...
	public static DD parse(String str) throws NumberFormatException {
		...
		// correct number of digits before decimal sign if we don't have a decimal sign
		// in the string
		if (!hasDecimalChar)
			numBeforeDec = numDigits;
	 ...
	 *
	 * @return -1,0 or 1 depending on whether this value is less than, equal to or
	 *         greater than the value of <tt>o</tt>
	 */
	 ...
	 * argument. A decimal point may be optionally inserted in the string of digits
	 * (as long as its position lies within the extracted digits - if not, the
	 * caller must prepend or append the appropriate zeroes and decimal point).
	 *
	 * @param y
	 *            the number to extract ( >= 0)
	 * @param decimalPointPos
	 *            the position in which to insert a decimal point
	 * @return the string containing the significant digits and possibly a decimal
	 *         point
	 */
	private String extractSignificantDigits(boolean insertDecimalPoint, int[] magnitude) {
		...
		for (int i = 0; i <= numDigits; i++) {
			...
			/**
			 * If a negative remainder is encountered, simply terminate the extraction. This
			 * is robust, but maybe slightly inaccurate. My current hypothesis is that
			 * negative remainders only occur for very small lo components, so the
			...
			/**
			 * Heuristic check: if the remaining portion of y is non-positive, assume that
			 * output is complete
			 */
			// if (y.hi <= 0.0)
			// if (y.hi < 0.0)
			...
			/**
			 * Check if remaining digits will be 0, and if so don't output them. Do this by
			 * comparing the magnitude of the remainder with the expected precision.
			 */
	...
	/**
	 * Returns the largest (closest to positive infinity) value that is not greater
	 * than the argument and is equal to a mathematical integer. Special cases:
	 *
	 * <ul>
	 * <li>If this value is NaN, returns NaN.
	 * </ul>
	 *
	 * @return the largest (closest to positive infinity) value that is not greater
	 *         than the argument and is equal to a mathematical integer.
	 */
	public DD floor() {
		if (isNaN())
	...
	/**
	 * Returns the string for this value if it has a known representation. (E.g. NaN
	 * or 0.0)
	 *
	 * @return the string for this special number or null if the number is not a
	 *         special number
	 */
	...
	/**
	 * Rounds this value to the nearest integer. The value is rounded to an integer
	 * by adding 1/2 and taking the floor of the result. Special cases:
	 *
	 * <ul>
	 * <li>If this value is NaN, returns NaN.
	 * </ul>
	...
	/**
	 * Multiplies this object by the argument, returning <tt>this</tt>. To prevent
	 * altering constants, this method <b>must only</b> be used on values known to
	 * be newly created.
	 *
	 * @param y
	...
	/**
	 * Squares this object. To prevent altering constants, this method <b>must
	 * only</b> be used on values known to be newly created.
	 *
	 * @return the square of this value.
	 */
	public DD selfSqr() {
	...
	/**
	 * Subtracts the argument from the value of <tt>this</tt>. To prevent altering
	 * constants, this method <b>must only</b> be used on values known to be newly
	 * created.
	 *
	 * @param y
	...
	/**
	 * Set the value for the DD object. This method supports the mutating operations
	 * concept described in the class documentation (see above).
	 *
	 * @param value
	 *            a DD instance supplying an extended-precision value.
	 ...
	 *
	 * @param value
	 *            a floating point value to be stored in the instance.
	 * @return a self-reference to the DD instance.
	 */
	 ...
	 *
	 * @return the positive square root of this number. If the argument is NaN or
	 ...
	 * 10<sup>-3</sup>, 10<sup>8</sup> ] standard notation will be used. Otherwise,
	...
	/**
	 * Returns the integer which is largest in absolute value and not further from
	 * zero than this value. Special cases:
	 *
	 * <ul>
	 * <li>If this value is NaN, returns NaN.
	 * </ul>
	 *
	 * @return the integer which is largest in absolute value and not further from
	 *         zero than this value
	 */
	public DD trunc() {
		if (isNaN())
			return NaN;
