 ...
 * eliminate artifacts such as narrow slivers, spikes and gores.
 *
 * <p>Too much snapping can result in invalid topology being created, so the number and location of
 * snapped vertices is decided using heuristics to determine when it is safe to snap. This can
 * result in some potential snaps being omitted, however.
 *
 * @author Martin Davis
 * @version 1.7
 */
public class GeometrySnapper {
   ...
   * valid (which fixes issues such as topology collapses in polygonal inputs).
   *
   * <p>Snapping a geometry to itself can remove artifacts such as very narrow slivers, gores and
   * spikes.
   *
  ...
  /**
   * Snaps the vertices in the component {@link LineString}s of the source geometry to the vertices
   * of the given snap geometry.
   *
  ...
  /**
   * Snaps the vertices in the component {@link LineString}s of the source geometry to the vertices
   * of the same geometry. Allows optionally cleaning the result to ensure it is topologically valid
  ...
  private Coordinate[] extractTargetCoordinates(Geometry g) {
    // TODO: should do this more efficiently.  Use CoordSeq filter to get points, KDTree for
    // uniqueness & queries
