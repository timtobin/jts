 ...
 * intended to be treated as immutable. This class "modifies" Geometrys by
 * traversing them, applying a user-defined {@link GeometryEditorOperation},
 * {@link CoordinateSequenceOperation} or {@link CoordinateOperation} and
 * creating new Geometrys with the same structure but (possibly) modified
 * components.
 *
 * <p>
 ...
 * modifying coordinates). The modified coordinate lists must be consistent with
 ...
 * <code>GeometryFactory</code> via the {@link #GeometryEditor(GeometryFactory)}
...
public class GeometryEditor {
	...
	/**
	 * Edit the input {@link Geometry} with the given edit operation. Clients can
	 * create subclasses of {@link GeometryEditorOperation} or
	...
	private Geometry editInternal(Geometry geometry, GeometryEditorOperation operation) {
		...
		if (geometry instanceof GeometryCollection collection) {
			return editGeometryCollection(collection, operation);
		}
	...
	public abstract static class CoordinateOperation implements GeometryEditorOperation {
		 ...
		 *
		 * <p>
		 * If it is desired to preserve the immutability of Geometrys, if the
	...
	public abstract static class CoordinateSequenceOperation implements GeometryEditorOperation {
		...
		public final Geometry edit(Geometry geometry, GeometryFactory factory) {
			if (geometry instanceof LinearRing ring) {
				return factory.createLinearRing(edit(ring.getCoordinateSequence(), geometry));
			}

			if (geometry instanceof LineString string) {
				return factory.createLineString(edit(string.getCoordinateSequence(), geometry));
			}

			if (geometry instanceof Point point) {
				return factory.createPoint(edit(point.getCoordinateSequence(), geometry));
			}
