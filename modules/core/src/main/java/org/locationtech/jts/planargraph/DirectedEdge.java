 ...
 * planar graphs may not require explicit Edge objects to be created). Usually a
 * client using a <code>PlanarGraph</code> will subclass
...
public class DirectedEdge extends GraphComponent implements Comparable {
	...
	public DirectedEdge(Node from, Node to, Coordinate directionPt, boolean edgeDirection) {
		...
		// Assert.isTrue(! (dx == 0 && dy == 0), "EdgeEnd with identical endpoints
		// found");
	}
	...
	/**
	 * Returns the angle that the start of this DirectedEdge makes with the positive
	 * x-axis, in radians.
	 */
	...
	/**
	 * Prints a detailed string representation of this DirectedEdge to the given
	 * PrintStream.
	 */
