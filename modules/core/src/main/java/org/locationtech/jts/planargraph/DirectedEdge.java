...
/**
 * Represents a directed edge in a {@link PlanarGraph}. A DirectedEdge may or may not have a
...
public class DirectedEdge extends GraphComponent implements Comparable {
  ...
  /**
   * Constructs a DirectedEdge connecting the <code>from</code> node to the <code>to</code> node.
   *
   * @param directionPt specifies this DirectedEdge's direction vector (determined by the vector
   *     from the <code>from</code> node to <code>directionPt</code>)
  ...
  /**
   * Returns a point to which an imaginary line is drawn from the from-node to specify this
   * DirectedEdge's orientation.
   */
  ...
  /**
   * Returns whether the direction of the parent Edge (if any) is the same as that of this Directed
   * Edge.
   */
  ...
  /**
   * Returns the angle that the start of this DirectedEdge makes with the positive x-axis, in
   * radians.
   */
  ...
  /**
   * Returns the symmetric DirectedEdge -- the other DirectedEdge associated with this
   * DirectedEdge's parent Edge.
   */
  ...
  /**
   * Returns 1 if this DirectedEdge has a greater angle with the positive x-axis than b", 0 if the
   * DirectedEdges are collinear, and -1 otherwise.
   *
   * <p>Using the obvious algorithm of simply computing the angle is not robust, since the angle
   * calculation is susceptible to roundoff. A robust algorithm is:
   *
   * <ul>
   *   <li>first compare the quadrants. If the quadrants are different, it it trivial to determine
   *       which vector is "greater".
   *   <li>if the vectors lie in the same quadrant, the robust {@link Orientation#index(Coordinate,
   *       Coordinate, Coordinate)} function can be used to decide the relative orientation of the
   *       vectors.
   * </ul>
   */
