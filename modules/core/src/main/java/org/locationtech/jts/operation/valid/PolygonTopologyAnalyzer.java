...
/**
 * Analyzes the topology of polygonal geometry to determine whether it is valid.
 *
 * <p>Analyzing polygons with inverted rings (shells or exverted holes) is performed if specified.
 * Inverted rings may cause a disconnected interior due to a self-touch; this is reported by {@link
 * #isInteriorDisconnectedBySelfTouch()}.
 *
 * @author mdavis
 */
class PolygonTopologyAnalyzer {
   ...
   * </ul>
   *
   * If the test ring start point is properly inside or outside, that provides the result. Otherwise
   * the start point is on the target ring, and the incident start segment (accounting for repeated
   * points) is tested for its topology relative to the target ring.
   *
  ...
  public static boolean isRingNested(LinearRing test, LinearRing target) {
    ...
    /**
     * The start point is on the boundary of the ring. Use the topology at the node to check if the
     * segment is inside or outside the ring.
     */
   ...
   * </ul>
   *
   * This works for both shells and holes, but the caller must know the ring role.
   *
   ...
   */
  private static boolean isIncidentSegmentInRing(
      Coordinate p0, Coordinate p1, Coordinate[] ringPts) {
  ...
  /**
   * Finds the ring vertex previous to a node point on a ring (which is contained in the index'th
   * segment, as either the start vertex or an interior point). Repeated points are skipped over.
   *
  ...
  /**
   * Finds the ring vertex next from a node point on a ring (which is contained in the index'th
   * segment, as either the start vertex or an interior point). Repeated points are skipped over.
   *
   * @param ringPts the ring
   * @param index the index of the segment containing the node
  ...
  /**
   * Tests whether the interior of the polygonal geometry is disconnected. If true, the
   * disconnection location is available from {@link #getDisconnectionLocation()}.
   *
  ...
  /**
   * Gets a location where the polyonal interior is disconnected. {@link #isInteriorDisconnected()}
  ...
  /**
   * Tests whether any polygon with holes has a disconnected interior by virtue of the holes (and
   * possibly shell) forming a hole cycle.
   *
   * <p>This is a global check, which relies on determining the touching graph of all holes in a
   * polygon.
   *
   * <p>If inverted rings disconnect the interior via a self-touch, this is checked by the {@link
   * PolygonIntersectionAnalyzer}. If inverted rings are part of a hole cycle this is detected here
   * as well.
   */
  ...
  /**
   * Tests if an area interior is disconnected by a self-touching ring. This must be evaluated after
   * other self-intersections have been analyzed and determined to not exist, since the logic relies
   * on the rings not self-crossing (winding).
   *
   * <p>If self-touching rings are not allowed, then the self-touch will previously trigger a
  ...
  private static List<SegmentString> createSegmentStrings(
      Geometry geom, boolean isInvertedRingValid) {
