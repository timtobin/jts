 ...
 * the image of the source point under the desired transformation.
 *
 * <p>
 * A transformation is well-defined by a set of three control vectors if and
 * only if the source points are not collinear. (In particular, the degenerate
 ...
 *
 * <p>
 * No such restriction applies to the destination points. However, if the
 * destination points are collinear or non-unique, a non-invertible
 * transformations will be generated.
 *
 * <p>
 * This technique of recovering a transformation from its effect on known points
 * is used in the Bilinear Interpolated Triangulation algorithm for warping
 * planar surfaces.
 *
 * @author Martin Davis
 */
public class AffineTransformationBuilder {
	...
	/**
	 * Constructs a new builder for the transformation defined by the given set of
	 ...
	 *            a control point
	 * @param dest0
	 *            the image of control point 0 under the required transformation
	 * @param dest1
	 *            the image of control point 1 under the required transformation
	 * @param dest2
	 *            the image of control point 2 under the required transformation
	 */
	public AffineTransformationBuilder(Coordinate src0, Coordinate src1, Coordinate src2, Coordinate dest0,
			Coordinate dest1, Coordinate dest2) {
		this.src0 = src0;
		this.src1 = src1;
		this.src2 = src2;
		this.dest0 = dest0;
	 ...
	 * mappings, or <code>
	 * null</code> if the control vectors do not determine a well-defined
	 * transformation.
	 *
	 * @return an affine transformation, or null if the control vectors do not
	 ...
	 *
	 * @param b
	 *            the vector for the right-hand side of the system
	 * @return the solution vector, or <code>null</code> if no solution could be
	 *         determined
	 */
	private double[] solve(double[] b) {
		double[][] a = new double[][]{{src0.x, src0.y, 1}, {src1.x, src1.y, 1}, {src2.x, src2.y, 1}};
