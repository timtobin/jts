...
public class DirectedEdgeStar extends EdgeEndStar {
   ...
   * the <code>next</code> pointer for an incoming dirEdge is set to the next outgoing edge.
   *
   ...
   * </ul>
   *
   * <p>Edges are linked in CCW order (the order they are stored). This means that rings have their
   * face on the Right (in other words, the topological location of the face is given by the RHS
   * label of the DirectedEdge)
   *
  ...
  public void linkResultDirectedEdges() {
    ...
    for (Object o : resultAreaEdgeList) {
      DirectedEdge nextOut = (DirectedEdge) o;
  ...
  public void findCoveredLineEdges() {
     ...
     * the edge, so the start location will be: - INTERIOR if the edge is outgoing - EXTERIOR if the
     * edge is incoming
     */
