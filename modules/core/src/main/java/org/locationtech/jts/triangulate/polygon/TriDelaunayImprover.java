...
/**
 * Improves the quality of a triangulation of {@link Tri}s via iterated Delaunay
 * flipping. This produces a Constrained Delaunay Triangulation with the
 * constraints being the boundary of the input triangulation.
 *
 * @author mdavis
 */
class TriDelaunayImprover {
	...
	/**
	 * Improves the quality of a triangulation of {@link Tri}s via iterated Delaunay
	 * flipping. The Tris are assumed to be linked into a Triangulation (e.g. via
	 * {@link TriangulationBuilder}).
	 *
	 * @param triList
	 *            the list of Tris to flip.
	 */
	 ...
	 * opp0-adj0-adj1 and opp1-adj1-adj0 are the triangle corners and hence are
	 * known to be convex. The quadrilateral is convex if the other corners
	 * opp0-adj0-opp1 and opp1-adj1-opp0 have the same orientation (since at least
	 * one must be convex).
	 *
	 * @param adj0
	 *            adjacent edge vertex 0
	 * @param adj1
	 *            adjacent edge vertex 1
	 * @param opp0
	 *            corner vertex of triangle 0
	 * @param opp1
	 *            corner vertex of triangle 1
	...
	/**
	 * Tests if either of a pair of adjacent triangles satisfy the Delaunay
	 * condition. The triangles are opp0-adj0-adj1 and opp1-adj1-adj0. The Delaunay
	 * condition is not met if one opposite vertex lies is in the circumcircle of
	 * the other triangle.
	 *
	 * @param adj0
	 *            adjacent edge vertex 0
	...
	private static boolean isDelaunay(Coordinate adj0, Coordinate adj1, Coordinate opp0, Coordinate opp1) {
		if (isInCircle(adj0, adj1, opp0, opp1))
			return false;
		if (isInCircle(adj1, adj0, opp1, opp0))
			return false;
		return true;
	}

	/**
	 * Tests whether a point p is in the circumcircle of a triangle abc (oriented
	 * clockwise).
	 *
	 * @param a
	...
	/**
	 * Does a flip of the common edge of two Tris if the Delaunay condition is not
	 * met.
	 *
	 * @param tri0
	 *            a Tri
	...
	private boolean improveNonDelaunay(Tri tri, int index) {
		...
		/**
		 * The candidate new edge is opp0 - opp1. Check if it is inside the
		 * quadrilateral formed by the two triangles. This is the case if the
	...
	/**
	 * Improves a triangulation by examining pairs of adjacent triangles (forming a
	 * quadrilateral) and testing if flipping the diagonal of the quadrilateral
	 * would produce two new triangles with larger minimum interior angles.
	 *
