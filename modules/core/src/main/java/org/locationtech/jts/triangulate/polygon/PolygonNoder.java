...
/**
 * Adds node vertices to the rings of a polygon where holes touch the shell or
 * each other. The structure of the polygon is preserved.
 *
 * <p>
 * This does not fix invalid polygon topology (such as self-touching or crossing
 * rings). Invalid input remains invalid after noding, and does not trigger an
 * error.
 */
class PolygonNoder {
	...
	/**
	 * A {@link SegmentIntersector} that added node vertices to
	 * {@link NodedSegmentStrings} where a segment touches another segment in its
	 * interior.
	 *
	 * @author mdavis
	 */
	private static class NodeAdder implements SegmentIntersector {
		...
		public void processIntersections(SegmentString ss0, int segIndex0, SegmentString ss1, int segIndex1) {
			...
			/**
			 * There should never be 2 intersection points, since that would imply collinear
			 * segments, and an invalid polygon
			 */
