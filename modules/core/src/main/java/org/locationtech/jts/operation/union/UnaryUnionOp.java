 ...
 * (which may be a {@link GeometryCollection}) together. By using this
 * special-purpose operation over a collection of geometries it is possible to
 * take advantage of various optimizations to improve performance. Heterogeneous
 ...
 *
 * <ul>
 * <li>Unioning a set of {@link Polygon}s has the effect of merging the areas
 * (i.e. the same effect as iteratively unioning all individual polygons
 * together).
 * <li>Unioning a set of {@link LineString}s has the effect of <b>noding</b> and
 * <b>dissolving</b> the input linework. In this context "fully noded" means
 * that there will be an endpoint or node in the result for every endpoint or
 * line segment crossing in the input. "Dissolved" means that any duplicate
 * (i.e. coincident) line segments or portions of line segments will be reduced
 * to a single line segment in the result. This is consistent with the semantics
 * of the {@link Geometry#union(Geometry)} operation. If <b>merged</b> linework
 * is required, the {@link LineMerger} class can be used.
 * <li>Unioning a set of {@link Point}s has the effect of merging all identical
 * points (producing a set with no duplicates).
 * </ul>
 *
...
public class UnaryUnionOp {
	/**
	 * Computes the geometric union of a {@link Collection} of {@link Geometry}s.
	 *
	 * @param geoms
	 *            a collection of geometries
	 * @return the union of the geometries, or <code>null</code> if the input is
	 *         empty
	 */
	 ...
	 *
	 * <p>
	 * If no input geometries were provided but a {@link GeometryFactory} was
	 * provided, an empty {@link GeometryCollection} is returned.
	 *
	 * @param geoms
	 *            a collection of geometries
	 * @param geomFact
	 *            the geometry factory to use if the collection is empty
	 * @return the union of the geometries, or an empty GEOMETRYCOLLECTION
	 */
	...
	/**
	 * Constructs a unary union operation for a {@link Geometry} (which may be a
	 * {@link GeometryCollection}).
	 *
	 * @param geom
	 *            a geometry to union
	 * @return the union of the elements of the geometry or an empty
	 *         GEOMETRYCOLLECTION
	 */
	...
	/**
	 * Constructs a unary union operation for a {@link Collection} of
	 * {@link Geometry}s, using the {@link GeometryFactory} of the input geometries.
	 *
	 * @param geoms
	 *            a collection of geometries
	 */
	 ...
	 *
	 * <ol>
	 * <li>If the input is empty and a dimension can be determined (i.e. an empty
	 * geometry is present), an empty atomic geometry of that dimension is returned.
	 * <li>If no input geometries were provided but a {@link GeometryFactory} was
	 * provided, an empty {@link GeometryCollection} is returned.
	 * <li>Otherwise, the return value is <code>null</code>.
	 * </ol>
	 *
	 * @return a Geometry containing the union, or an empty atomic geometry, or an
	 *         empty GEOMETRYCOLLECTION, or <code>null</code> if no GeometryFactory
	 *         was provided
	 */
	public Geometry union() {
		...
		/**
		 * For points and lines, only a single union operation is required, since the
		 * OGC model allows self-intersecting MultiPoint and MultiLineStrings. This is
		 * not the case for polygons, so Cascaded Union is required.
		 */
		...
		/**
		 * Performing two unions is somewhat inefficient, but is mitigated by unioning
		 * lines and polygons first (since point union can be done efficiently against a
		 * collection of lines and polygons)
		 */
	 ...
	 * efficient in the case of linear and puntal geometries. Uses robust version of
	 * overlay operation to ensure identical behaviour to the
