...
/**
 * Iterates over all {@link Geometry}s in a {@link Geometry}, (which may be either a collection or
 * an atomic geometry). The iteration sequence follows a pre-order, depth-first traversal of the
 * structure of the <code>GeometryCollection</code> (which may be nested). The original <code>
 * Geometry</code> object is returned as well (as the first object), as are all sub-collections and
 * atomic elements. It is simple to ignore the intermediate <code>GeometryCollection</code> objects
...
public class GeometryCollectionIterator implements Iterator {
  ...
  /**
   * The iterator over a nested <code>Geometry</code>, or <code>null</code> if this <code>
   * GeometryCollectionIterator</code> is not currently iterating over a nested <code>
   ...
   *
   * @param parent the geometry over which to iterate; also, the first element returned by the
   *     iterator.
   */
