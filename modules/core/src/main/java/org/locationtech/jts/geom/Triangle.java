...
public class Triangle {
	 ...
	 * perpendicular bisector of chord ab HCoordinate cab = perpendicularBisector(a,
	 * b); // compute the perpendicular bisector of chord bc HCoordinate cbc =
	 * perpendicularBisector(b, c); // compute the intersection of the bisectors
	 * (circle radii) HCoordinate hcc = new HCoordinate(cab, cbc); Coordinate cc =
	 ...
	 *
	 * //System.out.println("Acc = " + a.distance(cc) + ", Bcc = " + b.distance(cc)
	 * + ", Ccc = " + c.distance(cc) );
	 *
	 * return cc; }
	 */
	 ...
	 *
	 * <p>
	 * This method uses {@link DD} extended-precision arithmetic to provide more
	 * accurate results than
	 * {@link #circumcentre(Coordinate, Coordinate, Coordinate)}
	 *
	 * @param a
	 *            a vertex of the triangle
	 * @param b
	...
	/**
	 * Computes the incentre of a triangle. The <i>inCentre</i> of a triangle is the
	 * point which is equidistant from the sides of the triangle. It is also the
	 * point at which the bisectors of the triangle's angles meet. It is the centre
	 * of the triangle's <i>incircle</i>, which is the unique circle that is tangent
	 * to each of the triangle's three sides (and hence the Maximum Inscribed
	 * Circle).
	 *
	 * <p>
	...
	/**
	 * Computes the Z-value (elevation) of an XY point on a three-dimensional plane
	 * defined by a triangle whose vertices have Z-values. The defining triangle
	 * must not be degenerate (in other words, the triangle must enclose a non-zero
	 * area), and must not be parallel to the Z-axis.
	 *
	 * <p>
	...
	/**
	 * Tests whether a triangle is acute. A triangle is acute if all interior angles
	 * are acute. This is a strict test - right triangles will return
	 ...
	 * {@link Orientation#index(Coordinate, Coordinate, Coordinate)} for robust
	...
	/**
	 * Computes the circumcentre of this triangle. The circumcentre is the centre of
	 * the circumcircle, the smallest circle which passes through all the triangle
	 * vertices. It is also the common intersection point of the perpendicular
	...
	/**
	 * Computes the incentre of this triangle. The <i>incentre</i> of a triangle is
	...
	/**
	 * Computes the Z-value (elevation) of an XY point on a three-dimensional plane
	 * defined by this triangle (whose vertices must have Z-values). This triangle
	 * must not be degenerate (in other words, the triangle must enclose a non-zero
	 * area), and must not be parallel to the Z-axis.
	 *
	 * <p>
