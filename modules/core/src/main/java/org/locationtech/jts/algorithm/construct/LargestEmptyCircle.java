 ...
 * given accuracy distance tolerance (which can be specified or determined
 * automatically). The obstacles may be any combination of point, linear and
 * polygonal geometries.
 *
 * <p>
 * The Largest Empty Circle (LEC) is the largest circle whose interior does not
 * intersect with any obstacle and whose center lies within a polygonal
 * boundary. The circle center is the point in the interior of the boundary
 * which has the farthest distance from the obstacles (up to the accuracy of the
 * distance tolerance). The circle itself is determined by the center point and
 * a point lying on an obstacle determining the circle radius.
 *
 * <p>
 * The polygonal boundary may be supplied explicitly. If it is not specified the
 * convex hull of the obstacles is used as the boundary.
 *
 * <p>
 * To compute an LEC which lies <i>wholly</i> within a polygonal boundary,
 * include the boundary of the polygon(s) as a linear obstacle.
 *
 * <p>
 * The implementation uses a successive-approximation technique over a grid of
 * square cells covering the obstacles and boundary. The grid is refined using a
 * branch-and-bound algorithm. Point containment and distance are computed in a
 * performant way by using spatial indexes.
 *
 * @author Martin Davis
...
public class LargestEmptyCircle {
	...
	/**
	 * Computes the center point of the Largest Empty Circle interior-disjoint to a
	 * set of obstacles. The obstacles may be any collection of points, lines and
	 * polygons. The center of the LEC lies within the convex hull of the obstacles.
	 *
	 * @param obstacles
	...
	/**
	 * Computes the center point of the Largest Empty Circle interior-disjoint to a
	 * set of obstacles and within a polygonal boundary. The obstacles may be any
	 * collection of points, lines and polygons. The center of the LEC lies within
	 * the given boundary.
	 *
	 * @param obstacles
	 *            a geometry representing the obstacles
	...
	/**
	 * Computes the center point of the Largest Empty Circle interior-disjoint to a
	 * set of obstacles and within a polygonal boundary, with accuracy to a given
	 * tolerance distance. The obstacles may be any collection of points, lines and
	...
	/**
	 * Computes the center point of the Largest Empty Circle interior-disjoint to a
	 * set of obstacles, with accuracy to a given tolerance distance. The obstacles
	 * may be any collection of points, lines and polygons. The center of the LEC
	 * lies within the convex hull of the obstacles.
	 *
	 * @param obstacles
	 *            a geometry representing the obstacles
	 * @param tolerance
	...
	/**
	 * Computes a radius line of the Largest Empty Circle interior-disjoint to a set
	 * of obstacles and within a polygonal boundary, with accuracy to a given
	 * tolerance distance. The obstacles may be any collection of points, lines and
	 * polygons. The center of the LEC lies within the given boundary.
	 *
	 * @param obstacles
	...
	/**
	 * Computes a radius line of the Largest Empty Circle interior-disjoint to a set
	 * of obstacles, with accuracy to a given tolerance distance. The obstacles may
	 * be any collection of points, lines and polygons. The center of the LEC lies
	 ...
	 * interior-disjoint to a set of obstacle geometries and having its center
	 * within a polygonal boundary. The obstacles may be any collection of points,
	 * lines and polygons. If the boundary is null or empty the convex hull of the
	 * obstacles is used as the boundary.
	 *
	 * @param obstacles
	 *            a non-empty geometry representing the obstacles
	 * @param boundary
	 *            a polygonal geometry (may be null or empty)
	 */
	 ...
	 * within a polygonal boundary. The obstacles may be any collection of points,
	 * lines and polygons. If the boundary is null or empty the convex hull of the
	 * obstacles is used as the boundary. A zero tolerance aut0matically determines
	 ...
	 *            a polygonal geometry (may be null or empty)
	 * @param tolerance
	 *            a distance tolerance for computing the circle center point (a
	 *            non-negative value)
	 */
	...
	private void compute() {
		...
		// if boundaryPtLocater is not present then result is degenerate (represented as
		// zero-radius
		// circle)
	...
	/**
	 * Gets the center point of the Largest Empty Circle (up to the tolerance
	 * distance).
	 *
	 * @return the center point of the Largest Empty Circle
	 */
	...
	/**
	 * Gets a point defining the radius of the Largest Empty Circle. This is a point
	 * on the obstacles which is nearest to the computed center of the Largest Empty
	 * Circle. The line segment from the center to this point is a radius of the
	 * constructed circle, and this point lies on the boundary of the circle.
	 *
	...
	/**
	 * Tests whether a cell may contain the circle center, and thus should be
	 * refined (split into subcells to be investigated further.)
	 *
	 * @param cell
	 *            the cell to test
	...
	private boolean mayContainCircleCenter(Cell cell) {
		/**
		 * Every point in the cell lies outside the boundary, so they cannot be the
		 * center point
		 */
