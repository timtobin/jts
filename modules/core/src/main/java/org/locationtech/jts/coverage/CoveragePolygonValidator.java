...
/**
 * Validates that a polygon forms a valid polygonal coverage with the set of polygons adjacent to
 * it. If the polygon is coverage-valid an empty {@link LineString} is returned. Otherwise, the
 * result is a linear geometry containing the target polygon boundary linework causing the
 * invalidity.
 *
 * <p>A polygon is coverage-valid if:
 *
 * <ol>
 *   <li>The polygon interior does not intersect the interior of other polygons.
 *   <li>If the polygon boundary intersects another polygon boundary, the vertices and line segments
 *       of the intersection match exactly.
 * </ol>
 *
 * <p>The algorithm detects the following coverage errors:
 *
 * <ol>
 *   <li>the polygon is a duplicate of another one
 *   <li>a polygon boundary segment equals an adjacent segment (with same orientation). This
 *       determines that the polygons overlap
 *   <li>a polygon boundary segment is collinear and overlaps an adjacent segment but is not equal
 *       to it
 ...
 * </ol>
 *
 * <p>If any of these errors is present, the target polygon does not form a valid coverage with the
 * adjacent polygons.
 *
 * <p>The validity rules do not preclude properly-noded gaps between coverage polygons. However,
 * this class can detect narrow gaps, by specifying a maximum gap width using {@link
 * #setGapWidth(double)}. Note that this will also identify narrow gaps separating disjoint coverage
 * regions, and narrow gores. In some situations it may also produce false positives (i.e. linework
 * identified as part of a gap which is wider than the given width). To fully identify gaps it maybe
 * necessary to use {@link CoverageUnion} and analyze the holes in the result to see if they are
 * acceptable.
 *
 * <p>A polygon may be coverage-valid with respect to a set of surrounding polygons, but the
 * collection as a whole may not form a clean coverage. For example, the target polygon boundary may
 * be fully matched by adjacent boundary segments, but the adjacent set contains polygons which are
 * not coverage-valid relative to other ones in the set. A coverage is valid only if every polygon
 * in the coverage is coverage-valid. Use {@link CoverageValidator} to validate an entire set of
 * polygons.
 *
 * <p>The adjacent set may contain polygons which do not intersect the target polygon. These are
 * effectively ignored during validation (but may decrease performance).
 *
...
public class CoveragePolygonValidator {
  ...
  /**
   * Validates that a polygon is coverage-valid against the surrounding polygons in a polygonal
   * coverage.
   *
  ...
  /**
   * Validates that a polygon is coverage-valid against the surrounding polygons in a polygonal
   * coverage, and forms no gaps narrower than a specified width.
   *
   * <p>The set of surrounding polygons should include all polygons which are within the gap width
   * distance of the target polygon.
   *
   * @param targetPolygon the polygon to validate
   ...
   *
   * <p>If the gap width is specified, the set of surrounding polygons should include all polygons
   * which are within the gap width distance of the target polygon.
   *
  ...
  public Geometry validate() {
    ...
    /**
     * Mark matching segments first. Matched segments are not considered for further checks. This
     * improves performance substantially for mostly-valid coverages.
     */
  ...
  private void checkTargetRings(
    ...
    /**
     * Short-circuit if target is fully known (matched or invalid). This often happens in clean
     * coverages, when the target is surrounded by matching polygons. It can also happen in invalid
     * coverages which have polygons which are duplicates, or perfectly overlap other polygons.
     */
    if (CoverageRing.isKnown(targetRings)) return;

    /**
     * Here target has at least one unmatched segment. Do further checks to see if any of them are
     * are invalid.
     */
  ...
  /**
   * Marks matched segments. This improves the efficiency of validity testing, since in valid
   * coverages all segments (except exterior ones) are matched, and hence do not need to be tested
   * further. Segments which are equal and have same orientation are detected and marked invalid. In
   * fact, the entire target polygon may be matched and valid, which allows avoiding further tests.
   * Segments matched between adjacent polygons are also marked valid, since this prevents them from
   * being detected as misaligned, if this is being done.
   *
   * @param targetRings the target rings
   * @param adjRngs the adjacent rings
   * @param targetEnv the tolerance envelope of the target
   */
  private void markMatchedSegments(
  ...
  /**
   * Adds ring segments to the segment map, and detects if they match an existing segment. Matched
   * segments are marked.
   *
   * @param rings
   * @param envLimit
    ...
    for (CoverageRing ring : rings) {
      for (int i = 0; i < ring.size() - 1; i++) {
        ...
        if (segmentMap.containsKey(seg)) {
          ...
          /** Since inputs should be valid, the segments are assumed to be in different rings. */
          seg.match(segMatch);
        } else {
  ...
  /**
   * Models a segment in a CoverageRing. The segment is normalized so it can be compared with
   * segments in any orientation. Records valid matching segments in a coverage, which must have
   * opposite orientations. Also detects equal segments with identical orientation, and marks them
   * as coverage-invalid.
   */
  ...
  /**
   * Marks invalid target segments which cross an adjacent ring segment, lie partially in the
   * interior of an adjacent ring, or are nearly collinear with an adjacent ring segment up to the
   * distance tolerance
   *
   ...
   */
  private void markInvalidInteractingSegments(
    ...
    MCIndexSegmentSetMutualIntersector segSetMutInt =
        new MCIndexSegmentSetMutualIntersector(targetRings, distanceTolerance);
  ...
  /**
   * Marks invalid target segments which are fully interior to an adjacent polygon.
   *
   * @param targetRings the rings with segments to test
   * @param adjCovPolygons the adjacent polygons
   */
  private void markInvalidInteriorSegments(
    ...
    for (CoverageRing ring : targetRings) {
      ...
      for (int i = 0; i < ring.size() - 1; i += stride) {
        int iEnd = i + stride;
        if (iEnd >= ring.size()) iEnd = ring.size() - 1;
  ...
  /**
   * Marks invalid target segments in a section which are interior to an adjacent polygon.
   * Processing a section at a time dramatically improves efficiency. Due to the coherent
   * organization of polygon rings, sections usually have a high spatial locality. This means that
   * sections typically intersect only a few or often no adjacent polygons. The section envelope can
   * be computed and tested against adjacent polygon envelopes quickly. The section can be skipped
   * entirely if it does not interact with any polygons.
   *
   * @param ring
   * @param iStart
   * @param iEnd
   * @param adjPolygons
   */
  private void markInvalidInteriorSection(
  ...
  private void markInvalidInteriorSegment(CoverageRing ring, int i, CoveragePolygon adjPoly) {
    ...
    if (ring.isKnown(i)) return;

    /**
     * Check if vertex is in interior of an adjacent polygon. If so, the segments on either side are
     * in the interior. Mark them invalid, unless they are already matched.
     */
    ...
    if (adjPoly.contains(p)) {
      ...
      if (!ring.isKnown(iPrev)) ring.markInvalid(iPrev);
    }
  }
