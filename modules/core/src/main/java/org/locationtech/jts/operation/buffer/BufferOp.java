 ...
 *
 * <p>
 * In GIS, the positive (or negative) buffer of a geometry is defined as the
 * Minkowski sum (or difference) of the geometry with a circle of radius equal
 * to the absolute value of the buffer distance. In the CAD/CAM world buffers
 * are known as <i>offset curves</i>. In morphological analysis the operation of
 * positive and negative buffering is referred to as <i>erosion</i> and
 * <i>dilation</i>
 *
 * <p>
 ...
 * accuracy of the approximation by specifying the number of linear segments
 * used to approximate arcs. This is specified via
 * {@link BufferParameters#setQuadrantSegments(int)} or
 * {@link #setQuadrantSegments(int)}.
 *
 * <p>
 * The <b>end cap style</b> of a linear buffer may be
 * {@link BufferParameters#setEndCapStyle(int) specified}. The following end cap
 * styles are supported:
 *
 * <ul>
 * <li>{@link BufferParameters#CAP_ROUND} - the usual round end caps
 * <li>{@link BufferParameters#CAP_FLAT} - end caps are truncated flat at the
 * line ends
 * <li>{@link BufferParameters#CAP_SQUARE} - end caps are squared off at the
 * buffer distance beyond the line ends
 * </ul>
 *
 * <p>
 * The <b>join style</b> of the corners in a buffer may be
 * {@link BufferParameters#setJoinStyle(int) specified}. The following join
 * styles are supported:
 *
 * <ul>
 * <li>{@link BufferParameters#JOIN_ROUND} - the usual round join
 * <li>{@link BufferParameters#JOIN_MITRE} - corners are "sharp" (up to a
 * {@link BufferParameters#getMitreLimit() distance limit})
 * <li>{@link BufferParameters#JOIN_BEVEL} - corners are beveled (clipped off).
 * </ul>
 *
 * <p>
 * The buffer algorithm may perform simplification on the input to increase
 * performance. The simplification is performed a way that always increases the
 * buffer area (so that the simplified input covers the original input). The
 * degree of simplification can be
 * {@link BufferParameters#setSimplifyFactor(double) specified}, with a
 * {@link BufferParameters#DEFAULT_SIMPLIFY_FACTOR default} used otherwise. Note
 * that if the buffer distance is zero then so is the computed simplify
 * tolerance, no matter what the simplify factor.
 *
 * <p>
 * Buffer results are always valid geometry. Given this, computing a zero-width
 * buffer of an invalid polygonal geometry is an effective way to "validify" the
 * geometry. Note however that in the case of self-intersecting "bow-tie"
 * geometries, only the largest enclosed area will be retained.
 *
 * @version 1.7
 */
public class BufferOp {
	...
	/**
	 * Buffers a geometry with distance zero. The result can be computed using the
	 * maximum-signed-area orientation, or by combining both orientations.
	 *
	 * <p>
	 * This can be used to fix an invalid polygonal geometry to be valid (i.e. with
	 * no self-intersections). For some uses (e.g. fixing the result of a
	 * simplification) a better result is produced by using only the max-area
	 * orientation. Other uses (e.g. fixing geometry) require both orientations to
	 * be used.
	 *
	 * <p>
	 ...
	 *
	 * @param geom
	 *            the polygonal geometry to buffer by zero
	 * @param isBothOrientations
	 *            true if both orientations of input rings should be used
	...
	/*
	 * private static double OLDprecisionScaleFactor(Geometry g, double distance,
	 * int maxPrecisionDigits) { Envelope env = g.getEnvelopeInternal(); double
	 * envSize = Math.max(env.getHeight(), env.getWidth()); double expandByDistance
	 * = distance > 0.0 ? distance : 0.0; double bufEnvSize = envSize + 2 *
	 * expandByDistance;
	 *
	 * // the smallest power of 10 greater than the buffer envelope int bufEnvLog10
	 * = (int) (Math.log(bufEnvSize) / Math.log(10) + 1.0); int minUnitLog10 =
	 * bufEnvLog10 - maxPrecisionDigits; // scale factor is inverse of min Unit
	 * size, so flip sign of exponent double scaleFactor = Math.pow(10.0,
	 * -minUnitLog10); return scaleFactor; }
	 */
	...
	/**
	 * Combines the elements of two polygonal geometries together. The input
	 * geometries must be non-adjacent, to avoid creating an invalid result.
	 *
	 * @param poly0
	 *            a polygonal geometry (which may be empty)
	 * @param poly1
	 *            a polygonal geometry (which may be empty)
	 ...
	 * Geometry and buffer distance. The scale factor is determined by the number of
	 * digits of precision in the (geometry + buffer distance), limited by the
	 * supplied <code>maxPrecisionDigits</code> value.
	 *
	 * <p>
	 * The scale factor is based on the absolute magnitude of the (geometry + buffer
	 * distance). since this determines the number of digits of precision which must
	 * be handled.
	 *
	 * @param g
	 ...
	 *            the max # of digits that should be allowed by the precision
	 *            determined by the computed scale factor
	...
	private static double precisionScaleFactor(Geometry g, double distance, int maxPrecisionDigits) {
		...
		double envMax = MathUtil.max(Math.abs(env.getMaxX()), Math.abs(env.getMaxY()), Math.abs(env.getMinX()),
	...
	private void bufferFixedPrecision(PrecisionModel fixedPM) {
		 ...
		 *
		 * SnapRoundingNoder does not require rounded input, so could be used by itself.
		 * But using ScaledNoder may be faster, since it avoids rounding within
		 * SnapRoundingNoder. (Note this only works for buffering, because ScaledNoder
		 * may invalidate topology.)
		 */
	 ...
	 * {@link BufferParameters#CAP_ROUND}, {@link BufferParameters#CAP_FLAT}, and
	 * {@link BufferParameters#CAP_SQUARE}. The default is CAP_ROUND.
	 *
	 * @param endCapStyle
	...
	/**
	 * Sets the number of line segments in a quarter-circle used to approximate
	 * angle fillets for round end caps and joins.
	 *
	 * @param quadrantSegments
