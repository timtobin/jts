...
public class CGAlgorithmsDD {
   ...
   * @return 1 if q is counter-clockwise (left) from p1-p2 -1 if q is clockwise (right) from p1-p2 0
   ...
   *
   * @return -1 if the determinant is negative, 1 if the determinant is positive, 0 if the
   *     determinant is 0.
   */
   ...
   * the orientation index. Otherwise, a value i > 1 is returned. In this case the orientation index
   * must be computed using some other more robust method. The filter is fast to compute, so can be
   * used to avoid the use of slower robust methods except when they are really needed, thus
   * providing better average performance.
   *
   * <p>Uses an approach due to Jonathan Shewchuk, which is in the public domain.
   *
   ...
   */
  private static int orientationIndexFilter(
