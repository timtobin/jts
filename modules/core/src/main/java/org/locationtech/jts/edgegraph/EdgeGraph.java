...
/**
 * A graph comprised of {@link HalfEdge}s. It supports tracking the vertices in the graph via edges
 * incident on them, to allow efficient lookup of edges and vertices.
 *
 * <p>This class may be subclassed to use a different subclass of HalfEdge, by overriding {@link
 * #createEdge(Coordinate)}. If additional logic is required to initialize edges then {@link
 * EdgeGraph#addEdge(Coordinate, Coordinate)} can be overridden as well.
 *
 * @author Martin Davis
 */
public class EdgeGraph {
  ...
  /**
   * Adds an edge between the coordinates orig and dest to this graph. Only valid edges can be added
   * (in particular, zero-length segments cannot be added)
   *
  ...
  public HalfEdge addEdge(Coordinate orig, Coordinate dest) {
    ...
    /**
     * Attempt to find the edge already in the graph. Return it if found. Otherwise, use a found
     * edge with same origin (if any) to construct new edge.
     */
  ...
  /**
   * Gets all {@link HalfEdge}s in the graph. Both edges of edge pairs are included.
   *
