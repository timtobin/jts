...
/**
 * Tests whether a <code>Geometry</code> is simple as defined by the OGC SFS
 * specification.
 *
 * <p>
 ...
 * <li><b>LineString</b> geometries are simple if they do not self-intersect at
 * interior points (i.e. points other than the endpoints). Closed linestrings
 * which intersect only at their endpoints are simple (i.e. valid
 * <b>LinearRings</b>s.
 * <li><b>MultiLineString</b> geometries are simple if their elements are simple
 * and they intersect only at points which are boundary points of both elements.
 * (The notion of boundary points can be user-specified - see below).
 * <li><b>Polygonal</b> geometries have no definition of simplicity. The
 * <code>isSimple</code> code checks if all polygon rings are simple. (Note:
 * this means that <tt>isSimple</tt> cannot be used to test for <i>all</i>
 * self-intersections in <tt>Polygon</tt>s. In order to check if a
 * <tt>Polygonal</tt> geometry has self-intersections, use
 * {@link Geometry#isValid()}).
 * <li><b>GeometryCollection</b> geometries are simple if all their elements are
 * simple.
 * <li>Empty geometries are simple
 * </ul>
 *
 * For {@link Lineal} geometries the evaluation of simplicity can be customized
 * by supplying a {@link BoundaryNodeRule} to define how boundary points are
 * determined. The default is the SFS-standard
 * {@link BoundaryNodeRule#MOD2_BOUNDARY_RULE}.
 *
 * <p>
 * Note that under the <tt>Mod-2</tt> rule, closed <tt>LineString</tt>s (rings)
 * have no boundary. This means that an intersection at the endpoints of two
 * closed LineStrings makes the geometry non-simple. If it is required to test
 * whether a set of <code>LineString</code>s touch only at their endpoints, use
 * {@link BoundaryNodeRule#ENDPOINT_BOUNDARY_RULE}. For example, this can be
 * used to validate that a collection of lines form a topologically valid linear
 * network.
 *
 * <p>
 * By default this class finds a single non-simple location. To find all
 * non-simple locations, set {@link #setFindAllLocations(boolean)} before
 * calling {@link #isSimple()}, and retrieve the locations via
 * {@link #getNonSimpleLocations()}. This can be used to find all intersection
 * points in a linear network.
 *
...
public class IsSimpleOp {
	...
	private boolean isSimpleLinearGeometry(Geometry geom) {
		...
		NonSimpleIntersectionFinder segInt = new NonSimpleIntersectionFinder(isClosedEndpointsInInterior,
				isFindAllLocations, nonSimplePts);
	...
	/**
	 * Computes simplicity for polygonal geometries. Polygonal geometries are simple
	 * if and only if all of their component rings are simple.
	 *
	 * @param geom
	...
	private static class NonSimpleIntersectionFinder implements SegmentIntersector {
		/**
		 * Finds the vertex index in a segment of an intersection which is known to be a
		 * vertex.
		 *
		 * @param li
		 *            the line intersector
		 * @param segmentIndex
		 *            the intersection segment index
		 * @return the vertex index (0 or 1) in the segment vertex of the intersection
		 *         point
		 */
		 ...
		 *            the segmentString
		 * @param ssIndex
		 *            index of segment in segmentString
		 * @param li
		 *            the line intersector
		 * @param liSegmentIndex
		 *            index of segment in intersector
		 * @return true if the intersection vertex is an endpoint
		 */
		private static boolean isIntersectionEndpoint(SegmentString ss, int ssIndex, LineIntersector li,
				int liSegmentIndex) {
		...
		private boolean findIntersection(SegmentString ss0, int segIndex0, SegmentString ss1, int segIndex1) {
			...
			/**
			 * Check for equal segments (which will produce two intersection points). These
			 * also intersect in interior points, so are non-simple. (This is not triggered
			 * by zero-length segments, since they are filtered out by the MC index).
			 */
			...
			/**
			 * At this point there is a single intersection point which is a vertex in each
			 * segString. Classify them as endpoints or interior
			 */
			...
			/**
			 * Both intersection vertices must be endpoints. Final check is if one or both
			 * of them is interior due to being endpoint of a closed ring. This only applies
