...
/**
 * Detects invalid coverage topology where ring segments interact. The inputs to
 * {@link #processIntersections(SegmentString, int, SegmentString, int)} must be
 * {@link CoverageRing}s. If an invalid situation is detected the input target
 * segment is marked invalid using {@link CoverageRing#markInvalid(int)}.
 *
 * <p>
 * This class assumes it is used with {@link SegmentSetMutualIntersector}, so
 * that segments in the same ring are not evaluated.
 *
 * @author Martin Davis
 */
class InvalidSegmentDetector implements SegmentIntersector {

	private static boolean isNearlyParallel(Coordinate p00, Coordinate p01, Coordinate p10, Coordinate p11,
			double distanceTol) {
	...
	/**
	 * Checks if the segments are collinear, or if the target segment intersects the
	 * interior of the adjacent ring. Segments which are collinear must be non-equal
	 * and hence invalid, since matching segments have already been marked as valid
	 * and are not passed to this code.
	 *
	 * @param tgt0
	 * @param tgt1
	 * @param adj0
	 ...
	 * @return
	 */
	private boolean isCollinearOrInterior(Coordinate tgt0, Coordinate tgt1, Coordinate adj0, Coordinate adj1,
		...
		/**
		 * At this point the segments have a single intersection point which is an
	...
	/**
	 * Process interacting segments. The input order is important. The adjacent
	 * segment is first, the target is second. The inputs must be
	 * {@link CoverageRing}s.
	 */
	@Override
	public void processIntersections(SegmentString ssAdj, int iAdj, SegmentString ssTarget, int iTarget) {
		...
		// -- Assert: rings are not equal (because this is used with
		...
		/*
		 * //-- skip segments beyond distance tolerance if (distanceTol <
		 * Distance.segmentToSegment(t0, t1, adj0, adj1)) { return; }
		 */
