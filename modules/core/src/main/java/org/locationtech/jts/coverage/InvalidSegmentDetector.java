...
/**
 * Detects invalid coverage topology where ring segments interact. The inputs to {@link
 * #processIntersections(SegmentString, int, SegmentString, int)} must be {@link CoverageRing}s. If
 * an invalid situation is detected the input target segment is marked invalid using {@link
 * CoverageRing#markInvalid(int)}.
 *
 * <p>This class assumes it is used with {@link SegmentSetMutualIntersector}, so that segments in
 * the same ring are not evaluated.
 *
 * @author Martin Davis
 */
class InvalidSegmentDetector implements SegmentIntersector {
  ...
  /**
   * Process interacting segments. The input order is important. The adjacent segment is first, the
   * target is second. The inputs must be {@link CoverageRing}s.
   */
  @Override
  public void processIntersections(
    ...
    if (t0.equals2D(t1) || adj0.equals2D(adj1)) return;
    if (isEqual(t0, t1, adj0, adj1)) return;
  ...
  private boolean isEqual(Coordinate t0, Coordinate t1, Coordinate adj0, Coordinate adj1) {
    if (t0.equals2D(adj0) && t1.equals2D(adj1)) return true;
    if (t0.equals2D(adj1) && t1.equals2D(adj0)) return true;
    return false;
  }
  ...
  private boolean isInvalid(
    ...
    if (isCollinearOrInterior(tgt0, tgt1, adj0, adj1, adj, indexAdj)) return true;

    // -- segments which are nearly parallel for a significant length are invalid
    if (distanceTol > 0 && isNearlyParallel(tgt0, tgt1, adj0, adj1, distanceTol)) return true;

    return false;
  }

  /**
   * Checks if the segments are collinear, or if the target segment intersects the interior of the
   * adjacent ring. Segments which are collinear must be non-equal and hence invalid, since matching
   * segments have already been marked as valid and are not passed to this code.
   *
   * @param tgt0
   * @param tgt1
   * @param adj0
   ...
   * @return
   */
  private boolean isCollinearOrInterior(
      Coordinate tgt0,
      Coordinate tgt1,
      Coordinate adj0,
      Coordinate adj1,
    ...
    /**
     * At this point the segments have a single intersection point which is an endpoint of both
     * segments.
     *
     * <p>Check if the target segment lies in the interior of the adj ring.
     */
  ...
  private boolean isInteriorSegment(
      Coordinate intVertex, Coordinate tgt0, Coordinate tgt1, CoverageRing adj, int indexAdj) {
  ...
  private static boolean isNearlyParallel(
      Coordinate p00, Coordinate p01, Coordinate p10, Coordinate p11, double distanceTol) {
    LineSegment line0 = new LineSegment(p00, p01);
    LineSegment line1 = new LineSegment(p10, p11);
    LineSegment proj0 = line0.project(line1);
    if (proj0 == null) return false;
    LineSegment proj1 = line1.project(line0);
    if (proj1 == null) return false;

    if (proj0.getLength() <= distanceTol || proj1.getLength() <= distanceTol) return false;
