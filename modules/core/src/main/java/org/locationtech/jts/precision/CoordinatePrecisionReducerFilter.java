...
/**
 * Reduces the precision of the {@link Coordinate}s in a {@link CoordinateSequence} to match the
 * supplied {@link PrecisionModel}. Uses {@link PrecisionModel#makePrecise(double)}. The input is
 * modified in-place, so it should be cloned beforehand if the original should not be modified.
 *
 * @author mbdavis
 */
