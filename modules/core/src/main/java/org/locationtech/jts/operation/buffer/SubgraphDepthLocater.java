...
class SubgraphDepthLocater {
	 ...
	 *            the current list of {@link DepthSegments} intersecting the
	 *            stabbing line
	 */
	...
	static class DepthSegment implements Comparable {
		...
		/**
		 * Compares a point to a segment for left/right position, as long as the point
		 * lies within the segment Y extent. Otherwise the point is not comparable. If
		 * the point is not comparable or it lies on the segment returns 0.
		 *
		 * @param p
		 * @param seg
		 * @return
		...
		public int compareTo(Object obj) {
			...
			/**
			 * If segments are disjoint in X, X values provides ordering. This is the most
			 * common case.
			 */
			...
			/**
			 * Check if some segment point is left or right of the other segment in its Y
			 * extent.
			 */
			...
			/**
			 * If point checks in Y range are indeterminate, segments touch at a point and
			 * lie above and below that point, or are horizontal. Order according to their Y
			 * values. (The ordering in this case doesn't matter, it just has to be
			 * consistent)
			 */
