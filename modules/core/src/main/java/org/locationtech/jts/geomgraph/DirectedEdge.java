...
public class DirectedEdge extends EdgeEnd {
	...
	/**
	 * Computes the factor for the change in depth when moving from one location to
	 * another. E.g. if crossing from the {@link Location#INTERIOR} to
	 * the{@link Location#EXTERIOR} the depth decreases, so the factor is -1.
	 *
	 * @param currLocation
	 *            Current location
	 * @param nextLocation
	...
	/**
	 * The depth of each side (position) of this edge. The 0 element of the array is
	 * never used.
	 */
	...
	public boolean isInteriorAreaEdge() {
		...
		for (int i = 0; i < 2; i++) {
			if (!(label.isArea(i) && label.getLocation(i, Position.LEFT) == Location.INTERIOR
	...
	public boolean isLineEdge() {
		...
		boolean isExteriorIfArea0 = !label.isArea(0) || label.allPositionsEqual(0, Location.EXTERIOR);
		boolean isExteriorIfArea1 = !label.isArea(1) || label.allPositionsEqual(1, Location.EXTERIOR);
	 ...
	 *
	 * <p>
	 * You may also use {@link #setEdgeDepths(int, int)} to update depth and
	 * opposite depth together.
	 *
	 * @param position
	 *            Position to update
	 * @param depthVal
	...
	public void setDepth(int position, int depthVal) {
		if (depth[position] != -999) {
			...
			// Assert.isTrue(depth[position] == depthVal, "assigned depths do not match at "
			// +
			// getCoordinate());
		}
	 ...
	 *
	 * <p>
	 * This is used for edges corresponding to lines, which will only appear
	 * oriented in a single direction in the result.
	 *
	 * @param isVisited
