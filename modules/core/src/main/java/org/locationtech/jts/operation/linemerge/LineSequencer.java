...
/**
 * Builds a sequence from a set of LineStrings so that they are ordered end to
 * end. A sequence is a complete non-repeating list of the linear components of
 * the input. Each linestring is oriented so that identical endpoints are
 * adjacent in the list.
 *
 * <p>
 * A typical use case is to convert a set of unoriented geometric links from a
 * linear network (e.g. such as block faces on a bus route) into a continuous
 ...
 * expected. The computed output is a single {@link MultiLineString} containing
 ...
 *
 * <ul>
 * <li>If there is degree-1 node which is the start node of an linestring, use
 * that node as the start of the sequence
 * <li>If there is a degree-1 node which is the end node of an linestring, use
 * that node as the end of the sequence
 * <li>If the sequence has no degree-1 nodes, use any node as the start
 * </ul>
 *
 * Note that not all arrangements of lines can be sequenced. For a connected set
 * of edges in a graph, <i>Euler's Theorem</i> states that there is a sequence
 * containing each edge once <b>if and only if</b> there are no more than 2
 * nodes of odd degree. If it is not possible to find a sequence, the
...
public class LineSequencer {
	...
	/**
	 * Finds an {@link DirectedEdge} for an unvisited edge (if any), choosing the
	 * dirEdge which preserves orientation, if possible.
	 *
	 * @param node
	 *            the node to examine
	...
	/**
	 * Tests whether a {@link Geometry} is sequenced correctly. {@link LineString}s
	 * are trivially sequenced. {@link MultiLineString}s are checked for correct
	 * sequencing. Otherwise, <code>
	 * isSequenced</code> is defined to be <code>true</code> for geometries that are
	 * not lineal.
	 *
	 * @param geom
	...
	/**
	 * Adds a {@link Collection} of {@link Geometry}s to be sequenced. May be called
	...
	/**
	 * Adds a {@link Geometry} to be sequenced. May be called multiple times. Any
	 * dimension of Geometry may be added; the constituent linework will be
	 * extracted.
	 *
	 * @param geometry
	 *            the geometry to add
	...
	private void computeSequence() {
		...
		Assert.isTrue(sequencedGeometry instanceof LineString || sequencedGeometry instanceof MultiLineString,
	 ...
	 *
	 * @return the sequenced linestrings, or <code>null</code> if a valid sequence
	 *         does not exist
	 */
	...
	/**
	 * Computes a version of the sequence which is optimally oriented relative to
	 ...
	 *
	 * <ul>
	 * <li>If the path has a degree-1 node which is the start node of an linestring,
	 * use that node as the start of the sequence
	 * <li>If the path has a degree-1 node which is the end node of an linestring,
	 * use that node as the end of the sequence
	 * <li>If the sequence has no degree-1 nodes, use any node as the start (NOTE:
	 * in this case could orient the sequence according to the majority of the
	...
	private List orient(List seq) {
		...
		// (Could insert heuristic of taking direction of majority of lines as overall
		// direction)
	...
	/**
	 * Reverse the sequence. This requires reversing the order of the dirEdges, and
	 * flipping each dirEdge as well
	 *
	 * @param seq
