...
/**
 * Builds a sequence from a set of LineStrings so that they are ordered end to end. A sequence is a
 * complete non-repeating list of the linear components of the input. Each linestring is oriented so
 * that identical endpoints are adjacent in the list.
 *
 * <p>A typical use case is to convert a set of unoriented geometric links from a linear network
 * (e.g. such as block faces on a bus route) into a continuous oriented path through the network.
 *
 * <p>The input linestrings may form one or more connected sets. The input linestrings should be
 * correctly noded, or the results may not be what is expected. The computed output is a single
 * {@link MultiLineString} containing the ordered linestrings in the sequence.
 *
 * <p>The sequencing employs the classic <b>Eulerian path</b> graph algorithm. Since Eulerian paths
 * are not uniquely determined, further rules are used to make the computed sequence preserve as
 * much as possible of the input ordering. Within a connected subset of lines, the ordering rules
 * are:
 *
 * <ul>
 *   <li>If there is degree-1 node which is the start node of an linestring, use that node as the
 *       start of the sequence
 *   <li>If there is a degree-1 node which is the end node of an linestring, use that node as the
 *       end of the sequence
 *   <li>If the sequence has no degree-1 nodes, use any node as the start
 * </ul>
 *
 * Note that not all arrangements of lines can be sequenced. For a connected set of edges in a
 * graph, <i>Euler's Theorem</i> states that there is a sequence containing each edge once <b>if and
 * only if</b> there are no more than 2 nodes of odd degree. If it is not possible to find a
 * sequence, the {@link #isSequenceable()} method will return <code>false</code>.
 *
 * @version 1.7
 */
public class LineSequencer {
  ...
  /**
   * Tests whether a {@link Geometry} is sequenced correctly. {@link LineString}s are trivially
   * sequenced. {@link MultiLineString}s are checked for correct sequencing. Otherwise, <code>
   * isSequenced</code> is defined to be <code>true</code> for geometries that are not lineal.
   *
  ...
  /**
   * Adds a {@link Collection} of {@link Geometry}s to be sequenced. May be called multiple times.
  ...
  /**
   * Adds a {@link Geometry} to be sequenced. May be called multiple times. Any dimension of
  ...
  /**
   * Returns the {@link LineString} or {@link MultiLineString} built by the sequencing process, if
   * one exists.
   *
   * @return the sequenced linestrings, or <code>null</code> if a valid sequence does not exist
   */
  ...
  private void computeSequence() {
    ...
    Assert.isTrue(
        sequencedGeometry instanceof LineString || sequencedGeometry instanceof MultiLineString,
  ...
  private List findSequence(Subgraph graph) {
    ...
    while (lit.hasPrevious()) {
      ...
      if (unvisitedOutDE != null) addReverseSubpath(unvisitedOutDE.getSym(), lit, true);
    }

    /**
     * At this point, we have a valid sequence of graph DirectedEdges, but it is not necessarily
     * appropriately oriented relative to the underlying geometry.
     */
  ...
  /**
   * Finds an {@link DirectedEdge} for an unvisited edge (if any), choosing the dirEdge which
   * preserves orientation, if possible.
   *
  ...
  /**
   * Computes a version of the sequence which is optimally oriented relative to the underlying
   * geometry.
   *
   * <p>Heuristics used are:
   *
   * <ul>
   *   <li>If the path has a degree-1 node which is the start node of an linestring, use that node
   *       as the start of the sequence
   *   <li>If the path has a degree-1 node which is the end node of an linestring, use that node as
   *       the end of the sequence
   *   <li>If the sequence has no degree-1 nodes, use any node as the start (NOTE: in this case
   *       could orient the sequence according to the majority of the linestring orientations)
   * </ul>
   *
  ...
  private List orient(List seq) {
    ...
    boolean hasDegree1Node = startNode.getDegree() == 1 || endNode.getDegree() == 1;
  ...
  /**
   * Builds a geometry ({@link LineString} or {@link MultiLineString} ) representing the sequence.
   *
   * @param sequences a List of Lists of DirectedEdges with LineMergeEdges as their parent edges.
  ...
  private Geometry buildSequencedGeometry(List sequences) {
    ...
    for (Object sequence : sequences) {
      ...
      for (Object o : seq) {
        ...
        if (!de.getEdgeDirection() && !line.isClosed()) lineToAdd = reverse(line);

        lines.add(lineToAdd);
      }
    }
    if (lines.isEmpty()) return factory.createMultiLineString(new LineString[0]);
