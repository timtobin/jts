...
public class Bintree {
	/**
	 * Ensure that the Interval for the inserted item has non-zero extents. Use the
	 * current minExtent to pad it, if necessary
	 */
	 ...
	 *
	 * <p>
	 * minExtent is the minimum extent of all items inserted into the tree so far.
	 * It is used as a heuristic value to construct non-zero extents for features
	 * with zero extent. Start with a non-zero extent, in case the first feature
	 * inserted has a zero extent in both directions. This value may be non-optimal,
	 * but only one feature will be inserted with this value.
	 */
	...
	public void insert(Interval itemInterval, Object item) {
		 ...
		 * "   node size = " + nodeSize()); if (newSize <= oldSize) {
	 ...
	 * interval. If the query interval is <tt>null</tt>, all items in the tree are
	 * found.
	 *
	 * <p>
	...
	public List query(Interval interval) {
		/**
		 * the items that are matched are all items in intervals which overlap the query
		 * interval
		 */
		List foundItems = new ArrayList();
	 ...
	 * given collection. If the query interval is <tt>null</tt>, add all items in
	 * the tree.
	 *
	 * @param interval
	 ...
	 *
	 * @param itemInterval
	 *            the interval of the item to be removed
	 * @param item
	 *            the item to remove
