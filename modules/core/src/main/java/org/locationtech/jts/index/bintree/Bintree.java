...
/**
 * An <code>BinTree</code> (or "Binary Interval Tree") is a 1-dimensional version of a quadtree. It
 * indexes 1-dimensional intervals (which may be the projection of 2-D objects on an axis). It
 * supports range searching (where the range may be a single point). This structure is dynamic - new
 * items can be added at any time, and it will support deletion of items (although this is not
 ...
 *
 * <p>The bintree structure is used to provide a primary filter for interval queries. The query()
 * method returns a list of all objects which <i>may</i> intersect the query interval. Note that it
 * may return objects which do not in fact intersect. A secondary filter is required to test for
 * exact intersection. Of course, this secondary filter may consist of other tests besides
 * intersection, such as testing other kinds of spatial relationships.
 *
 * <p>This index is different to the Interval Tree of Edelsbrunner or the Segment Tree of Bentley.
 *
 * @version 1.7
 */
public class Bintree {
   ...
   * Statistics
   *
   * <p>minExtent is the minimum extent of all items inserted into the tree so far. It is used as a
   * heuristic value to construct non-zero extents for features with zero extent. Start with a
   * non-zero extent, in case the first feature inserted has a zero extent in both directions. This
   * value may be non-optimal, but only one feature will be inserted with this value.
   */
  ...
  /**
   * Queries the tree to find all candidate items which may overlap the query interval. If the query
   * interval is <tt>null</tt>, all items in the tree are found.
   *
   * <p>min and max may be the same value
   */
  public List query(Interval interval) {
    /** the items that are matched are all items in intervals which overlap the query interval */
    List foundItems = new ArrayList();
