...
/**
 * Constructs the Maximum Inscribed Circle for a polygonal {@link Geometry}, up
 * to a specified tolerance (which can be specified or determined
 * automatically). The Maximum Inscribed Circle is determined by a point in the
 * interior of the area which has the farthest distance from the area boundary,
 * along with a boundary point at that distance.
 *
 * <p>
 * In the context of geography the center of the Maximum Inscribed Circle is
 * known as the <b>Pole of Inaccessibility</b>. A cartographic use case is to
 * determine a suitable point to place a map label within a polygon.
 *
 * <p>
 * The radius length of the Maximum Inscribed Circle is a measure of how
 * "narrow" a polygon is. It is the distance at which the negative buffer
 * becomes empty. The class supports testing whether a polygon is "narrower"
 * than a specified distance via {@link #isRadiusWithin(Geometry, double)} or
 * {@link #isRadiusWithin(double)}. Testing for the maximum radius is generally
 * much faster than computing the actual radius value, since short-circuiting is
 * used to limit the approximation iterations.
 *
 * <p>
 * The class supports polygons with holes and multipolygons.
 *
 * <p>
 * For small polygons (currently triangles and convex quadrilaterals) the MIC is
 * determined exactly. For other polygons the implementation uses a
 * successive-approximation technique over a grid of square cells covering the
 * area geometry. The grid is refined using a branch-and-bound algorithm. Point
 * containment and distance are computed in a performant way by using spatial
 * indexes.
 *
 * <h3>Future Enhancements</h3>
 *
 * <ul>
 * <li>Support a polygonal constraint on placement of center point, for example
 * to produce circle-packing constructions, or support multiple labels.
 * </ul>
 *
 * @author Martin Davis
...
public class MaximumInscribedCircle {
	...
	/**
	 * Computes the maximum number of iterations allowed. Uses a heuristic based on
	 * the size of the input geometry and the tolerance distance. A smaller
	 * tolerance distance allows more iterations. This is a rough heuristic,
	 * intended to prevent huge iterations for very thin geometries.
	 *
	 * @param geom
	 *            the input geometry
	 * @param toleranceDist
	...
	/**
	 * Computes the center point of the Maximum Inscribed Circle of a polygonal
	 * geometry.
	 *
	 * @param polygonal
	...
	/**
	 * Computes the center point of the Maximum Inscribed Circle of a polygonal
	 * geometry, up to a given tolerance distance.
	 *
	 * @param polygonal
	 *            a polygonal geometry
	 * @param tolerance
	...
	/**
	 * Computes a radius line of the Maximum Inscribed Circle of a polygonal
	 * geometry.
	 *
	 * @param polygonal
	 *            a polygonal geometry
	...
	/**
	 * Tests if the radius of the maximum inscribed circle is no longer than the
	 * specified distance. The approximation tolerance is determined automatically
	 ...
	 *             if the tolerance is negative, or the input geometry is
	...
	/**
	 * Creates a new instance of a Maximum Inscribed Circle computation, with an
	 * approximation tolerance distance. A zero tolerance aut0matically determines
	 * an approximation tolerance.
	 *
	 * @param polygonal
	 *            an areal geometry
	 * @param tolerance
	 *            the distance tolerance for computing the centre point (must be
	 *            non-negative)
	 * @throws IllegalArgumentException
	 *             if the tolerance is negative, or the input geometry is
	 *             non-polygonal or empty.
	...
	private void computeApproximation() {
		...
		while (!cellQueue.isEmpty() && iter < maxIter) {
			 ...
			 * since no point in it is further than the current farthest distance (up to
			 * tolerance).
			 *
			 * <p>
			 * The tolerance can be automatically determined as a fraction of the current
			 * farthest distance. For a very small actual MIC distance this may cause many
			 * iterations, but the iter limit prevents an infinite loop
			 */
			double requiredTol = tolerance > 0 ? tolerance : farthestCell.getDistance() * AUTO_TOLERANCE_FRACTION;
	...
	/**
	 * Computes the signed distance from a point to the area boundary. Points
	 * outside the polygon are assigned a negative distance. Their containing cells
	 * will be last in the priority queue (but may still end up being tested since
	...
	/**
	 * Gets the center point of the maximum inscribed circle (up to the tolerance
	 * distance).
	 *
	 * @return the center point of the maximum inscribed circle
	 */
	...
	/**
	 * Gets a point defining the radius of the Maximum Inscribed Circle. This is a
	 * point on the boundary which is nearest to the computed center of the Maximum
	 * Inscribed Circle. The line segment from the center to this point is a radius
	 * of the constructed circle, and this point lies on the boundary of the circle.
	 *
	...
	/**
	 * Tests if the radius of the maximum inscribed circle is no longer than the
	 * specified distance. This method determines the distance tolerance
	 * automatically as a fraction of the maxRadius value. After this method is
	 * called the center and radius points provide locations demonstrating where the
	 * radius exceeds the specified maximum.
	 *
	 * @param maxRadius
	 *            the (non-negative) radius value to test
	 * @return true if the max in-circle radius is no longer than the max radius
	 */
	...
	private static class Cell implements Comparable<Cell> {
		...
		/**
		 * For maximum efficieny sort the PriorityQueue with largest maxDistance at
		 * front. Since Java PQ sorts least-first, need to invert the comparison
		 */
