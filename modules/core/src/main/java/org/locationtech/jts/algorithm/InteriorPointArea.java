...
/**
 * Computes a point in the interior of an areal geometry. The point will lie in
 * the geometry interior in all except certain pathological cases.
 *
 * <h2>Algorithm</h2>
 *
 ...
 *
 * <ul>
 * <li>Determine a horizontal scan line on which the interior point will be
 * located. To increase the chance of the scan line having non-zero-width
 * intersection with the polygon the scan line Y ordinate is chosen to be near
 * the centre of the polygon's Y extent but distinct from all of vertex Y
 * ordinates.
 * <li>Compute the sections of the scan line which lie in the interior of the
 * polygon.
 * <li>Choose the widest interior section and take its midpoint as the interior
 * point.
 * </ul>
 *
 * The final interior point is chosen as the one occurring in the widest
 * interior section.
 *
 * <p>
 * This algorithm is a tradeoff between performance and point quality (where
 * points further from the geometry boundary are considered to be higher
 * quality) Priority is given to performance. This means that the computed
 * interior point may not be suitable for some uses (such as label positioning).
 *
 * <p>
 * The algorithm handles some kinds of invalid/degenerate geometry, including
 ...
 * <li>If the input polygon is <i>extremely</i> narrow the computed point may
 * not lie in the interior of the polygon.
 * </ul>
 *
 * @version 1.17
 */
public class InteriorPointArea {
	 ...
	 *
	 * @return the coordinate of an interior point or <code>null</code> if the input
	 *         geometry is empty
	 */
	...
	/**
	 * Processes a geometry to determine the best interior point for all component
	 * polygons.
	 *
	 * @param geom
	...
	/**
	 * Computes an interior point of a component Polygon and updates current best
	 * interior point if appropriate.
	 *
	 * @param polygon
	...
	/**
	 * Computes an interior point in a single {@link Polygon}, as well as the width
	 * of the scan-line section it occurs in to allow choosing the widest section
	 * occurrence.
	 *
	 * @author mdavis
	 */
	private static class InteriorPointPolygon {
		/**
		 * Computes the intersection of a segment with a horizontal line. The segment is
		 * expected to cross the horizontal line - this condition is not checked.
		 * Computation uses regular double-precision arithmetic. Test seems to indicate
		 * this is as good as using DD arithmetic.
		 *
		 * @param p0
		 ...
		 *            an endpoint of the segment
		 * @param Y
		 *            the Y-ordinate of the horizontal line
		 * @return
		 */
		 ...
		 * crossing situations are not counted, to ensure that the list of crossings
		 ...
		 * {@link #interiorPoint} location and the {@link #interiorSectionWidth}
		 *
		 * @param crossings
		 ...
		 * Gets the computed interior point.
		 *
		 * @return the interior point coordinate, or <code>null</code> if the input
		 *         geometry is empty
		 */
		public Coordinate getInteriorPoint() {
			return interiorPoint;
		...
		/**
		 * Gets the width of the scanline section containing the interior point. Used to
		 * determine the best point to use.
		 *
		 * @return the width
		 */
		...
		/*
		 * // for testing only private static void checkIntersectionDD(Coordinate p0,
		 * Coordinate p1, double scanY, double xInt) { double xIntDD =
		 * intersectionDD(p0, p1, scanY); System.out.println( ((xInt != xIntDD) ? ">>" :
		 * "") + "IntPt x - DP: " + xInt + ", DD: " + xIntDD + "   y: " + scanY + "   "
		 * + WKTWriter.toLineString(p0, p1) ); }
		 *
		 * private static double intersectionDD(Coordinate p0, Coordinate p1, double Y)
		 * { double x0 = p0.getX(); double x1 = p1.getX();
		 *
		 * if ( x0 == x1 ) return x0;
		 *
		 * DD segDX = DD.valueOf(x1).selfSubtract(x0); // Assert: segDX is non-zero, due
		 * to previous equality test DD segDY =
		 * DD.valueOf(p1.getY()).selfSubtract(p0.getY()); DD m = segDY.divide(segDX); DD
		 * dy = DD.valueOf(Y).selfSubtract(p0.getY()); DD dx = dy.divide(m); DD xInt =
		 * DD.valueOf(x0).selfAdd(dx); return xInt.doubleValue(); }
		 */
	}

	/**
	 * Finds a safe scan line Y ordinate by projecting the polygon segments to the Y
	 * axis and finding the Y-axis interval which contains the centre of the Y
