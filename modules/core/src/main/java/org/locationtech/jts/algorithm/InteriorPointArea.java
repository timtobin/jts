 ...
 *
 * <ul>
 *   <li>Determine a horizontal scan line on which the interior point will be located. To increase
 *       the chance of the scan line having non-zero-width intersection with the polygon the scan
 *       line Y ordinate is chosen to be near the centre of the polygon's Y extent but distinct from
 *       all of vertex Y ordinates.
 ...
 *
 * <p>This algorithm is a tradeoff between performance and point quality (where points further from
 * the geometry boundary are considered to be higher quality) Priority is given to performance. This
 * means that the computed interior point may not be suitable for some uses (such as label
 * positioning).
 *
 * <p>The algorithm handles some kinds of invalid/degenerate geometry, including zero-area and
 * self-intersecting polygons.
 *
 * <p>Empty geometry is handled by returning a <code>null</code> point.
 *
 * <h3>KNOWN BUGS</h3>
 *
 * <ul>
 *   <li>If a fixed precision model is used, in some cases this method may return a point which does
 *       not lie in the interior.
 *   <li>If the input polygon is <i>extremely</i> narrow the computed point may not lie in the
...
public class InteriorPointArea {
   ...
   * @return the computed interior point, or <code>null</code> if the geometry has no polygonal
   *     components
   */
  ...
  private static class InteriorPointPolygon {
    ...
    private void scanRing(LinearRing ring, List<Double> crossings) {
      ...
      if (!intersectsHorizontalLine(ring.getEnvelopeInternal(), interiorPointY)) return;
     ...
     * cross the horizontal line - this condition is not checked. Computation uses regular
     * double-precision arithmetic. Test seems to indicate this is as good as using DD arithmetic.
     *
     * @param p0 an endpoint of the segment
     * @param p1 an endpoint of the segment
   ...
   * returned as the scan line Y-ordinate.
   *
   * <p>Note that in the case of (degenerate, invalid) zero-area polygons the computed Y value may
   * be equal to a vertex Y-ordinate.
   *
   * @author mdavis
   */
