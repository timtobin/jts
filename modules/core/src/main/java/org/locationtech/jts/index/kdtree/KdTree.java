...
public class KdTree {
	...
	/**
	 * Converts a collection of {@link KdNode}s to an array of {@link Coordinate}s,
	 * specifying whether repeated nodes should be represented by multiple
	 * coordinates.
	 *
	 * @param kdnodes
	 *            a collection of nodes
	...
	/**
	 * Finds the node in the tree which is the best match for a point being
	 * inserted. The match is made deterministic by returning the lowest of any
	 * nodes which lie the same distance from the point. There may be no match if
	 * the point is not within the distance tolerance of any existing node.
	 *
	 * @param p
	 ...
	 *         node is returned with its counter incremented. This can be checked by
	 *         testing returnedNode.getCount() &gt; 1.
	 */
	public KdNode insert(Coordinate p, Object data) {
		...
		/**
		 * Check if the point is already in the tree, up to tolerance. If tolerance is
		 * zero, this phase of the insertion can be skipped.
		 */
		if (tolerance > 0) {
	...
	/**
	 * Inserts a point known to be beyond the distance tolerance of any existing
	 * node. The point is inserted at the bottom of the exact splitting path, so
	 * that tree shape is deterministic.
	 *
	 * @param p
	 *            the point to insert
	 * @param data
	 ...
	 *            the query point
	 * @param n
	 *            the number of nearest nodes to find
