...
public class LineString extends Geometry implements Lineal {
	/**
	 * The minimum number of vertices allowed in a valid non-empty linestring. Empty
	 * linestrings with 0 vertices are also valid.
	 */
	 ...
	 *
	 * @param points
	 *            the points of the linestring, or <code>null</code> to create the
	 *            empty geometry.
	 ...
	 * @param points
	 *            the points of the linestring, or <code>null</code> to create the
	 *            empty geometry. This array must not contain <code>null</code>
	...
	private void init(CoordinateSequence points) {
		...
		if (points.size() > 0 && points.size() < MINIMUM_VALID_SIZE) {
			throw new IllegalArgumentException("Invalid number of points in LineString (found " + points.size()
	...
	/**
	 * Normalizes a LineString. A normalized linestring has the first point which is
	 * not equal to it's reflected point less than the reflected point.
	 */
