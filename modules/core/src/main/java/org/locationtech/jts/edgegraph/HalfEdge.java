 ...
 * graph formed by the edges.
 *
 * <p>To support graphs where the edges are sequences of coordinates each edge may also have a
 * direction point supplied. This is used to determine the ordering of the edges around the origin.
 * HalfEdges with the same origin are ordered so that the ring of edges formed by them is oriented
 * CCW.
 *
 * <p>By design HalfEdges carry minimal information about the actual usage of the graph they
 * represent. They can be subclassed to carry more information if required.
 *
 * <p>HalfEdges form a complete and consistent data structure by themselves, but an {@link
...
public class HalfEdge {
  ...
  /**
   * Gets the previous edge CW around the origin vertex of this edge, with that vertex being its
   * destination.
   *
   * <p>It is always true that <code>e.next().prev() == e</code>
   *
   * <p>Note that this requires a scan of the origin edges, so may not be efficient for some uses.
   *
   ...
   *
   * <p><code>e.oNext()</code> is equal to <code>e.sym().next()</code>
   *
  ...
  private HalfEdge insertionEdge(HalfEdge eAdd) {
    ...
    do {
       ...
       *
       * <p>Insert edge here if it lies between ePrev and eNext.
       */
       ...
       *
       * <p>Insert edge here if it lies in the gap between ePrev and eNext across the origin.
       */
   ...
   *
   * <p>The angle of edge a is greater than the angle of edge b, where the angle of an edge is the
   * angle made by the first segment of the edge with the positive x-axis
   *
   * <p>When applied to a list of edges originating at the same point, this produces a CCW ordering
   * of the edges around the point.
   *
   * <p>Using the obvious algorithm of computing the angle is not robust, since the angle
   * calculation is susceptible to roundoff error. A robust algorithm is:
   *
   * <ul>
   *   <li>First, compare the quadrants the edge vectors lie in. If the quadrants are different, it
   *       is trivial to determine which edge has a greater angle.
   *   <li>if the vectors lie in the same quadrant, the {@link Orientation#index(Coordinate,
   *       Coordinate, Coordinate)} function can be used to determine the relative orientation of
   *       the vectors.
   * </ul>
   */
  ...
  /**
   * Provides a string representation of the edges around the origin node of this edge. Uses the
   * subclass representation for each edge.
   *
  ...
  /**
   * Finds the first node previous to this edge, if any. A node has degree {@code <> 2}. If no such
   * node exists (i.e. the edge is part of a ring) then null is returned.
   *
