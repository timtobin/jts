...
/**
 * Represents a directed component of an edge in an {@link EdgeGraph}. HalfEdges
 * link vertices whose locations are defined by {@link Coordinate}s. HalfEdges
 * start at an <b>origin</b> vertex, and terminate at a <b>destination</b>
 * vertex. HalfEdges always occur in symmetric pairs, with the {@link #sym()}
 * method giving access to the oppositely-oriented component. HalfEdges and the
 * methods on them form an edge algebra, which can be used to traverse and query
 * the topology of the graph formed by the edges.
 *
 * <p>
 ...
 * of the edges around the origin. HalfEdges with the same origin are ordered so
 * that the ring of edges formed by them is oriented CCW.
 *
 * <p>
 ...
 * {@link EdgeGraph} is useful to allow retrieving edges by vertex and edge
...
public class HalfEdge {
	...
	/**
	 * Creates a HalfEdge pair representing an edge between two vertices located at
	 * coordinates p0 and p1.
	 *
	 * @param p0
	 *            a vertex coordinate
	 * @param p1
	 ...
	 * an edge is the angle made by the first segment of the edge with the positive
	 * x-axis
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * Using the obvious algorithm of computing the angle is not robust, since the
	 * angle calculation is susceptible to roundoff error. A robust algorithm is:
	 *
	 * <ul>
	 * <li>First, compare the quadrants the edge vectors lie in. If the quadrants
	 * are different, it is trivial to determine which edge has a greater angle.
	 * <li>if the vectors lie in the same quadrant, the
	 * {@link Orientation#index(Coordinate, Coordinate, Coordinate)} function can be
	 * used to determine the relative orientation of the vectors.
	 * </ul>
	 */
	...
	/**
	 * Compares edges which originate at the same vertex based on the angle they
	 * make at their origin vertex with the positive X-axis. This allows sorting
	 * edges around their origin vertex in CCW order.
	 */
	...
	/**
	 * Computes the degree of the origin vertex. The degree is the number of edges
	...
	/**
	 * Gets the direction point of this edge. In the base case this is the dest
	 * coordinate of the edge. Subclasses may override to allow a HalfEdge to
	 * represent an edge with more than two coordinates.
	 *
	 ...
	 * @param p1
	 *            the destination vertex to test
	...
	/**
	 * Finds the edge starting at the origin of this edge with the given dest
	 * vertex, if any.
	 *
	 * @param dest
	 *            the dest vertex to search for
	 * @return the edge with the required dest vertex, if it exists, or null
	 */
	 ...
	 * ensuring that the edges remain ordered CCW. The inserted edge must have the
	 * same origin as this edge.
	 *
	 * @param eAdd
	 *            the edge to insert
	 */
	...
	/**
	 * Insert an edge with the same origin after this one. Assumes that the inserted
	 * edge is in the correct position around the ring.
	 *
	 * @param e
	...
	/**
	 * Finds the insertion edge for a edge being added to this origin, ensuring that
	 * the star of edges around the origin remains fully CCW.
	 *
	 * @param eAdd
	...
	private HalfEdge insertionEdge(HalfEdge eAdd) {
		...
		do {
			 ...
			 *
			 * <p>
			 * Insert edge here if it lies in the gap between ePrev and eNext across the
			 * origin.
			 */
	...
	/**
	 * Tests whether the edges around the origin are sorted correctly. Note that
	 * edges must be strictly increasing, which implies no two edges can have the
	 * same direction point.
	 *
	...
	/**
	 * Links this edge with its sym (opposite) edge. This must be done for each pair
	 * of edges created.
	 *
	 * @param sym
	...
	/**
	 * Gets the previous edge CW around the origin vertex of this edge, with that
	 * vertex being its destination.
	 *
	 * <p>
	...
	/**
	 * Finds the first node previous to this edge, if any. A node has degree
	 * {@code <> 2}. If no such node exists (i.e. the edge is part of a ring) then
	 * null is returned.
	 *
	 * @return an edge originating at the node prior to this edge, if any, or null
	 *         if no node exists
	 */
	...
	public String toString() {
		return "HE(" + orig.x + " " + orig.y + ", " + sym.orig.x + " " + sym.orig.y + ")";
	}

	/**
	 * Provides a string representation of the edges around the origin node of this
	 * edge. Uses the subclass representation for each edge.
	 *
