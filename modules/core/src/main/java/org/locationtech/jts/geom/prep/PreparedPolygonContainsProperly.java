...
/**
 * Computes the <tt>containsProperly</tt> spatial relationship predicate for {@link
 * PreparedPolygon}s relative to all other {@link Geometry} classes. Uses short-circuit tests and
 * indexing to improve performance.
 *
 * <p>A Geometry A <tt>containsProperly</tt> another Geometry B iff all points of B are contained in
 * the Interior of A. Equivalently, B is contained in A AND B does not intersect the Boundary of A.
 *
 * <p>The advantage to using this predicate is that it can be computed efficiently, with no need to
 * compute topology at individual points. In a situation with many geometries intersecting the
 * boundary of the target geometry, this can make a performance difference.
 *
 * @author Martin Davis
 */
class PreparedPolygonContainsProperly extends PreparedPolygonPredicate {
  /**
   * Computes the </tt>containsProperly</tt> predicate between a {@link PreparedPolygon} and a
   * {@link Geometry}.
   *
  ...
  public boolean containsProperly(Geometry geom) {
     ...
     * result.
     *
     * <p>If a point of any test components does not lie in the target interior, result is false
     */
    ...
    /**
     * Given that no segments intersect, if any vertex of the target is contained in some test
     * component. the test is NOT properly contained.
     */
