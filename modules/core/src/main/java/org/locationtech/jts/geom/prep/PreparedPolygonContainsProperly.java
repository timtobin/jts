...
/**
 * Computes the <tt>containsProperly</tt> spatial relationship predicate for
 ...
 *
 * <p>
 * A Geometry A <tt>containsProperly</tt> another Geometry B iff all points of B
 * are contained in the Interior of A. Equivalently, B is contained in A AND B
 * does not intersect the Boundary of A.
 *
 * <p>
 * The advantage to using this predicate is that it can be computed efficiently,
 * with no need to compute topology at individual points. In a situation with
 * many geometries intersecting the boundary of the target geometry, this can
...
class PreparedPolygonContainsProperly extends PreparedPolygonPredicate {
	/**
	 * Computes the </tt>containsProperly</tt> predicate between a
	 * {@link PreparedPolygon} and a {@link Geometry}.
	 *
	 * @param prep
	...
	public boolean containsProperly(Geometry geom) {
		 ...
		 *
		 * <p>
		 * If a point of any test components does not lie in the target interior, result
		 * is false
		 */
		...
		/**
		 * Given that no segments intersect, if any vertex of the target is contained in
		 * some test component. the test is NOT properly contained.
		 */
