 ...
 * specified, and respect the supplied rule when computing the results of the operation.
 *
 * <p>An example use case for a non-SFS-standard Boundary Node Rule is that of checking that a set
 * of {@link LineString}s have valid linear network topology, when turn-arounds are represented as
 ...
 * that closed rings have <b>no</b> boundary points.
 *
 * <p>This interface and its subclasses follow the <tt>Strategy</tt> design pattern.
 *
 * @author Martin Davis
 * @version 1.7
 * @see RelateOp
...
public interface BoundaryNodeRule {
   ...
   * LinearRing}s and closed {@link LineString}s have an empty boundary.
   *
   * <p>This is the rule specified by the <i>OGC SFS</i>, and is the default rule used in JTS.
   *
   * @author Martin Davis
   * @version 1.7
   */
   ...
   * boundary (the common endpoint of the underlying LineString).
   *
   * <p>This rule is useful when dealing with linear networks. For example, it can be used to check
   * whether linear networks are correctly noded. The usual network topology constraint is that
   * linear segments may touch only at endpoints. In the case of a segment touching a closed segment
   * (ring) at one point, the Mod2 rule cannot distinguish between the permitted case of touching at
   * the node point and the invalid case of touching at some other interior (non-node) point. The
   * EndPoint rule does distinguish between these cases, so is more appropriate for use.
   *
   * @author Martin Davis
   * @version 1.7
   */
  ...
  /**
   * A {@link BoundaryNodeRule} which determines that only endpoints with valency of exactly 1 are
   * on the boundary. This corresponds to the boundary of a {@link MultiLineString} being all the
