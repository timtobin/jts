 ...
 * JTS operations (such as {@link RelateOp}, {@link BoundaryOp} and
 * {@link IsSimpleOp}) allow the BoundaryNodeRule to be specified, and respect
 * the supplied rule when computing the results of the operation.
 *
 * <p>
 * An example use case for a non-SFS-standard Boundary Node Rule is that of
 * checking that a set of {@link LineString}s have valid linear network
 * topology, when turn-arounds are represented as closed rings. In this
 * situation, the entry road to the turn-around is only valid when it touches
 * the turn-around ring at the single (common) endpoint. This is equivalent to
 * requiring the set of <tt>LineString</tt>s to be <b>simple</b> under the
 * {@link EndPointBoundaryNodeRule}. The SFS-standard
 * {@link Mod2BoundaryNodeRule} is not sufficient to perform this test, since it
 * states that closed rings have <b>no</b> boundary points.
 *
 * <p>
 * This interface and its subclasses follow the <tt>Strategy</tt> design
 * pattern.
 *
 * @author Martin Davis
 * @version 1.7
...
public interface BoundaryNodeRule {
	...
	/**
	 * Tests whether a point that lies in <tt>boundaryCount</tt> geometry component
	 * boundaries is considered to form part of the boundary of the parent geometry.
	 *
	 * @param boundaryCount
	...
	/**
	 * A {@link BoundaryNodeRule} which specifies that any points which are
	 * endpoints of lineal components are in the boundary of the parent geometry.
	 * This corresponds to the "intuitive" topological definition of boundary. Under
	 * this rule {@link LinearRing}s have a non-empty boundary (the common endpoint
	...
	/**
	 * A {@link BoundaryNodeRule} which determines that only endpoints with valency
	 * of exactly 1 are on the boundary. This corresponds to the boundary of a
	 * {@link MultiLineString} being all the "unattached" endpoints.
	 *
	 * @author Martin Davis
	 * @version 1.7
	 */
	...
	/**
	 * A {@link BoundaryNodeRule} which determines that only endpoints with valency
	 * greater than 1 are on the boundary. This corresponds to the boundary of a
	 * {@link MultiLineString} being all the "attached" endpoints, but not the
	 * "unattached" ones.
	 *
	 * @author Martin Davis
	 * @version 1.7
