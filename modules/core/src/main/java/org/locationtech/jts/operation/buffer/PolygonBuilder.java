...
class PolygonBuilder {
	 ...
	 *
	 * @return containing EdgeRing, if there is one or null if no containing
	 *         EdgeRing is found
	 */
	...
	private List buildMinimalEdgeRings(List maxEdgeRings, List shellList, List freeHoleList) {
		...
		for (Object maxEdgeRing : maxEdgeRings) {
			...
			if (er.getMaxNodeDegree() > 2) {
				...
				// at this point we can go ahead and attempt to place holes, if this EdgeRing is
				// a polygon
	 ...
	 * returned.
	 *
	 * @return the shell EdgeRing, if there is one or null, if all the rings are
	 *         holes
	 */
	...
	/**
	 * Determines finds a containing shell for all holes which have not yet been
	 * assigned to a shell.
	 *
	 * <p>
	 * Holes which do not lie in any shell are (probably) an eroded element, so are
	 * simply discarded
	 */
