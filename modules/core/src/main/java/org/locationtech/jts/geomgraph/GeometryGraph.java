...
public class GeometryGraph extends PlanarGraph {
	 ...
	 * whether a component (node or edge) that appears multiple times in elements of
	 * a MultiGeometry is in the boundary or the interior of the Geometry <br>
	 * The SFS uses the "Mod-2 Rule", which this function implements <br>
	 * An alternative (and possibly more intuitive) rule would be the "At Most One
	 * Rule": isInBoundary = (componentCount == 1)
	 */
	/*
	...
	/*
	 * This constructor is used by clients that wish to add Edges explicitly, rather
	 * than adding a Geometry. (An example is BufferOp).
	 */
	// no longer used
	...
	private void addLineString(LineString line) {
		...
		insertEdge(e);
		/*
		 * Add the boundary points of the LineString, if any. Even if the LineString is
		 * closed, add both points as if they were endpoints. This allows for the case
		 * that the node already exists and is a boundary point.
		 */
	...
	/**
	 * Add a point computed externally. The point is assumed to be a Point Geometry
	 * part, which has a location of INTERIOR.
	 *
	 * @param pt
	 *            Coordinate
	 */
	 ...
	 *            if <code>false</code>, intersection checks are optimized to not
	 *            test rings for self-intersection
	 * @return the computed SegmentIntersector containing information about the
	 *         intersections found
	 */
	public SegmentIntersector computeSelfNodes(LineIntersector li, boolean computeRingSelfNodes) {
		...
		boolean isRings = parentGeom instanceof LinearRing || parentGeom instanceof Polygon
	...
	/**
	 * Determines the {@link Location} of the given {@link Coordinate} in this
	 * geometry.
	 *
	 * @param pt
	 *            the point to test
