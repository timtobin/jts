 ...
 * allow robust computation.
 *
 * <p>The overlay can be used to determine any of the following set-theoretic operations (boolean
 * combinations) of the geometries:
 *
 * <ul>
 ...
 * </ul>
 *
 * Input geometries may have different dimension. Input collections must be homogeneous (all
 * elements must have the same dimension). Inputs may be <b>simple</b> {@link GeometryCollection}s.
 * A GeometryCollection is simple if it can be flattened into a valid Multi-geometry; i.e. it is
 * homogeneous and does not contain any overlapping Polygons.
 *
 * <p>The precision model used for the computation can be supplied independent of the precision
 * model of the input geometry. The main use for this is to allow using a fixed precision for
 * geometry with a floating precision model. This does two things: ensures robust computation; and
 * forces the output to be validly rounded to the precision model.
 *
 * <p>For fixed precision models noding is performed using a {@link SnapRoundingNoder}. This
 * provides robust computation (as long as precision is limited to around 13 decimal digits).
 *
 * <p>For floating precision an {@link MCIndexNoder} is used. This is not fully robust, so can
 * sometimes result in {@link TopologyException}s being thrown. For robust full-precision overlay
 * see {@link OverlayNGRobust}.
 *
 * <p>A custom {@link Noder} can be supplied. This allows using a more performant noding strategy in
 * specific cases, for instance in {@link CoverageUnion}.
 *
 * <p><b>Note:</b If a {@link SnappingNoder} is used it is best to specify a fairly small snap
 * tolerance, since the intersection clipping optimization can interact with the snapping to alter
 * the result.
 *
 * <p>Optionally the overlay computation can process using strict mode (via {@link
 * #setStrictMode(boolean)}. In strict mode result semantics are:
 *
 * <ul>
 *   <li>Lines and Points resulting from topology collapses are not included in the result
 *   <li>Result geometry is homogeneous for the {@link #INTERSECTION} and {@link #DIFFERENCE}
 *       operations.
 *   <li>Result geometry is homogeneous for the {@link #UNION} and {@link #SYMDIFFERENCE} operations
 *       if the inputs have the same dimension
 * </ul>
 *
 * <p>Strict mode has the following benefits:
 *
 * <ul>
 *   <li>Results are simpler
 *   <li>Overlay operations are chainable without needing to remove lower-dimension elements
 * </ul>
 *
 * <p>The original JTS overlay semantics corresponds to non-strict mode.
 *
 * <p>If a robustness error occurs, a {@link TopologyException} is thrown. These are usually caused
 * by numerical rounding causing the noding output to not be fully noded. For robust computation
 * with full-precision {@link OverlayNGRobust} can be used.
 *
 * @author mdavis
 * @see OverlayNGRobust
 */
public class OverlayNG {
   ...
   *
   * <p>The original JTS overlay semantics used non-strict result semantics, including; - An
   * Intersection result can be mixed-dimension, due to inclusion of intersection components of all
   * dimensions - Results can include lines caused by Area topology collapse
   */
   ...
   * contained in the result of overlaying the geometries using a given overlay operation.
   *
   * <p>The method handles arguments of {@link Location#NONE} correctly
   *
  ...
  static boolean isResultOfOp(int overlayOpCode, int loc0, int loc1) {
    ...
    return switch (overlayOpCode) {
      case INTERSECTION -> loc0 == Location.INTERIOR && loc1 == Location.INTERIOR;
      case UNION -> loc0 == Location.INTERIOR || loc1 == Location.INTERIOR;
      case DIFFERENCE -> loc0 == Location.INTERIOR && loc1 != Location.INTERIOR;
      case SYMDIFFERENCE ->
  ...
  public static Geometry overlay(
      Geometry geom0, Geometry geom1, int opCode, PrecisionModel pm, Noder noder) {
    OverlayNG ov = new OverlayNG(geom0, geom1, pm, opCode);
    ov.setNoder(noder);
    Geometry geomOv = ov.getResult();
    return geomOv;
   ...
   * geometry. and an appropriate noder.
   *
   * <p>The noder is chosen according to the precision model specified.
   *
   * <ul>
   *   <li>For {@link PrecisionModel#FIXED} a snap-rounding noder is used, and the computation is
   *       robust.
   *   <li>For {@link PrecisionModel#FLOATING} a non-snapping noder is used, and this computation
   *       may not be robust. If errors occur a {@link TopologyException} is thrown.
   * </ul>
   *
  ...
  /**
   * Computes a union operation on the given geometry, with the supplied precision model.
   *
   * <p>The input must be a valid geometry. Collections must be homogeneous.
   *
   * <p>To union an overlapping set of polygons in a more performant way use {@link UnaryUnionNG}.
   * To union a polyonal coverage or linear network in a more performant way, use {@link
   * CoverageUnion}.
   *
   ...
   *
   * <p>The primary use of this is to support coverage union. Because of this the overlay is
   * performed using strict mode.
   *
   ...
   * noding strategy is determined by the precision model.
   *
   ...
   *   <li>Result geometry is homogeneous for the {@link #INTERSECTION} and {@link #DIFFERENCE}
   *       operations.
   *   <li>Result geometry is homogeneous for the {@link #UNION} and {@link #SYMDIFFERENCE}
   ...
   *
   * @return the result of the overlay operation.
   * @throws IllegalArgumentException if the input is not supported (e.g. a mixed-dimension
   *     geometry)
  ...
  public Geometry getResult() {
    ...
    ElevationModel elevModel =
        ElevationModel.create(inputGeom.getGeometry(0), inputGeom.getGeometry(1));
    Geometry result;
    if (inputGeom.isAllPoints()) {
      // handle Point-Point inputs
      result =
          OverlayPoints.overlay(opCode, inputGeom.getGeometry(0), inputGeom.getGeometry(1), pm);
  ...
  private Geometry computeEdgeOverlay() {
    ...
    /**
     * Heuristic check on result area. Catches cases where noding causes vertex to move and make
     * topology graph area "invert".
     */
   ...
   *
   * <p>This method implements the semantic that the result of an intersection operation is
   * homogeneous with highest dimension. In other words, if an intersection has components of a
   * given dimension no lower-dimension components are output. For example, if two polygons
   * intersect in an area, no linestrings or points are included in the result, even if portions of
   * the input do meet in lines or points. This semantic choice makes more sense for typical usage,
