...
/**
 * Computes the geometric overlay of two {@link Geometry}s, using an explicit
 * precision model to allow robust computation.
 *
 * <p>
 ...
 * <li>{@link #DIFFERENCE} - all points which lie in the first geometry but not
 * the second
 * <li>{@link #SYMDIFFERENCE} - all points which lie in one geometry but not
 * both
 * </ul>
 *
 * Input geometries may have different dimension. Input collections must be
 * homogeneous (all elements must have the same dimension). Inputs may be
 * <b>simple</b> {@link GeometryCollection}s. A GeometryCollection is simple if
 * it can be flattened into a valid Multi-geometry; i.e. it is homogeneous and
 * does not contain any overlapping Polygons.
 *
 * <p>
 ...
 * {@link SnapRoundingNoder}. This provides robust computation (as long as
 * precision is limited to around 13 decimal digits).
 *
 * <p>
 * For floating precision an {@link MCIndexNoder} is used. This is not fully
 * robust, so can sometimes result in {@link TopologyException}s being thrown.
 * For robust full-precision overlay see {@link OverlayNGRobust}.
 *
 * <p>
 * A custom {@link Noder} can be supplied. This allows using a more performant
 * noding strategy in specific cases, for instance in {@link CoverageUnion}.
 *
 * <p>
 * <b>Note:</b If a {@link SnappingNoder} is used it is best to specify a fairly
 * small snap tolerance, since the intersection clipping optimization can
 * interact with the snapping to alter the result.
 *
 * <p>
 * Optionally the overlay computation can process using strict mode (via
 * {@link #setStrictMode(boolean)}. In strict mode result semantics are:
 *
 * <ul>
 * <li>Lines and Points resulting from topology collapses are not included in
 * the result
 * <li>Result geometry is homogeneous for the {@link #INTERSECTION} and
 * {@link #DIFFERENCE} operations.
 * <li>Result geometry is homogeneous for the {@link #UNION} and
 * {@link #SYMDIFFERENCE} operations if the inputs have the same dimension
 * </ul>
 *
 * <p>
 ...
 * <li>Overlay operations are chainable without needing to remove
 ...
 *
 * <p>
 * If a robustness error occurs, a {@link TopologyException} is thrown. These
 * are usually caused by numerical rounding causing the noding output to not be
 * fully noded. For robust computation with full-precision
 * {@link OverlayNGRobust} can be used.
 *
 * @author mdavis
 * @see OverlayNGRobust
 */
public class OverlayNG {
	 ...
	 *
	 * <p>
	 * The original JTS overlay semantics used non-strict result semantics,
	 * including; - An Intersection result can be mixed-dimension, due to inclusion
	 * of intersection components of all dimensions - Results can include lines
	 * caused by Area topology collapse
	 */
	static final boolean STRICT_MODE_DEFAULT = false;

	/**
	 * Tests whether a point with a given topological {@link Label} relative to two
	 * geometries is contained in the result of overlaying the geometries using a
	 * given overlay operation.
	 *
	 * <p>
	...
	/**
	 * Tests whether a point with given {@link Location}s relative to two geometries
	 * would be contained in the result of overlaying the geometries using a given
	 * overlay operation. This is used to determine whether components computed
	 * during the overlay process should be included in the result geometry.
	 *
	 * <p>
	 * The method handles arguments of {@link Location#NONE} correctly.
	 *
	 ...
	 * @return true if a point with given locations is in the result of the overlay
	 *         operation
	 */
	...
	/**
	 * Computes an overlay operation for the given geometry operands, with the
	...
	/**
	 * Computes an overlay operation on the given geometry operands, using a
	 * supplied {@link Noder}.
	 *
	 * @param geom0
	 *            the first geometry argument
	 ...
	 *            the code for the desired overlay operation
	 * @param pm
	 *            the precision model to use (which may be null if the noder does
	 *            not use one)
	 * @param noder
	 *            the noder to use
	 * @return the result of the overlay operation
	...
	/**
	 * Computes an overlay operation on the given geometry operands, using the
	 ...
	 *
	 * <ul>
	 * <li>For {@link PrecisionModel#FIXED} a snap-rounding noder is used, and the
	 * computation is robust.
	 * <li>For {@link PrecisionModel#FLOATING} a non-snapping noder is used, and
	 * this computation may not be robust. If errors occur a
	 * {@link TopologyException} is thrown.
	 * </ul>
	 *
	 * @param geom0
	...
	/**
	 * Computes a union operation on the given geometry, with the supplied precision
	 * model.
	 *
	 * <p>
	 ...
	 * {@link UnaryUnionNG}. To union a polyonal coverage or linear network in a
	 * more performant way, use {@link CoverageUnion}.
	 *
	 * @param geom0
	 *            the geometry
	 * @param pm
	...
	/**
	 * Creates an overlay operation on the given geometries, with a defined
	 * precision model. The noding strategy is determined by the precision model.
	 *
	 * @param geom0
	...
	/**
	 * Creates an overlay operation on the given geometries using the precision
	...
	/**
	 * Sets whether overlay processing optimizations are enabled. It may be useful
	 * to disable optimizations for testing purposes. Default is TRUE (optimization
	 * enabled).
	 *
	 * @param isOptimized
	...
	/**
	 * Sets whether the result can contain only {@link Polygon} components. This is
	 * used if it is known that the result must be an (possibly empty) area.
	 *
	 ...
	 *             if the input is not supported (e.g. a mixed-dimension geometry)
	...
	private Geometry computeEdgeOverlay() {
		...
		/**
		 * Heuristic check on result area. Catches cases where noding causes vertex to
		 * move and make topology graph area "invert".
		 */
		if (OverlayUtil.isFloating(pm)) {
			boolean isAreaConsistent = OverlayUtil.isResultAreaConsistent(inputGeom.getGeometry(0),
					inputGeom.getGeometry(1), opCode, result);
			if (!isAreaConsistent)
	...
	/**
	 * Extracts the result geometry components from the fully labelled topology
	 * graph.
	 *
	 * <p>
	...
	private Geometry extractResult(int opCode, OverlayGraph graph) {
		...
		if (!isAreaResultOnly) {
			// --- Build lines
			boolean allowResultLines = !hasResultAreaComponents || isAllowMixedIntResult || opCode == SYMDIFFERENCE
					|| opCode == UNION;
			if (allowResultLines) {
			...
			/**
			 * Operations with point inputs are handled elsewhere. Only an Intersection op
			 * can produce point results from non-point inputs.
			 */
	...
	private Geometry createEmptyResult() {
		return OverlayUtil.createEmptyResult(
				OverlayUtil.resultDimension(opCode, inputGeom.getDimension(0), inputGeom.getDimension(1)), geomFact);
	}
}
