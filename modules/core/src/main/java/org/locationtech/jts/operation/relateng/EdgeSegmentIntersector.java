...
/**
 * Tests segments of {@link RelateSegmentString}s and if they intersect adds the
 * intersection(s) to the {@link TopologyComputer}.
 *
 * @author Martin Davis
 */
class EdgeSegmentIntersector implements SegmentIntersector {
	...
	private void addIntersections(RelateSegmentString ssA, int segIndexA, RelateSegmentString ssB, int segIndexB) {
		...
		for (int i = 0; i < li.getIntersectionNum(); i++) {
			 ...
			 * segments. Proper intersections lie on a unique segment so do not need to be
			 * checked. And it is important that the Containing Segment check not be used,
			 * since due to intersection computation roundoff, it is not reliable in that
			 * situation.
			 */
			if (li.isProper()
