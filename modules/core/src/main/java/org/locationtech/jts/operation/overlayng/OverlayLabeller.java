...
/**
 * Implements the logic to compute the full labeling for the edges in an
 * {@link OverlayGraph}.
 *
 * @author mdavis
 */
class OverlayLabeller {
	/**
	 * Finds all OverlayEdges which are linear (i.e. line or collapsed) and have a
	 * known location for the given input geometry.
	 *
	 * @param geomIndex
	...
	/**
	 * Finds a boundary edge for this geom originating at the given node, if one
	 * exists. A boundary edge should exist if this is a node on the boundary of the
	 * parent area geometry.
	 *
	 * @param nodeEdge
	...
	private static void propagateLinearLocationAtNode(OverlayEdge eNode, int geomIndex, boolean isInputLine,
		...
		// Debug.println("propagateLinearLocationAtNode ----- using location for " +
		// geomIndex + " from:
		// " + eNode);
		...
		do {
			...
			if (label.isLineLocationUnknown(geomIndex)) {
				...
				/**
				 * Add sym edge to stack for graph traversal (Don't add e itself, since e origin
				 * node has now been scanned)
				 */
	...
	public void computeLabelling() {
		 ...
		 * disconnected from the area edges of the parent. This can occur with a
		 * collapsed hole or shell. The edges can be labeled based on their parent ring
		 * role (shell or hole).
		 */
		labelCollapsedEdges();
		labelConnectedLinearEdges();
	...
	/**
	 * Labels edges around nodes based on the arrangement of incident area boundary
	 * edges. Also propagates the labeling to connected linear edges.
	 *
	 * @param nodes
	 *            the nodes to label
	 */
	...
	private void labelCollapsedEdge(OverlayEdge edge, int geomIndex) {
		...
		if (!label.isCollapse(geomIndex))
			return;
		/**
		 * This must be a collapsed edge which is disconnected from any area edges (e.g.
		 * a fully collapsed shell or hole). It can be labeled according to its parent
		 * source ring role.
		 */
	...
	/**
	 * At this point collapsed edges with unknown location must be disconnected from
	 * the boundary edges of the parent (because otherwise the location would have
	 * been propagated from them). They can be now located based on their parent
	 * ring role (shell or hole). (This cannot be done earlier, because the location
	 * based on the boundary edges must take precedence. There are situations where
	 * a collapsed edge has a location which is different to its ring role - e.g. a
	 * narrow gore in a polygon, which is in the interior of the reduced polygon,
	 * but whose ring role would imply the location EXTERIOR.)
	 *
	 * <p>
	 * Note that collapsed edges can NOT have location determined via a PIP location
	 * check, because that is done against the unreduced input geometry, which may
	 * give an invalid result due to topology collapse.
	 *
	 * <p>
	 * The labeling is propagated to other connected linear edges, since there may
	 * be NOT_PART edges which are connected, and they can be labeled in the same
	 * way. (These would get labeled anyway during subsequent disconnected labeling
	 * pass, but may be more efficient and accurate to do it here.)
	 */
	...
	/**
	 * There can be edges which have unknown location but are connected to a linear
	 * edge with known location. In this case linear location is propagated to the
	 * connected edges.
	 */
	...
	/**
	 * Determines the location of an edge relative to a target input geometry. The
	 * edge has no location information because it is disconnected from other edges
	 * that would provide that information. The location is determined by checking
	...
	private void labelDisconnectedEdge(OverlayEdge edge, int geomIndex) {
		...
		// Debug.print("BEFORE: " + edge.toStringNode());
		/**
		 * Locate edge in input area using a Point-In-Poly check. This should be safe
		 * even with precision reduction, because since the edge has remained
		 * disconnected its interior-exterior relationship can be determined relative to
	...
	/**
	 * At this point there may still be edges which have unknown location relative
	 * to an input geometry. This must be because they are NOT_PART edges for that
	 * geometry, and are disconnected from any edges of that geometry. An example of
	 * this is rings of one geometry wholly contained in another geometry. The
	 * location must be fully determined to compute a correct result for all overlay
	 * operations.
	 *
	 * <p>
	 * If the input geometry is an Area the edge location can be determined via a
	 * PIP test. If the input is not an Area the location is EXTERIOR.
	 */
	...
	/**
	 * Determines the {@link Location} for an edge within an Area geometry via
	 * point-in-polygon location.
	 *
	 * <p>
	 * NOTE this is only safe to use for disconnected edges, since the test is
	 * carried out against the original input geometry, and precision reduction may
	 * cause incorrect results for edges which are close enough to a boundary to
	 * become connected.
	 *
	 * @param geomIndex
	 *            the parent geometry index
	...
	/**
	 * Determines the {@link Location} for an edge within an Area geometry via
	 * point-in-polygon location, by checking that both endpoints are interior to
	 * the target geometry. Checking both endpoints ensures correct results in the
	 * presence of topology collapse.
	 *
	 * <p>
	 * NOTE this is only safe to use for disconnected edges, since the test is
	 * carried out against the original input geometry, and precision reduction may
	...
	private int locateEdgeBothEnds(int geomIndex, OverlayEdge edge) {
		/*
		 * To improve the robustness of the point location, check both ends of the edge.
	...
	/**
	 * Marks an edge which forms part of the boundary of the result area. This is
	 * determined by the overlay operation being executed, and the location of the
	 * edge. The relevant location is either the right side of a boundary edge, or
	 * the line location of a non-boundary edge.
	 *
	 * @param e
	 *            the edge to mark
	 * @param overlayOpCode
	...
	/**
	 * Scans around a node CCW, propagating the side labels for a given area
	 * geometry to all edges (and their sym) with unknown locations for that
	 * geometry.
	 *
	 * @param e2
	 * @param geomIndex
	...
	public void propagateAreaLocations(OverlayEdge nodeEdge, int geomIndex) {
		...
		if (!inputGeometry.isArea(geomIndex))
			return;
		/**
		 * No need to propagate if node has only one edge. This handles dangling edges
		 * created by overlap limiting.
		 */
		...
		// Debug.println("\npropagateSideLabels geomIndex = " + geomIndex + " : " +
		// eStart);
		...
		do {
			...
			if (!label.isBoundary(geomIndex)) {
				/**
				 * If this is not a Boundary edge for this input area, its location is now known
				 * relative to this input area
				 */
				...
				Assert.isTrue(label.hasSides(geomIndex));
				/**
				 * This is a boundary edge for the input area geom. Update the current location
				 * from its labels. Also check for topological consistency.
				 */
				int locRight = e.getLocation(geomIndex, Position.RIGHT);
				if (locRight != currLoc) {
					/*
					 * Debug.println("side location conflict: index= " + geomIndex + " R loc " +
					 * Location.toLocationSymbol(locRight) + " <>  curr loc " +
					 * Location.toLocationSymbol(currLoc) + " for " + e); //
					 */
	...
	/**
	 * Performs a breadth-first graph traversal to find and label connected linear
	 * edges.
	 *
	 * @param geomIndex
	 ...
	 * This has the effect of merging edge-adjacent result areas, as required by
