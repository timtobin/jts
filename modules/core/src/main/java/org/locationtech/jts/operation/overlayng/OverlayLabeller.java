...
class OverlayLabeller {
   ...
   *
   * <p>Note that collapsed edges can NOT have location determined via a PIP location check, because
   * that is done against the unreduced input geometry, which may give an invalid result due to
   * topology collapse.
   *
   * <p>The labeling is propagated to other connected linear edges, since there may be NOT_PART
   * edges which are connected, and they can be labeled in the same way. (These would get labeled
   * anyway during subsequent disconnected labeling pass, but may be more efficient and accurate to
   * do it here.)
   */
  ...
  private static void propagateLinearLocationAtNode(
      OverlayEdge eNode, int geomIndex, boolean isInputLine, Deque<OverlayEdge> edgeStack) {
    ...
    // Debug.println("propagateLinearLocationAtNode ----- using location for " + geomIndex + " from:
    // " + eNode);
   ...
   * overlay operations.
   *
   * <p>If the input geometry is an Area the edge location can be determined via a PIP test. If the
   ...
   * location.
   *
   * <p>NOTE this is only safe to use for disconnected edges, since the test is carried out against
   * the original input geometry, and precision reduction may cause incorrect results for edges
   ...
   * endpoints ensures correct results in the presence of topology collapse.
   *
   * <p>NOTE this is only safe to use for disconnected edges, since the test is carried out against
   * the original input geometry, and precision reduction may cause incorrect results for edges
   * which are close enough to a boundary to become connected.
