...
/**
 * Computes the location of points relative to a {@link Polygonal} {@link Geometry}, using a simple
 * <tt>O(n)</tt> algorithm.
 *
 * <p>The algorithm used reports if a point lies in the interior, exterior, or exactly on the
 * boundary of the Geometry.
 *
 * <p>Instance methods are provided to implement the interface {@link SimplePointInAreaLocator}.
 * However, they provide no performance advantage over the class methods.
 *
 * <p>This algorithm is suitable for use in cases where only a few points will be tested. If many
 * points will be tested, {@link IndexedPointInAreaLocator} may provide better performance.
 *
 * @version 1.7
 */
public class SimplePointInAreaLocator implements PointOnGeometryLocator {
  ...
  /**
   * Determines the {@link Location} of a point in an areal {@link Geometry}. The return value is
   * one of:
   *
   * <ul>
  ...
  /**
   * Determines whether a point is contained in a {@link Geometry}, or lies on its boundary. This is
   ...
   * </ul>
   *
   * This method is provided for backwards compatibility only. Use {@link #locate(Coordinate,
   * Geometry)} instead.
   *
   * @param p the point to test
  ...
  /**
   * Determines whether a point lies in a {@link Polygon}. If the point lies on the polygon boundary
  ...
  /**
   * Determines whether a point lies in a LinearRing, using the ring envelope to short-circuit if
   * possible.
   *
   * @param p the point to test
