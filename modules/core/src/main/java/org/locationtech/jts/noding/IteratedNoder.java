...
/**
 * Nodes a set of {@link NodedSegmentString}s completely. The set of segment strings is fully noded;
 * i.e. noding is repeated until no further intersections are detected.
 *
 * <p>Iterated noding using a FLOATING precision model is not guaranteed to converge, due to
 * roundoff error. This problem is detected and an exception is thrown. Clients can choose to rerun
 * the noding using a lower precision model.
 *
 * @version 1.7
 */
public class IteratedNoder implements Noder {
  ...
  /**
   * Sets the maximum number of noding iterations performed before the noding is aborted. Experience
   * suggests that this should rarely need to be changed from the default. The default is MAX_ITER.
   *
  ...
  /**
   * Fully nodes a list of {@link SegmentString}s, i.e. performs noding iteratively until no
   * intersections are found between segments. Maintains labelling of edges correctly through the
   * noding.
   *
  ...
  public void computeNodes(Collection segStrings) throws TopologyException {
    ...
    do {
      ...
      /**
       * Fail if the number of nodes created is not declining. However, allow a few iterations at
       * least before doing this
       */
