...
/**
 * Models a <b>Dimensionally Extended Nine-Intersection Model (DE-9IM)</b>
 * matrix. DE-9IM matrix values (such as "212FF1FF2") specify the topological
 * relationship between two {@link Geometry}s. This class can also represent
 * matrix patterns (such as "T*T******") which are used for matching instances
 * of DE-9IM matrices.
 *
 * <p>
 ...
 * {@link Location#INTERIOR}, {@link Location#BOUNDARY}, and
 * {@link Location#EXTERIOR}.
 *
 * <p>
 * When used to specify the topological relationship between two geometries, the
 * matrix entries represent the possible dimensions of each intersection:
 * {@link Dimension#A} = 2, {@link Dimension#L} = 1, {@link Dimension#P} = 0 and
 * {@link Dimension#FALSE} = -1. When used to represent a matrix pattern entries
 * can have the additional values {@link Dimension#TRUE} {"T") and
 * {@link Dimension#DONTCARE} ("*").
 *
 * <p>
 ...
 *
 * <ul>
 * <li><i><a href="http://www.opengis.org/techno/specs.htm">OGC 99-049 OpenGIS
 * Simple Features Specification for SQL</a></i> , Section 2.1.13
 * <li><i><a href=
 * "http://portal.opengeospatial.org/files/?artifact_id=25355">OGC 06-103r4
 * OpenGIS Implementation Standard for Geographic information - Simple feature
 * access - Part 1: Common architecture</a></i> , Section 6.1.15 (which provides
 * some further details on certain predicate specifications).
 ...
 * <li>test if a matrix (possibly with geometry dimensions) matches a standard
...
public class IntersectionMatrix implements Cloneable {
	...
	/**
	 * Tests if the dimension value matches <tt>TRUE</tt> (i.e. has value 0, 1, 2 or
	 * TRUE).
	 *
	 * @param actualDimensionValue
	 *            a number that can be stored in the <code>IntersectionMatrix</code>
	 *            . Possible values are
	...
	/**
	 * Tests if each of the actual dimension symbols in a matrix string satisfies
	 * the corresponding required dimension symbol in a pattern string.
	 *
	 * @param actualDimensionSymbols
	 *            nine dimension symbols to validate. Possible values are <code>
	 *     {T, F, * , 0, 1, 2}</code>.
	 * @param requiredDimensionSymbols
	 *            nine dimension symbols to validate against. Possible values are
	 *            <code>{T, F, * , 0, 1, 2}</code>.
	 * @return true if each of the required dimension symbols encompass the
	 ...
	 *            a character used in the string representation of an <code>
	 *     IntersectionMatrix</code>. Possible values are
	 *            <code>{T, F, * , 0, 1, 2}</code>.
	 ...
	 *
	 * @param im
	 *            an intersection matrix string value or pattern.
	...
	/**
	 * Creates an <code>IntersectionMatrix</code> with <code>FALSE</code> dimension
	 * values.
	 */
	 ...
	 *
	 * @param row
	 *            the row of this <code>IntersectionMatrix</code>, indicating the
	 *            interior, boundary or exterior of the first <code>Geometry</code>
	 * @param column
	 *            the column of this <code>IntersectionMatrix</code>, indicating the
	 *            interior, boundary or exterior of the second <code>Geometry</code>
	...
	/**
	 * Tests if this matrix matches <code>[T*F**F***]</code> or
	 * <code>[*TF**F***]</code> or <code>
	 * [**FT*F***]</code> or <code>[**F*TF***]</code>
	 *
	 * @return <code>true</code> if the first <code>Geometry</code> is covered by
	 *         the second
	 */
	public boolean isCoveredBy() {
		boolean hasPointInCommon = isTrue(matrix[Location.INTERIOR][Location.INTERIOR])
		...
		return hasPointInCommon && matrix[Location.INTERIOR][Location.EXTERIOR] == Dimension.FALSE
				&& matrix[Location.BOUNDARY][Location.EXTERIOR] == Dimension.FALSE;
	}

	/**
	 * Tests if this matrix matches <code>[T*****FF*]</code> or
	 * <code>[*T****FF*]</code> or <code>
	 * [***T**FF*]</code> or <code>[****T*FF*]</code>
	 *
	...
	public boolean isCovers() {
		...
		return hasPointInCommon && matrix[Location.EXTERIOR][Location.INTERIOR] == Dimension.FALSE
				&& matrix[Location.EXTERIOR][Location.BOUNDARY] == Dimension.FALSE;
	}
	...
	/**
	 * Tests whether the argument dimensions are equal and this matrix matches the
	 * pattern <tt>[T*F**FFF*]</tt>.
	 *
	 * <p>
	 * <b>Note:</b> This pattern differs from the one stated in <i>Simple feature
	 * access - Part 1: Common architecture</i>. That document states the pattern as
	 * <tt>[TFFFTFFFT]</tt>. This would specify that two identical <tt>POINT</tt>s
	 * are not equal, which is not desirable behaviour. The pattern used here has
	 * been corrected to compute equality in this situation.
	 *
	 * @param dimensionOfGeometryA
	 *            the dimension of the first <code>Geometry</code>
	 * @param dimensionOfGeometryB
	 *            the dimension of the second <code>Geometry</code>
	 * @return <code>true</code> if the two <code>Geometry</code>s related by this
	 *         matrix are equal; the <code>Geometry</code>s must have the same
	 ...
	 *            the dimension of the second <code>Geometry</code>
	 * @return <code>true</code> if the two <code>Geometry</code>s related by this
	 *         matrix overlap. For this function to return <code>true</code>, the
	...
	/**
	 * Tests if this matrix matches <code>[FT*******]</code>,
	 * <code>[F**T*****]</code> or <code>
	 * [F***T****]</code>.
	 *
	 * @param dimensionOfGeometryA
	 *            the dimension of the first <code>Geometry</code>
	 ...
	 *            the dimension of the second <code>Geometry</code>
	 * @return <code>true</code> if the two <code>Geometry</code> s related by this
	 *         matrix touch; Returns false if both <code>Geometry</code>s are
	 *         points.
	 */
	 ...
	 *
	 * @return <code>true</code> if the first <code>Geometry</code> is within the
	 *         second
	 */
	 ...
	 *
	 * @param pattern
	 *            A pattern containing nine dimension symbols with which to compare
	 *            the entries of this matrix. Possible symbol values are
	 *            <code>{T, F, * , 0, 1, 2}</code>.
	...
	/**
	 * Changes the elements of this <code>IntersectionMatrix</code> to the dimension
	 * symbols in <code>
	 * dimensionSymbols</code>.
	 *
	 * @param dimensionSymbols
	 *            nine dimension symbols to which to set this
	 *            <code>IntersectionMatrix
	...
	/**
	 * For each element in this <code>IntersectionMatrix</code>, changes the element
	 * to the corresponding minimum dimension symbol if the element is less.
	 *
	 * @param minimumDimensionSymbols
	 *            nine dimension symbols with which to compare the elements of this
	 *            <code>IntersectionMatrix</code>. The order of dimension values
	 ...
	 *            interior, boundary or exterior of the first <code>Geometry</code>
	 * @param column
	 *            the column of this <code>IntersectionMatrix</code> , indicating
	 *            the interior, boundary or exterior of the second
	 *            <code>Geometry</code>
	 * @param minimumDimensionValue
	 *            the dimension value with which to compare the element. The order
	 ...
	 * </code> .
	 *
	 * @return the nine dimension symbols of this <code>IntersectionMatrix</code> in
	 *         row-major order.
	 */
