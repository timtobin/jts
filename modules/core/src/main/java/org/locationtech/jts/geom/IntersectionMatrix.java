...
/**
 * Models a <b>Dimensionally Extended Nine-Intersection Model (DE-9IM)</b> matrix. DE-9IM matrix
 * values (such as "212FF1FF2") specify the topological relationship between two {@link Geometry}s.
 * This class can also represent matrix patterns (such as "T*T******") which are used for matching
 * instances of DE-9IM matrices.
 *
 * <p>DE-9IM matrices are 3x3 matrices with integer entries. The matrix indices {0,1,2} represent
 * the topological locations that occur in a geometry (Interior, Boundary, Exterior). These are
 * provided by the constants {@link Location#INTERIOR}, {@link Location#BOUNDARY}, and {@link
 * Location#EXTERIOR}.
 *
 * <p>When used to specify the topological relationship between two geometries, the matrix entries
 * represent the possible dimensions of each intersection: {@link Dimension#A} = 2, {@link
 * Dimension#L} = 1, {@link Dimension#P} = 0 and {@link Dimension#FALSE} = -1. When used to
 * represent a matrix pattern entries can have the additional values {@link Dimension#TRUE} {"T")
 * and {@link Dimension#DONTCARE} ("*").
 *
 * <p>For a description of the DE-9IM and the spatial predicates derived from it, see the following
 * references:
 *
 * <ul>
 *   <li><i><a href="http://www.opengis.org/techno/specs.htm">OGC 99-049 OpenGIS Simple Features
 *       Specification for SQL</a></i> , Section 2.1.13
 *   <li><i><a href="http://portal.opengeospatial.org/files/?artifact_id=25355">OGC 06-103r4 OpenGIS
 *       Implementation Standard for Geographic information - Simple feature access - Part 1: Common
 *       architecture</a></i> , Section 6.1.15 (which provides some further details on certain
 *       predicate specifications).
 *   <li>Wikipedia article on <a href="https://en.wikipedia.org/wiki/DE-9IM">DE-9IM</a>
 * </ul>
 *
 ...
 *   <li>test if a matrix (possibly with geometry dimensions) matches a standard named spatial
 *       predicate
 * </UL>
 *
 * @version 1.7
...
public class IntersectionMatrix implements Cloneable {
   ...
   *     {T, F, * , 0, 1, 2}</code>.
   * @param requiredDimensionSymbols nine dimension symbols to validate against. Possible values are
   *     <code>{T, F, * , 0, 1, 2}</code>.
  ...
  /**
   * Changes the elements of this <code>IntersectionMatrix</code> to the dimension symbols in <code>
   ...
   *     of dimension values from least to greatest is <code>{DONTCARE, TRUE, FALSE, 0, 1, 2}</code>
   *     .
   */
   ...
   *     this <code>IntersectionMatrix</code>. The order of dimension values from least to greatest
   ...
   *
   * @return <code>true</code> if the two <code>Geometry</code>s related by this matrix are disjoint
   */
  ...
  /**
   * Tests if this matrix matches <code>[FT*******]</code>, <code>[F**T*****]</code> or <code>
   * [F***T****]</code>.
   *
   ...
   * @return <code>true</code> if the two <code>Geometry</code> s related by this matrix touch;
   ...
   *
   * <p>The SFS defined this predicate only for P/L, P/A, L/L, and L/A situations. JTS extends the
   * definition to apply to L/P, A/P and A/L situations as well. This makes the relation symmetric.
   *
   * @param dimensionOfGeometryA the dimension of the first <code>Geometry</code>
   * @param dimensionOfGeometryB the dimension of the second <code>Geometry</code>
   * @return <code>true</code> if the two <code>Geometry</code>s related by this matrix cross.
   */
  ...
  /**
   * Tests if this matrix matches <code>[T*****FF*]</code> or <code>[*T****FF*]</code> or <code>
  ...
  /**
   * Tests if this matrix matches <code>[T*F**F***]</code> or <code>[*TF**F***]</code> or <code>
  ...
  /**
   * Tests whether the argument dimensions are equal and this matrix matches the pattern
   * <tt>[T*F**FFF*]</tt>.
   *
   * <p><b>Note:</b> This pattern differs from the one stated in <i>Simple feature access - Part 1:
   * Common architecture</i>. That document states the pattern as <tt>[TFFFTFFFT]</tt>. This would
   * specify that two identical <tt>POINT</tt>s are not equal, which is not desirable behaviour. The
   * pattern used here has been corrected to compute equality in this situation.
   *
   * @param dimensionOfGeometryA the dimension of the first <code>Geometry</code>
   * @param dimensionOfGeometryB the dimension of the second <code>Geometry</code>
   * @return <code>true</code> if the two <code>Geometry</code>s related by this matrix are equal;
   *     the <code>Geometry</code>s must have the same dimension to be equal
   */
   ...
   * @param dimensionOfGeometryA the dimension of the first <code>Geometry</code>
   * @param dimensionOfGeometryB the dimension of the second <code>Geometry</code>
   * @return <code>true</code> if the two <code>Geometry</code>s related by this matrix overlap. For
   *     this function to return <code>true</code>, the <code>Geometry</code>s must be two points,
   ...
   *
   * @param pattern A pattern containing nine dimension symbols with which to compare the entries of
   *     this matrix. Possible symbol values are <code>{T, F, * , 0, 1, 2}</code>.
