...
/**
 * Computes the minimum diameter of a {@link Geometry}. The minimum diameter is
 * defined to be the width of the smallest band that contains the geometry,
 * where a band is a strip of the plane defined by two parallel lines. This can
 * be thought of as the smallest hole that the geometry can be moved through,
 * with a single rotation.
 *
 * <p>
 * The first step in the algorithm is computing the convex hull of the Geometry.
 * If the input Geometry is known to be convex, a hint can be supplied to avoid
 * this computation.
 *
 * <p>
 ...
 * <li>the <b>minimum-width rectangle</b> of the input geometry. The rectangle
 * has width equal to the minimum diameter, and has one side parallel to the
 * supporting segment. In degenerate cases the rectangle may be a LineString or
 * a Point. (Note that this may not be the enclosing rectangle with minimum
 * area; use {@link MinimumAreaRectangle} to compute this.)
 * </ul>
 *
 * @see ConvexHull
...
public class MinimumDiameter {
	...
	private static LineSegment computeSegmentForLine(double a, double b, double c) {
		...
		Coordinate p1;
		/*
		 * Line eqn is ax + by = c Slope is a/b. If slope is steep, use y values as the
		 * inputs
		 */
	...
	/**
	 * Gets the minimum-width rectangular {@link Polygon} which encloses the input
	 * geometry and is based along the supporting segment. The rectangle has width
	 * equal to the minimum diameter, and a longer length. If the convex hull of the
	 * input is degenerate (a line or point) a {@link LineString} or {@link Point}
	 * is returned.
	 *
	 * <p>
	 * This is not necessarily the rectangle with minimum area. Use
	 * {@link MinimumAreaRectangle} to compute this.
	 *
	 * @param geom
	 *            the geometry
	...
	/**
	 * Compute a minimum diameter for a giver {@link Geometry}, with a hint if the
	 * Geometry is convex (e.g. a convex Polygon or LinearRing, or a two-point
	...
	/**
	 * Compute the width information for a ring of {@link Coordinate}s. Leaves the
	 * width information in the instance variables.
	 *
	 * @param pts
	 */
	...
	/**
	 * Gets the rectangular {@link Polygon} which encloses the input geometry and is
	 * based on the minimum diameter supporting segment. The rectangle has width
	 * equal to the minimum diameter, and a longer length. If the convex hull of the
	 * input is degenerate (a line or point) a {@link LineString} or {@link Point}
	 * is returned.
	 *
	...
	public Geometry getMinimumRectangle() {
		...
		LinearRing shell = inputGeom.getFactory().createLinearRing(new Coordinate[]{p0, p1, p2, p3, p0});
