 ...
 *
 * <p>
 * The major benefit of doing this is to reduce the number of points and the
 * complexity of shape which will be buffered. This improve performance and
 * robustness. It also reduces the risk of gores created by the quantized fillet
 * arcs (although this issue should be eliminated by the offset curve generation
 * logic).
 *
 * <p>
 ...
 *
 * <p>
 * Another important heuristic used is that the end segments of linear inputs
 * are never simplified. This ensures that the client buffer code is able to
 * generate end caps faithfully. Ring inputs can have end segments removed by
 * simplification.
 *
 * <p>
...
public class BufferInputLineSimplifier {
	...
	private boolean deleteShallowConcavities() {
		/**
		 * Do not simplify end line segments of lines. This ensures that end caps are
	 ...
	 * helps prevents the simplification from incrementally "skipping" over points
	...
	private boolean isShallowSampled(Coordinate p0, Coordinate p2, int i0, int i2, double distanceTol) {
		...
		for (int i = i0; i < i2; i += inc) {
			if (!isShallow(p0, inputLine[i], p2, distanceTol))
				return false;
		}
		return true;
	}
	 ...
	 *
	 * @param index
	 * @return the next non-deleted index, if any or inputLine.length if there are
	 *         no more non-deleted indices
	 */
