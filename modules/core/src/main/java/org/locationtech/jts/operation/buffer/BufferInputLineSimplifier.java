 ...
 *
 * <p>The major benefit of doing this is to reduce the number of points and the complexity of shape
 * which will be buffered. This improve performance and robustness. It also reduces the risk of
 * gores created by the quantized fillet arcs (although this issue should be eliminated by the
 * offset curve generation logic).
 *
 * <p>A key aspect of the simplification is that it affects inside (concave or inward) corners only.
 * Convex (outward) corners are preserved, since they are required to ensure that the generated
 * buffer curve lies at the correct distance from the input geometry.
 *
 * <p>Another important heuristic used is that the end segments of linear inputs are never
 * simplified. This ensures that the client buffer code is able to generate end caps faithfully.
 * Ring inputs can have end segments removed by simplification.
 *
 * <p>No attempt is made to avoid self-intersections in the output. This is acceptable for use for
 * generating a buffer offset curve, since the buffer algorithm is insensitive to invalid polygonal
 * geometry. However, this means that this algorithm cannot be used as a general-purpose polygon
...
public class BufferInputLineSimplifier {
  /**
   * Simplify the input coordinate list. If the distance tolerance is positive, concavities on the
   * LEFT side of the line are simplified. If the supplied distance tolerance is negative,
  ...
  /**
   * Uses a sliding window containing 3 vertices to detect shallow angles in which the middle vertex
   * can be deleted, since it does not affect the shape of the resulting buffer in a significant
   * way.
   *
  ...
  private boolean deleteShallowConcavities() {
    /**
     * Do not simplify end line segments of lines. This ensures that end caps are generated
     * consistently.
     */
   ...
   *
   * @param index
   * @return the next non-deleted index, if any or inputLine.length if there are no more non-deleted
   *     indices
   */
  private int nextIndex(int index) {
    int next = index + 1;
    while (next < inputLine.length && isDeleted[next]) next++;
    return next;
  }
  ...
  /**
   * Checks for shallowness over a sample of points in the given section. This helps prevents the
   * simplification from incrementally "skipping" over points which are in fact non-shallow.
   *
   ...
   * @return
   */
  private boolean isShallowSampled(
