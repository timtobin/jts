...
/**
 * Computes an overlay where one input is Point(s) and one is not. This class
 * supports overlay being used as an efficient way to find points within or
 * outside a polygon.
 *
 * <p>
 * Input semantics are:
 ...
 *
 * <ul>
 * <li>Points are rounded to the precision model (if present) <ii>An empty
 * result is an empty atomic geometry with dimension determined by the inputs
 * and the operation, as per overlay semantics
 * <li>
 * </ul>
 *
 ...
 *
 * <ul>
 * <li>Input points are not included in the noding of the non-point input
 * geometry (in particular, they do not participate in snap-rounding if that is
 * used).
 * <li>If the non-point input geometry is not included in the output it is not
 * rounded and noded. This means that points are compared to the non-rounded
 * geometry. This will be apparent in the result.
 * </ul>
 *
 * @author Martin Davis
 */
class OverlayMixedPoints {
	...
	/**
	 * Copy the non-point input geometry if not already done by precision reduction
	 * process.
	 *
