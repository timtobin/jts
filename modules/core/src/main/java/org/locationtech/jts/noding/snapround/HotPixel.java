 ...
 * lies in a unique hot pixel. It also matches the rounding semantics for numbers.
 *
 * <p>The hot pixel operations are all computed in the integer domain to avoid rounding problems.
 *
 * <p>Hot Pixels support being marked as nodes. This is used to prevent introducing nodes at line
 * vertices which do not have other lines snapped to them.
 *
 * @version 1.7
 */
public class HotPixel {
  ...
  /**
   * Creates a new hot pixel centered on a rounded point, using a given scale factor. The scale
   * factor must be strictly positive (non-zero).
   *
  ...
  private boolean intersectsScaled(double p0x, double p0y, double p1x, double p1y) {
     ...
     *
     * <p>Compute orientation WRT each pixel corner. If corner orientation == 0, segment intersects
     * the corner. From the corner and whether segment is heading up or down, can determine
     * intersection or not.
     *
     * <p>Otherwise, check whether segment crosses interior of pixel side This is the case if the
