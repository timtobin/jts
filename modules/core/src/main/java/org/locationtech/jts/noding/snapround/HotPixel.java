 ...
 * of width equal to the size of the scale factor. It is a partially open
 * region, which contains the interior of the tolerance square and the boundary
 * <b>minus</b> the top and right segments. This ensures that every point of the
 * space lies in a unique hot pixel. It also matches the rounding semantics for
 * numbers.
 *
 * <p>
...
public class HotPixel {
	...
	/**
	 * Creates a new hot pixel centered on a rounded point, using a given scale
	 * factor. The scale factor must be strictly positive (non-zero).
	 *
	 * @param pt
	 *            the coordinate at the centre of the pixel (already rounded)
	 * @param scaleFactor
	...
	/**
	 * Test whether a segment intersects the closure of this hot pixel. This is NOT
	 * the test used in the standard snap-rounding algorithm, which uses the
	 * partially-open tolerance square instead. This method is provided for testing
	 * purposes only.
	 *
	 * @param p0
	...
	private boolean intersectsScaled(double p0x, double p0y, double p1x, double p1y) {
		...
		/**
		 * Report false if segment env does not intersect pixel env. This check reflects
		 * the fact that the pixel Top and Right sides are open (not part of the pixel).
		 */
		// check Right side
		double maxx = hpx + TOLERANCE;
		...
		/**
		 * Vertical or horizontal segments must now intersect the segment interior or
		 * Left or Bottom sides.
		 */
		 ...
		 * segment intersects the corner. From the corner and whether segment is heading
		 * up or down, can determine intersection or not.
		 *
		 * <p>
		 * Otherwise, check whether segment crosses interior of pixel side This is the
		 * case if the orientations for each corner of the side are different.
		 */
	...
	/**
	 * Scale without rounding. This ensures intersections are checked against
	 * original linework. This is required to ensure that intersections are not
	 * missed because the segment is moved by snapping.
	 *
	 * @param val
	 * @return
	 */
