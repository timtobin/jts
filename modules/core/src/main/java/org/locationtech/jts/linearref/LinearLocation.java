 ...
 * The referenced geometry is not maintained within this location, but must be
 * provided for operations which require it. Various methods are provided to
 * manipulate the location value and query the geometry it references.
 */
public class LinearLocation implements Comparable {
	/**
	 * Compares two sets of location values for order.
	 *
	 * @param componentIndex0
	 *            a component index
	 * @param segmentIndex0
	 *            a segment index
	 * @param segmentFraction0
	 *            a segment fraction
	 * @param componentIndex1
	 *            another component index
	 * @param segmentIndex1
	 *            another segment index
	 * @param segmentFraction1
	 *            another segment fraction
	 * @return a negative integer, zero, or a positive integer as the first set of
	 *         location values is less than, equal to, or greater than the second
	 *         set of locationValues
	 */
	public static int compareLocationValues(int componentIndex0, int segmentIndex0, double segmentFraction0,
	...
	/**
	 * Gets the count of the number of line segments in a {@link LineString}. This
	 * is one less than the number of coordinates.
	 *
	 * @param line
	 *            a LineString
	...
	/**
	 * Computes the {@link Coordinate} of a point a given fraction along the line
	 * segment <tt>(p0,
	 * p1)</tt>. If the fraction is greater than 1.0 the last point of the segment
	 * is returned. If the fraction is less than or equal to 0.0 the first point of
	 * the segment is returned. The Z ordinate is interpolated from the Z-ordinates
	 * of the given points, if they are specified.
	 *
	 * @param p0
	 ...
	 *
	 * @param o
	 *            the <code>LineStringLocation</code> with which this
	...
	/**
	 * Gets the {@link Coordinate} along the given linear {@link Geometry} which is
	...
	/**
	 * Gets a {@link LineSegment} representing the segment of the given linear
	 * {@link Geometry} which contains this location.
	 *
	 * @param linearGeom
	 *            a linear geometry
	...
	/**
	 * Gets the length of the segment in the given Geometry containing this
	 * location.
	 *
	 * @param linearGeom
	 *            a linear geometry
	...
	/**
	 * Tests whether this location is an endpoint of the linear component it refers
	 * to.
	 *
	 * @param linearGeom
	 *            the linear geometry referenced by this location
	...
	/**
	 * Ensures the individual values are locally valid. Does <b>not</b> ensure that
	 * the indexes are valid for a particular linear geometry.
	 *
	 * @see clamp
	 */
	 ...
	 * {@link Geometry}, if the vertex is closer than <tt>minDistance</tt>.
	 *
	 * @param linearGeom
	 *            a linear geometry
	 * @param minDistance
	...
	/**
	 * Converts a linear location to the lowest equivalent location index. The
	 * lowest index has the lowest possible component and segment indices.
	 *
	 * <p>
	 * Specifically:
	 *
	 * <ul>
	 * <li>if the location point is an endpoint, a location value is returned as
	 * (nseg-1, 1.0)
	 * <li>if the location point is ambiguous (i.e. an endpoint and a startpoint),
	 * the lowest endpoint location is returned
	 * </ul>
	 *
	 * If the location index is already the lowest possible value, the original
