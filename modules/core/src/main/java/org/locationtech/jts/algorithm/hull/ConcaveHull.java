...
/**
 * Constructs a concave hull of a set of points. A concave hull is a concave or
 * convex polygon containing all the input points, whose vertices are a subset
 * of the vertices in the input. A given set of points has a sequence of hulls
 * of increasing concaveness, determined by a numeric target parameter.
 *
 * <p>
 ...
 *
 * <ul>
 * <li><b>Maximum Edge Length</b> - the length of the longest edge of the hull
 * is no larger than this value.
 * <li><b>Maximum Edge Length Ratio</b> - determines the Maximum Edge Length by
 * a fraction of the difference between the longest and shortest edge lengths in
 * the Delaunay Triangulation. This normalizes the <b>Maximum Edge Length</b> to
 * be scale-free. A value of 1 produces the convex hull; a value of 0 produces
 * maximum concaveness.
 * <li><b>Alpha</b> - produces Alpha-shapes, by removing border triangles with a
 * circumradius greater than alpha. Large values produce the convex hull; a
 * value of 0 produces maximum concaveness.
 * </ul>
 *
 * The preferred criterion is the <b>Maximum Edge Length Ratio</b>, since it is
 * scale-free and local (so that no assumption needs to be made about the total
 * amount of concaveness present).
 *
 * <p>
 * Other length criteria can be used by setting the Maximum Edge Length
 * directly. For example, use a length relative to the longest edge length in
 * the Minimum Spanning Tree of the point set. Or, use a length derived from the
 * {@link #uniformGridEdgeLength(Geometry)} value.
 *
 * <p>
 * The computed hull is always a single connected {@link Polygon} (unless it is
 * degenerate, in which case it will be a {@link Point} or a
 * {@link LineString}). This constraint may cause the concave hull to fail to
 * meet the target criterion.
 *
 * <p>
 * Optionally the concave hull can be allowed to contain holes by calling
 * {@link #setHolesAllowed(boolean)}.
 *
 * @author Martin Davis
 */
public class ConcaveHull {
	private static final int PARAM_ALPHA = 2;

	private static final int PARAM_EDGE_LENGTH = 1;

	/**
	 * Computes the alpha shape of a geometry as a polygon. The alpha parameter is
	 * the radius of the eroding disc.
	 *
	 * @param geom
	 *            the input geometry
	 * @param alpha
	 *            the radius of the eroding disc
	 * @param isHolesAllowed
	 *            whether holes are allowed in the result
	...
	/**
	 * Computes a concave hull of the vertices in a geometry using the target
	 * criterion of maximum edge length.
	 *
	 * @param geom
	 *            the input geometry
	 * @param maxLength
	...
	/**
	 * Computes a concave hull of the vertices in a geometry using the target
	 * criterion of maximum edge length, and optionally allowing holes.
	 *
	 * @param geom
	 *            the input geometry
	 * @param maxLength
	...
	/**
	 * Computes a concave hull of the vertices in a geometry using the target
	 * criterion of maximum edge length ratio. The edge length ratio is a fraction
	 * of the length difference between the longest and shortest edges in the
	 * Delaunay Triangulation of the input points.
	 *
	 * @param geom
	 *            the input geometry
	 * @param lengthRatio
	 *            the target edge length factor
	 * @return the concave hull
	 */
	...
	/**
	 * Computes a concave hull of the vertices in a geometry using the target
	 * criterion of maximum edge length factor, and optionally allowing holes. The
	 * edge length factor is a fraction of the length difference between the longest
	 * and shortest edges in the Delaunay Triangulation of the input points.
	 *
	 * @param geom
	 *            the input geometry
	 * @param maxLength
	...
	/**
	 * Finds tris which may be the start of holes. Only tris which have a long
	 * enough edge and which do not touch the current hull boundary are included.
	 * This avoids the risk of disconnecting the result polygon. The list is sorted
	 * in decreasing order of size.
	 *
	 * @param triList
	 * @param maxSizeInHull
	 *            maximum tri size which is not in a hole
	 * @return
	 */
	...
	/**
	 * Computes the approximate edge length of a uniform square grid having the same
	 * number of points as a geometry and the same area as its convex hull. This
	 * value can be used to determine a suitable length threshold value for
	 * computing a concave hull. A value from 2 to 4 times the uniform grid length
	...
	/**
	 * Adds a Tri to the queue. Only add tris with a single border edge, since
	 * otherwise that would risk isolating a vertex if the tri ends up being eroded
	 * from the hull. Sets the tri size according to the threshold parameter being
	 * used.
	 *
	 * @param tri
	 *            the Tri to add
	 * @param queue
	 *            the priority queue to add to
	 */
	...
	/**
	 * Computes the concave hull using edge length as the target criterion. The
	 * erosion is done in two phases: first the border, then any internal holes (if
	 * required). This allows an fast connection check to be used when eroding
	 * holes, which makes this much more efficient than the area-based algorithm.
	 *
	 * @param triList
	 */
	...
	/**
	 * Tests if a tri is included in the hull. Tris with size less than the maximum
	...
	private boolean isRemovableBorder(HullTri tri) {
		/**
		 * Tri must have exactly 2 adjacent tris (i.e. a single boundary edge). If it it
		 * has only 0 or 1 adjacent then removal would remove a vertex. If it has 3
		 * adjacent then it is not on border.
		 */
		if (tri.numAdjacent() != 2)
			return false;
		/**
		 * The tri cannot be removed if it is connecting, because this would create more
		 * than one result polygon.
		 */
	...
	private boolean isRemovableHole(HullTri tri) {
		 ...
		 * Tri must have exactly 2 adjacent tris (i.e. a single boundary edge). If it it
		 * has only 0 or 1 adjacent then removal would remove a vertex. If it has 3
		 * adjacent then it is not connected to hole.
		 */
		if (tri.numAdjacent() != 2)
			return false;
		/**
		 * Ensure removal does not disconnect hull area. This is a fast check which
		 * ensure holes and boundary do not touch at single points. (But it is slightly
		 * over-strict, since it prevents any touching holes.)
		 */
		return !tri.hasBoundaryTouch();
	}

	/**
	 * Erodes a hole starting at a given triangle, and eroding all adjacent
	 * triangles with boundary edge length above target.
	 *
	 * @param triList
	 *            the triangulation
	 * @param triHole
	 *            triangle which is a hole
	 */
	...
	/**
	 * Sets the alpha parameter to compute an alpha shape of the input. Alpha is the
	 * radius of the eroding disc. Border triangles with circumradius greater than
	 * alpha are removed.
	 *
	 * @param alpha
	 *            the alpha radius
	...
	/**
	 * Sets the target maximum edge length for the concave hull. The length value
	 * must be zero or greater.
	 *
	 * <ul>
	 * <li>The value 0.0 produces the concave hull of smallest area that is still
	 * connected.
	 * <li>Larger values produce less concave results. A value equal or greater than
	 * the longest Delaunay Triangulation edge length produces the convex hull.
	 * </ul>
	 *
	 * The {@link #uniformGridEdgeLength(Geometry)} value may be used as the basis
	 * for estimating an appropriate target maximum edge length.
	 *
	 * @param edgeLength
	...
	/**
	 * Sets the target maximum edge length ratio for the concave hull. The edge
	 * length ratio is a fraction of the difference between the longest and shortest
	 * edge lengths in the Delaunay Triangulation of the input points. It is a value
	 * in the range 0 to 1.
	 *
	 * <ul>
	 * <li>The value 0.0 produces a concave hull of minimum area that is still
	 * connected.
	 * <li>The value 1.0 produces the convex hull.
	 * <ul>
	 *
	 * @param edgeLengthRatio
