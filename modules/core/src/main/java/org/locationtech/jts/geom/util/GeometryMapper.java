...
public class GeometryMapper {
  ...
  /**
   * Maps the atomic elements of a {@link Geometry} (which may be atomic or composite) using a
   * {@link MapOp} mapping operation into an atomic <tt>Geometry</tt> or a flat collection of the
   * most specific type. <tt>null</tt> and empty values returned from the mapping operation are
   * discarded.
   *
  ...
  public static Geometry flatMap(Geometry geom, int emptyDim, MapOp op) {
    ...
    if (mapped.size() == 1) return mapped.getFirst();
    return geom.getFactory().buildGeometry(mapped);
  }
  ...
  /**
   * An interface for geometry functions that map a geometry input to a geometry output. The output
   * may be <tt>null</tt> if there is no valid output value for the given input value.
   *
   * @author Martin Davis
   */
