...
/**
 * Methods to map various collections of {@link Geometry}s via defined mapping
 * functions.
 *
 * @author Martin Davis
 */
public class GeometryMapper {
	...
	/**
	 * Maps the atomic elements of a {@link Geometry} (which may be atomic or
	 * composite) using a {@link MapOp} mapping operation into an atomic
	 * <tt>Geometry</tt> or a flat collection of the most specific type.
	 * <tt>null</tt> and empty values returned from the mapping operation are
	 * discarded.
	 *
	 * @param geom
	 *            the geometry to map
	 * @param emptyDim
	 *            the dimension of empty geometry to create
	 * @param op
	...
	/**
	 * Maps the members of a {@link Geometry} (which may be atomic or composite)
	 * into another <tt>Geometry</tt> of most specific type. <tt>null</tt> results
	 * are skipped. In the case of hierarchical {@link GeometryCollection}s, only
	 * the first level of members are mapped.
	 *
	 * @param geom
	...
	/**
	 * An interface for geometry functions that map a geometry input to a geometry
	 * output. The output may be <tt>null</tt> if there is no valid output value for
