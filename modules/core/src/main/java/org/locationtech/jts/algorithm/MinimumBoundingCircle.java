...
/**
 * Computes the <b>Minimum Bounding Circle</b> (MBC) for the points in a {@link Geometry}. The MBC
 * is the smallest circle which <tt>cover</tt>s all the input points (this is also known as the
 * <b>Smallest Enclosing Circle</b>). This is equivalent to computing the Maximum Diameter of the
 * input point set.
 *
 * <p>The computed circle can be specified in two equivalent ways, both of which are provide as
 * output by this class:
 *
 * <ul>
 *   <li>As a centre point and a radius
 *   <li>By the set of points defining the circle. Depending on the number of points in the input
 *       and their relative positions, this set contains from 0 to 3 points.
 *       <ul>
 ...
 * </ul>
 *
 * The class can also output a {@link Geometry} which approximates the shape of the Minimum Bounding
 * Circle (although as an approximation it is <b>not</b> guaranteed to <tt>cover</tt> all the input
 * points.)
 *
 * <p>The Maximum Diameter of the input point set can be computed as well. The Maximum Diameter is
 * defined by the pair of input points with maximum distance between them. The points of the maximum
 * diameter are two of the extremal points of the Minimum Bounding Circle. They lie on the convex
 * hull of the input. However, that the maximum diameter is not a diameter of the Minimum Bounding
 * Circle in the case where the MBC is defined by an inscribed triangle.
 *
 * @author Martin Davis
 * @see MinimumDiameter
 */
public class MinimumBoundingCircle {
  ...
  /**
   * Creates a new object for computing the minimum bounding circle for the point set defined by the
  ...
  /**
   * Gets a geometry which represents the Minimum Bounding Circle. If the input is degenerate (empty
   * or a single unique point), this method will return an empty geometry or a single Point
   * geometry. Otherwise, a Polygon will be returned which approximates the Minimum Bounding Circle.
   * (Note that because the computed polygon is only an approximation, it may not precisely contain
  ...
  public Geometry getCircle() {
    ...
    compute();
    if (centre == null) return input.getFactory().createPolygon();
    Point centrePoint = input.getFactory().createPoint(centre);
    if (radius == 0.0) return centrePoint;
    return centrePoint.buffer(radius);
  }

  /**
   * Gets a geometry representing the maximum diameter of the input. The maximum diameter is the
   * longest line segment between any two points of the input.
   *
   * <p>The points are two of the extremal points of the Minimum Bounding Circle. They lie on the
  ...
  public Geometry getMaximumDiameter() {
    ...
    return switch (extremalPts.length) {
      ...
      case 2 ->
          input.getFactory().createLineString(new Coordinate[] {extremalPts[0], extremalPts[1]});
      default -> {
  ...
  /**
   * Gets the extremal points which define the computed Minimum Bounding Circle. There may be zero,
   * one, two or three of these points, depending on the number of points in the input and the
   * geometry of those points.
   *
   * <ul>
   *   <li>0 or 1 points indicate an empty or trivial input point arrangement.
   *   <li>2 points define the diameter of the Minimum Bounding Circle.
   *   <li>3 points define an inscribed triangle of which the Minimum Bounding Circle is the
   *       circumcircle. The longest chords of the circle are the line segments [0-1] and [1-2]
   * </ul>
   *
  ...
  private void computeCirclePoints() {
    ...
    /**
     * The problem is simplified by reducing to the convex hull. Computing the convex hull also has
     * the useful effect of eliminating duplicate points
     */
    ...
    /**
     * Iterate over the remaining points to find a pair or triplet of points which determine the
     * minimal circle. By the design of the algorithm, at most <tt>pts.length</tt> iterations are
     * required to terminate with a correct result.
     */
  ...
  private static Coordinate pointWithMinAngleWithSegment(
      Coordinate[] pts, Coordinate P, Coordinate Q) {
