...
/**
 * A {@link CoordinateSequence} implementation based on a packed arrays. In this
 * implementation, {@link Coordinate}s returned by #toArray and #get are copies
 * of the internal values. To change the actual values, use the provided
 * setters.
 *
 * <p>
 * For efficiency, created Coordinate arrays are cached using a soft reference.
 * The cache is cleared each time the coordinate sequence contents are modified
...
public abstract class PackedCoordinateSequence implements CoordinateSequence, Serializable {
	...
	/**
	 * A soft reference to the Coordinate[] representation of this sequence. Makes
	 * repeated coordinate array accesses more efficient.
	 */
	 ...
	 *
	 * @param dimension
	 *            the total number of ordinates that make up a {@link Coordinate} in
	 *            this sequence.
	 * @param measures
	 *            the number of measure-ordinates each {@link Coordinate} in this
	 *            sequence has.
	 */
	 ...
	 * Warning: for performance reasons the ordinate index is not checked - if it is
	 * over dimensions you may not get an exception but a meaningless value.
	 *
	 * @param index
	 *            the coordinate index
	 * @param ordinate
	...
	public static class Double extends PackedCoordinateSequence {
		 ...
		 *
		 * @param coords
		 *            an array of <code>double</code> values that contains the ordinate
		 ...
		 *
		 * @param coords
		 *            an array of <code>float</code> values that contains the ordinate
		 *            values of the sequence
		 * @param dimension
		 *            the total number of ordinates that make up a {@link Coordinate} in
		 *            this sequence.
		...
		/**
		 * @see PackedCoordinateSequence#getOrdinate(int, int) Beware, for performance
		 *      reasons the ordinate index is not checked, if it's over dimensions you
	...
	public static class Float extends PackedCoordinateSequence {
		...
		/**
		 * @see PackedCoordinateSequence#getOrdinate(int, int) For performance reasons
		 *      the ordinate index is not checked. If it is larger than the dimension a
		 *      meaningless value may be returned.
		 */
		public double getOrdinate(int index, int ordinate) {
			return coords[index * dimension + ordinate];
		}
