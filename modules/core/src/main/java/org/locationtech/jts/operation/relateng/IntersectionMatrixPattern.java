 ...
 *
 * Matrix patterns are specified as a 9-character string containing the pattern symbols for the
 * DE-9IM 3x3 matrix entries, listed row-wise. The pattern symbols are:
 *
 * <ul>
...
public class IntersectionMatrixPattern {
  ...
  /**
   * A DE-9IM pattern to detect whether two polygonal geometries are adjacent along an edge, but do
   * not overlap.
   */
  ...
  /**
   * A DE-9IM pattern to detect if two geometries intersect in their interiors. This can be used to
   * determine if a polygonal coverage contains any overlaps (although not whether they are
   * correctly noded).
   */
