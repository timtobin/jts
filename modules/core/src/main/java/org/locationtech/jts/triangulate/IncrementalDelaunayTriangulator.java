...
/**
 * Computes a Delaunay Triangulation of a set of {@link Vertex}es, using an incremental insertion
 * algorithm.
 *
 * @author Martin Davis
 * @version 1.0
...
public class IncrementalDelaunayTriangulator {
   ...
   * the tolerance of the supplied subdivision.
   *
  ...
  /**
   * Sets whether the triangulation is forced to have a convex boundary. Because of the use of a
   * finite-size frame, this condition requires special logic to enforce. The default is true, since
   * this is a requirement for some uses of Delaunay Triangulations (such as Concave Hull
   * generation). However, forcing the triangulation boundary to be convex may cause the overall
   * frame triangulation to be non-Delaunay. This can cause a problem for Voronoi generation, so the
   * logic can be disabled via this method.
   *
  ...
  /**
   * Inserts all sites in a collection. The inserted vertices <b>MUST</b> be unique up to the
   * provided tolerance value. (i.e. no two vertices should be closer than the provided tolerance
   * value). They do not have to be rounded to the tolerance grid, however.
   *
   ...
   * affected edges so that the result is still a Delaunay triangulation.
   *
   * <p>
   *
  ...
  public QuadEdge insertSite(Vertex v) {
    ...
    /**
     * Examine suspect edges to ensure that the Delaunay condition is satisfied. If it is not, flip
     * the edge and continue scanning.
     *
     * <p>Since the frame is not infinitely far away, edges which touch the frame or are adjacent to
     * it require special logic to ensure the inner triangulation maintains a convex boundary.
     */
    do {
  ...
  /**
   * Tests if a edge touching a frame vertex creates a concavity in the triangulation boundary.
   *
  ...
  /**
   * Tests if the quadrilateral surrounding an edge is concave at the edge origin. Used to determine
   * if the triangulation boundary has a concavity.
   *
   * @param e
   * @return
   */
  ...
  /**
   * Edges whose adjacent triangles contain a frame vertex and the inserted vertex must not be
   * flipped.
   *
   * @param e the edge to test
