...
public class IncrementalDelaunayTriangulator {
	...
	/**
	 * Creates a new triangulator using the given {@link QuadEdgeSubdivision}. The
	 * triangulator uses the tolerance of the supplied subdivision.
	 *
	 * @param subdiv
	 ...
	 * and fixes the affected edges so that the result is still a Delaunay
	 * triangulation.
	 *
	 * <p>
	 *
	...
	public QuadEdge insertSite(Vertex v) {
		...
		/**
		 * This code is based on Guibas and Stolfi (1985), with minor modifications and
		 * a bug fix from Dani Lischinski (Graphic Gems 1993). (The modification I
		 * believe is the test for the inserted site falling exactly on an existing
		 * edge. Without this test zero-width triangles have been observed to be
		 * created)
		 */
		...
		/**
		 * Examine suspect edges to ensure that the Delaunay condition is satisfied. If
		 * it is not, flip the edge and continue scanning.
		 *
		 * <p>
		 * Since the frame is not infinitely far away, edges which touch the frame or
		 * are adjacent to it require special logic to ensure the inner triangulation
	 ...
	 * closer than the provided tolerance value). They do not have to be rounded to
	 * the tolerance grid, however.
	 *
	 * @param vertices
	...
	/**
	 * Edges whose adjacent triangles contain a frame vertex and the inserted vertex
	 * must not be flipped.
	 *
	 * @param e
	 *            the edge to test
	...
	/**
	 * Tests if a edge touching a frame vertex creates a concavity in the
