 ...
 *
 * Because it is not clear at this time what semantics for spatial analysis methods involving <code>
 * GeometryCollection</code>s would be useful, <code>GeometryCollection</code>s are not supported as
 * arguments to binary predicates or the <code>relate</code> method.
 *
 ...
 * heterogeneous a <code>GeometryCollection</code> will be returned.
 *
 * <p>Because it is not clear at this time what semantics for set-theoretic methods involving <code>
 * GeometryCollection</code>s would be useful, <code>GeometryCollections</code> are not supported as
 * arguments to the set-theoretic methods.
 *
 ...
 * of representing a point set as a <code>Geometry</code>.
 *
 * <p>The SFS does not specify an unambiguous representation of a given point set returned from a
 ...
 *
 * <UL>
 *   <LI>Simple means that the Geometry returned will be simple according to the JTS definition of
 *       <code>isSimple</code>.
 *   <LI>Noded applies only to overlays involving <code>LineString</code>s. It means that all
 *       intersection points on <code>LineString</code>s will be present as endpoints of <code>
 *       LineString</code>s in the result.
 * </UL>
 *
 * This definition implies that non-simple geometries which are arguments to spatial analysis
 * methods must be subjected to a line-dissolve process to ensure that the results are simple.
 *
 ...
 * to fit the <code>PrecisionModel</code>.
 *
 * <p>Unfortunately, truncating coordinates moves them slightly. Line segments which would not be
 ...
 * than it would in the exact result.
 *
 * <p>When JTS detects topology collapses during the computation of spatial analysis methods, it
 * will throw an exception. If possible the exception will report the location of the collapse.
 *
 * <p>
 *
 ...
 * comparison based on exact, structural pointwise equality. The {@link #equals(Object)} is a
 * synonym for this method, to provide structural equality semantics for use in Java collections. It
 * is important to note that structural pointwise equality is easily affected by things like ring
 * order and component order. In many situations it will be desirable to normalize geometries before
 ...
 * due to the potential for confusion with {@link #equals(Object)} its use is discouraged.
 *
 * <p>Since {@link #equals(Object)} and {@link #hashCode()} are overridden, Geometries can be used
 * effectively in Java collections.
 *
 * @version 1.7
 */
public abstract class Geometry implements Cloneable, Comparable, Serializable {
  @Serial private static final long serialVersionUID = 8763622679187376702L;
  ...
  private static final GeometryComponentFilter geometryChangedFilter =
      geom -> geom.geometryChangedAction();
   ...
   *
   * @param geometries an array of <code>Geometry</code>s; no elements may be <code>null</code>
   * @return <code>true</code> if any of the <code>Geometry</code>s <code>isEmpty</code> methods
   ...
   * @return <code>true</code> if any of <code>array</code>s elements are <code>null</code>
   */
   ...
   *
   * <p>JTS supports Spatial Reference System information in the simple way defined in the SFS. A
   * Spatial Reference System ID (SRID) is present in each <code>Geometry</code> object. <code>
   * Geometry</code> provides basic accessor operations for this field, but no others. The SRID is
   ...
   *
   * <p><b>NOTE:</b> This method should only be used for exceptional circumstances or for backwards
   * compatibility. Normally the SRID should be set on the {@link GeometryFactory} used to create
   * the geometry. SRIDs set using this method will <i>not</i> be propagated to geometries returned
  ...
  /**
   * Returns the number of {@link Geometry}s in a {@link GeometryCollection} (or 1, if the geometry
   * is not a collection).
   *
   ...
   * might be to add an object representing a Coordinate Reference System.
   *
   * <p>Note that user data objects are not present in geometries created by construction methods.
   *
   * @param userData an object, the semantics for which are defined by the application using this
   *     Geometry
   */
  ...
  /**
   * Returns a vertex of this geometry (usually, but not necessarily, the first one), or <code>null
   * </code> if the geometry is empty. The returned coordinate should not be assumed to be an actual
   * <code>Coordinate</code> object used in the internal representation.
   *
   ...
   *
   * <p>In general, the array cannot be assumed to be the actual internal storage for the vertices.
   * Thus modifying the array may not modify the geometry itself. Use the {@link
   * CoordinateSequence#setOrdinate} method (possibly on the components) to modify the underlying
   * data. If the coordinates are modified, {@link #geometryChanged} must be called afterwards.
   *
   ...
   * or other anomalous points.
   *
   * <p>Simplicity is defined for each {@link Geometry} subclass as follows:
   *
   * <ul>
   *   <li>Valid polygonal geometries are simple, since their rings must not self-intersect. <code>
   *       isSimple</code> tests for this condition and reports <code>false</code> if it is not met.
   *       (This is a looser test than checking for validity).
   *   <li>Linear rings have the same semantics.
   *   <li>Linear geometries are simple if they do not self-intersect at points other than boundary
   *       points.
   *   <li>Zero-dimensional geometries (points) are simple if they have no repeated points.
   ...
   * specification.
   *
   * <p>For validity rules see the Javadoc for the specific Geometry subclass.
   *
   ...
   *
   * <p>Note this test is for topological emptiness, not structural emptiness. A collection
   * containing only empty elements is reported as empty. To check structural emptiness use {@link
   * #getNumGeometries()}.
   *
  ...
  /**
   * Returns the minimum distance between this <code>Geometry</code> and another <code>Geometry
   * </code>.
   *
   ...
   *
   * <p>The centroid of an empty geometry is <code>POINT EMPTY</code>.
   *
   ...
   *
   * <p>The interior point of an empty geometry is <code>POINT EMPTY</code>.
   *
   ...
   * values are in the set {0,1,2}.
   *
   * <p>Note that this is a different concept to the dimension of the vertex {@link Coordinate}s.
   * The geometry dimension can never be greater than the coordinate dimension. For example, a
   * 0-dimensional geometry (e.g. a Point) may have a coordinate dimension of 3 (X,Y,Z).
   *
  ...
  /**
   * Tests whether an atomic geometry or any element of a collection has the specified dimension. In
   * particular, this can be used with mixed-dimension {@link GeometryCollection}s to test if they
   * contain an element of the specified dimension.
   *
  ...
  /**
   * Returns the boundary, or an empty geometry of appropriate dimension if this <code>Geometry
   * </code> is empty. (In the case of zero-dimensional geometries, ' an empty GeometryCollection is
   * returned.) For a discussion of this function, see the OpenGIS Simple Features Specification. As
   * stated in SFS Section 2.1.13.1, "the boundary of a Geometry is a set of Geometries of the next
   * lower dimension."
   *
   ...
   *     this object is the empty geometry. Returns <code>Dimension.FALSE</code> if the boundary is
   *     the empty geometry.
   */
   ...
   *
   * <p>If this <code>Geometry</code> is:
   *
   * <ul>
   ...
   *   <li>otherwise, returns a <code>Polygon</code> whose vertices are (minx miny, minx maxy, maxx
  ...
  /**
   * Gets an {@link Envelope} containing the minimum and maximum x and y values in this <code>
   * Geometry</code>. If the geometry is empty, an empty <code>Envelope</code> is returned.
   *
   ...
   *   <li><code>! g.intersects(this) = true</code> <br>
   *       (<code>disjoint</code> is the inverse of <code>intersects</code>)
   * </ul>
   *
   * @param g the <code>Geometry</code> with which to compare this <code>Geometry</code>
   * @return <code>true</code> if the two <code>Geometry</code>s are disjoint
   ...
   *
   * @param g the <code>Geometry</code> with which to compare this <code>Geometry</code>
   * @return <code>true</code> if the two <code>Geometry</code>s touch; Returns <code>false</code>
   ...
   *         <li><code>[****T****]</code>
   *       </ul>
   *   <li><code>! g.disjoint(this) = true</code> <br>
  ...
  public boolean intersects(Geometry g) {
     ...
     *
     * <p>- for P-A case: If P is in env(A), test for point-in-poly
     *
     * <p>- for A-A case: If env(A1).overlaps(env(A2)) test for overlaps via point-in-poly first
     * (both ways) Possibly optimize selection of point to test by finding point of A1 closest to
     * centre of env(A2). (Is there a test where we shouldn't bother - e.g. if env A is much smaller
   ...
   *
   * <p>The SFS defined this predicate only for P/L, P/A, L/L, and L/A situations. To make the
   * relation symmetric JTS extends the definition to apply to L/P, A/P and A/L situations as well.
   *
   * @param g the <code>Geometry</code> with which to compare this <code>Geometry</code>
   ...
   *   <li><code>g.contains(this) = true</code> <br>
   ...
   * geometry B, <code>A.within(B) = false</code> (As a concrete example, take A to be a LineString
   * which lies in the boundary of a Polygon B.) For a predicate with similar behaviour but avoiding
   * this subtle limitation, see {@link #coveredBy}.
   *
   * @param g the <code>Geometry</code> with which to compare this <code>Geometry</code>
   * @return <code>true</code> if this <code>Geometry</code> is within <code>g</code>
   ...
   *       [T*****FF*]</code>
   *   <li><code>g.within(this) = true</code> <br>
   ...
   * the boundary of a Polygon B.) For a predicate with similar behaviour but avoiding this subtle
   ...
   *       (for two points or two surfaces) or <code>[1*T***T**]</code> (for two curves)
   * </ul>
   *
   ...
   *         <li><code>[****T*FF*]</code>
   *       </ul>
   *   <li><code>g.coveredBy(this) = true</code> <br>
   ...
   * distinguish between points in the boundary and in the interior of geometries. For most
   * situations, <code>covers</code> should be used in preference to <code>contains</code>. As an
   * added benefit, <code>covers</code> is more amenable to optimization, and hence should be more
   * performant.
   *
   * @param g the <code>Geometry</code> with which to compare this <code>Geometry</code>
   ...
   *         <li><code>[**F*TF***]</code>
   *       </ul>
   *   <li><code>g.covers(this) = true</code> <br>
  ...
  /**
   * Tests whether the elements in the DE-9IM {@link IntersectionMatrix} for the two <code>Geometry
   * </code>s match the elements in <code>intersectionPattern</code>. The pattern is a 9-character
   * string, with symbols drawn from the following set:
   *
   * <UL>
   *   <LI>0 (dimension 0)
   *   <LI>1 (dimension 1)
   ...
   * </UL>
   *
   * For more information on the DE-9IM, see the <i>OpenGIS Simple Features Specification</i>.
   *
   * @param g the <code>Geometry</code> with which to compare this <code>Geometry</code>
   * @param intersectionPattern the pattern against which to check the intersection matrix for the
   *     two <code>Geometry</code>s
   ...
   *
   * @param g the <code>Geometry</code> with which to compare this <code>Geometry</code>
   * @return an {@link IntersectionMatrix} describing the intersections of the interiors, boundaries
   *     and exteriors of the two <code>Geometry</code>s
   */
   ...
   *
   * <p>This method is included for backward compatibility reasons. It has been superseded by the
   * {@link #equalsTopo(Geometry)} method, which has been named to clearly denote its functionality.
   *
   * <p>This method should NOT be confused with the method {@link #equals(Object)}, which implements
   ...
   * SFS <code>equals</code> predicate.
   *
   * <p>The SFS <code>equals</code> predicate has the following equivalent definitions:
   *
   * <ul>
   ...
   * </ul>
   *
   * <b>Note</b> that this method computes <b>topologically equality</b>. For structural equality,
   * see {@link #equalsExact(Geometry)}.
   *
   * @param g the <code>Geometry</code> with which to compare this <code>Geometry</code>
  ...
  /**
   * Tests whether this geometry is structurally and numerically equal to a given <code>Object
   * </code>. If the argument <code>Object</code> is not a <code>Geometry</code>, the result is
   * <code>false</code>. Otherwise, the result is computed using {@link #equalsExact(Geometry)}.
   *
   * <p>This method is provided to fulfill the Java contract for value-based object equality. In
   * conjunction with {@link #hashCode()} it provides semantics which are most useful for using
   * <code>Geometry</code>s as keys and values in Java collections.
   *
   * <p>Note that to produce the expected result the input geometries should be in normal form. It
   * is the caller's responsibility to perform this where required (using {@link Geometry#norm()} or
   ...
   * the Minkowski sum or difference of the geometry with a disc of radius <code>abs(distance)
   * </code>.
   *
   * <p>Mathematically-exact buffer area boundaries can contain circular arcs. To represent these
   * arcs using linear geometry they must be approximated with line segments. The buffer geometry is
   * constructed using 8 segments per quadrant to approximate the circular arcs. The end cap style
   * is <code>CAP_ROUND</code>.
   *
   * <p>The buffer operation always returns a polygonal result. The negative or zero-distance buffer
   * of lines and points is always an empty {@link Polygon}. This is also the result for the buffers
   * of degenerate (zero-area) polygons.
   *
   ...
   * accuracy of approximation for circular arcs.
   *
   * <p>Mathematically-exact buffer area boundaries can contain circular arcs. To represent these
   * arcs using linear geometry they must be approximated with line segments. The <code>
   * quadrantSegments</code> argument allows controlling the accuracy of the approximation by
   * specifying the number of line segments used to represent a quadrant of a circle
   *
   * <p>The buffer operation always returns a polygonal result. The negative or zero-distance buffer
   * of lines and points is always an empty {@link Polygon}. This is also the result for the buffers
   * of degenerate (zero-area) polygons.
  ...
  /**
   * Computes a buffer area around this geometry having the given width and with a specified
   * accuracy of approximation for circular arcs, and using a specified end cap style.
   *
   * <p>Mathematically-exact buffer area boundaries can contain circular arcs. To represent these
   * arcs using linear geometry they must be approximated with line segments. The <code>
   * quadrantSegments</code> argument allows controlling the accuracy of the approximation by
   * specifying the number of line segments used to represent a quadrant of a circle
   *
   * <p>The end cap style specifies the buffer geometry that will be created at the ends of
   * linestrings. The styles provided are:
   *
   * <ul>
  ...
  /**
   * Computes the smallest convex <code>Polygon</code> that contains all the points in the <code>
   * Geometry</code>. This obviously applies only to <code>Geometry</code> s which contain 3 or more
   * points; the results for degenerate cases are specified as follows:
   *
   * <TABLE>
   *    <TR>
  ...
  /**
   * Computes a <code>Geometry</code> representing the point-set which is common to both this <code>
   * Geometry</code> and the <code>other</code> Geometry.
   *
   * <p>The intersection of two geometries of different dimension produces a result geometry of
   * dimension less than or equal to the minimum dimension of the input geometries. The result
   * geometry may be a heterogeneous {@link GeometryCollection}. If the result is empty, it is an
   * atomic geometry with the dimension of the lowest input dimension.
   *
   * <p>Intersection of {@link GeometryCollection}s is supported only for homogeneous collection
   * types.
   *
   * <p>Non-empty heterogeneous {@link GeometryCollection} arguments are not supported.
   *
   ...
   * @throws TopologyException if a robustness error occurs
   * @throws IllegalArgumentException if the argument is a non-empty heterogeneous <code>
   *     GeometryCollection</code>
   */
   ...
   *
   * <p>The union of two geometries of different dimension produces a result geometry of dimension
   * equal to the maximum dimension of the input geometries. The result geometry may be a
   * heterogeneous {@link GeometryCollection}. If the result is empty, it is an atomic geometry with
   * the dimension of the highest input dimension.
   *
   * <p>Unioning {@link LineString}s has the effect of <b>noding</b> and <b>dissolving</b> the input
   * linework. In this context "noding" means that there will be a node or endpoint in the result
   * for every endpoint or line segment crossing in the input. "Dissolving" means that any duplicate
   * (i.e. coincident) line segments or portions of line segments will be reduced to a single line
   * segment in the result. If <b>merged</b> linework is required, the {@link LineMerger} class can
   * be used.
   *
   * <p>Non-empty {@link GeometryCollection} arguments are not supported.
   *
   * @param other the <code>Geometry</code> with which to compute the union
   * @return a point-set combining the points of this <code>Geometry</code> and the points of <code>
   *     other</code>
   * @throws TopologyException if a robustness error occurs
   ...
   * Geometry.
   *
   * <p>If the result is empty, it is an atomic geometry with the dimension of the left-hand input.
   *
   * <p>Non-empty {@link GeometryCollection} arguments are not supported.
   *
   * @param other the <code>Geometry</code> with which to compute the difference
   ...
   * Geometry, with the points in the <code>other</code> Geometry not contained in this <code>
   * Geometry</code>. If the result is empty, it is an atomic geometry with the dimension of the
   * highest input dimension.
   *
   * <p>Non-empty {@link GeometryCollection} arguments are not supported.
   *
   * @param other the <code>Geometry</code> with which to compute the symmetric difference
   * @return a Geometry representing the point-set symmetric difference of this <code>Geometry
   *     </code> with <code>other</code>
   * @throws TopologyException if a robustness error occurs
   * @throws IllegalArgumentException if either input is a non-empty GeometryCollection
   */
   ...
   *
   * <p>This method supports {@link GeometryCollection}s (which the other overlay operations
   * currently do not).
   *
   * <p>The result obeys the following contract:
   *
   * <ul>
   *   <li>Unioning a set of {@link LineString}s has the effect of fully noding and dissolving the
   *       linework.
   *   <li>Unioning a set of {@link Polygon}s always returns a {@link Polygonal} geometry (unlike
   *       {@link #union(Geometry)}, which may return geometries of lower dimension if a topology
   *       collapse occurred).
   * </ul>
   *
   ...
   * tolerance. Two Geometries are exactly equal within a distance tolerance if and only if:
   *
   * <ul>
   *   <li>they have the same structure
   *   <li>they have the same values for their vertices, within the given tolerance distance, in
   *       exactly the same order.
   * </ul>
   *
   * This method does <i>not</i> test the values of the <code>GeometryFactory</code>, the <code>SRID
   * </code>, or the <code>userData</code> fields.
   *
   * <p>To properly test equality between different geometries, it is usually necessary to {@link
   * #normalize()} them first.
   *
   * @param other the <code>Geometry</code> with which to compare this <code>Geometry</code>
   * @param tolerance distance at or below which two <code>Coordinate</code>s are considered equal
   * @return <code>true</code> if this and the other <code>Geometry</code> have identical structure
   *     and point values, up to the distance tolerance.
   * @see #equalsExact(Geometry)
   * @see #normalize()
   * @see #norm()
   */
   ...
   * <ul>
   *   <li>they have the same structure
   *   <li>they have the same values for their vertices, in exactly the same order.
   * </ul>
   *
   * This provides a stricter test of equality than {@link #equalsTopo(Geometry)}, which is more
   * useful in certain situations (such as using geometries as keys in collections).
   *
   * <p>This method does <i>not</i> test the values of the <code>GeometryFactory</code>, the <code>
   * SRID</code>, or the <code>userData</code> fields.
   *
   * <p>To properly test equality between different geometries, it is usually necessary to {@link
   * #normalize()} them first.
   *
   ...
   *
   * <p>This method is relatively expensive to compute. For maximum performance, the client should
   * instead perform normalization on the individual geometries at an appropriate point during
   * processing.
   *
   * @param g a Geometry
   ...
   *
   * @param filter the filter to apply to this <code>Geometry</code>'s coordinates
   */
  ...
  /**
   * Performs an operation with or on this <code>Geometry</code> and its subelement <code>Geometry
   * </code>s (if any). Only GeometryCollections and subclasses have subelement Geometry's.
   *
   * @param filter the filter to apply to this <code>Geometry</code> (and its children, if it is a
   ...
   * <tt>userData</tt>).
   *
   * <p><b>NOTE:</b> the userData object reference (if present) is copied, but the value itself is
   * not copied. If a deep copy is required this must be performed by the caller.
   *
   ...
   * extension of this ordering to sequences of coordinates.
   *
   * <p>NOTE that this method mutates the value of this geometry in-place. If this is not safe
   * and/or wanted, the geometry should be cloned prior to normalization.
   */
   ...
   * <code>Geometry</code>.
   *
   * <p>If their classes are different, they are compared using the following ordering:
   *
   * <UL>
   *   <LI>Point (lowest)
   *   <LI>MultiPoint
   ...
   * </UL>
   *
   * If the two <code>Geometry</code>s have the same class, their first elements are compared. If
   * those are the same, the second elements are compared, etc.
   *
   * @param o a <code>Geometry</code> with which to compare this <code>Geometry</code>
   * @return a positive number, 0, or a negative number, depending on whether this object is greater
   *     than, equal to, or less than <code>o</code>, as defined in "Normal Form For Geometry" in
   *     the JTS Technical Specifications
   */
  ...
  /**
   * Returns whether this <code>Geometry</code> is greater than, equal to, or less than another
   * <code>Geometry</code>, using the given {@link CoordinateSequenceComparator}.
   *
   * <p>If their classes are different, they are compared using the following ordering:
   *
   * <UL>
  ...
  /**
   * Returns whether the two <code>Geometry</code>s are equal, from the point of view of the <code>
   * equalsExact</code> method. Called by <code>equalsExact</code> . In general, two <code>Geometry
   * </code> classes are considered to be "equivalent" only if they are the same class. An exception
   * is <code>LineString</code> , which is considered to be equivalent to its subclasses.
   *
   * @param other the <code>Geometry</code> with which to compare this <code>Geometry</code> for
   *     equality
   ...
   * @throws IllegalArgumentException if <code>g</code> is a <code>GeometryCollection</code> but not
  ...
  /**
   * Returns the minimum and maximum x and y values in this <code>Geometry</code> , or a null <code>
   * Envelope</code> if this <code>Geometry</code> is empty. Unlike <code>getEnvelopeInternal</code>
   * , this method calculates the <code>Envelope</code> each time it is called; <code>
   * getEnvelopeInternal</code> caches the result of this method.
   *
   * @return this <code>Geometry</code>s bounding box; if the <code>Geometry</code> is empty, <code>
   *     Envelope#isNull</code> will return <code>true</code>
   */
  ...
  /**
   * Returns the first non-zero result of <code>compareTo</code> encountered as the two <code>
   * Collection</code>s are iterated over. If, by the time one of the iterations is complete, no
   * non-zero result has been encountered, returns 0 if the other iteration is also complete. If
   ...
   * @return the first non-zero <code>compareTo</code> result, if any; otherwise, zero
   */
  ...
  private Point createPointFromInternalCoord(Coordinate coord, Geometry exemplar) {
    ...
    if (coord == null) return exemplar.getFactory().createPoint();
