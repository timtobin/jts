 ...
 * methods involving <code>
 * GeometryCollection</code>s would be useful, <code>GeometryCollection</code>s
 * are not supported as arguments to binary predicates or the
 * <code>relate</code> method.
 *
 * <H3>Overlay Methods</H3>
 *
 * The overlay methods return the most specific class possible to represent the
 * result. If the result is homogeneous, a <code>Point</code>,
 ...
 * involving <code>
 * GeometryCollection</code>s would be useful, <code>GeometryCollections</code>
 ...
 * <code>Geometry</code>s returned from overlay methods. The canonical form is a
 * <code>Geometry</code> which is simple and noded:
 *
 * <UL>
 ...
 *
 * There are two ways of comparing geometries for equality: <b>structural
 * equality</b> and <b>topological equality</b>.
 *
 * <h4>Structural Equality</h4>
 *
 * Structural Equality is provided by the {@link #equalsExact(Geometry)} method.
 * This implements a comparison based on exact, structural pointwise equality.
 * The {@link #equals(Object)} is a synonym for this method, to provide
 * structural equality semantics for use in Java collections. It is important to
 * note that structural pointwise equality is easily affected by things like
 * ring order and component order. In many situations it will be desirable to
 * normalize geometries before comparing them (using the {@link #norm()} or
 * {@link #normalize()} methods). {@link #equalsNorm(Geometry)} is provided as a
 * convenience method to compute equality over normalized geometries, but it is
 * expensive to use. Finally, {@link #equalsExact(Geometry, double)} allows
 * using a tolerance value for point comparison.
 *
 * <h4>Topological Equality</h4>
 *
 * Topological Equality is provided by the {@link #equalsTopo(Geometry)} method.
 * It implements the SFS definition of point-set equality defined in terms of
 * the DE-9IM matrix. To support the SFS naming convention, the method
 * {@link #equals(Geometry)} is also provided as a synonym. However, due to the
 * potential for confusion with {@link #equals(Object)} its use is discouraged.
 *
 * <p>
...
public abstract class Geometry implements Cloneable, Comparable, Serializable {
	...
	/**
	 * Throws an exception if <code>g</code>'s type is a
	...
	/**
	 * An object reference which can be used to carry ancillary data defined by the
	 * client.
	 */
	...
	/**
	 * Performs an operation with or on this <code>Geometry</code>'s coordinates. If
	 * this method modifies any coordinate values, {@link #geometryChanged} must be
	 * called to update the geometry state. Note that you cannot use this method to
	 ...
	 * {@link CoordinateSequence}s. If the filter reports that a coordinate value
	 * has been changed, {@link #geometryChanged} will be called automatically.
	 *
	 * @param filter
	 *            the filter to apply
	 */
	 ...
	 * subelement <code>Geometry
	 * </code>s (if any). Only GeometryCollections and subclasses have subelement
	 * Geometry's.
	 *
	 * @param filter
	 ...
	 * a disc of radius <code>abs(distance)
	 * </code>.
	 *
	 * <p>
	 ...
	 * Mathematically-exact buffer area boundaries can contain circular arcs. To
	 * represent these arcs using linear geometry they must be approximated with
	 * line segments. The <code>
	 * quadrantSegments</code> argument allows controlling the accuracy of the
	 * approximation by specifying the number of line segments used to represent a
	 ...
	 * <li>{@link BufferParameters#CAP_FLAT} - a straight line perpendicular to the
	 * end segment
	 ...
	 * the two <code>
	 * Collection</code>s are iterated over. If, by the time one of the iterations
	 * is complete, no non-zero result has been encountered, returns 0 if the other
	 * iteration is also complete. If <code>b</code> completes before
	 * <code>a</code>, a positive number is returned; if a before b, a negative
	 * number.
	 *
	 * @param a
	 ...
	 * @param b
	 *            a <code>Collection</code> of <code>Comparable</code>s
	 * @return the first non-zero <code>compareTo</code> result, if any; otherwise,
	 *         zero
	 */
	...
	/**
	 * Returns whether this <code>Geometry</code> is greater than, equal to, or less
	 * than another <code>Geometry</code>.
	 *
	 * <p>
	 ...
	 * </UL>
	 *
	 * If the two <code>Geometry</code>s have the same class, their first elements
	 * are compared. If those are the same, the second elements are compared, etc.
	 *
	 * @param o
	 *            a <code>Geometry</code> with which to compare this
	 *            <code>Geometry</code>
	...
	/**
	 * Returns whether this <code>Geometry</code> is greater than, equal to, or less
	 * than another <code>Geometry</code>, using the given
	 ...
	 *            <code>Geometry</code>
	 * @param comp
	 *            a <code>CoordinateSequenceComparator</code>
	 * @return a positive number, 0, or a negative number, depending on whether this
	 *         object is greater than, equal to, or less than <code>o</code>, as
	 *         defined in "Normal Form For Geometry" in the JTS Technical
	 *         Specifications
	...
	/**
	 * Returns whether this <code>Geometry</code> is greater than, equal to, or less
	 * than another <code>Geometry</code> having the same class.
	 *
	 * @param o
	...
	/**
	 * Returns whether this <code>Geometry</code> is greater than, equal to, or less
	 * than another <code>Geometry</code> of the same class. using the given
	 * {@link CoordinateSequenceComparator}.
	 *
	 * @param o
	 *            a <code>Geometry</code> having the same class as this
	 ...
	 * , or a null <code>
	 * Envelope</code> if this <code>Geometry</code> is empty. Unlike
	 * <code>getEnvelopeInternal</code> , this method calculates the
	 * <code>Envelope</code> each time it is called; <code>
	 * getEnvelopeInternal</code> caches the result of this method.
	 *
	 * @return this <code>Geometry</code>s bounding box; if the
	 *         <code>Geometry</code> is empty, <code>
	 ...
	 * <li>The DE-9IM Intersection Matrix for the two geometries matches the pattern
	 * <code>
	 *       [T*****FF*]</code>
	 ...
	 * boundary of a geometry B, <code>
	 * B.contains(A) = false</code>. (As a concrete example, take A to be a
	 * LineString which lies in the boundary of a Polygon B.) For a predicate with
	 * similar behaviour but avoiding this subtle limitation, see {@link #covers}.
	 *
	 * @param g
	 ...
	 * points in the <code>
	 * Geometry</code>. This obviously applies only to <code>Geometry</code> s which
	 * contain 3 or more points; the results for degenerate cases are specified as
	 * follows:
	 *
	 * <TABLE>
	 * <TR>
	 ...
	 * </TABLE>
	 *
	 * @return the minimum-area convex polygon containing this
	...
	/**
	 * Creates a deep copy of this {@link Geometry} object. Coordinate sequences
	 * contained in it are copied. All instance fields are copied (i.e.
	 * <code>envelope</code>, <tt>SRID</tt> and <tt>userData</tt>).
	 *
	 * <p>
	 * <b>NOTE:</b> the userData object reference (if present) is copied, but the
	 * value itself is not copied. If a deep copy is required this must be performed
	 * by the caller.
	 *
	 ...
	 * <li>The DE-9IM Intersection Matrix for the two geometries matches at least
	 ...
	 *            the <code>Geometry</code> with which to compare this
	 *            <code>Geometry</code>
	 * @return <code>true</code> if this <code>Geometry</code> is covered by
	 *         <code>g</code>
	 * @see Geometry#within
	 * @see Geometry#covers
	 */
	 ...
	 * returns <code>
	 * true</code> for more cases). In particular, unlike <code>contains</code> it
	 * does not distinguish between points in the boundary and in the interior of
	 * geometries. For most situations, <code>covers</code> should be used in
	 * preference to <code>contains</code>. As an added benefit, <code>covers</code>
	 ...
	 * <li>The DE-9IM Intersection Matrix for the two geometries matches one of the
	 * following patterns:
	 * <ul>
	 ...
	 *
	 * <p>
	 * The SFS defined this predicate only for P/L, P/A, L/L, and L/A situations. To
	 * make the relation symmetric JTS extends the definition to apply to L/P, A/P
	 * and A/L situations as well.
	 *
	 * @param g
	 *            the <code>Geometry</code> with which to compare this
	 *            <code>Geometry</code>
	...
	/**
	 * Computes a <code>Geometry</code> representing the closure of the point-set of
	 * the points contained in this <code>Geometry</code> that are not contained in
	 * the <code>other</code> Geometry.
	 *
	 * <p>
	 * If the result is empty, it is an atomic geometry with the dimension of the
	 * left-hand input.
	 *
	 * <p>
	 ...
	 *
	 * @param other
	 *            the <code>Geometry</code> with which to compute the difference
	 * @return a Geometry representing the point-set difference of this
	 ...
	 *
	 * @param g
	 *            the <code>Geometry</code> from which to compute the distance
	 ...
	 *
	 * <p>
	 * This method is included for backward compatibility reasons. It has been
	 * superseded by the {@link #equalsTopo(Geometry)} method, which has been named
	 * to clearly denote its functionality.
	 *
	 * <p>
	 * This method should NOT be confused with the method {@link #equals(Object)},
	 ...
	 * <code>Object
	 * </code>. If the argument <code>Object</code> is not a <code>Geometry</code>,
	 * the result is <code>false</code>. Otherwise, the result is computed using
	 ...
	 * are most useful for using <code>Geometry</code>s as keys and values in Java
	 * collections.
	 *
	 * <p>
	 ...
	 * </ul>
	 *
	 * This provides a stricter test of equality than {@link #equalsTopo(Geometry)},
	 * which is more useful in certain situations (such as using geometries as keys
	 * in collections).
	 *
	 * <p>
	 * This method does <i>not</i> test the values of the
	 * <code>GeometryFactory</code>, the <code>
	 * SRID</code>, or the <code>userData</code> fields.
	 *
	 * <p>
	 * To properly test equality between different geometries, it is usually
	 * necessary to {@link #normalize()} them first.
	 *
	 * @param other
	 *            the <code>Geometry</code> with which to compare this
	 *            <code>Geometry</code>
	 * @return <code>true</code> if this and the other <code>Geometry</code> have
	 *         identical structure and point values.
	 ...
	 *
	 * This method does <i>not</i> test the values of the
	 * <code>GeometryFactory</code>, the <code>SRID
	 ...
	 *            <code>Geometry</code>
	 * @param tolerance
	 *            distance at or below which two <code>Coordinate</code>s are
	 *            considered equal
	 * @return <code>true</code> if this and the other <code>Geometry</code> have
	 *         identical structure and point values, up to the distance tolerance.
	 * @see #equalsExact(Geometry)
	 * @see #normalize()
	 * @see #norm()
	 */
	 ...
	 * geometries before computing {@link #equalsExact(Geometry)}.
	 *
	 * <p>
	 * This method is relatively expensive to compute. For maximum performance, the
	 * client should instead perform normalization on the individual geometries at
	 * an appropriate point during processing.
	 *
	 * @param g
	 *            a Geometry
	 * @return true if the input geometries are exactly equal in their normalized
	 *         form
	 */
	 ...
	 *            the <code>Geometry</code> with which to compare this
	 *            <code>Geometry</code>
	 * @return <code>true</code> if the two <code>Geometry</code>s are topologically
	 *         equal
	 * @see #equalsExact(Geometry)
	 */
	 ...
	 * party (for example, via a {@link CoordinateFilter}). When this method is
	 * called the geometry will flush and/or update any derived information it has
	 * cached (such as its {@link Envelope} ). The operation is applied to all
	...
	/**
	 * Notifies this Geometry that its Coordinates have been changed by an external
	 * party. When #geometryChanged is called, this method will be called for this
	 * Geometry and its component Geometries.
	 *
	 ...
	 * <code>Geometry
	 * </code> is empty. (In the case of zero-dimensional geometries, ' an empty
	 * GeometryCollection is returned.) For a discussion of this function, see the
	 * OpenGIS Simple Features Specification. As stated in SFS Section 2.1.13.1,
	 * "the boundary of a Geometry is a set of Geometries of the next lower
	 * dimension."
	 *
	 * @return the closure of the combinatorial boundary of this
	 *         <code>Geometry</code>
	 */
	...
	/**
	 * Computes the centroid of this <code>Geometry</code>. The centroid is equal to
	 * the centroid of the set of component Geometries of highest dimension (since
	 ...
	 * one), or <code>null
	 * </code> if the geometry is empty. The returned coordinate should not be
	 * assumed to be an actual <code>Coordinate</code> object used in the internal
	 * representation.
	 *
	 ...
	 * <li>otherwise, returns a <code>Polygon</code> whose vertices are (minx miny,
	...
	/**
	 * Gets an {@link Envelope} containing the minimum and maximum x and y values in
	 * this <code>
	 * Geometry</code>. If the geometry is empty, an empty <code>Envelope</code> is
	 * returned.
	 *
	 * <p>
	 * The returned object is a copy of the one maintained internally, to avoid
	 * aliasing issues. For best performance, clients which access this envelope
	 * frequently should cache the return value.
	 *
	...
	/**
	 * Computes an interior point of this <code>Geometry</code>. An interior point
	 * is guaranteed to lie in the interior of the Geometry, if it possible to
	 ...
	 * <code>Geometry</code> s contained by composite <code>Geometry</code>s must be
	 ...
	 * <code>Geometry</code> object. <code>
	 * Geometry</code> provides basic accessor operations for this field, but no
	 * others. The SRID is represented as an integer.
	 *
	 * @return the ID of the coordinate space in which the <code>Geometry</code> is
	 *         defined.
	 */
	public int getSRID() {
		return SRID;
	...
	/**
	 * Tests whether an atomic geometry or any element of a collection has the
	 * specified dimension. In particular, this can be used with mixed-dimension
	 * {@link GeometryCollection}s to test if they contain an element of the
	 * specified dimension.
	 *
	 * @param dim
	...
	/**
	 * Computes a <code>Geometry</code> representing the point-set which is common
	 * to both this <code>
	 ...
	 * geometry of dimension less than or equal to the minimum dimension of the
	 * input geometries. The result geometry may be a heterogeneous
	 * {@link GeometryCollection}. If the result is empty, it is an atomic geometry
	 * with the dimension of the lowest input dimension.
	 *
	 * <p>
	 * Intersection of {@link GeometryCollection}s is supported only for homogeneous
	 * collection types.
	 *
	 * <p>
	 * Non-empty heterogeneous {@link GeometryCollection} arguments are not
	 * supported.
	 *
	 * @param other
	 *            the <code>Geometry</code> with which to compute the intersection
	 * @return a Geometry representing the point-set common to the two
	 *         <code>Geometry</code>s
	 * @throws TopologyException
	 *             if a robustness error occurs
	 * @throws IllegalArgumentException
	 *             if the argument is a non-empty heterogeneous <code>
	...
	public boolean intersects(Geometry g) {
		 ...
		 *
		 * <p>
		 * - for A-A case: If env(A1).overlaps(env(A2)) test for overlaps via
		 * point-in-poly first (both ways) Possibly optimize selection of point to test
		 * by finding point of A1 closest to centre of env(A2). (Is there a test where
		 * we shouldn't bother - e.g. if env A is much smaller than env B, maybe there's
		 * no point in testing pt(B) in env(A)?
		 */
	 ...
	 *
	 * <p>
	 * Note this test is for topological emptiness, not structural emptiness. A
	 * collection containing only empty elements is reported as empty. To check
	 * structural emptiness use {@link #getNumGeometries()}.
	 *
	 * @return <code>true</code> if this <code>Geometry</code> does not cover any
	 *         points
	 */
	public abstract boolean isEmpty();

	/**
	 * Returns whether the two <code>Geometry</code>s are equal, from the point of
	 * view of the <code>
	 * equalsExact</code> method. Called by <code>equalsExact</code> . In general,
	 * two <code>Geometry
	 * </code> classes are considered to be "equivalent" only if they are the same
	 * class. An exception is <code>LineString</code> , which is considered to be
	 * equivalent to its subclasses.
	 *
	 * @param other
	 *            the <code>Geometry</code> with which to compare this
	 *            <code>Geometry</code> for equality
	 * @return <code>true</code> if the classes of the two <code>Geometry</code> s
	 *         are considered to be equal by the <code>equalsExact</code> method.
	 */
	...
	/**
	 * Tests whether this {@link Geometry} is simple. The SFS definition of
	 ...
	 *
	 * <ul>
	 * <li>Valid polygonal geometries are simple, since their rings must not
	 * self-intersect. <code>
	 *       isSimple</code> tests for this condition and reports <code>false</code>
	 * if it is not met. (This is a looser test than checking for validity).
	 * <li>Linear rings have the same semantics.
	 * <li>Linear geometries are simple if they do not self-intersect at points
	 * other than boundary points.
	 * <li>Zero-dimensional geometries (points) are simple if they have no repeated
	 * points.
	...
	/**
	 * Tests whether this <code>Geometry</code> is topologically valid, according to
	...
	/**
	 * Tests whether the distance from this <code>Geometry</code> to another is less
	...
	/**
	 * Converts this <code>Geometry</code> to <b>normal form</b> (or <b> canonical
	 * form</b> ). Normal form is a unique representation for <code>Geometry</code>
	 * s. It can be used to test whether two <code>Geometry</code>s are equal in a
	 * way that is independent of the ordering of the coordinates within them.
	 * Normal form equality is a stronger condition than topological equality, but
	 * weaker than pointwise equality. The definitions for normal form use the
	 * standard lexicographical ordering for coordinates. "Sorted in order of
	 * coordinates" means the obvious extension of this ordering to sequences of
	 * coordinates.
	 *
	 * <p>
	 * NOTE that this method mutates the value of this geometry in-place. If this is
	 * not safe and/or wanted, the geometry should be cloned prior to normalization.
	 */
	 ...
	 * <code>false</code>. This predicate is symmetric.
	 *
	 * @param g
	 *            the <code>Geometry</code> with which to compare this
	 *            <code>Geometry</code>
	 ...
	 *            the <code>Geometry</code> with which to compare this
	 *            <code>Geometry</code>
	 * @return an {@link IntersectionMatrix} describing the intersections of the
	 *         interiors, boundaries and exteriors of the two <code>Geometry</code>s
	 */
	 ...
	 * two <code>Geometry
	 * </code>s match the elements in <code>intersectionPattern</code>. The pattern
	 * is a 9-character string, with symbols drawn from the following set:
	 *
	 * <UL>
	 * <LI>0 (dimension 0)
	 * <LI>1 (dimension 1)
	 ...
	 *            the pattern against which to check the intersection matrix for the
	 *            two <code>Geometry</code>s
	 * @return <code>true</code> if the DE-9IM intersection matrix for the two
	 *         <code>Geometry</code>s match <code>intersectionPattern</code>
	...
	/**
	 * Computes a new geometry which has all component coordinate sequences in
	 * reverse order (opposite orientation) to this one.
	 *
	 ...
	 * {@link GeometryFactory} used to create the geometry. SRIDs set using this
	 * method will <i>not</i> be propagated to geometries returned by constructive
	 * methods.
	 *
	 * @see GeometryFactory
	 */
	 ...
	 *
	 * @param userData
	 *            an object, the semantics for which are defined by the application
	 *            using this Geometry
	 */
	 ...
	 * contained in the <code>other</code> Geometry, with the points in the
	 * <code>other</code> Geometry not contained in this <code>
	 * Geometry</code>. If the result is empty, it is an atomic geometry with the
	 * dimension of the highest input dimension.
	 *
	 * <p>
	 * Non-empty {@link GeometryCollection} arguments are not supported.
	 *
	 * @param other
	 *            the <code>Geometry</code> with which to compute the symmetric
	 *            difference
	 * @return a Geometry representing the point-set symmetric difference of this
	 *         <code>Geometry
	...
	/**
	 * Returns the Well-known Text representation of this <code>Geometry</code>. For
	 * a definition of the Well-known Text format, see the OpenGIS Simple Features
	 * Specification.
	 *
	 ...
	 * <code>false</code>, since points have only interiors. This predicate is
	 * symmetric.
	 *
	 * @param g
	 *            the <code>Geometry</code> with which to compare this
	 *            <code>Geometry</code>
	 * @return <code>true</code> if the two <code>Geometry</code>s touch; Returns
	 *         <code>false</code> if both <code>Geometry</code>s are points
	 */
	 ...
	 *
	 * <p>
	 * This method supports {@link GeometryCollection}s (which the other overlay
	 * operations currently do not).
	 *
	 * <p>
	 * The result obeys the following contract:
	 *
	 * <ul>
	 * <li>Unioning a set of {@link LineString}s has the effect of fully noding and
	 * dissolving the linework.
	 * <li>Unioning a set of {@link Polygon}s always returns a {@link Polygonal}
	 * geometry (unlike {@link #union(Geometry)}, which may return geometries of
	 * lower dimension if a topology collapse occurred).
	 * </ul>
	 *
	 ...
	 * contained in both this <code>Geometry</code> and the <code>other</code>
	 * Geometry.
	 *
	 * <p>
	 * The union of two geometries of different dimension produces a result geometry
	 * of dimension equal to the maximum dimension of the input geometries. The
	 * result geometry may be a heterogeneous {@link GeometryCollection}. If the
	 * result is empty, it is an atomic geometry with the dimension of the highest
	 * input dimension.
	 *
	 * <p>
	 * Unioning {@link LineString}s has the effect of <b>noding</b> and
	 * <b>dissolving</b> the input linework. In this context "noding" means that
	 * there will be a node or endpoint in the result for every endpoint or line
	 * segment crossing in the input. "Dissolving" means that any duplicate (i.e.
	 * coincident) line segments or portions of line segments will be reduced to a
	 * single line segment in the result. If <b>merged</b> linework is required, the
	 * {@link LineMerger} class can be used.
	 *
	 * <p>
	 * Non-empty {@link GeometryCollection} arguments are not supported.
	 *
	 ...
	 * @return a point-set combining the points of this <code>Geometry</code> and
	 *         the points of <code>
	 *     other</code>
	 * @throws TopologyException
	 *             if a robustness error occurs
	 ...
	 * points in the boundary of a geometry B, <code>A.within(B) = false</code> (As
	 * a concrete example, take A to be a LineString which lies in the boundary of a
	 * Polygon B.) For a predicate with similar behaviour but avoiding this subtle
	 * limitation, see {@link #coveredBy}.
	 *
	 * @param g
	 *            the <code>Geometry</code> with which to compare this
	 *            <code>Geometry</code>
