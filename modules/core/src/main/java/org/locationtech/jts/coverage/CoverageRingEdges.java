...
/**
 * Models a polygonal coverage as a set of unique {@link CoverageEdge}s, linked
 * to the parent rings in the coverage polygons. Each edge has either one or two
 * parent rings, depending on whether it is an inner or outer edge of the
 * coverage. The source coverage is represented as a array of polygonal
 * geometries (either {@link Polygon}s or {@link MultiPolygon}s).
 *
 * <p>
 * Coverage edges are found by identifying vertices which are nodes in the
 * coverage, splitting edges at nodes, and then identifying unique edges. The
 * unique edges are associated to their parent ring (in order), to allow
 * reforming the coverage polygons.
 *
 * <p>
 * One ring in each polygonal geometry is marked as primary. Primary edges are
 * always retained during coverage processing (such as simplification), to
 * ensure the geometry does not disappear entirely. The primary ring is the
 * shell of the largest polygon in the geometry.
 *
 * @author Martin Davis
 */
class CoverageRingEdges {
	 ...
	 *
	 * @param coverage
	 *            the set of polygonal geometries in the coverage
	...
	private final List<CoverageEdge> edges;

	private final Map<LinearRing, List<CoverageEdge>> ringEdgesMap;
	...
	/**
	 * Detects nodes occurring at vertices which are between a boundary segment and
	 * an inner (shared) segment. These occur where two polygons are adjacent at the
	 * coverage boundary (this is not detected by
	 * {@link #findMultiRingNodes(Geometry[])}).
	 *
	 * @param ring
	 *            the ring to extract edges from
	 * @param boundarySegs
	 *            the coverage boundary segments
	 * @param nodes
	 *            nodes in the coverage
	 */
	...
	/**
	 * Adds the {@link CoverageEdge}s for a ring to the map of ring edges.
	 *
	 * @param index
	 *            the index of the geometry in the coverage
	 * @param ring
	 *            the ring to extract edges from
	 * @param isPrimary
	 *            true if the ring is primary (must not be removed)
	 * @param nodes
	 *            nodes in the coverage
	 * @param boundarySegs
	 *            the coverage boundary segments
	 ...
	 *            map of unique edges
	 */
	private void addRingEdges(int index, LinearRing ring, boolean isPrimary, Set<Coordinate> nodes,
			Set<LineSegment> boundarySegs, HashMap<LineSegment, CoverageEdge> uniqueEdgeMap) {
	...
	private LinearRing buildRing(LinearRing ring) {
		...
		boolean isRemoved = ringEdges.size() == 1 && ringEdges.getFirst().getCoordinates().length == 0;
		if (isRemoved)
			return null;

		CoordinateList ptsList = new CoordinateList();
		for (int i = 0; i < ringEdges.size(); i++) {
			Coordinate lastPt = !ptsList.isEmpty() ? ptsList.getCoordinate(ptsList.size() - 1) : null;
	 ...
	 *
	 * @param ring
	 *            ring to create edge for
	 * @param start
	 *            start index of ring section; -1 indicates edge is entire ring
	 * @param end
	 *            end index of ring section
	 * @param index
	 *            the index of the parent geometry in the coverage
	 * @param isPrimary
	 *            true if the ring is primary (must not be removed)
	 * @param uniqueEdgeMap
	 *            map of edges
	 ...
	 */
	private CoverageEdge createEdge(Coordinate[] ring, int start, int end, int index, boolean isPrimary,
	 ...
	 */
	private List<CoverageEdge> extractRingEdges(int index, LinearRing ring, boolean isPrimary,
			HashMap<LineSegment, CoverageEdge> uniqueEdgeMap, Set<Coordinate> nodes) {
	...
	/**
	 * Finds nodes occurring between boundary segments. Nodes on boundaries occur at
	 * vertices which have 3 or more incident boundary segments. This detects
	 * situations where two rings touch only at a vertex (i.e. two polygons touch,
	 * or a polygon shell touches a hole) These nodes lie in only 2 adjacent rings,
	...
	private Set<Coordinate> findBoundaryNodes(Set<LineSegment> boundarySegments) {
		...
		return counter.entrySet().stream().filter(e -> e.getValue() > 2).map(Map.Entry::getKey)
	 ...
	 *
	 * @param geom
	 *            the polygonal geometry to scan
	 ...
	 *
	 * @param coverage
	 *            a list of polygonal geometries
