...
class RelateSegmentString extends BasicSegmentString {
  ...
  public static RelateSegmentString createLine(
      Coordinate[] pts, boolean isA, int elementId, RelateGeometry parent) {
    return createSegmentString(pts, isA, Dimension.L, elementId, -1, null, parent);
  }

  public static RelateSegmentString createRing(
      Coordinate[] pts,
      boolean isA,
      int elementId,
      int ringId,
  ...
  private static RelateSegmentString createSegmentString(
      Coordinate[] pts,
      boolean isA,
      int dim,
      int elementId,
  ...
  public NodeSection createNodeSection(int segIndex, Coordinate intPt) {
    ...
    NodeSection a =
        new NodeSection(
            isA, dimension, id, ringId, parentPolygonal, isNodeAtVertex, prev, intPt, next);
    return a;
  }
  ...
  private Coordinate prevVertex(int segIndex, Coordinate pt) {
    ...
    if (!segStart.equals2D(pt)) return segStart;
    // -- pt is at segment start, so get previous vertex
    if (segIndex > 0) return getCoordinate(segIndex - 1);
    if (isClosed()) return prevInRing(segIndex);
    return null;
  }
  ...
  private Coordinate nextVertex(int segIndex, Coordinate pt) {
    ...
    if (!segEnd.equals2D(pt)) return segEnd;
    // -- pt is at seg end, so get next vertex
    if (segIndex < size() - 2) return getCoordinate(segIndex + 2);
    if (isClosed()) return nextInRing(segIndex + 1);
  ...
  public boolean isContainingSegment(int segIndex, Coordinate pt) {
    ...
    if (pt.equals2D(getCoordinate(segIndex))) return true;
    if (pt.equals2D(getCoordinate(segIndex + 1))) {
      boolean isFinalSegment = segIndex == size() - 2;
      if (isClosed() || !isFinalSegment) return false;
