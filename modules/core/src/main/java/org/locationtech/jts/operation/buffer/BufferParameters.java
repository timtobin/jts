...
/**
 * A value class containing the parameters which specify how a buffer should be
 * constructed.
 *
 * <p>
...
public class BufferParameters {
	 ...
	 * the accuracy of the default Quadrant Segments parameter.
	 */
	...
	/**
	 * Creates a set of parameters with the given quadrantSegments and endCapStyle
	 * values.
	 *
	 * @param quadrantSegments
	 *            the number of quadrant segments to use
	...
	/**
	 * Gets the number of quadrant segments which will be used to approximate angle
	 * fillets in round endcaps and joins.
	 *
	 ...
	 * {@link #CAP_ROUND}, {@link #CAP_FLAT}, and {@link #CAP_SQUARE}. The default
	 * is {@link #CAP_ROUND}.
	 *
	 * @param endCapStyle
	...
	/**
	 * Sets the join style for outside (reflex) corners between line segments. The
	 * styles supported are {@link #JOIN_ROUND}, {@link #JOIN_MITRE} and {link
	 * JOIN_BEVEL}. The default is {@link #JOIN_ROUND}.
	 *
	 * @param joinStyle
	 ...
	 * angle fillets in round endcaps and joins. The value should be at least 1.
	 *
	 * <p>
	 * This determines the error in the approximation to the true buffer curve. The
	 * default value of 8 gives less than 2% error in the buffer distance. For a
	 * error of &lt; 1%, use QS = 12. For a error of &lt; 0.1%, use QS = 18. The
	 * error is always less than the buffer distance (in other words, the computed
	 * buffer curve is always inside the true curve).
	 *
	 * @param quadSegs
	...
	/**
	 * Sets the factor used to determine the simplify distance tolerance for input
	 * simplification. Simplifying can increase the performance of computing
	 * buffers. Generally the simplify factor should be greater than 0. Values
	 * between 0.01 and .1 produce relatively good accuracy for the generate buffer.
