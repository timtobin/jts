 ...
 *
 * <p>The parameters allow control over:
 *
 * <ul>
...
public class BufferParameters {
  ...
  /**
   * Gets the number of quadrant segments which will be used to approximate angle fillets in round
   * endcaps and joins.
   *
  ...
  /**
   * Sets the number of line segments in a quarter-circle used to approximate angle fillets in round
   * endcaps and joins. The value should be at least 1.
   *
   * <p>This determines the error in the approximation to the true buffer curve. The default value
   * of 8 gives less than 2% error in the buffer distance. For a error of &lt; 1%, use QS = 12. For
   * a error of &lt; 0.1%, use QS = 18. The error is always less than the buffer distance (in other
   * words, the computed buffer curve is always inside the true curve).
   *
   ...
   * #CAP_ROUND}, {@link #CAP_FLAT}, and {@link #CAP_SQUARE}. The default is {@link #CAP_ROUND}.
   *
  ...
  /**
   * Sets the join style for outside (reflex) corners between line segments. The styles supported
   * are {@link #JOIN_ROUND}, {@link #JOIN_MITRE} and {link JOIN_BEVEL}. The default is {@link
   * #JOIN_ROUND}.
   *
   ...
   * on only one side of each input line.
   *
   * <p>The side used is determined by the sign of the buffer distance:
   *
   * <ul>
   ...
   *
   * <p>The End Cap Style for single-sided buffers is always ignored, and forced to the equivalent
   * of <tt>CAP_FLAT</tt>.
   *
