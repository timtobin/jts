...
/**
 * Models a triangle formed from {@link QuadEdge}s in a
 * {@link QuadEdgeSubdivision} which forms a triangulation. The class provides
 * methods to access the topological and geometric properties of the triangle
 * and its neighbours in the triangulation. Triangle vertices are ordered in CCW
 * orientation in the structure.
 *
 * <p>
 * QuadEdgeTriangles support having an external data attribute attached to them.
 * Alternatively, this class can be subclassed and attributes can be defined in
 * the subclass. Subclasses will need to define their own
 * <tt>BuilderVisitor</tt> class and <tt>createOn</tt> method.
 *
 * @author Martin Davis
 * @version 1.0
 */
public class QuadEdgeTriangle {
	...
	public static boolean contains(QuadEdge[] tri, Coordinate pt) {
		Coordinate[] ring = new Coordinate[]{tri[0].orig().getCoordinate(), tri[1].orig().getCoordinate(),
	...
	public static boolean contains(Vertex[] tri, Coordinate pt) {
		Coordinate[] ring = new Coordinate[]{tri[0].getCoordinate(), tri[1].getCoordinate(), tri[2].getCoordinate(),
				tri[0].getCoordinate()};
		return PointLocation.isInRing(pt, ring);
	}
	 ...
	 * {@link QuadEdgeSubdivision} representing a triangulation. The <tt>data</tt>
	 * attributes of the {@link QuadEdge}s in the subdivision will be set to point
	 * to the triangle which contains that edge. This allows tracing the neighbour
	...
	public static Geometry toPolygon(QuadEdge[] e) {
		Coordinate[] ringPts = new Coordinate[]{e[0].orig().getCoordinate(), e[1].orig().getCoordinate(),
	...
	public static Geometry toPolygon(Vertex[] v) {
		Coordinate[] ringPts = new Coordinate[]{v[0].getCoordinate(), v[1].getCoordinate(), v[2].getCoordinate(),
				v[0].getCoordinate()};
		GeometryFactory fact = new GeometryFactory();
		LinearRing ring = fact.createLinearRing(ringPts);
		Polygon tri = fact.createPolygon(ring);
	 ...
	 * @param e
	 *            a QuadEdge
	 * @return the index of the edge in this triangle or -1 if the edge is not an
	 ...
	 * @return the index of the edge starting at the vertex or -1 if the vertex is
	 *         not in the triangle
	 */
	...
	/**
	 * Gets the neighbours of this triangle. If there is no neighbour triangle, the
	...
	/**
	 * Gets the triangles which are adjacent (include) to a given vertex of this
	 * triangle.
	 *
	 * @param vertexIndex
	 *            the vertex to query
