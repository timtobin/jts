 ...
 * {@link Coordinate} or {@link Geometry}. An affine transformation is a mapping of the 2D plane
 * into itself via a series of transformations of the following basic types:
 *
 * <ul>
 ...
 * </ul>
 *
 * In general, affine transformations preserve straightness and parallel lines, but do not preserve
 * distance or shape.
 *
 * <p>An affine transformation can be represented by a 3x3 matrix in the following form:
 *
 * <blockquote>
 *
 * <pre>
 ...
 * </blockquote>
 *
 * A coordinate P = (x, y) can be transformed to a new coordinate P' = (x', y') by representing it
 * as a 3x1 matrix and using matrix multiplication to compute:
 *
 * <blockquote>
 *
 * <pre>
 ...
 *
 * Affine transformations can be composed using the {@link #compose} method. Composition is computed
 * via multiplication of the transformation matrices, and is defined as:
 *
 * <blockquote>
 *
 * <code>
 ...
 * </blockquote>
 *
 * This produces a transformation whose effect is that of A followed by B. The methods {@link
 * #reflect}, {@link #rotate}, {@link #scale}, {@link #shear}, and {@link #translate} have the
 * effect of composing a transformation of that type with the transformation they are invoked on.
 *
 * <p>The composition of transformations is in general <i>not</i> commutative.
 *
 * <h3>Transformation Inversion</h3>
 *
 * Affine transformations may be invertible or non-invertible. If a transformation is invertible,
 * then there exists an inverse transformation which when composed produces the identity
 * transformation. The {@link #getInverse} method computes the inverse of a transformation, if one
 * exists.
 *
 * @author Martin Davis
 */
public class AffineTransformation implements Cloneable, CoordinateSequenceFilter {
   ...
   */
  public static AffineTransformation reflectionInstance(
   ...
   * @return a transformation for the rotation
   */
  public static AffineTransformation rotationInstance(
   ...
   * @return a transformation for the scaling
   */
  public static AffineTransformation scaleInstance(
   ...
   * @return this transformation, with an updated matrix
   */
  public AffineTransformation setTransformation(
      double m00, double m01, double m02, double m10, double m11, double m12) {
    this.m00 = m00;
    this.m01 = m01;
    this.m02 = m02;
   ...
   * entries are returned, in the sequence:
   *
   * <pre>
   ...
   * </blockquote>
   *
   * If the determinant is zero, the transform is singular (not invertible), and operations which
   * attempt to compute an inverse will throw a <tt>NoninvertibleTransformException</tt>.
   *
   ...
   * exists this method will throw a <tt>NoninvertibleTransformationException</tt>.
   *
   * <p>The matrix of the inverse is equal to the inverse of the matrix for the transformation. It
   ...
   * transformation for a reflection is computed by:
   *
   * <blockquote>
   *
   * <code>
   ...
   * angle <tt>theta</tt> has the value:
   *
   * <blockquote>
   *
   * <pre>
   ...
   * </blockquote>
   *
   * Note that a shear of (1, 1) is <i>not</i> equal to shear(1, 0) composed with shear(0, 1).
   * Instead, shear(1, 1) corresponds to a mapping onto the line x = y.
   *
   ...
   * transformation matrix has the value:
   *
   * <blockquote>
   *
   * <pre>
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof AffineTransformation trans)) return false;
