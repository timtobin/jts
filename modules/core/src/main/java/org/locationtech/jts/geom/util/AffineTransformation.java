...
/**
 * Represents an affine transformation on the 2D Cartesian plane. It can be used
 * to transform a {@link Coordinate} or {@link Geometry}. An affine
 ...
 * by representing it as a 3x1 matrix and using matrix multiplication to
 * compute:
 *
 * <blockquote>
 *
 ...
 * Composition is computed via multiplication of the transformation matrices,
 * and is defined as:
 *
 * <blockquote>
 *
 ...
 * </blockquote>
 *
 * This produces a transformation whose effect is that of A followed by B. The
 * methods {@link #reflect}, {@link #rotate}, {@link #scale}, {@link #shear},
 * and {@link #translate} have the effect of composing a transformation of that
 * type with the transformation they are invoked on.
 *
 * <p>
 ...
 *
 * Affine transformations may be invertible or non-invertible. If a
 * transformation is invertible, then there exists an inverse transformation
 * which when composed produces the identity transformation. The
 * {@link #getInverse} method computes the inverse of a transformation, if one
 * exists.
 *
 * @author Martin Davis
 */
public class AffineTransformation implements Cloneable, CoordinateSequenceFilter {
	...
	/**
	 * Creates a transformation for a rotation by an angle <i>theta</i>, specified
	 * by the sine and cosine of the angle. This allows providing exact values for
	 * sin(theta) and cos(theta) for the common case of rotations of multiples of
	 * quarter-circles.
	 *
	 * @param sinTheta
	...
	/**
	 * Creates a transformation for a rotation about the point (x,y) by an angle
	 * <i>theta</i>, specified by the sine and cosine of the angle. This allows
	 * providing exact values for sin(theta) and cos(theta) for the common case of
	...
	/**
	 * Constructs a transformation which maps the given source points into the given
	 * destination points.
	 *
	 * @param src0
	 *            source point 0
	 ...
	 */
	public AffineTransformation(Coordinate src0, Coordinate src1, Coordinate src2, Coordinate dest0, Coordinate dest1,
			Coordinate dest2) {
	...
	/**
	 * Updates this transformation to be the composition of this transformation with
	 * the given {@link AffineTransformation}. This produces a transformation whose
	 * effect is equal to applying this transformation followed by the argument
	 * transformation. Mathematically,
	 *
	 * <blockquote>
	 *
	 * <code>
	 ...
	 * {@link AffineTransformation} with this transformation. This produces a
	 * transformation whose effect is equal to applying the argument transformation
	 * followed by this transformation. Mathematically,
	 *
	 * <blockquote>
	 *
	 * <code>
	...
	public boolean equals(Object obj) {
		...
		return m00 == trans.m00 && m01 == trans.m01 && m02 == trans.m02 && m10 == trans.m10 && m11 == trans.m11
				&& m12 == trans.m12;
	}
	 ...
	 * </blockquote>
	 *
	 * If the determinant is zero, the transform is singular (not invertible), and
	 * operations which attempt to compute an inverse will throw a
	...
	/**
	 * Computes the inverse of this transformation, if one exists. The inverse is
	 * the transformation which when composed with this one produces the identity
	 * transformation. A transformation has an inverse if and only if it is not
	 * singular (i.e. its determinant is non-zero). Geometrically, an transformation
	 * is non-invertible if it maps the plane to a line or a point. If no inverse
	 * exists this method will throw a
	 * <tt>NoninvertibleTransformationException</tt>.
	 *
	 * <p>
	 * The matrix of the inverse is equal to the inverse of the matrix for the
	 * transformation. It is computed as follows:
	 *
	 * <blockquote>
	 *
	 * <pre>
	...
	/**
	 * Gets an array containing the entries of the transformation matrix. Only the 6
	 * non-trivial entries are returned, in the sequence:
	 *
	 * <pre>
	...
	/**
	 * Reports that this filter should continue to be executed until all coordinates
	...
	/**
	 * Updates the value of this transformation to that of a rotation transformation
	 * composed with the current value. Positive angles correspond to a rotation in
	...
	/**
	 * Updates the value of this transformation to that of a rotation around the
	 * origin composed with the current value, with the sin and cos of the rotation
	 * angle specified directly.
	 *
	 * @param sinTheta
	 ...
	 * point composed with the current value. Positive angles correspond to a
	...
	/**
	 * Updates the value of this transformation to that of a rotation around a given
	 * point composed with the current value, with the sin and cos of the rotation
	 * angle specified directly.
	 *
	 * @param sinTheta
	 *            the sine of the angle to rotate by
	...
	/**
	 * Updates the value of this transformation to that of a scale transformation
	 * composed with the current value.
	 *
	 * @param xScale
	 *            the value to scale by in the x direction
	 * @param yScale
	...
	/**
	 * Sets this transformation to be a reflection about the line defined by vector
	 * (x,y). The transformation for a reflection is computed by:
	 *
	 * <blockquote>
	 *
	 * <code>
	...
	public AffineTransformation setToReflection(double x, double y) {
		...
		/**
		 * Handle special case - x = y. This case is specified explicitly to avoid
		 * roundoff error.
		 */
		if (x == y) {
			m00 = 0.0;
	...
	/**
	 * Sets this transformation to be a rotation around the origin. A positive
	 * rotation angle corresponds to a counter-clockwise rotation. The
	 * transformation matrix for a rotation by an angle <tt>theta</tt> has the
	 * value:
	 *
	 * <blockquote>
	 *
	...
	/**
	 * Sets this transformation to be a rotation around the origin by specifying the
	 * sin and cos of the rotation angle directly. The transformation matrix for the
	 * rotation has the value:
	 *
	 * <blockquote>
	 *
	 * <pre>
	...
	/**
	 * Sets this transformation to be a rotation around a given point (x,y). A
	 * positive rotation angle corresponds to a counter-clockwise rotation. The
	 * transformation matrix for a rotation by an angle <tt>theta</tt> has the
	 * value:
	 *
	 * <blockquote>
	...
	/**
	 * Sets this transformation to be a rotation around a given point (x,y) by
	 * specifying the sin and cos of the rotation angle directly. The transformation
	 * matrix for the rotation has the value:
	 *
	 * <blockquote>
	 *
	 * <pre>
	...
	/**
	 * Sets this transformation to be a shear. The transformation matrix for a shear
	 * has the value:
	 *
	 * <blockquote>
	 *
	 ...
	 * </blockquote>
	 *
	 * Note that a shear of (1, 1) is <i>not</i> equal to shear(1, 0) composed with
	 * shear(0, 1). Instead, shear(1, 1) corresponds to a mapping onto the line x =
	 * y.
	 *
	 * @param xShear
	...
	/**
	 * Sets this transformation to be a translation. For a translation by the vector
	 * (x, y) the transformation matrix has the value:
	 *
	 * <blockquote>
	 *
	 * <pre>
	 ...
	 * @return this transformation, with an updated matrix
	 */
	public AffineTransformation setTransformation(double m00, double m01, double m02, double m10, double m11,
			double m12) {
		this.m00 = m00;
		this.m01 = m01;
		this.m02 = m02;
	...
	/**
	 * Updates the value of this transformation to that of a shear transformation
	 * composed with the current value.
	 *
	 * @param xShear
	 *            the value to shear by in the x direction
	...
	/**
	 * Applies this transformation to the <tt>src</tt> coordinate and places the
	 * results in the <tt>dest</tt> coordinate (which may be the same as the
	 * source).
	 *
	 * @param src
	...
	/**
	 * Applies this transformation to the i'th coordinate in the given
	 * CoordinateSequence.
	 *
	 * @param seq
	 *            a <code>CoordinateSequence</code>
	...
	/**
	 * Creates a new {@link Geometry} which is the result of this transformation
	 * applied to the input Geometry.
	 *
	 * @param g
	 *            a <code>Geometry</code>
