...
/**
 * A ring of a polygon being analyzed for topological validity. The shell and hole rings of valid
 * polygons touch only at discrete points. The "touch" relationship induces a graph over the set of
 * rings. The interior of a valid polygon must be connected. This is the case if there is no "chain"
 * of touching rings (which would partition off part of the interior). This is equivalent to the
 * touch graph having no cycles. Thus the touch graph of a valid polygon is a forest - a set of
 * disjoint trees.
 *
 * <p>Also, in a valid polygon two rings can touch only at a single location, since otherwise they
 * disconnect a portion of the interior between them. This is checked as the touches relation is
 * built (so the touch relation representation for a polygon ring does not need to support more than
 * one touch location for each adjacent ring).
 *
 * <p>The cycle detection algorithm works for polygon rings which also contain self-touches
 * (inverted shells and exverted holes).
 *
 * <p>Polygons with no holes do not need to be checked for a connected interior, unless self-touches
 * are allowed. The class also records the topology at self-touch nodes, to support checking if an
 * invalid self-touch disconnects the polygon.
 *
 * @author mdavis
 */
class PolygonRing {
  ...
  /**
   * Records a touch location between two rings, and checks if the rings already touch in a
   * different location.
   *
  ...
  /**
   * Finds a location (if any) where a chain of holes forms a cycle in the ring touch graph. The
   * shell may form part of the chain as well. This indicates that a set of holes disconnects the
  ...
  /**
   * Finds a location of an interior self-touch in a list of rings, if one exists. This indicates
   * that a self-touch disconnects the interior of a polygon, which is invalid.
   *
   * @param polyRings the list of rings to check
  ...
  private final PolygonRing shell;
  private final LinearRing ring;

  /**
   * The root of the touch graph tree containing this ring. Serves as the id for the graph partition
   * induced by the touch relation.
   */
  private PolygonRing touchSetRoot = null;

  // lazily created
  /**
   * The set of PolygonRingTouch links for this ring. The set of all touches in the rings of a
   * polygon forms the polygon touch graph. This supports detecting touch cycles, which reveal the
   * condition of a disconnected interior.
   *
   * <p>Only a single touch is recorded between any two rings, since more than one touch between two
   * rings indicates interior disconnection as well.
   */
  private Map<Integer, PolygonRingTouch> touches = null;

  /** The set of self-nodes in this ring. This supports checking valid ring self-touch topology. */
  ...
  private void addTouch(PolygonRing ring, Coordinate pt) {
    ...
    touches.computeIfAbsent(ring.id, k -> new PolygonRingTouch(ring, pt));
  }

  public void addSelfTouch(
      Coordinate origin, Coordinate e00, Coordinate e01, Coordinate e10, Coordinate e11) {
    if (selfNodes == null) {
  ...
  /**
   * Tests if this ring touches a given ring at the single point specified.
   *
  ...
  /**
   * Detects whether the subgraph of holes linked by touch to this ring contains a hole cycle. If no
   * cycles are detected, the set of touching rings is a tree. The set is marked using this ring as
   * the root.
   *
  ...
  private static void init(PolygonRing root, Deque<PolygonRingTouch> touchStack) {
   ...
   */
  private Coordinate scanForHoleCycle(
      PolygonRingTouch currentTouch, PolygonRing root, Deque<PolygonRingTouch> touchStack) {
    PolygonRing ring = currentTouch.getRing();
    Coordinate currentPt = currentTouch.getCoordinate();

    /**
     * Scan the touched rings Either they form a hole cycle, or they are added to the touch set and
     * pushed on the stack for scanning
     */
    for (PolygonRingTouch touch : ring.getTouches()) {
      /**
       * Don't check touches at the entry point to avoid trivial cycles. They will already be
       * processed or on the stack from the previous ring (which touched all the rings at that point
       * as well)
       */
      if (currentPt.equals2D(touch.getCoordinate())) continue;

      /**
       * Test if the touched ring has already been reached via a different touch path. This is
       * indicated by it already being marked as part of the touch set. This indicates a hole cycle
       * has been found.
       */
  ...
  /**
   * Finds the location of an invalid interior self-touch in this ring, if one exists.
   *
   * @return the location of an interior self-touch node, or null if there are none
   */
  public Coordinate findInteriorSelfNode() {
    ...
    /**
     * Determine if the ring interior is on the Right. This is the case if the ring is a shell and
     * is CW, or is a hole and is CCW.
     */
...
/**
 * Records a point where a {@link PolygonRing} touches another one. This forms an edge in the
 * induced ring touch graph.
 *
 * @author mdavis
 */
class PolygonRingTouch {
  private final PolygonRing ring;
  private final Coordinate touchPt;
...
/**
 * Represents a ring self-touch node, recording the node (intersection point) and the endpoints of
 * the four adjacent segments.
 *
 * <p>This is used to evaluate validity of self-touching nodes, when they are allowed.
 *
 * @author mdavis
 */
class PolygonRingSelfNode {
  private final Coordinate nodePt;
  private final Coordinate e00;
  private final Coordinate e01;
  private final Coordinate e10;

  // private Coordinate e11;

  public PolygonRingSelfNode(
      Coordinate nodePt, Coordinate e00, Coordinate e01, Coordinate e10, Coordinate e11) {
    this.nodePt = nodePt;
    this.e00 = e00;
    this.e01 = e01;
    this.e10 = e10;
  ...
  /**
   * Tests if a self-touch has the segments of each half of the touch lying in the exterior of a
   * polygon. This is a valid self-touch. It applies to both shells and holes. Only one of the four
   * possible cases needs to be tested, since the situation has full symmetry.
   *
  ...
  public boolean isExterior(boolean isInteriorOnRight) {
    /**
     * Note that either corner and either of the other edges could be used to test. The situation is
     * fully symmetrical.
     */
