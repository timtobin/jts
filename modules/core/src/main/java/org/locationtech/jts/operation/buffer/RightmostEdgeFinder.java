...
class RightmostEdgeFinder {
	...
	private void checkForRightmostCoordinate(DirectedEdge de) {
		...
		for (int i = 0; i < coord.length - 1; i++) {
			...
			// segment
			// <FIX> MD 19 Sep 03 - NO! we can test all vertices, since the rightmost must
			// have a
	...
	private void findRightmostEdgeAtVertex() {
		/**
		 * The rightmost point is an interior vertex, so it has a segment on either side
		 * of it. If these segments are both above or below the rightmost point, we need
		 * to determine their relative orientation to decide which is rightmost.
		 */
