 ...
 *
 * <p>
 * Incorporates heuristics to optimize checking for degenerate results, and to
 * reduce the number of points processed for large inputs.
 *
 * @version 1.7
 */
public class ConvexHull {
	...
	/**
	 * Extracts unique coordinates from an array of coordinates, up to an (optional)
	 * maximum count of values. If more than the given maximum of unique values are
	 * found, this is reported by returning <code>null</code>. This avoids scanning
	 * all input points if not needed. If the maximum points is not specified, all
	 * unique points are extracted.
	 *
	 * @param pts
	 *            an array of Coordinates
	 * @param maxPts
	 *            the maximum number of unique points to scan, or -1
	...
	private static Coordinate[] extractUnique(Coordinate[] pts, int maxPts) {
		...
		for (Coordinate pt : pts) {
			...
			if (maxPts >= 0 && uniquePts.size() > maxPts)
				return null;
		}
	...
	/**
	 * Computes the extremal points of an inner octolateral. Some points may be
	 * duplicates - these are collapsed later.
	 *
	 * @param inputPts
	 *            the points to compute the octolateral for
	...
	/**
	 * Checks if there are <= 2 unique points, which produce an obviously degenerate
	 * result. If there are more points, returns null to indicate this.
	 *
	 * <p>
	 * This is a fast check for an obviously degenerate result. If the result is not
	 * obviously degenerate (at least 3 unique points found) the full uniquing of
	 * the entire point set is done only once during the reduce phase.
	 *
	 * @return a degenerate hull geometry, or null if the number of input points is
	 *         large
	 */
	 ...
	 * geometry. The returned geometry contains the minimal number of points needed
	 * to represent the convex hull. In particular, no more than two consecutive
	 * points will be collinear.
	 *
	 * @return if the convex hull contains 3 or more points, a {@link Polygon}; 2
	 *         points, a {@link LineString}; 1 point, a {@link Point}; 0 points, an
	...
	private Coordinate[] preSort(Coordinate[] pts) {
		...
		/**
		 * find the lowest point in the set. If two or more points have the same minimum
		 * Y coordinate choose the one with the minimum X. This focal point is put in
		 * array location pts[0].
		 */
	 ...
	 * The heuristic is to find a polygon guaranteed to be in (or on) the hull, and
	 * eliminate all points inside it. A quadrilateral defined by the extremal
	 * points in the four orthogonal directions can be used, but even more inclusive
	 * is to use an octilateral defined by the points in the 8 cardinal directions.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * To satisfy the requirements of the Graham Scan algorithm, the returned array
	 * has at least 3 entries.
	 *
	 * <p>
	 * This has the side effect of making the reduced points unique, as required by
	 * the convex hull algorithm used.
	 *
	 * @param pts
	...
	private Coordinate[] reduce(Coordinate[] inputPts) {
		 ...
		 * points of the interior polygon are forced to be in the reduced set.
		 */
	...
	/**
	 * An alternative to Stack.toArray, which is not present in earlier versions of
	 * Java.
	 */
	 ...
	 * origin. The origin is assumed to be lower in Y and then X than all other
	 * point inputs. The points are ordered CCW around the origin.
	 *
	 * @author Martin Davis
	 * @version 1.7
	 */
	private static class RadialComparator implements Comparator<Coordinate> {
		 ...
		 * about point o. First checks radial ordering. using a CCW orientation. If the
		 * points are collinear, the comparison is based on their distance to the
		 * origin.
		 *
		 * <p>
		 * p < q iff
		...
		private static int polarCompare(Coordinate o, Coordinate p, Coordinate q) {
			...
			/**
			 * The points are collinear, so compare based on distance from the origin. The
			 * points p and q are >= to the origin, so they lie in the closed half-plane
			 * above the origin. If they are not in a horizontal line, the Y ordinate can be
			 * tested to determine distance. This is more robust than computing the distance
			 * explicitly.
			 */
			if (p.y > q.y)
				return 1;
		...
		/**
		 * Creates a new comparator using a given origin. The origin must be lower in Y
		 * and then X to all compared points.
		 *
		 * @param origin
		 *            the origin of the radial comparison
		 */
