...
/**
 * A memory-efficient representation of a triangle in a triangulation. Contains
 * three vertices, and links to adjacent Tris for each edge. Tris are
 * constructed independently, and if needed linked into a triangulation using
 * {@link TriangulationBuilder}.
 *
 * <p>
 * An edge of a Tri in a triangulation is called a boundary edge if it has no
 * adjacent triangle. The set of Tris containing boundary edges are called the
...
public class Tri {
	...
	/**
	 * Creates a triangle with the given vertices. The vertices should be oriented
	 * clockwise.
	 *
	 * @param p0
	...
	/**
	 * Creates a triangle from an array with three vertex coordinates. The vertices
	...
	/**
	 * Computes the vertex or edge index which is the next one (clockwise) around
	 * the triangle.
	 *
	 * @param index
	 *            the index
	...
	/**
	 * Creates a {@link GeometryCollection} of {@link Polygon}s representing the
	 * triangles in a list.
	 *
	 * @param tris
	...
	/**
	 * triN is the adjacent triangle across the edge pN - pNN. pNN is the next
	 * vertex CW from pN.
	 */
	protected Tri tri0;
	...
	public Tri(Coordinate p0, Coordinate p1, Coordinate p2) {
		...
		this.p1 = p1;
		this.p2 = p2;
		// Assert.isTrue( Orientation.CLOCKWISE != Orientation.index(p0, p1, p2), "Tri
		// is not oriented
		// correctly");
	}

	/**
	 * Computes the degree of a Tri vertex, which is the number of tris containing
	 * it. This must be done by searching the entire triangulation, since the
	 * containing tris may not be adjacent or edge-connected.
	 *
	 * @param index
	 *            the vertex index
	 * @param triList
	...
	private void flip(Tri tri, int index0, int index1, Coordinate adj0, Coordinate adj1, Coordinate opp0,
			Coordinate opp1) {
		...
		/**
		 * Order: 0: opp0-adj0 edge, 1: opp0-adj1 edge, 2: opp1-adj0 edge, 3: opp1-adj1
		 * edge
		 */
	...
	/**
	 * Interchanges the vertices of this triangle and a neighbor so that their
	 * common edge becomes the the other diagonal of the quadrilateral they form.
	...
	/**
	 * Gets the triangles adjacent to the quadrilateral formed by this triangle and
	 * an adjacent one. The triangles are returned in the following order:
	 *
	 * <p>
	 * Order: 0: opp0-adj0 edge, 1: opp0-adj1 edge, 2: opp1-adj0 edge, 3: opp1-adj1
	 * edge
	 ...
	 * @param index
	 *            the index of the common edge in this triangle
	 * @param index1
	 *            the index of the common edge in the adjacent triangle
	 * @return
	 */
	...
	/**
	 * Gets the edge index which a triangle is adjacent to (if any), based on the
	 * adjacent triangle link.
	 *
	 * @param tri
	 *            the tri to find
	 ...
	 */
	/*
	 * //TODO: define when needed public Coordinate[] getEdge(Tri neighbor) { int
	 * index = getIndex(neighbor); int next = next(index);
	 *
	 * Coordinate e0 = getCoordinate(index); Coordinate e1 = getCoordinate(next);
	 * assert (neighbor.hasCoordinate(e0)); assert (neighbor.hasCoordinate(e1)); int
	 * iN = neighbor.getIndex(e0); int iNPrev = prev(iN); assert
	 * (neighbor.getIndex(e1) == iNPrev);
	 *
	 * return new Coordinate[] { getCoordinate(index), getCoordinate(next) }; }
	 *
	 * public Coordinate getEdgeStart(int i) { return getCoordinate(i); }
	 *
	 * public Coordinate getEdgeEnd(int i) { return getCoordinate(next(i)); }
	 *
	 * public boolean hasCoordinate(Coordinate v) { if ( p0.equals(v) ||
	 * p1.equals(v) || p2.equals(v) ) { return true; } return false; }
	 */
	...
	/**
	 * Tests if a tri contains a boundary edge, and thus on the border of the
	 * triangulation containing it.
	 *
	...
	/**
	 * Tests if a tri vertex is interior. A vertex of a triangle is interior if it
	...
	/**
	 * Computes the number of triangle adjacent to this triangle. This is a number
	 * in the range [0,2].
	 *
	...
	/**
	 * Removes this triangle from a triangulation. All adjacent references and the
	 * references to this Tri in the adjacent Tris are set to <code>null</code.
	 */
	public void remove() {
		remove(0);
		remove(1);
	...
	/**
	 * Removes this tri from the triangulation containing it. All links between the
	 * tri and adjacent ones are nulled.
	 *
	 * @param triList
	 *            the triangulation
	 */
	 ...
	 *            an adjacent triangle
	 * @param triNew
	 *            the triangle to replace it with
	 */
	...
	/**
	 * Sets the triangle adjacent to the edge originating at a given vertex. The
	 * vertices of the adjacent triangles are assumed to match the appropriate
	...
	/**
	 * Sets the adjacent triangles. The vertices of the adjacent triangles are
	...
	/**
	 * Sets the triangle adjacent to an edge. The vertices of the adjacent triangle
	 * are assumed to match the appropriate vertices in this triangle.
	 *
	 * @param edgeIndex
	...
	/**
	 * Spits a triangle by a point located inside the triangle. Creates the three
	 * new resulting triangles with adjacent links set correctly. Returns the new
	 * triangle whose 0'th vertex is the splitting point.
	 *
	 * @param p
	 *            the point to insert
	...
	public String toString() {
		return "POLYGON ((%s, %s, %s, %s))".formatted(WKTWriter.format(p0), WKTWriter.format(p1), WKTWriter.format(p2),
				WKTWriter.format(p0));
	}

	/**
	 * Validates that a tri is correct. Currently just checks that orientation is
	 * CW.
	 *
