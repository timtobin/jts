...
/**
 * Extracts the subline of a linear {@link Geometry} between two
 * {@link LinearLocation}s on the line.
 */
class ExtractLineByLocation {
	/**
	 * Computes the subline of a {@link LineString} between two
	 * {@link LinearLocation}s on the line. If the start location is after the end
	 * location, the computed linear geometry has reverse orientation to the input
	 * line.
	 *
	 * @param line
	 *            the line to use as the baseline
	 * @param start
	 *            the start location
	 * @param end
	 *            the end location
	...
	private LineString computeLine(LinearLocation start, LinearLocation end) {
		...
		// Assert.isTrue(end.getSegmentFraction() <= 1.0, "invalid segment fraction
		// value");
		...
		/**
		 * Ensure there is enough coordinates to build a valid line. Make a 2-point line
		 * with duplicate coordinates, if necessary. There will always be at least one
		 * coordinate in the coordList.
		 */
	...
	/**
	 * Extracts a subline of the input. If <code>end < start</code> the linear
	 * geometry computed will be reversed.
	 *
	 * @param start
	 *            the start location
	 * @param end
	...
	/**
	 * Computes a valid and normalized location compatible with the values in a
	 * LinearIterator. (I.e. segmentFractions of 1.0 are converted to the next
	 * highest coordinate index)
	 */
	/*
	 ...
	 * segmentFraction = loc.getSegmentFraction();
	 *
	 * if (segmentFraction < 0.0) { segmentFraction = 0.0; } if (segmentFraction >
	 * 1.0) { segmentFraction = 1.0; }
	 *
	 * if (componentIndex < 0) { componentIndex = 0; segmentIndex = 0;
	 * segmentFraction = 0.0; } if (segmentIndex < 0) { segmentIndex = 0;
	 * segmentFraction = 0.0; }
	 *
	 * if (segmentFraction == 1.0) { segmentFraction = 0.0; segmentIndex += 1; }
	 *
