 ...
 * <code>isSimple</code> trivially returns true. (Note: this means that
 * <tt>isSimple</tt> cannot be used to test for (invalid) self-intersections in
 * <tt>Polygon</tt>s. In order to check if a <tt>Polygonal</tt> geometry has
 * self-intersections, use {@link Geometry#isValid()}).
 * <li><b>Linear</b> geometries are simple if they do <i>not</i> self-intersect
 * at interior points (i.e. points other than boundary points). This is
 * equivalent to saying that no two linear components satisfy the SFS
 * {@link Geometry#touches(Geometry)} predicate.
 * <li><b>Zero-dimensional (point)</b> geometries are simple if and only if they
 ...
 * {@link BoundaryNodeRule#MOD2_BOUNDARY_RULE}. Note that under the
 * <tt>Mod-2</tt> rule, closed <tt>LineString</tt>s (rings) will never satisfy
 * the <tt>touches</tt> predicate at their endpoints, since these are interior
 * points, not boundary points. If it is required to test whether a set of
 * <code>LineString</code>s touch only at their endpoints, use <code>IsSimpleOp
 * </code> with {@link BoundaryNodeRule#ENDPOINT_BOUNDARY_RULE}. For example,
 * this can be used to validate that a set of lines form a topologically valid
 * linear network.
 *
...
public class IsSimpleOp {
	 ...
	 * (i.e. where it has a non-boundary self-intersection). {@link #isSimple} must
	...
	/**
	 * Semantics for GeometryCollection is simple if all components are simple.
	 *
	 * @param geom
	 * @return true if the geometry is simple
	 */
