...
public class CGAlgorithms {
	 ...
	 * @param q
	 *            the point to compute the relative orientation of
	 * @return 1 if q is counter-clockwise from p1-p2, or -1 if q is clockwise from
	 *         p1-p2, or 0 if q is collinear with p1-p2
	...
	public static double distanceLineLine(Coordinate A, Coordinate B, Coordinate C, Coordinate D) {
		 ...
		 * (Bx-Ax)(Dy-Cy)-(By-Ay)(Dx-Cx)
		 *
		 * Let P be the position vector of the intersection point, then P=A+r(B-A) or
		 * Px=Ax+r(Bx-Ax) Py=Ay+r(By-Ay) By examining the values of r & s, you can also
		 * determine some other limiting conditions: If 0<=r<=1 & 0<=s<=1, intersection
		 * exists r<0 or r>1 or s<0 or s>1 line segments do not intersect If the
		 * denominator in eqn 1 is zero, AB & CD are parallel If the numerator in eqn 1
		 * is also zero, AB & CD are collinear.
		 */
		...
		if (noIntersection) {
			return MathUtil.min(distancePointLine(A, C, D), distancePointLine(B, C, D), distancePointLine(C, A, B),
	...
	public static double distancePointLine(Coordinate p, Coordinate A, Coordinate B) {
		 ...
		 * extension of AB r>1 P is on the forward extension of AB 0<r<1 P is interior
		 * to AB
		 */
	...
	public static boolean isCCW(Coordinate[] ring) {
		...
		/*
		 * If disc is exactly 0, lines are collinear. There are two possible cases: (1)
		 * the lines lie along the x axis in opposite directions (2) the lines lie on
		 * top of one another
		 *
	 ...
	 * coordinates.
	 *
	 * @return true if the point is a vertex of the line or lies in the interior of
	 *         a line segment in the linestring
	 ...
	 *
	 * <p>
	 * This method does <i>not</i> first check the point against the envelope of the
	 * ring.
	 *
	 * @param p
	 ...
	 * @deprecated Use {@link PointLocation#locateInRing(Coordinate, Coordinate[])}
	 *             instead.
	 */
	...
	/**
	 * Returns the index of the direction of the point {@code q} relative to a
	 * vector specified by {@code p1-p2}.
	 *
	 * @param p1
	 ...
	 * @return {@code 1} if q is counter-clockwise (left) from p1-p2 {@code -1} if q
	 *         is clockwise (right) from p1-p2 {@code 0} if q is collinear with
	 *         p1-p2
	 * @deprecated Use {@link Orientation#index(Coordinate, Coordinate, Coordinate)}
	 *             instead.
	 */
