...
/**
 * Dissolves a noded collection of {@link SegmentString}s to produce a set of
 * merged linework with unique segments. A custom {@link SegmentStringMerger}
 * merging strategy can be supplied. This strategy will be called when two
 * identical (up to orientation) strings are dissolved together. The default
 * merging strategy is simply to discard one of the merged strings.
 *
 * <p>
 * A common use for this class is to merge noded edges while preserving
 * topological labelling. This requires a custom merging strategy to be supplied
...
public class SegmentStringDissolver {
	...
	public void dissolve(SegmentString segString) {
		...
		if (existing == null) {
			...
			if (merger != null) {
				boolean isSameOrientation = CoordinateArrays.equals(existing.getCoordinates(),
	 ...
	 * (OrientedCoordinateArray) i.next(); if (oca.compareTo(addedOCA) == 0) return
	 * true; } return false; }
	 */

	/**
	 * A merging strategy which can be used to update the context data of
	 * {@link SegmentString}s which are merged during the dissolve process.
	 *
	 * @author mbdavis
	 */
	public interface SegmentStringMerger {
		/**
		 * Updates the context data of a SegmentString when an identical (up to
		 * orientation) one is found during dissolving.
		 *
		 * @param mergeTarget
		 ...
		 *            <code>true</code> if the strings are in the same direction, <code>
		 *     false</code> if they are opposite
		 */
