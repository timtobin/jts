...
public class DistanceOp {
	/**
	 * Compute the the closest points of two geometries. The points are presented in
	...
	public static boolean isWithinDistance(Geometry g0, Geometry g1, double distance) {
		...
		// MD - could improve this further with a positive short-circuit based on
		// envelope MinMaxDist
	...
	private void computeContainmentDistance(int polyGeomIndex, GeometryLocation[] locPtPoly) {
		...
		if (!polys.isEmpty()) {
			...
			if (minDistance <= terminateDistance) {
				// this assigment is determined by the order of the args in the computeInside
				// call above
