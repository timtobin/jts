...
/**
 * Transforms a polygon with holes into a single self-touching (invalid) ring by
 * joining holes to the exterior shell or to another hole with out-and-back line
 * segments. The holes are added in order of their envelopes (leftmost/lowest
 * first). As the result shell develops, a hole may be added to what was
 * originally another hole.
 *
 * <p>
 * There is no attempt to optimize the quality of the join lines. In particular,
 * holes may be joined by lines longer than is optimal. However, holes which
 * touch the shell or other holes are joined at the touch point.
 *
 * <p>
 * The class does not require the input polygon to have normal orientation
 * (shell CW and rings CCW). The output ring is always CW.
 */
public class PolygonHoleJoiner {
	...
	private static SegmentSetMutualIntersector createBoundaryIntersector(Coordinate[] shellRing,
			Coordinate[][] holeRings) {
	...
	/**
	 * Joins the shell and holes of a polygon and returns the result as sequence of
	 * Coordinates.
	 *
	 * @param inputPolygon
	 *            the polygon to join
	...
	/**
	 * Joins the shell and holes of a polygon and returns the result as an (invalid)
	 * Polygon.
	 *
	 * @param inputPolygon
	 *            the polygon to join
	 ...
	 *
	 * @param poly
	 *            polygon that contains the holes
	...
	/**
	 * Add hole vertices at proper position in shell vertex list. This code assumes
	 * that if hole touches (shell or other hole), it touches at a node. This
	 * requires an initial noding step. In this case, the code avoids duplicating
	 * join vertices.
	 *
	 * <p>
	 ...
	 *
	 * @param joinIndex
	 *            index of join vertex in shell
	 * @param holeCoords
	 *            the vertices of the hole to be inserted
	 * @param holeJoinIndex
	 *            index of join vertex in hole
	 */
	private void addJoinedHole(int joinIndex, Coordinate[] holeCoords, int holeJoinIndex) {
		...
		// -- check for touching (zero-length) join to avoid inserting duplicate
		// vertices
	...
	/**
	 * Creates the new section of vertices for ad added hole, including any required
	 * vertices from the shell at the join point, and ensuring join vertices are not
	 * duplicated.
	 *
	 * @param holeCoords
	 *            the hole vertices
	 * @param holeJoinIndex
	 *            the index of the join vertex
	 * @param joinPt
	 *            the shell join vertex
	...
	private List<Coordinate> createHoleSection(Coordinate[] holeCoords, int holeJoinIndex, Coordinate joinPt) {
		...
		/**
		 * Add all hole vertices, including duplicate at hole join vertex Except if hole
		 * DOES touch, join vertex is already in shell ring
		 */
		...
		/**
		 * Add duplicate shell vertex at end of the return join line. Except if hole
		 * DOES touch, join line is zero-length so do not need dup vertex
		 */
		if (isNonTouchingHole) {
	...
	/**
	 * Finds the vertex index of a hole where it touches the current shell (if it
	 * does). If a hole does touch, it must touch at a single vertex (otherwise, the
	 * polygon is invalid).
	 *
	 * @param holeCoords
	 *            the hole
	...
	/**
	 * Gets the join ring vertex index that the hole is joined after. A vertex can
	 * occur multiple times in the join ring, so it is necessary to choose the one
	 * which forms a corner having the join line in the ring interior.
	 *
	 * @param joinCoord
	 *            the join ring vertex
	 * @param holeJoinCoord
	 *            the hole join vertex
	...
	/**
	 * Finds a shell vertex that is joinable to the hole join vertex. One must
	 * always exist, since the hole join vertex is on the left of the hole, and thus
	 * must always have at least one shell vertex visible to it.
	 *
	 * <p>
	 * There is no attempt to optimize the selection of shell vertex to join to
	 * (e.g. by choosing one with shortest distance).
	 *
	 * @param holeJoinCoord
	 *            the hole join vertex
	...
	/**
	 * Tests if a line between a ring corner vertex and a given point is interior to
	 * the ring corner.
	 *
	 * @param ring
	 *            a ring of points
	 * @param ringIndex
	 *            the index of a ring vertex
	 * @param linePt
	 *            the point to be joined to the ring
	...
	/**
	 * Joins a hole to the shell only if the hole touches the shell. Otherwise,
	 * reports the hole is non-touching.
	 *
	 * @param holeCoords
	 *            the hole to join
	...
	private boolean joinTouchingHole(Coordinate[] holeCoords) {
		...
		/**
		 * Find shell corner which contains the hole, by finding corner which has a hole
		 * segment at the join pt in interior
		 */
