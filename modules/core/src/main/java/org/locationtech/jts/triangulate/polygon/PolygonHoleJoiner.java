...
/**
 * Transforms a polygon with holes into a single self-touching (invalid) ring by joining holes to
 * the exterior shell or to another hole with out-and-back line segments. The holes are added in
 * order of their envelopes (leftmost/lowest first). As the result shell develops, a hole may be
 * added to what was originally another hole.
 *
 * <p>There is no attempt to optimize the quality of the join lines. In particular, holes may be
 * joined by lines longer than is optimal. However, holes which touch the shell or other holes are
 * joined at the touch point.
 *
 * <p>The class does not require the input polygon to have normal orientation (shell CW and rings
 * CCW). The output ring is always CW.
 */
public class PolygonHoleJoiner {
  ...
  public Coordinate[] compute() {
    ...
    if (holeRings.length > 0) nodeRings();
    joinedRing = copyToList(shellRing);
    if (holeRings.length > 0) joinHoles();
  ...
  /**
   * Joins a hole to the shell only if the hole touches the shell. Otherwise, reports the hole is
   * non-touching.
   *
  ...
  private boolean joinTouchingHole(Coordinate[] holeCoords) {
    ...
    /**
     * Find shell corner which contains the hole, by finding corner which has a hole segment at the
     * join pt in interior
     */
  ...
  /**
   * Finds the vertex index of a hole where it touches the current shell (if it does). If a hole
   * does touch, it must touch at a single vertex (otherwise, the polygon is invalid).
   *
  ...
  private int findHoleTouchIndex(Coordinate[] holeCoords) {
    for (int i = 0; i < holeCoords.length; i++) {
      if (joinedPts.contains(holeCoords[i])) return i;
    }
    return -1;
  }
  ...
  /**
   * Finds a shell vertex that is joinable to the hole join vertex. One must always exist, since the
   * hole join vertex is on the left of the hole, and thus must always have at least one shell
   * vertex visible to it.
   *
   * <p>There is no attempt to optimize the selection of shell vertex to join to (e.g. by choosing
   * one with shortest distance).
   *
  ...
  /**
   * Gets the join ring vertex index that the hole is joined after. A vertex can occur multiple
   * times in the join ring, so it is necessary to choose the one which forms a corner having the
  ...
  /**
   * Tests if a line between a ring corner vertex and a given point is interior to the ring corner.
   *
  ...
  private static int prev(int i, int size) {
    int prev = i - 1;
    if (prev < 0) return size - 2;
    return prev;
  }

  private static int next(int i, int size) {
    int next = i + 1;
    if (next > size - 2) return 0;
    return next;
  }

  /**
   * Add hole vertices at proper position in shell vertex list. This code assumes that if hole
   * touches (shell or other hole), it touches at a node. This requires an initial noding step. In
   * this case, the code avoids duplicating join vertices.
   *
   * <p>Also adds hole points to ordered coordinates.
   *
  ...
  /**
   * Creates the new section of vertices for ad added hole, including any required vertices from the
   * shell at the join point, and ensuring join vertices are not duplicated.
   *
   ...
   */
  private List<Coordinate> createHoleSection(
      Coordinate[] holeCoords, int holeJoinIndex, Coordinate joinPt) {
    List<Coordinate> section = new ArrayList<>();

    boolean isNonTouchingHole = joinPt != null;
    /**
     * Add all hole vertices, including duplicate at hole join vertex Except if hole DOES touch,
     * join vertex is already in shell ring
     */
    if (isNonTouchingHole) section.add(holeCoords[holeJoinIndex].copy());
    ...
    /**
     * Add duplicate shell vertex at end of the return join line. Except if hole DOES touch, join
     * line is zero-length so do not need dup vertex
     */
  ...
  /**
   * Tests whether the interior of a line segment intersects the polygon boundary. If so, the line
   * is not a valid join line.
   *
  ...
  private boolean intersectsBoundary(Coordinate p0, Coordinate p1) {
    SegmentString segString = new BasicSegmentString(new Coordinate[] {p0, p1}, null);
  ...
  private static class InteriorIntersectionDetector implements SegmentIntersector {
    ...
    public void processIntersections(
      ...
      if (li.getIntersectionNum() == 0) {
        ...
        if (li.isInteriorIntersection()) hasIntersection = true;
  ...
  private static SegmentSetMutualIntersector createBoundaryIntersector(
