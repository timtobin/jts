...
/**
 * Implements the SFS <tt>relate()</tt> generalized spatial predicate on two
 * {@link Geometry}s.
 *
 * <p>
 * The class supports specifying a custom {@link BoundaryNodeRule} to be used
 * during the relate computation.
 *
 * <p>
 * If named spatial predicates are used on the result {@link IntersectionMatrix}
 * of the RelateOp, the result may or not be affected by the choice of
 * <tt>BoundaryNodeRule</tt>, depending on the exact nature of the pattern. For
 * instance, {@link IntersectionMatrix#isIntersects()} is insensitive to the
 * choice of <tt>BoundaryNodeRule</tt>, whereas
 * {@link IntersectionMatrix#isTouches(int, int)} is affected by the rule
 * chosen.
 *
 * <p>
 * <b>Note:</b> custom Boundary Node Rules do not (currently) affect the results
 * of other {@link Geometry} methods (such as {@link Geometry#getBoundary}. The
...
public class RelateOp extends GeometryGraphOperation {
	 ...
	 * two {@link Geometry}s, using the default (OGC SFS) Boundary Node Rule
	 *
	 * @param a
	 *            a Geometry to test
	 * @param b
	 *            a Geometry to test
	 * @return the IntersectionMatrix for the spatial relationship between the
	 *         geometries
	 */
	 ...
	 * @return the IntersectionMatrix for the spatial relationship between the input
	 *         geometries
	 */
	...
	/**
	 * Creates a new Relate operation, using the default (OGC SFS) Boundary Node
	 * Rule.
	 *
	 * @param g0
