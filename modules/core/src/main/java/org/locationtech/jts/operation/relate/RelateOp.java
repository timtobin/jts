 ...
 *
 * <p>The class supports specifying a custom {@link BoundaryNodeRule} to be used during the relate
 * computation.
 *
 * <p>If named spatial predicates are used on the result {@link IntersectionMatrix} of the RelateOp,
 * the result may or not be affected by the choice of <tt>BoundaryNodeRule</tt>, depending on the
 * exact nature of the pattern. For instance, {@link IntersectionMatrix#isIntersects()} is
 * insensitive to the choice of <tt>BoundaryNodeRule</tt>, whereas {@link
 * IntersectionMatrix#isTouches(int, int)} is affected by the rule chosen.
 *
 * <p><b>Note:</b> custom Boundary Node Rules do not (currently) affect the results of other {@link
...
public class RelateOp extends GeometryGraphOperation {
  /**
   * Computes the {@link IntersectionMatrix} for the spatial relationship between two {@link
   * Geometry}s, using the default (OGC SFS) Boundary Node Rule
   *
  ...
  /**
   * Computes the {@link IntersectionMatrix} for the spatial relationship between two {@link
   * Geometry}s using a specified Boundary Node Rule.
   *
   * @param a a Geometry to test
   * @param b a Geometry to test
   ...
   */
  public static IntersectionMatrix relate(
