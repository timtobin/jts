...
/**
 * Supports linear referencing along a linear {@link Geometry} using the length
 * along the line as the index. Negative length values are taken as measured in
 * the reverse direction from the end of the geometry. Out-of-range index values
 * are handled by clamping them to the valid range of values. Non-simple lines
 * (i.e. which loop back to cross or touch themselves) are supported.
 */
public class LengthIndexedLine {
	private final Geometry linearGeom;

	/**
	 * Constructs an object which allows a linear {@link Geometry} to be linearly
	 * referenced using length as an index.
	 *
	 * @param linearGeom
	...
	/**
	 * Computes a valid index for this line by clamping the given index to the valid
	...
	/**
	 * Computes the {@link LineString} for the interval on the line between the
	 * given indices. If the endIndex lies before the startIndex, the computed
	...
	public Geometry extractLine(double startIndex, double endIndex) {
		...
		// if extracted line is zero-length, resolve start lower as well to ensure they
		// are equal
	...
	/**
	 * Computes the {@link Coordinate} for the point on the line at the given index.
	 * If the index is out of range the first or last point on the line will be
	 * returned. The Z-ordinate of the computed point will be interpolated from the
	 * Z-ordinates of the line segment containing it, if they exist.
	 *
	 * @param index
	...
	/**
	 * Computes the {@link Coordinate} for the point on the line at the given index,
	 * offset by the given distance. If the index is out of range the first or last
	 * point on the line will be returned. The computed point is offset to the left
	 * of the line if the offset distance is positive, to the right if negative.
	 *
	 * <p>
	 * The Z-ordinate of the computed point will be interpolated from the
	 * Z-ordinates of the line segment containing it, if they exist.
	 *
	 * @param index
	 *            the index of the desired point
	...
	/**
	 * Computes the minimum index for a point on the line. If the line is not simple
	 * (i.e. loops back on itself) a single point may have more than one possible
	 * index. In this case, the smallest index is returned.
	 *
	 * <p>
	 * The supplied point does not <i>necessarily</i> have to lie precisely on the
	 * line, but if it is far from the line the accuracy and performance of this
	 * function is not guaranteed. Use {@link #project} to compute a guaranteed
	 ...
	 * index. If no such index exists, returns <tt>minIndex</tt>. This method can be
	 * used to determine all indexes for a point which occurs more than once on a
	 * non-simple line. It can also be used to disambiguate cases where the given
	 * point lies slightly off the line and is equidistant from two different points
	 * on the line.
	 *
	 * <p>
	 * The supplied point does not <i>necessarily</i> have to lie precisely on the
	 ...
	 * <b>conform</b> to the line; that is, all vertices in the subline (except
	 * possibly the first and last) must be vertices of the line and occur in the
	 * same order).
	 *
	 * @param subLine
	...
	/**
	 * Computes the index for the closest point on the line to the given point. If
	 * more than one point has the closest distance the first one along the line is
	 * returned. (The point does not necessarily have to lie precisely on the line.)
	 *
	 * @param pt
	 *            a point on the line
