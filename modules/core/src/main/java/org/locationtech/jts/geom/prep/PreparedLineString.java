...
public class PreparedLineString extends BasicPreparedGeometry {
	...
	public synchronized FastSegmentSetIntersectionFinder getIntersectionFinder() {
		/**
		 * MD - Another option would be to use a simple scan for segment testing for
		 * small geometries. However, testing indicates that there is no particular
	...
	/**
	 * There's not much point in trying to optimize contains, since contains for
	 * linear targets requires the entire test geometry to exactly match the target
	 * linework.
	 */
}
