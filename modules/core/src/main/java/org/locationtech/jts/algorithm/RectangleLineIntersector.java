 ...
 *
 * <p>Rectangles contain a large amount of inherent symmetry (or to put it another way, although
 * they contain four coordinates they only actually contain 4 ordinates worth of information). The
 * algorithm used takes advantage of the symmetry of the geometric situation to optimize performance
 * by minimizing the number of line intersection tests.
 *
 * @author Martin Davis
 */
public class RectangleLineIntersector {
  ...
  public boolean intersects(Coordinate p0, Coordinate p1) {
     ...
     * completely.
     *
     * <p>In the case of a crossing, the segment must intersect a diagonal of the rectangle.
     *
     * <p>To distinguish these two cases, it is sufficient to test intersection with a single
     * diagonal of the rectangle, namely the one with slope "opposite" to the slope of the segment.
     * (Note that if the segment is axis-parallel, it must intersect both diagonals, so this is
     * still sufficient.)
     */
    if (isSegUpwards) {
