 ...
 * symmetry of the geometric situation to optimize performance by minimizing the
 * number of line intersection tests.
 *
 * @author Martin Davis
 */
public class RectangleLineIntersector {
	...
	public RectangleLineIntersector(Envelope rectEnv) {
		...
		/**
		 * Up and Down are the diagonal orientations relative to the Left side of the
		 * rectangle. Index 0 is the left side, 1 is the right side.
		 */
	...
	public boolean intersects(Coordinate p0, Coordinate p1) {
		...
		/**
		 * If the segment envelope is disjoint from the rectangle envelope, there is no
		 * intersection
		 */
		...
		/**
		 * Normalize segment. This makes p0 less than p1, so that the segment runs to
		 * the right, or vertically upwards.
		 */
		 ...
		 * are two possible situations: 1) the segment is disjoint to the rectangle 2)
		 * the segment crosses the rectangle completely.
		 *
		 * <p>
		 ...
		 *
		 * <p>
		 * To distinguish these two cases, it is sufficient to test intersection with a
		 * single diagonal of the rectangle, namely the one with slope "opposite" to the
		 * slope of the segment. (Note that if the segment is axis-parallel, it must
		 * intersect both diagonals, so this is still sufficient.)
		 */
		if (isSegUpwards) {
