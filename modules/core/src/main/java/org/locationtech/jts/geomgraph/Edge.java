...
public class Edge extends GraphComponent {
  ...
  public static void updateIM(Label label, IntersectionMatrix im) {
    ...
    if (label.isArea()) {
      im.setAtLeastIfValid(
          label.getLocation(0, Position.LEFT), label.getLocation(1, Position.LEFT), 2);
      im.setAtLeastIfValid(
          label.getLocation(0, Position.RIGHT), label.getLocation(1, Position.RIGHT), 2);
    }
  }
   ...
   *
   * @return zero-width V area edge, consisting of two segments which are equal and of oppose
   *     orientation
   */
   ...
   *
   * <p>e1 equals e2 <b>iff</b> the coordinates of e1 are the same or the reverse of the coordinates
   * in e2
   */
