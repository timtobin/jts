...
public class AffineTransformationFactory {
   ...
   *
   * @param src the source points of the vectors
   * @param dest the destination points of the vectors
   * @return the computed transformation
   * @throws IllegalArgumentException if the control vector arrays are too short, long or of
   *     different lengths
   */
  public static AffineTransformation createFromControlVectors(Coordinate[] src, Coordinate[] dest) {
    ...
    if (src.length == 2) return createFromControlVectors(src[0], src[1], dest[0], dest[1]);
