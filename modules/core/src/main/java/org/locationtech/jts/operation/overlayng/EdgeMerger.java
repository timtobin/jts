 ...
 *       topology locations are "summed" to provide a final assignment of side location
 *   <li>Coincident edges from the same linear geometry can simply be merged using the same ON
 *       location
 * </ul>
 *
 ...
 * lines.
 *
 * <p>The merger also preserves the order of the edges in the input. This means that for
 * polygon-line overlay the result lines will be in the same order as in the input (possibly with
 * multiple result lines for a single input line).
 *
 * @author mdavis
 */
class EdgeMerger {
