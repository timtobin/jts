 ...
 *
 * <p>
 * The planar Hilbert Curve is a continuous space-filling curve. In the limit
 * the Hilbert curve has infinitely many vertices and fills the space of the
 * unit square. A sequence of finite approximations to the infinite Hilbert
 * curve is defined by the level number. The finite Hilbert curve at level n
 * H<sub>n</sub> contains 2<sup>n + 1</sup> points. Each finite Hilbert curve
 * defines an ordering of the points in the 2-dimensional range square
 * containing the curve. Curves fills the range square of side
 * 2<sup>level</sup>. Curve points have ordinates in the range [0,
 * 2<sup>level</sup> - 1]. The index of a point along a Hilbert curve is called
 * the Hilbert code. The code for a given point is specific to the level chosen.
 *
 * <p>
 * This implementation represents codes using 32-bit integers. This allows
 * levels 0 to 16 to be handled. The class supports encoding points in the range
 * of a given level curve and decoding the point for a given code value.
 *
 * <p>
 * The Hilbert order has the property that it tends to preserve locality. This
 * means that codes which are near in value will have spatially proximate
 * points. The converse is not always true - the delta between codes for nearby
 * points is not always small. But the average delta is small enough that the
 * Hilbert order is an effective way of linearizing space to support range
 * queries.
 *
 * @author Martin Davis
...
public class HilbertCode {
	...
	/**
	 * Encodes a point (x,y) in the range of the the Hilbert curve at a given level
	...
	/**
	 * The level of the finite Hilbert curve which contains at least the given
	 * number of points.
	 *
	 * @param numPoints
	...
	/**
	 * The maximum ordinate value for points in the curve for the given level. The
	 * maximum ordinate is 2<sup>level</sup> - 1.
	 *
	 * @param level
	...
	/**
	 * The number of points in the curve for the given level. The number of points
	 * is 2<sup>2 * level</sup>.
	 *
	 * @param level
	 *            the level of the curve
