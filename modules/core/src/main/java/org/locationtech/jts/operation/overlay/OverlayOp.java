...
public class OverlayOp extends GeometryGraphOperation {
  /**
   * The spatial functions supported by this class. These operations implement various boolean
   * combinations of the resultants of the overlay.
   */
  ...
  /**
   * Tests whether a point with given {@link Location}s relative to two geometries is contained in
   * the result of overlaying the geometries using a given overlay operation.
   *
   * <p>The method handles arguments of {@link Location#NONE} correctly
   *
  ...
  public OverlayOp(Geometry g0, Geometry g1) {
    ...
    /**
     * Use factory of primary geometry. Note that this does NOT handle mixed-precision arguments
   ...
   *
   * <p>Note: this method can be called once only.
   *
  ...
  private void computeOverlay(int opCode) {
     ...
     *
     * <p>This test is slow, but necessary in order to catch robustness failure situations. If an
  ...
  /**
   * If either of the GeometryLocations for the existing label is exactly opposite to the one in the
   * labelToMerge, this indicates a dimensional collapse has happened. In this case, convert the
   * label for that Geometry to a Line label
   */
  /* NOT NEEDED?
  ...
  */
        ...
        for (int i = 0; i < 2; i++) {
          if (!lbl.isNull(i) && lbl.isArea() && !depth.isNull(i)) {
            ...
            if (depth.getDelta(i) == 0) {
               ...
               */
              Assert.isTrue(
                  !depth.isNull(i, Position.LEFT), "depth of LEFT side has not been initialized");
              lbl.setLocation(i, Position.LEFT, depth.getLocation(i, Position.LEFT));
              Assert.isTrue(
                  !depth.isNull(i, Position.RIGHT), "depth of RIGHT side has not been initialized");
   ...
   * incident on them.
   *
   * <p>Isolated nodes are found because nodes in one graph which don't intersect nodes in the other
  ...
  private Geometry computeGeometry(
      List resultPointList, List resultLineList, List resultPolyList, int opcode) {
   ...
   * not a collection.
   *
   * <p>The empty result is constructed using the following rules:
   *
   * <ul>
   ...
   *   <li>{@link #SYMDIFFERENCE} - result has the dimension of the highest input dimension (since
   ...
   */
  public static Geometry createEmptyResult(
