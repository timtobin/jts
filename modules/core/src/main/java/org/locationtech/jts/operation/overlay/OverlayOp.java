...
/**
 * Computes the geometric overlay of two {@link Geometry}s. The overlay can be
 * used to determine any boolean combination of the geometries.
 *
 * @version 1.7
 */
public class OverlayOp extends GeometryGraphOperation {
	 ...
	 * is contained in the result of overlaying the geometries using a given overlay
	 * operation.
	 *
	 * <p>
	 * The method handles arguments of {@link Location#NONE} correctly
	...
	public OverlayOp(Geometry g0, Geometry g1) {
		 ...
		 * mixed-precision arguments where the second arg has greater precision than the
		 * first.
		 */
	 ...
	 *
	 * @param overlayOpCode
	 *            the overlay operation to perform
	...
	private void computeOverlay(int opCode) {
		 ...
		 * snapping will be performed, which will hopefully avoid the problem. In the
		 * future hopefully a faster check can be developed.
		 */
		...
		/**
		 * The ordering of building the result Geometries is important. Areas must be
		 * built before lines, which must be built before points. This is so that lines
		 * which are covered by areas are not included explicitly, and similarly for
		 * points.
		 */
	...
	/**
	 * If either of the GeometryLocations for the existing label is exactly opposite
	 * to the one in the labelToMerge, this indicates a dimensional collapse has
	 * happened. In this case, convert the label for that Geometry to a Line label
	 */
	/*
	 ...
	 * depths are first normalized. Then, if the depths for the edge are equal, this
	 * edge must have collapsed into a line edge. If the depths are not equal,
	 * update the label with the locations corresponding to the depths (i.e. a depth
	 * of 0 corresponds to a Location of EXTERIOR, a depth of 1 corresponds to
	 * INTERIOR)
	 */
	private void computeLabelsFromDepths() {
		for (Iterator it = edgeList.iterator(); it.hasNext();) {
			...
			/**
			 * Only check edges for which there were duplicates, since these are the only
			 * ones which might be the result of dimensional collapses.
			 */
			if (!depth.isNull()) {
				depth.normalize();
				for (int i = 0; i < 2; i++) {
					if (!lbl.isNull(i) && lbl.isArea() && !depth.isNull(i)) {
						/**
						 * if the depths are equal, this edge is the result of the dimensional collapse
						 * of two or more edges. It has the same location on both sides of the edge, so
						 * it has collapsed to a line.
						 */
						if (depth.getDelta(i) == 0) {
							lbl.toLine(i);
						} else {
							/**
							 * This edge may be the result of a dimensional collapse, but it still has
							 * different locations on both sides. The label of the edge must be updated to
							 * reflect the resultant side locations indicated by the depth values.
							 */
	 ...
	 * a node may be an intersection node with a previously computed label of
	 * BOUNDARY, but in the original arg Geometry it is actually in the interior due
	 * to the Boundary Determination Rule)
	 */
	 ...
	 * both Geometries) only if they are incident on a node which has edges for both
	 * Geometries
	 */
	 ...
	 * adding nodes to the nodeList. To complete the labelling we need to check for
	 * nodes that lie in the interior of edges, and in the interior of areas.
	 *
	 * <p>
	 * When each node labelling is completed, the labelling of the incident edges is
	...
	private void findResultAreaEdges(int opCode) {
		for (Object o : graph.getEdgeEnds()) {
			...
			if (label.isArea() && !de.isInteriorAreaEdge() && isResultOfOp(label.getLocation(0, Position.RIGHT),
	...
	/**
	 * If both a dirEdge and its sym are marked as being in the result, cancel them
	 * out.
	 */
	 ...
	 * @return true if the coordinate point is covered by a result Line or Area
	 *         geometry
	 */
	...
	/**
	 * Creates an empty result geometry of the appropriate dimension, based on the
	 * given overlay operation and the dimensions of the inputs. The created
	 ...
	 *
	 * <ul>
	 * <li>{@link #INTERSECTION} - result has the dimension of the lowest input
	 * dimension
	 ...
	 * <li>{@link #SYMDIFFERENCE} - result has the dimension of the highest input
	 * dimension (since the symmetric Difference is the union of the differences).
	 * </ul>
	 *
	 * @param overlayOpCode
