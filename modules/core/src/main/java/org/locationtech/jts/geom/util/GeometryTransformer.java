 ...
 * {@link #transformMultiPolygon(MultiPolygon, Geometry)} method to ensure that
 * if input Polygons change type the result is a <tt>GeometryCollection</tt>,
 ...
 * transform<i>XXX</i></code> methods should always return valid geometry - if
 * they cannot do this they should return <code>null</code> (for instance, it
 * may not be possible for a transformLineString implementation to return at
 * least two points - in this case, it should return <code>null</code>). The
 * {@link #transform(Geometry)} method itself will always return a non-null
...
public class GeometryTransformer {
	...
	/**
	 * <code>true</code> if the output from a collection argument should still be a
	 * collection
	 */
	 ...
	 * degenerate ring of 3 or fewer points). In this case a LineString is returned.
	...
	protected Geometry transformPolygon(Polygon geom, Geometry parent) {
		...
		if (isAllValidLinearRings)
			return factory.createPolygon((LinearRing) shell, (LinearRing[]) holes.toArray(new LinearRing[]{}));
		else {
