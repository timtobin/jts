 ...
 * particular Geometry types.
 *
 * <p>A typically usage would be a transformation class that transforms <tt>Polygons</tt> into
 * <tt>Polygons</tt>, <tt>LineStrings</tt> or <tt>Points</tt>, depending on the geometry of the
 * input (For instance, a simplification operation). This class would likely need to override the
 * {@link #transformMultiPolygon(MultiPolygon, Geometry)} method to ensure that if input Polygons
 * change type the result is a <tt>GeometryCollection</tt>, not a <tt>MultiPolygon</tt>.
 *
 * <p>The default behaviour of this class is simply to recursively transform each Geometry component
 * into an identical object by deep copying down to the level of, but not including, coordinates.
 *
 * <p>All <code>transformX</code> methods may return <code>null</code>, to avoid creating empty or
 * invalid geometry objects. This will be handled correctly by the transformer. <code>
 * transform<i>XXX</i></code> methods should always return valid geometry - if they cannot do this
 * they should return <code>null</code> (for instance, it may not be possible for a
 * transformLineString implementation to return at least two points - in this case, it should return
 * <code>null</code>). The {@link #transform(Geometry)} method itself will always return a non-null
 * Geometry object (but this may be empty).
 *
 * @version 1.7
 * @see GeometryEditor
 */
public class GeometryTransformer {
  ...
  public final Geometry transform(Geometry inputGeom) {
    ...
    if (inputGeom instanceof Point point) return transformPoint(point, null);
    if (inputGeom instanceof MultiPoint point) return transformMultiPoint(point, null);
    if (inputGeom instanceof LinearRing ring) return transformLinearRing(ring, null);
    if (inputGeom instanceof LineString string) return transformLineString(string, null);
    if (inputGeom instanceof MultiLineString string) return transformMultiLineString(string, null);
    if (inputGeom instanceof Polygon polygon) return transformPolygon(polygon, null);
    if (inputGeom instanceof MultiPolygon polygon) return transformMultiPolygon(polygon, null);
  ...
  /**
   * Transforms a {@link CoordinateSequence}. This method should always return a valid coordinate
   * list for the desired result type. (E.g. a coordinate list for a LineString must have 0 or at
   * least 2 points). If this is not possible, return an empty sequence - this will be pruned out.
   *
  ...
  protected Geometry transformPoint(Point geom, Geometry parent) {
    return factory.createPoint(transformCoordinates(geom.getCoordinateSequence(), geom));
  }
   ...
   * which does not form a structurally valid ring (i.e. a degenerate ring of 3 or fewer points). In
  ...
  protected Geometry transformPolygon(Polygon geom, Geometry parent) {
    ...
    if (shellIsNullOrEmpty || !(shell instanceof LinearRing)) isAllValidLinearRings = false;
