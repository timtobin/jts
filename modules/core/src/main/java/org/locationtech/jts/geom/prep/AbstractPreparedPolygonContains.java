...
/**
 * A base class containing the logic for computes the <tt>contains</tt> and <tt>covers</tt> spatial
 * relationship predicates for a {@link PreparedPolygon} relative to all other {@link Geometry}
 * classes. Uses short-circuit tests and indexing to improve performance.
 *
 * <p>Contains and covers are very similar, and differ only in how certain cases along the boundary
 * are handled. These cases require full topological evaluation to handle, so all the code in this
 * class is common to both predicates.
 *
 * <p>It is not possible to short-circuit in all cases, in particular in the case where line
 * segments of the test geometry touches the polygon linework. In this case full topology must be
 * computed. (However, if the test geometry consists of only points, this <i>can</i> be evaluated in
...
abstract class AbstractPreparedPolygonContains extends PreparedPolygonPredicate {
   ...
   *
   * <p>For contains the value is true. For covers the value is false.
   */
  ...
  protected boolean eval(Geometry geom) {
     ...
     * important cases, finding a proper intersection implies that the test geometry is NOT
  ...
  /**
   * Evaluation optimized for Point geometries. This provides about a 2x performance increase, and
   * less memory usage.
   *
  ...
  private boolean isProperIntersectionImpliesNotContainedSituation(Geometry testGeom) {
    /**
     * If the test geometry is polygonal we have the A/A situation. In this case, a proper
     * intersection indicates that the Epsilon-Neighbourhood Exterior Intersection condition exists.
     * This condition means that in some small area around the intersection point, there must exist
     * a situation where the interior of the test intersects the exterior of the target. This
     * implies the test is NOT contained in the target.
     */
    if (testGeom instanceof Polygonal) return true;
    /**
     * A single shell with no holes allows concluding that a proper intersection implies not
     * contained (due to the Epsilon-Neighbourhood Exterior Intersection condition)
     */
