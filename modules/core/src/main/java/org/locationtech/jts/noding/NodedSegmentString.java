 ...
 * {@link Coordinate}s. Intended to optimize the noding of contiguous segments
 * by reducing the number of allocated objects. {@link SegmentString}s can carry
 * a context object, which is useful for preserving topological or parentage
 * information. All noded substrings are initialized with the same context
 * object.
 *
 * <p>
 * For read-only applications use {@link BasicSegmentString}, which is
...
public class NodedSegmentString implements NodableSegmentString {
	/**
	 * Gets the {@link SegmentString}s which result from splitting this string at
	 * node points.
	 *
	 * @param segStrings
	...
	/**
	 * Adds the noded {@link SegmentString}s which result from splitting this string
	 * at node points.
	 *
	 * @param segStrings
	 *            a Collection of NodedSegmentStrings
