...
/**
 * A {@link CoordinateSequence} backed by an array of {@link Coordinate}s. This
 * is the implementation that {@link Geometry}s use by default. Coordinates
 * returned by #toArray and #getCoordinate are live -- modifications to them are
 * actually changing the CoordinateSequence's underlying data. A dimension may
 * be specified for the coordinates in the sequence, which may be 2 or 3. The
 * actual coordinates will always have 3 ordinates, but the dimension is useful
 * as metadata in some situations.
 *
 * @version 1.7
 */
public class CoordinateArraySequence implements CoordinateSequence, Serializable {
	 ...
	 * {@link CoordinateArrays#enforceConsistency(Coordinate[])} ).
	 *
	 * @param coordinates
	 *            the coordinate array that will be referenced.
	 * @param dimension
