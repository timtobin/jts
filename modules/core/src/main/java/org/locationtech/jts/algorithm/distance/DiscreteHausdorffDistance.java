...
/**
 * An algorithm for computing a distance metric which is an approximation to the
 * Hausdorff Distance based on a discretization of the input {@link Geometry}.
 * The algorithm computes the Hausdorff distance restricted to discrete points
 * for one of the geometries. The points can be either the vertices of the
 * geometries (the default), or the geometries with line segments densified by a
 * given fraction. The class can also determine two points of the geometries
 ...
 *
 * <ul>
 * <li>computing distance between Linestrings that are roughly parallel to each
 * other, and roughly equal in length. This occurs in matching linear networks.
 ...
 * </pre>
 *
 * The class can compute the oriented Hausdorff distance from A to B. This
 * computes the distance to the farthest point on A from B.
 *
 * <blockquote>
 *
 * <i>OHD(A, B) = max<sub>a &isin; A</sub>( Distance(a, B) )</i> <br>
 * with <br>
 ...
 * distance of another one B. This is more efficient than testing whether A is
 * covered by a buffer of B.
 *
...
public class DiscreteHausdorffDistance {
	...
	/**
	 * Computes the Hausdorff distance between two geometries, with each segment
	 ...
	 *            the second input
	 * @param densifyFrac
	 *            the densification fraction (in [0, 1])
	 * @return the Hausdorff distance between g0 and g1
	 */
	...
	/**
	 * Computes a line containing points indicating the Hausdorff distance between
	 * two geometries.
	 *
	 * @param g0
	 *            the first input
	...
	/**
	 * Computes a line containing points indicating the Hausdorff distance between
	 * two geometries, with each segment densified by the given fraction.
	 *
	 * @param g0
	 *            the first input
	 * @param g1
	...
	/**
	 * Computes the oriented Hausdorff distance from one geometry to another, with
	 * each segment densified by the given fraction.
	 *
	 * @param g0
	 *            the first input
	 * @param g1
	...
	/**
	 * Computes a line containing points indicating the computed oriented Hausdorff
	 * distance from one geometry to another.
	 *
	 * @param g0
	 *            the first input
	 * @param g1
	...
	/**
	 * Computes a line containing points indicating the computed oriented Hausdorff
	 * distance from one geometry to another, with each segment densified by the
	 * given fraction.
	 *
	 * @param g0
	 *            the first input
	...
	private static class MaxDensifiedByFractionDistanceFilter implements CoordinateSequenceFilter {
	...
	private static class MaxPointDistanceFilter implements CoordinateFilter {
		private final Geometry geom;
		private final PointPairDistance maxPtDist = new PointPairDistance();
		private final PointPairDistance minPtDist = new PointPairDistance();
