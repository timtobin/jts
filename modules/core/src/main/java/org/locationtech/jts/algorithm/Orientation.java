...
public class Orientation {
	...
	/**
	 * Returns the orientation index of the direction of the point <code>q</code>
	 * relative to a directed infinite line specified by <code>p1-p2</code>. The
	 * index indicates whether the point lies to the {@link #LEFT} or {@link #RIGHT}
	 * of the line, or lies on it {@link #COLLINEAR}. The index also indicates the
	 * orientation of the triangle formed by the three points (
	 ...
	 * @return -1 ( {@link #CLOCKWISE} or {@link #RIGHT} ) if q is clockwise (right)
	 *         from p1-p2; 1 ( {@link #COUNTERCLOCKWISE} or {@link #LEFT} ) if q is
	 *         counter-clockwise (left) from p1-p2; 0 ( {@link #COLLINEAR} or
	 *         {@link #STRAIGHT} ) if q is collinear with p1-p2
	 */
	public static int index(Coordinate p1, Coordinate p2, Coordinate q) {
		 ...
		 *
		 * For instance, the following situation produces identical results in spite of
		 * the inverse orientation of the line segment:
		 *
	...
	/**
	 * Tests if a ring defined by a {@link CoordinateSequence} is oriented
	 * counter-clockwise.
	 *
	 * <ul>
	 * <li>The list of points is assumed to have the first and last points equal.
	 * <li>This handles coordinate lists which contain repeated points.
	 * <li>This handles rings which contain collapsed segments (in particular, along
	 * the top of the ring).
	 * </ul>
	 *
	 ...
	 *
	 * @param ring
	 *            a CoordinateSequence forming a ring (with first and last point
	 *            identical)
	...
	public static boolean isCCW(CoordinateSequence ring) {
		...
		/**
		 * Find first highest point after a lower point, if one exists (e.g. a rising
		 * segment) If one does not exist, hiIndex will remain 0 and the ring must be
		 * flat. Note this relies on the convention that rings have the same start and
		 * end point.
		 */
		...
		/**
		 * Find the next lower point after the high point (e.g. a falling segment). This
		 * must exist since ring is not flat.
		 */
		int iDownLow = iUpHi;
		do {
		...
		/**
		 * Two cases can occur: 1) the hiPt and the downPrevPt are the same. This is the
		 * general position case of a "pointed cap". The ring orientation is determined
		 * by the orientation of the cap 2) The hiPt and the downPrevPt are different.
		 * In this case the top of the cap is flat. The ring orientation is given by the
		 * direction of the flat segment
		 */
		if (upHiPt.equals2D(downHiPt)) {
			/**
			 * Check for the case where the cap has configuration A-B-A. This can happen if
			 * the ring does not contain 3 distinct points (including the case where the
			 * input array has fewer than 4 elements), or it contains coincident line
			 * segments.
			 */
			...
			/**
			 * It can happen that the top segments are coincident. This is an invalid ring,
			 * which cannot be computed correctly. In this case the orientation is 0, and
			 * the result is false.
			 */
	...
	/**
	 * Tests if a ring defined by an array of {@link Coordinate}s is oriented
	 * counter-clockwise.
	 *
	 * <ul>
	 * <li>The list of points is assumed to have the first and last points equal.
	 ...
	 *
	 * @param ring
	 *            an array of Coordinates forming a ring (with first and last point
	 *            identical)
	 * @return true if the ring is oriented counter-clockwise.
	...
	/**
	 * Tests if a ring defined by an array of {@link Coordinate}s is oriented
	 * counter-clockwise, using the signed area of the ring.
	 *
	 * <ul>
	 * <li>The list of points is assumed to have the first and last points equal.
	 * <li>This handles coordinate lists which contain repeated points.
