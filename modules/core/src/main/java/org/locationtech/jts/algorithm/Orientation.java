 ...
 * generally geometry on two-dimensional manifolds).
 *
 * <p>Determining triangle orientation is notoriously subject to numerical precision errors in the
 * case of collinear or nearly collinear points. JTS uses extended-precision arithmetic to increase
...
public class Orientation {
   ...
   * @return -1 ( {@link #CLOCKWISE} or {@link #RIGHT} ) if q is clockwise (right) from p1-p2; 1 (
   *     {@link #COUNTERCLOCKWISE} or {@link #LEFT} ) if q is counter-clockwise (left) from p1-p2; 0
   *     ( {@link #COLLINEAR} or {@link #STRAIGHT} ) if q is collinear with p1-p2
   */
  ...
  /**
   * Tests if a ring defined by an array of {@link Coordinate}s is oriented counter-clockwise.
   *
   * <ul>
   *   <li>The list of points is assumed to have the first and last points equal.
   *   <li>This handles coordinate lists which contain repeated points.
   *   <li>This handles rings which contain collapsed segments (in particular, along the top of the
   *       ring).
   * </ul>
   *
   * This algorithm is guaranteed to work with valid rings. It also works with "mildly invalid"
   * rings which contain collapsed (coincident) flat segments along the top of the ring. If the ring
   * is "more" invalid (e.g. self-crosses or touches), the computed result may not be correct.
   *
  ...
  /**
   * Tests if a ring defined by a {@link CoordinateSequence} is oriented counter-clockwise.
   *
   * <ul>
   *   <li>The list of points is assumed to have the first and last points equal.
   *   <li>This handles coordinate lists which contain repeated points.
  ...
  /**
   * Tests if a ring defined by an array of {@link Coordinate}s is oriented counter-clockwise, using
   ...
   * </ul>
   *
   * This algorithm is guaranteed to work with valid rings. For invalid rings (containing
   * self-intersections), the algorithm determines the orientation of the largest enclosed area
   * (including overlaps). This provides a more useful result in some situations, such as buffering.
   *
   * <p>However, this approach may be less accurate in the case of rings with almost zero area.
   * (Note that the orientation of rings with zero area is essentially undefined, and hence
   * non-deterministic.)
   *
   * @param ring an array of Coordinates forming a ring (with first and last point identical)
   * @return true if the ring is oriented counter-clockwise.
