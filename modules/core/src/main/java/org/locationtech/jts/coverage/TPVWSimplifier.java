...
/**
 * Computes a Topology-Preserving Visvalingam-Whyatt simplification of a set of
 * input lines. The simplified lines will contain no more intersections than are
 * present in the original input. Line and ring endpoints are preserved, except
 * for rings which are flagged as "free". Rings which are smaller than the
 * tolerance area may be removed entirely, as long as they are flagged as
 * removable.
 *
 * <p>
 * The amount of simplification is determined by a tolerance value, which is a
 * non-zero quantity. It is the square root of the area tolerance used in the
 * Visvalingam-Whyatt algorithm. This equates roughly to the maximum distance by
 * which a simplified line can change from the original.
 *
 * @author mdavis
 */
class TPVWSimplifier {
	 ...
	 * themselves and a set of linear constraints. The endpoints of lines are
	 * preserved. The endpoint of rings are preserved as well, unless the ring is
	 * indicated as "free" via a bit flag with the same index.
	 *
	 * @param lines
	 *            the lines to simplify
	 * @param freeRings
	 *            flags indicating which ring edges do not have node endpoints
	 * @param constraintLines
	 *            the linear constraints (may be null)
	...
	public static class Edge {
		...
		/**
		 * Creates a new edge. The endpoints of the edge are preserved during
		 * simplification, unless it is a ring and the {@Link #isFreeRing} flag is set.
		 *
		 * @param pts
		 *            the line or ring
		 * @param distanceTolerance
		 * @param isFreeRing
		 *            whether a ring endpoint can be removed
		 * @param isFreeRing
		 * @param isRemovable
		 */
		...
		/**
		 * Tests if any vertices in a line intersect the corner triangle. Uses the
		 * vertex spatial index for efficiency.
		 *
		 * @param corner
		 *            the corner vertices
		 * @param cornerEnv
		 *            the envelope of the corner
		 * @param edge
		 *            the hull to test
		...
		/**
		 * Removes a corner by removing the apex vertex from the ring. Two new corners
		 * are created with apexes at the other vertices of the corner (if they are
		 * non-convex and thus removable).
		 *
		 * @param corner
		 *            the corner to remove
		 * @param cornerArea
		 * @param areaTolerance
		 * @param cornerQueue
		 *            the corner queue
		 */
		private void removeCorner(Corner corner, double areaTolerance, CornerArea cornerArea,
		...
		public void updateRemoved(double removableSizeFactor) {
			if (!isRemovable)
				return;
			double areaTolerance = distanceTolerance * distanceTolerance;
			isRemoved = CoordinateArrays.isRing(pts) && Area.ofRing(pts) < removableSizeFactor * areaTolerance;
		}
	}
