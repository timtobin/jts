...
/**
 * Algorithms for computing values and predicates associated with triangles. For
 * some algorithms extended-precision implementations are provided, which are
 * more robust (i.e. they produce correct answers in more cases). Also, some
...
public class TrianglePredicate {
	 ...
	 * a, b, c (oriented counter-clockwise). The computation uses {@link DD}
	...
	/**
	 * Tests if a point is inside the circle defined by the triangle with vertices
	 * a, b, c (oriented counter-clockwise). This test uses simple double-precision
	 * arithmetic, and thus may not be robust.
	 *
	 * @param a
	 *            a vertex of the triangle
	 * @param b
	 ...
	 * Tests if a point is inside the circle defined by the triangle with vertices
	 * a, b, c (oriented counter-clockwise). This test uses simple double-precision
	 * arithmetic, and thus is not 100% robust. However, by using normalization to
	 * the origin it provides improved robustness and increased performance.
	 *
	 * <p>
	...
	/**
	 * Tests if a point is inside the circle defined by the triangle with vertices
	 * a, b, c (oriented counter-clockwise). This method uses more robust
	 * computation.
	 *
	 * @param a
	 *            a vertex of the triangle
	 ...
	 *
	 * @param ax
	 *            the x ordinate of a vertex of the triangle
	 * @param ay
	...
	/**
	 * Checks if the computed value for isInCircle is correct, using double-double
	 ...
	 */
	/*
	 * private static void checkRobustInCircle(Coordinate a, Coordinate b,
	 * Coordinate c, Coordinate p) { boolean nonRobustInCircle =
	 * isInCircleNonRobust(a, b, c, p); boolean isInCircleDD =
	 * TrianglePredicate.isInCircleDDSlow(a, b, c, p); boolean isInCircleCC =
	 * TrianglePredicate.isInCircleCC(a, b, c, p);
	 *
	 * Coordinate circumCentre = Triangle.circumcentre(a, b, c);
	 * System.out.println("p radius diff a = " + Math.abs(p.distance(circumCentre) -
	 * a.distance(circumCentre)) / a.distance(circumCentre));
	 *
	 * if (nonRobustInCircle != isInCircleDD || nonRobustInCircle != isInCircleCC) {
	 * System.out.println("inCircle robustness failure (double result = " +
	 * nonRobustInCircle + ", DD result = " + isInCircleDD + ", CC result = " +
	 * isInCircleCC + ")"); System.out.println(WKTWriter.toLineString(new
	 * CoordinateArraySequence( new Coordinate[] { a, b, c, p })));
