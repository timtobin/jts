...
/**
 * Algorithms for computing values and predicates associated with triangles. For some algorithms
 * extended-precision implementations are provided, which are more robust (i.e. they produce correct
 * answers in more cases). Also, some more robust formulations of some algorithms are provided,
 * which utilize normalization to the origin.
 *
 * @author Martin Davis
 */
public class TrianglePredicate {
  /**
   * Tests if a point is inside the circle defined by the triangle with vertices a, b, c (oriented
   * counter-clockwise). This test uses simple double-precision arithmetic, and thus may not be
   * robust.
   *
  ...
  /**
   * Tests if a point is inside the circle defined by the triangle with vertices a, b, c (oriented
   * counter-clockwise). This test uses simple double-precision arithmetic, and thus is not 100%
   * robust. However, by using normalization to the origin it provides improved robustness and
   * increased performance.
   *
   * <p>Based on code by J.R.Shewchuk.
   *
   * @param a a vertex of the triangle
   * @param b a vertex of the triangle
   * @param c a vertex of the triangle
  ...
  /**
   * Tests if a point is inside the circle defined by the triangle with vertices a, b, c (oriented
   * counter-clockwise). This method uses more robust computation.
   *
   * @param a a vertex of the triangle
   * @param b a vertex of the triangle
   * @param c a vertex of the triangle
  ...
  /**
   * Tests if a point is inside the circle defined by the triangle with vertices a, b, c (oriented
   * counter-clockwise). The computation uses {@link DD} arithmetic for robustness.
   *
   * @param a a vertex of the triangle
   * @param b a vertex of the triangle
   * @param c a vertex of the triangle
  ...
  public static boolean isInCircleDDSlow(Coordinate a, Coordinate b, Coordinate c, Coordinate p) {
    ...
    DD aTerm =
        (ax.multiply(ax).add(ay.multiply(ay))).multiply(triAreaDDSlow(bx, by, cx, cy, px, py));
    DD bTerm =
        (bx.multiply(bx).add(by.multiply(by))).multiply(triAreaDDSlow(ax, ay, cx, cy, px, py));
    DD cTerm =
        (cx.multiply(cx).add(cy.multiply(cy))).multiply(triAreaDDSlow(ax, ay, bx, by, px, py));
    DD pTerm =
        (px.multiply(px).add(py.multiply(py))).multiply(triAreaDDSlow(ax, ay, bx, by, cx, cy));
  ...
  /**
   * Computes twice the area of the oriented triangle (a, b, c), i.e., the area is positive if the
   * triangle is oriented counterclockwise. The computation uses {@link DD} arithmetic for
   * robustness.
   *
  ...
  public static boolean isInCircleDDFast(Coordinate a, Coordinate b, Coordinate c, Coordinate p) {
    DD aTerm = (DD.sqr(a.x).selfAdd(DD.sqr(a.y))).selfMultiply(triAreaDDFast(b, c, p));
    DD bTerm = (DD.sqr(b.x).selfAdd(DD.sqr(b.y))).selfMultiply(triAreaDDFast(a, c, p));
    DD cTerm = (DD.sqr(c.x).selfAdd(DD.sqr(c.y))).selfMultiply(triAreaDDFast(a, b, p));
  ...
  /**
   * Computes the inCircle test using distance from the circumcentre. Uses standard double-precision
   * arithmetic.
   *
   * <p>In general this doesn't appear to be any more robust than the standard calculation. However,
   * there is at least one case where the test point is far enough from the circumcircle that this
