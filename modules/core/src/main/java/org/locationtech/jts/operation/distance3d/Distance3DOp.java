...
/**
 * Find two points on two 3D {@link Geometry}s which lie within a given distance, or else are the
 * nearest points on the geometries (in which case this also provides the distance between the
 * geometries).
 *
 * <p>3D geometries have vertex Z ordinates defined. 3D {@link Polygon}s are assumed to lie in a
 * single plane (which is enforced if not actually the case). 3D {@link LineString}s and {@link
 * Point}s may have any configuration.
 *
 * <p>The distance computation also finds a pair of points in the input geometries which have the
 * minimum distance between them. If a point lies in the interior of a line segment, the coordinate
 * computed is a close approximation to the exact point for X and Y ordinates. Z ordinate is not
 * interpolated.
 *
 * <p>The algorithms used are straightforward O(n^2) comparisons. This worst-case performance could
 * be improved on by using Voronoi techniques or spatial indexes.
 *
 * @version 1.7
 */
public class Distance3DOp {
  ...
  public Coordinate[] nearestPoints() {
    ...
    Coordinate[] nearestPts =
        new Coordinate[] {
          minDistanceLocation[0].getCoordinate(), minDistanceLocation[1].getCoordinate()
        };
    return nearestPts;
  }
  ...
  private void updateDistance(
    ...
    if (minDistance < terminateDistance) isDone = true;
  }
  ...
  /**
   * Finds the index of the "most polygonal" input geometry. This optimizes the computation of the
   * best-fit plane, since it is cached only for the left-hand geometry.
   *
  ...
  private int mostPolygonalIndex() {
    ...
    if (dim0 >= 2 && dim1 >= 2) {
      if (geom[0].getNumPoints() > geom[1].getNumPoints()) return 0;
      return 1;
    }
  ...
  private void computeMinDistanceMultiMulti(Geometry g0, Geometry g1, boolean flip) {
    if (g0 instanceof GeometryCollection) {
      ...
      } else computeMinDistanceOneMulti(g0, g1, flip);
    }
  }
   ...
   *
   * <p>To compute the distance, compute the distance between the rings of one polygon and the other
   * polygon, and vice-versa. If the polygons intersect, then at least one ring must intersect the
   * other polygon. Note that it is NOT sufficient to test only the shell rings. A counter-example
   * is a "figure-8" polygon A and a simple polygon B at right angles to A, with the ring of B
   * passing through the holes of A. The polygons intersect, but A's shell does not intersect B, and
   * B's shell does not intersect A.
   *
   * @param poly0
   * @param poly1
   * @param geomIndex
   */
  private void computeMinDistancePolygonPolygon(
      PlanarPolygon3D poly0, Polygon poly1, boolean flip) {
   ...
   * @param geomIndex
   */
  private void computeMinDistancePolygonRings(
      PlanarPolygon3D poly, Polygon ringPoly, boolean flip) {
    // compute shell ring
  ...
  private Coordinate intersection(PlanarPolygon3D poly, LineString line) {
    ...
    for (int i = 0; i < seq.size() - 1; i++) {
      ...
      /**
       * If the oriented distances of the segment endpoints have the same sign, the segment does not
       * cross the plane, and is skipped.
       */
      if (d0 * d1 > 0) continue;

      /**
       * Compute segment-plane intersection point which is then used for a point-in-polygon test.
       * The endpoint distances to the plane d0 and d1 give the proportional distance of the
  ...
  private void computeMinDistancePolygonPoint(
