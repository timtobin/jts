...
/**
 * Find two points on two 3D {@link Geometry}s which lie within a given
 * distance, or else are the nearest points on the geometries (in which case
 * this also provides the distance between the geometries).
 *
 * <p>
 * 3D geometries have vertex Z ordinates defined. 3D {@link Polygon}s are
 * assumed to lie in a single plane (which is enforced if not actually the
 * case). 3D {@link LineString}s and {@link Point}s may have any configuration.
 *
 * <p>
...
public class Distance3DOp {
	...
	/**
	 * Computes a point at a distance along a segment specified by two relatively
	 * proportional values. The fractional distance along the segment is d0/(d0+d1).
	 *
	 * @param p0
	...
	private void computeMinDistancePointPoint(Point point0, Point point1, boolean flip) {
		...
		if (dist < minDistance) {
			updateDistance(dist, new GeometryLocation(point0, 0, point0.getCoordinate()),
	...
	private void computeMinDistancePolygonLine(PlanarPolygon3D poly, LineString line, boolean flip) {
		...
		if (intPt != null) {
			updateDistance(0, new GeometryLocation(poly.getPolygon(), 0, intPt), new GeometryLocation(line, 0, intPt),
					flip);
			return;
		}
	 ...
	 *
	 * <p>
	 * To compute the distance, compute the distance between the rings of one
	 * polygon and the other polygon, and vice-versa. If the polygons intersect,
	 * then at least one ring must intersect the other polygon. Note that it is NOT
	 * sufficient to test only the shell rings. A counter-example is a "figure-8"
	 * polygon A and a simple polygon B at right angles to A, with the ring of B
	 * passing through the holes of A. The polygons intersect, but A's shell does
	 * not intersect B, and B's shell does not intersect A.
	 *
	 * @param poly0
	 * @param poly1
	 * @param geomIndex
	 ...
	 *
	 * @return the distance between the geometries, or 0 if either input geometry is
	 *         empty
	...
	private Coordinate intersection(PlanarPolygon3D poly, LineString line) {
		...
		for (int i = 0; i < seq.size() - 1; i++) {
			...
			/**
			 * If the oriented distances of the segment endpoints have the same sign, the
			 * segment does not cross the plane, and is skipped.
			 */
			if (d0 * d1 > 0)
				continue;

			/**
			 * Compute segment-plane intersection point which is then used for a
			 * point-in-polygon test. The endpoint distances to the plane d0 and d1 give the
			 * proportional distance of the intersection point along the segment.
			 */
	...
	/**
	 * Finds the index of the "most polygonal" input geometry. This optimizes the
	 * computation of the best-fit plane, since it is cached only for the left-hand
	 * geometry.
	 *
