...
/**
 * Interface for classes which process triangles visited during traversals of a {@link
 * QuadEdgeSubdivision}
 *
 * @author Martin Davis
 */
public interface TraversalVisitor {
   ...
   * neighbouring triangle should be added to the queue so its neighbours are visited. Often it will
   * perform processing on the neighbour triangle as well, in order to mark it as processed
   * (visited) and/or to determine if it should be visited. Note that choosing <b>not</b> to visit
   * the neighbouring triangle is the terminating condition for many traversal algorithms. In
   * particular, if the neighbour triangle has already been visited, it should not be visited again.
   *
