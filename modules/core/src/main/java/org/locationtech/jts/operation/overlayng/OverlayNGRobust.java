 ...
 * series of increasingly robust (but slower) noding strategies.
 *
 * <p>The noding strategies used are:
 *
 * <ol>
 *   <li>A simple, fast noder using FLOATING precision.
 *   <li>A {@link SnappingNoder} using an automatically-determined snap tolerance
 *   <li>First snapping each geometry to itself, and then overlaying them using a <code>
 *       SnappingNoder</code>.
 *   <li>The above two strategies are repeated with increasing snap tolerance, up to a limit.
 *   <li>Finally a {@link org.locationtech.jts.noding.snapround.SnapRoundingNoder} is used with a
 *       automatically-determined scale factor intended to preserve input precision while still
 *       preventing robustness problems.
 * </ol>
 *
 * If all of the above attempts fail to compute a valid overlay, the original {@link
 * TopologyException} is thrown. In practice this is extremely unlikely to occur.
 *
 * <p>This algorithm relies on each overlay operation execution throwing a {@link TopologyException}
 * if it is unable to compute the overlay correctly. Generally this occurs because the noding phase
 * does not produce a valid noding. This requires the use of a {@link ValidatingNoder} in order to
 * check the results of using a floating noder.
 *
 * @author Martin Davis
 * @see OverlayNG
 */
public class OverlayNGRobust {
  ...
  private static final UnionStrategy OVERLAY_UNION =
      new UnionStrategy() {
   ...
   * @return the computed overlay result, or null if the overlay fails
   */
  private static Geometry overlaySnapping(
   ...
   * @return the computed overlay result, or null if the overlay fails
   */
  private static Geometry overlaySnapBoth(
      Geometry geom0, Geometry geom1, int opCode, double snapTol) {
    try {
  ...
  private static Geometry overlaySnapTol(
      Geometry geom0, Geometry geom1, int opCode, double snapTol) {
    SnappingNoder snapNoder = new SnappingNoder(snapTol);
