...
/**
 * Performs an overlay operation using {@link OverlayNG}, providing full
 * robustness by using a series of increasingly robust (but slower) noding
 * strategies.
 *
 * <p>
 ...
 * <li>First snapping each geometry to itself, and then overlaying them using a
 * <code>
 *       SnappingNoder</code>.
 * <li>The above two strategies are repeated with increasing snap tolerance, up
 * to a limit.
 * <li>Finally a {@link org.locationtech.jts.noding.snapround.SnapRoundingNoder}
 * is used with a automatically-determined scale factor intended to preserve
 * input precision while still preventing robustness problems.
 * </ol>
 *
 * If all of the above attempts fail to compute a valid overlay, the original
 * {@link TopologyException} is thrown. In practice this is extremely unlikely
 * to occur.
 *
 * <p>
 * This algorithm relies on each overlay operation execution throwing a
 * {@link TopologyException} if it is unable to compute the overlay correctly.
 * Generally this occurs because the noding phase does not produce a valid
 * noding. This requires the use of a {@link ValidatingNoder} in order to check
 * the results of using a floating noder.
 *
 * @author Martin Davis
 * @see OverlayNG
 */
public class OverlayNGRobust {
	...
	/**
	 * A factor for a snapping tolerance distance which should allow noding to be
	 * computed robustly.
	 */
	private static final double SNAP_TOL_FACTOR = 1e12;

	/**
	 * Computes the largest magnitude of the ordinates of a geometry, based on the
	 * geometry envelope.
	 *
	 * @param geom
	 *            a geometry
	...
	/**
	 * Overlay two geometries, using heuristics to ensure computation completes
	...
	public static Geometry overlay(Geometry geom0, Geometry geom1, int opCode) {
		...
		/**
		 * First try overlay with a FLOAT noder, which is fast and causes least change
		 * to geometry coordinates By default the noder is validated, which is required
		 * in order to detect certain invalid noding situations which otherwise cause
		 * incorrect overlay output.
		 */
		try {
			...
			/**
			 * Capture original exception, so it can be rethrown if the remaining strategies
			 * all fail.
			 */
			exOriginal = ex;
		}
		...
		/**
		 * On failure retry using snapping noding with a "safe" tolerance. if this
		 * throws an exception just let it go, since it is something that is not a
		 * TopologyException
		 */
		...
		/**
		 * On failure retry using snap-rounding with a heuristic scale factor (grid
		 * size).
		 */
	...
	/**
	 * Attempt Overlay using Snap-Rounding with an automatically-determined scale
	 * factor.
	 *
	 * @param geom0
	 * @param geom1
	...
	/**
	 * Attempt overlay using snapping with repeated tries with increasing snap
	 * tolerances.
	 *
	 * @param geom0
	 * @param geom1
	 ...
	 * which improves robustness. Collapsed artifacts are removed from the result to
	 * allow using it in further overlay operations.
	 *
	 * @param geom
	...
	private static Geometry snapSelf(Geometry geom, double snapTol) {
		...
		/**
		 * Ensure the result is not mixed-dimension, since it will be used in further
		 * overlay computation. It may however be lower dimension, if it collapses
		 * completely due to snapping.
		 */
	...
	/**
	 * Computes a heuristic snap tolerance distance for overlaying a pair of
	 * geometries using a {@link SnappingNoder}.
	 *
	 * @param geom0
	 * @param geom1
	...
	/**
	 * Computes the unary union of a collection of geometries using robust
	 * computation.
	 *
	 * @param geoms
	 ...
	 * System.out.println(msg); System.out.println(geom0);
