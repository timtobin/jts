...
/**
 * Provides an efficient method of unioning a collection of {@link Polygonal}
 * geometries. The geometries are indexed using a spatial index, and unioned
 * recursively in index order. For geometries with a high degree of overlap,
 * this has the effect of reducing the number of vertices early in the process,
 * which increases speed and robustness.
 *
 * <p>
...
public class CascadedPolygonUnion {
	 ...
	 * Testing indicates that a smaller capacity is better. For an STRtree, 4 is
	 * probably a good number (since this produces 2x2 "squares").
	 */
	 ...
	 * Extracts the {@link Polygon}s from the input and returns them as an
	 * appropriate {@link Polygonal} geometry.
	 *
	 * <p>
	 ...
	 */
	/*
	 * private Geometry repeatedUnion(List geoms) { Geometry union = null; for
	 * (Iterator i = geoms.iterator(); i.hasNext(); ) { Geometry g = (Geometry)
	 * i.next(); if (union == null) union = g.copy(); else union =
	 * unionFun.union(union, g); } return union; }
	 */

	// =======================================

	/**
	 * Unions a list of geometries by treating the list as a flattened binary tree,
	...
	/**
	 * Unions a section of a list using a recursive binary union on each half of the
	 * section.
	 *
	 * @param geoms
	...
	/**
	 * Reduces a tree of geometries to a list of geometries by recursively unioning
	 * the subtrees in the list.
	 *
	 * @param geomTree
	 ...
	 *
	 * <p>
	 * This method discards the input geometries as they are processed. In many
	 * input cases this reduces the memory retained as the operation proceeds.
	 * Optimal memory usage is achieved by disposing of the original input
	 * collection before calling this method.
	 *
	 * @return the union of the input geometries or null if no input geometries were
	 *         provided
	...
	public Geometry union() {
		 ...
		 * This makes unioning more efficient, since vertices are more likely to be
		 * eliminated on each round.
		 */
	...
	private Geometry unionTree(List geomTree) {
		/**
		 * Recursively unions all subtrees in the list into single geometries. The
		 * result is a list of Geometrys only
		 */
