...
class MaximalEdgeRing {
   ...
   * <b>incoming</b> result edge is set to the next <b>outgoing</b> result edge.
   *
   * <p>Edges are linked when:
   *
   * <ul>
   ...
   * </ul>
   *
   * <p>Edges are linked in CCW order (which is the order they are linked in the underlying graph).
   * This means that rings have their face on the Right (in other words, the topological location of
   * the face is given by the RHS label of the DirectedEdge). This produces rings with CW
   * orientation.
   *
   * <p>PRECONDITIONS: - This edge is in the result - This edge is not yet linked - The edge and its
   * sym are NOT both marked as being in the result
   */
  ...
  private static OverlayEdge linkMaxInEdge(
      OverlayEdge currOut, OverlayEdge currMaxRingOut, MaximalEdgeRing maxEdgeRing) {
    OverlayEdge currIn = currOut.symOE();
