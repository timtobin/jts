...
class MaximalEdgeRing {
	...
	/**
	 * Tests if an edge of the maximal edge ring is already linked into a minimal
	 * {@link OverlayEdgeRing}. If so, this node has already been processed earlier
	...
	/**
	 * Links the edges of a {@link MaximalEdgeRing} around this node into minimal
	 * edge rings ({@link OverlayEdgeRing}s). Minimal ring edges are linked in the
	 * opposite orientation (CW) to the maximal ring. This changes self-touching
	...
	/**
	 * Traverses the star of edges originating at a node and links consecutive
	 * result edges together into <b>maximal</b> edge rings. To link two edges the
	 * <code>resultNextMax</code> pointer for an <b>incoming</b> result edge is set
	 * to the next <b>outgoing</b> result edge.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * PRECONDITIONS: - This edge is in the result - This edge is not yet linked -
	...
	public static void linkResultAreaMaxRingAtNode(OverlayEdge nodeEdge) {
		...
		// Assert.isTrue(! nodeEdge.symOE().isInResultArea(), "Found both half-edges in
		// result");

		/**
		 * Since the node edge is an out-edge, make it the last edge to be linked by
		 * starting at the next edge. The node edge cannot be an in-edge as well, but
		 * the next one may be the first in-edge.
		 */
		...
		do {
			/**
			 * If an edge is linked this node has already been processed so can skip further
			 * processing
			 */
