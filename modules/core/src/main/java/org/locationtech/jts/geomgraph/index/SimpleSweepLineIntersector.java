...
/**
 * Finds all intersections in one or two sets of edges, using a simple x-axis
 * sweepline algorithm. While still O(n^2) in the worst case, this algorithm
...
public class SimpleSweepLineIntersector extends EdgeSetIntersector {
	...
	private void processOverlaps(int start, int end, SweepLineEvent ev0, SegmentIntersector si) {
		...
		/**
		 * Since we might need to test for self-intersections, include current INSERT
		 * event object in list of event objects to test. Last index can be skipped,
		 * because it must be a Delete event.
		 */
