...
/**
 * Implements some 2D matrix operations (in particular, solving systems of
 * linear equations).
 *
 * @author Martin Davis
 */
public class Matrix {
	/**
	 * Solves a system of equations using Gaussian Elimination. In order to avoid
	 * overhead the algorithm runs in-place on A - if A should not be modified the
	 * client must supply a copy.
	 *
	 * @param a
	 *            an nxn matrix in row/column order )modified by this method)
	 * @param b
	 *            a vector of length n
	 * @return a vector containing the solution (if any) or null if the system has
	 *         no or no unique solution
