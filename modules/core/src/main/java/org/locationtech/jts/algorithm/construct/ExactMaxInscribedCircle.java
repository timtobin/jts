...
/**
 * Computes the Maximum Inscribed Circle for some kinds of convex polygons. It determines the circle
 * center point by computing Voronoi node points and testing them for distance to generating edges.
 * This is more precise than iterated approximation, and faster for small polygons (such as
...
class ExactMaxInscribedCircle {
  ...
  /**
   * Tests whether a given geometry is supported by this class. Currently only triangles and convex
  ...
  public static boolean isSupported(Geometry geom) {
    if (!isSimplePolygon(geom)) return false;
    Polygon polygon = (Polygon) geom;
    if (isTriangle(polygon)) return true;
    if (isQuadrilateral(polygon) && isConvex(polygon)) return true;
    return false;
  }

  private static boolean isSimplePolygon(Geometry geom) {
    return geom instanceof Polygon p && p.getNumInteriorRing() == 0;
  }
  ...
  public static Coordinate[] computeRadius(Polygon polygon) {
    ...
    if (ring.length == 4) return computeTriangle(ring);
    else if (ring.length == 5) return computeConvexQuadrilateral(ring);
  ...
  /**
   * The Voronoi nodes of a convex polygon occur at the intersection point of two bisectors of each
   * triplet of edges. The Maximum Inscribed Circle center is the node with the farthest distance
   * from the generating edges. For a quadrilateral there are 4 distinct edge triplets, at each edge
  ...
  private static LineSegment computeConvexBisector(Coordinate[] pts, int index, double len) {
    ...
    if (isConcave(pPrev, basePt, pNext)) throw new IllegalStateException("Input is not convex");
  ...
  private static boolean isConvex(CoordinateSequence ring) {
    /** A ring cannot be all concave, so if it has a consistent orientation it must be convex. */
    int n = ring.size();
    if (n < 4) return false;
    ...
    for (int i = 0; i < n - 1; i++) {
      ...
      int orient =
          Orientation.index(ring.getCoordinate(i), ring.getCoordinate(i1), ring.getCoordinate(i2));
      if (orient == Orientation.COLLINEAR) continue;
      if (ringOrient == 0) {
        ringOrient = orient;
