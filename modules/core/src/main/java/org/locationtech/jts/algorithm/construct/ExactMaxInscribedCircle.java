...
/**
 * Computes the Maximum Inscribed Circle for some kinds of convex polygons. It
 * determines the circle center point by computing Voronoi node points and
 * testing them for distance to generating edges. This is more precise than
 * iterated approximation, and faster for small polygons (such as triangles and
...
class ExactMaxInscribedCircle {
	...
	/**
	 * The Voronoi nodes of a convex polygon occur at the intersection point of two
	 * bisectors of each triplet of edges. The Maximum Inscribed Circle center is
	 * the node with the farthest distance from the generating edges. For a
	 * quadrilateral there are 4 distinct edge triplets, at each edge with its
	 * adjacent edges.
	 *
	 * @param ring
	 *            the polygon ring
	...
	private static boolean isConvex(CoordinateSequence ring) {
		/**
		 * A ring cannot be all concave, so if it has a consistent orientation it must
		 * be convex.
		 */
		int n = ring.size();
		if (n < 4)
		...
		for (int i = 0; i < n - 1; i++) {
			...
			int orient = Orientation.index(ring.getCoordinate(i), ring.getCoordinate(i1), ring.getCoordinate(i2));
	...
	/**
	 * Tests whether a given geometry is supported by this class. Currently only
	 * triangles and convex quadrilaterals are supported.
	 *
	 * @param geom
	 *            an areal geometry
