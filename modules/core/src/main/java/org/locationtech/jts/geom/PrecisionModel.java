 ...
 * a <code>Geometry</code>. A precision model may be <b>floating</b>
 * ({@link #FLOATING} or {@link #FLOATING_SINGLE}), in which case normal
 * floating-point value semantics apply.
 *
 * <p>
 * For a {@link #FIXED} precision model the {@link #makePrecise(Coordinate)}
 * method allows rounding a coordinate to a "precise" value; that is, one whose
 ...
 * model. Constructive methods (such as boolean operations) always round
 * computed coordinates to the appropriate precision model.
 *
 * <p>
 ...
 * <li>FIXED - represents a model with a fixed number of decimal places. A Fixed
 * Precision Model is specified by a <b>scale factor</b>. The scale factor
 * specifies the size of the grid which numbers are rounded to. Input
 * coordinates are mapped to fixed coordinates according to the following
 * equations:
 * <UL>
 ...
 * </ul>
 *
 * For example, to specify 3 decimal places of precision, use a scale factor of
 * 1000. To specify -3 decimal places of precision (i.e. rounding to the nearest
 * 1000), use a scale factor of 0.001.
 *
 * <p>
 * It is also supported to specify a precise <b>grid size</b> by providing it as
 * a negative scale factor. This allows setting a precise grid size rather than
 * using a fractional scale, which provides more accurate and robust rounding.
 * For example, to specify rounding to the nearest 1000 use a scale factor of
 * -1000.
 *
 * <p>
 * Coordinates are represented internally as Java double-precision values. Java
 * uses the IEEE-394 floating point standard, which provides 53 bits of
 * precision. (Thus the maximum precisely representable <i>integer</i> is
...
public class PrecisionModel implements Serializable, Comparable {
	 ...
	 * places. The number of decimal places is determined by the log10 of the scale
	 * factor.
	 */
	...
	/**
	 * Determines which of two {@link PrecisionModel}s is the most precise (allows
	 * the greatest number of significant digits).
	 *
	 * @param pm1
	 *            a PrecisionModel
	 * @param pm2
	...
	/**
	 * If non-zero, the precise grid size specified. In this case, the scale is also
	 * valid and is computed from the grid size. If zero, the scale is used to
	 * compute the grid size where needed.
	 */
	...
	/**
	 * Copy constructor to create a new <code>PrecisionModel</code> from an existing
	 * one.
	 */
	...
	/**
	 * Creates a <code>PrecisionModel</code> that specifies an explicit precision
	 * model type. If the model type is FIXED the scale factor will default to 1.
	 *
	 * @param modelType
	 ...
	 * which are rounded to the grid defined by the scale factor. The provided scale
	 * may be negative, to specify an exact grid size. The scale is then computed as
	 * the reciprocal.
	 *
	 * @param scale
	 *            amount by which to multiply a coordinate after subtracting the
	 *            offset, to obtain a precise coordinate. Must be non-zero.
	 */
	...
	public boolean equals(Object other) {
		if (!(other instanceof PrecisionModel otherPrecisionModel)) {
			return false;
		}
	...
	/**
	 * Returns the maximum number of significant digits provided by this precision
	 * model. Intended for use by routines which need to print out decimal
	 * representations of precise values (such as {@link WKTWriter}).
	 *
	 * <p>
	 * This method would be more correctly called <tt>getMinimumDecimalPlaces</tt>,
	 * since it actually computes the number of decimal places that is required to
	 * correctly display the full precision of an ordinate value.
	 *
	 * <p>
	 ...
	 *
	 * @return the amount by which to subtract the x-coordinate before multiplying
	 *         by the scale
	 ...
	 *
	 * @return the amount by which to subtract the y-coordinate before multiplying
	 *         by the scale
	 * @deprecated Offsets are no longer used
	 */
	...
	/**
	 * Computes the grid size for a fixed precision model. This is equal to the
	 * reciprocal of the scale factor. If the grid size has been set explicity (via
	 * a negative scale factor) it will be returned.
	 *
	...
	/**
	 * Sets the multiplying factor used to obtain a precise coordinate. This method
	 * is private because PrecisionModel is an immutable (value) type.
	 */
	private void setScale(double scale) {
		/**
		 * A negative scale indicates the grid size is being set. The scale is set as
		 * well, as the reciprocal.
		 */
		if (scale < 0) {
	 ...
	 * @return the coordinate whose values will be changed to the external
	 ...
	 *            the original coordinate
	 * @param external
	 *            the coordinate whose values will be changed to the external
	...
	public static class Type implements Serializable {
		private static final Map nameToTypeMap = new HashMap();
		@Serial
