 ...
 * specifies the grid of allowable points for a <code>Geometry</code>. A precision model may be
 * <b>floating</b> ({@link #FLOATING} or {@link #FLOATING_SINGLE}), in which case normal
 * floating-point value semantics apply.
 *
 * <p>For a {@link #FIXED} precision model the {@link #makePrecise(Coordinate)} method allows
 * rounding a coordinate to a "precise" value; that is, one whose precision is known exactly.
 *
 * <p>Coordinates are assumed to be precise in geometries. That is, the coordinates are assumed to
 * be rounded to the precision model given for the geometry. All internal operations assume that
 * coordinates are rounded to the precision model. Constructive methods (such as boolean operations)
 * always round computed coordinates to the appropriate precision model.
 *
 * <p>Three types of precision model are supported:
 *
 * <ul>
 ...
 *       is specified by a <b>scale factor</b>. The scale factor specifies the size of the grid
 *       which numbers are rounded to. Input coordinates are mapped to fixed coordinates according
 ...
 * </ul>
 *
 * For example, to specify 3 decimal places of precision, use a scale factor of 1000. To specify -3
 * decimal places of precision (i.e. rounding to the nearest 1000), use a scale factor of 0.001.
 *
 * <p>It is also supported to specify a precise <b>grid size</b> by providing it as a negative scale
 * factor. This allows setting a precise grid size rather than using a fractional scale, which
 * provides more accurate and robust rounding. For example, to specify rounding to the nearest 1000
 * use a scale factor of -1000.
 *
 * <p>Coordinates are represented internally as Java double-precision values. Java uses the IEEE-394
...
public class PrecisionModel implements Serializable, Comparable {
  ...
  @Serial private static final long serialVersionUID = 7777263578777803835L;

  /** The types of Precision Model which JTS supports. */
  public static class Type implements Serializable {
    @Serial private static final long serialVersionUID = -5528602631731589822L;
  ...
  /**
   * If non-zero, the precise grid size specified. In this case, the scale is also valid and is
   * computed from the grid size. If zero, the scale is used to compute the grid size where needed.
   */
   ...
   * Creates a <code>PrecisionModel</code> that specifies Fixed precision. Fixed-precision
   * coordinates are represented as precise internal coordinates, which are rounded to the grid
   * defined by the scale factor. The provided scale may be negative, to specify an exact grid size.
   * The scale is then computed as the reciprocal.
   *
   * @param scale amount by which to multiply a coordinate after subtracting the offset, to obtain a
   *     precise coordinate. Must be non-zero.
   */
   ...
   * {@link WKTWriter}).
   *
   * <p>This method would be more correctly called <tt>getMinimumDecimalPlaces</tt>, since it
   * actually computes the number of decimal places that is required to correctly display the full
   * precision of an ordinate value.
   *
   * <p>Since it is difficult to compute the required number of decimal places for scale factors
   * which are not powers of 10, the algorithm uses a very rough approximation in this case. This
   * has the side effect that for scale factors which are powers of 10 the value returned is 1
  ...
  /**
   * Computes the grid size for a fixed precision model. This is equal to the reciprocal of the
   * scale factor. If the grid size has been set explicity (via a negative scale factor) it will be
   * returned.
   *
   * @return the grid size at a fixed precision scale.
   */
  public double gridSize() {
    if (isFloating()) return Double.NaN;
  ...
  private void setScale(double scale) {
    /**
     * A negative scale indicates the grid size is being set. The scale is set as well, as the
     * reciprocal.
     */
    if (scale < 0) {
   ...
   * @param internal the coordinate whose values will be changed to the precise representation of
   *     <code>external</code>
   ...
   *
   * @param external the original coordinate
   * @return the coordinate whose values will be changed to the precise representation of <code>
   *     external</code>
   * @deprecated use makePrecise instead
   */
   ...
   * @return the coordinate whose values will be changed to the external representation of <code>
   *     internal</code>
   ...
   *
   * @param internal the original coordinate
   * @param external the coordinate whose values will be changed to the external representation of
   *     <code>internal</code>
   * @deprecated no longer needed, since internal representation is same as external representation
   */
   ...
   *
   * <p>This method has no effect on NaN values.
   *
   * <p><b>Note:</b> Java's <code>Math#rint</code> uses the "Banker's Rounding" algorithm, which is
   * not suitable for precision operations elsewhere in JTS.
   */
