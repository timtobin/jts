 ...
 *
 * <p>This Quadtree index provides a <b>primary filter</b> for range rectangle queries. The various
 ...
 * It will automatically expand to accommodate any extent of dataset.
 *
 * <p>This data structure is also known as an <i>MX-CIF quadtree</i> following the terminology of
 * Samet and others.
 *
 * @version 1.7
 */
public class Quadtree implements SpatialIndex, Serializable {
  @Serial private static final long serialVersionUID = -7461163625812743604L;

  /**
   * Ensure that the envelope for the inserted item has non-zero extents. Use the current minExtent
   * to pad the envelope, if necessary
   */
  ...
  /**
   * minExtent is the minimum envelope extent of all items inserted into the tree so far. It is used
   * as a heuristic value to construct non-zero envelopes for features with zero X and/or Y extent.
   * Start with a non-zero extent, in case the first feature inserted has a zero extent in both
   * directions. This value may be non-optimal, but only one feature will be inserted with this
   * value.
   */
  ...
  */
  ...
  /**
   * Queries the tree and returns items which may lie in the given search envelope. Precisely, the
   * items that are returned are all items in the tree whose envelope <b>may</b> intersect the
   * search Envelope. Note that some items with non-intersecting envelopes may be returned as well;
   * the client is responsible for filtering these out. In most situations there will be many items
   * in the tree which do not intersect the search envelope and which are not returned - thus
  ...
  /**
   * Queries the tree and visits items which may lie in the given search envelope. Precisely, the
   * items that are visited are all items in the tree whose envelope <b>may</b> intersect the search
   * Envelope. Note that some items with non-intersecting envelopes may be visited as well; the
   * client is responsible for filtering these out. In most situations there will be many items in
   * the tree which do not intersect the search envelope and which are not visited - thus providing
   * improved performance over a simple linear scan.
   *
   * @param searchEnv the envelope of the desired query area.
