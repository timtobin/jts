 ...
 * items beforehand. It will automatically expand to accommodate any extent of
 * dataset.
 *
 * <p>
...
public class Quadtree implements SpatialIndex, Serializable {
	...
	/**
	 * minExtent is the minimum envelope extent of all items inserted into the tree
	 * so far. It is used as a heuristic value to construct non-zero envelopes for
	 * features with zero X and/or Y extent. Start with a non-zero extent, in case
	 * the first feature inserted has a zero extent in both directions. This value
	 * may be non-optimal, but only one feature will be inserted with this value.
	 */
	...
	/*
	 * public List OLDquery(Envelope searchEnv) { /** the items that are matched are
	 * the items in quads which overlap the search envelope
	 */
	/*
	 * List foundItems = new ArrayList(); root.addAllItemsFromOverlapping(searchEnv,
	 * foundItems); return foundItems; }
	 */
	 ...
	 * whose envelope <b>may</b> intersect the search Envelope. Note that some items
	 * with non-intersecting envelopes may be returned as well; the client is
	...
	public List query(Envelope searchEnv) {
		/**
		 * the items that are matched are the items in quads which overlap the search
		 * envelope
		 */
