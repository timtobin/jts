...
/**
 * Simplifies a {@link Geometry} using the Douglas-Peucker algorithm. Ensures that any polygonal
 * geometries returned are valid. Simple lines are not guaranteed to remain simple after
 * simplification. All geometry types are handled. Empty and point geometries are returned
 * unchanged. Empty geometry components are deleted.
 *
 * <p>Note that in general D-P does not preserve topology - e.g. polygons can be split, collapse to
 * lines or disappear holes can be created or disappear, and lines can cross. To simplify geometry
 * while preserving topology use {@link TopologyPreservingSimplifier}. (However, using D-P is
 * significantly faster).
 *
 * <h2>KNOWN BUGS</h2>
 *
 * <ul>
 *   <li>In some cases the approach used to clean invalid simplified polygons can distort the output
 *       geometry severely.
 * </ul>
 *
 * @version 1.7
...
public class DouglasPeuckerSimplifier {
  ...
  /**
   * Controls whether simplified polygons will be "fixed" to have valid topology. The caller may
   * choose to disable this because:
   *
   * <ul>
   *   <li>valid topology is not required
   *   <li>fixing topology is a relative expensive operation
   *   <li>in some pathological cases the topology fixing operation may either fail or run for too
   *       long
   * </ul>
   *
  ...
  static class DPTransformer extends GeometryTransformer {
    ...
    protected CoordinateSequence transformCoordinates(CoordinateSequence coords, Geometry parent) {
      ...
      if (inputPts.length == 0) {
        ...
        newPts =
            DouglasPeuckerLineSimplifier.simplify(inputPts, distanceTolerance, isPreserveEndpoint);
      }
    ...
    /**
     * Simplifies a LinearRing. If the simplification results in a degenerate ring, remove the
     * component.
     *
     ...
     * self-intersections). Since buffer can handle invalid topology, but always returns valid
     * geometry, constructing a 0-width buffer "corrects" the topology. Note this only works for
     * area geometries, since buffer always returns areas. This also may return empty geometries, if
     * the input has no actual area. If the input is empty or is not polygonal, this ensures that
     * POLYGON EMPTY is returned.
     *
    ...
    private Geometry createValidArea(Geometry rawAreaGeom) {
      ...
      if (isEnsureValidTopology && !isValidArea) return rawAreaGeom.buffer(0.0);
      return rawAreaGeom;
    }
  }
}
