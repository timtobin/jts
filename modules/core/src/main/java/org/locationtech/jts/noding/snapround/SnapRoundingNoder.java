 ...
 * SegmentString}s, in a performant way, and avoiding unnecessary noding.
 *
 * <p>Implements the Snap Rounding technique described in the papers by Hobby, Guibas &amp;
 * Marimont, and Goodrich et al. Snap Rounding enforces that all output vertices lie on a uniform
 * grid, which is determined by the provided {@link PrecisionModel}.
 *
 * <p>Input vertices do not have to be rounded to the grid beforehand; this is done during the
 * snap-rounding process. In fact, rounding cannot be done a priori, since rounding vertices by
 * themselves can distort the rounded topology of the arrangement (i.e. by moving segments away from
 * hot pixels that would otherwise intersect them, or by moving vertices across segments).
 *
 * <p>To minimize the number of introduced nodes, the Snap-Rounding Noder avoids creating nodes at
 * edge vertices if there is no intersection or snap at that location. However, if two different
 * input edges contain identical segments, each of the segment vertices will be noded. This still
 * provides fully-noded output. This is the same behaviour provided by other noders, such as {@link
 * MCIndexNoder} and {@link org.locationtech.jts.noding.snap.SnappingNoder}.
 *
 * @version 1.7
 */
public class SnapRoundingNoder implements Noder {
  /**
   * The division factor used to determine nearness distance tolerance for intersection detection.
   */
  ...
  private NodedSegmentString computeSegmentSnaps(NodedSegmentString ss) {
    ...
    for (int i = 0; i < pts.length - 1; i++) {
       ...
       * important to check original segment because rounding can move it enough to intersect other
       * hot pixels not intersecting original segment)
       */
