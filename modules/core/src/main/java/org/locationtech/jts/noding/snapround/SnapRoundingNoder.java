 ...
 * of {@link SegmentString}s, in a performant way, and avoiding unnecessary
 * noding.
 *
 * <p>
 ...
 * since rounding vertices by themselves can distort the rounded topology of the
 * arrangement (i.e. by moving segments away from hot pixels that would
 * otherwise intersect them, or by moving vertices across segments).
 *
 * <p>
 ...
 * output. This is the same behaviour provided by other noders, such as
 * {@link MCIndexNoder} and
 * {@link org.locationtech.jts.noding.snap.SnappingNoder}.
 *
 * @version 1.7
 */
public class SnapRoundingNoder implements Noder {
	 ...
	 * and adds nodes for them to the segment strings. Also creates HotPixel nodes
	...
	/**
	 * Add nodes for any vertices in hot pixels that were added as nodes during
	 * segment noding.
	 *
	 * @param ss
	...
	/**
	 * Creates HotPixels for each vertex in the input segStrings. The HotPixels are
	 * not marked as nodes, since they will only be nodes in the final line
	 * arrangement if they interact with other segments (or they are already created
	...
	/**
	 * Computes the nodes in the snap-rounding line arrangement. The nodes are added
	 * to the {@link NodedSegmentString}s provided as the input.
	 *
	...
	/**
	 * Add snapped vertices to a segment string. If the segment string collapses
	 * completely due to rounding, null is returned.
	 *
	 * @param ss
	...
	private NodedSegmentString computeSegmentSnaps(NodedSegmentString ss) {
		// Coordinate[] pts = ss.getCoordinates();
		/**
		 * Get edge coordinates, including added intersection nodes. The coordinates are
		 * now rounded to the grid, in preparation for snapping to the Hot Pixels
		 */
		...
		for (int i = 0; i < pts.length - 1; i++) {
			 ...
			 * (It is important to check original segment because rounding can move it
			 * enough to intersect other hot pixels not intersecting original segment)
			 */
	...
	/**
	 * Computes new segment strings which are rounded and contain intersections
	...
	/**
	 * Gets a list of the rounded coordinates. Duplicate (collapsed) coordinates are
	 * removed.
	 *
	 * @param pts
	...
	private List<NodedSegmentString> snapRound(Collection<NodedSegmentString> segStrings) {
		/**
		 * Determine hot pixels for intersections and vertices. This is done BEFORE the
		 * input lines are rounded, to avoid distorting the line arrangement (rounding
		 * can cause vertices to move across edges).
		 */
	...
	private void snapSegment(Coordinate p0, Coordinate p1, NodedSegmentString ss, int segIndex) {
		pixelIndex.query(p0, p1, node -> {
			 ...
			 * then that vertex is the source for the hot pixel. To avoid over-noding a node
			 * is not added at this point. The hot pixel may be subsequently marked as a
			 * node, in which case the intersection will be added during the final vertex
			 * noding phase.
			 */
			if (!hp.isNode()) {
			...
			/**
			 * Add a node if the segment intersects the pixel. Mark the HotPixel as a node
			 * (since it may not have been one before). This ensures the vertex for it is
			 * added as a node during the final vertex noding phase.
			 */
