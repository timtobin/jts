...
/**
 * Tris which are used to form a concave hull. If a Tri has an edge (or edges)
 * with no adjacent tri the tri is on the border of the triangulation. The edge
 * is a boundary edge. The union of those edges forms the (linear) boundary of
 * the triangulation. The triangulation area may be a Polygon or MultiPolygon,
...
class HullTri extends Tri implements Comparable<HullTri> {
	...
	/**
	 * Tests if a triangulation is edge-connected, if a triangle is removed. NOTE:
	...
	public int adjacent2VertexIndex() {
		...
		if (hasAdjacent(1) && hasAdjacent(2))
			return 2;
		if (hasAdjacent(2) && hasAdjacent(0))
			return 0;
		return -1;
	}
	...
	/**
	 * Gets the most CCW boundary edge index. This assumes there is at least one
	 * non-boundary edge.
	 *
	...
	/**
	 * Gets the most CW boundary edge index. This assumes there is at least one
	 * non-boundary edge.
	 *
	...
	/**
	 * Sorts tris in decreasing order. Since PriorityQueues sort in <i>ascending</i>
	 * order, to sort with the largest at the head, smaller sizes must compare as
	 * greater than larger sizes. (i.e. the normal numeric comparison is reversed).
	 * If the sizes are identical (which should be an infrequent case), the areas
	 * are compared, with larger areas sorting before smaller. (The rationale is
	 * that larger areas indicate an area of lower point density, which is more
	 * likely to be in the exterior of the computed hull.) This improves the
	 * determinism of the queue ordering.
	 */
	@Override
	...
	/**
	 * Tests if this tri has a vertex which is in the boundary, but not in a
	 * boundary edge.
	 *
	...
	/**
	 * Tests if a tri is the only one connecting its 2 adjacents. Assumes that the
	 * tri is on the border of the triangulation and that the triangulation does not
	 * contain holes
	 *
	 * @param tri
	 *            the tri to test
	...
	/**
	 * Tests whether some vertex of this Tri has degree = 1. In this case it is not
	 * in any other Tris.
	 *
	 * @param tri
	 * @param triList
	...
	/**
	 * Sets the size to be the length of the boundary edges. This is used when
	 * constructing hull without holes, by erosion from the triangulation border.
	 */
