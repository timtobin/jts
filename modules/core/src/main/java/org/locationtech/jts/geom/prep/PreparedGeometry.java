...
/**
 * An interface for classes which prepare {@link Geometry}s in order to optimize
 * the performance of repeated calls to specific geometric operations.
 *
 * <p>
 * A given implementation may provide optimized implementations for only some of
 * the specified methods, and delegate the remaining methods to the original
 * {@link Geometry} operations. An implementation may also only optimize certain
 * situations, and delegate others. See the implementing classes for
 * documentation about which methods and situations they optimize.
 *
 * <p>
 * Subclasses are intended to be thread-safe, to allow
 * <code>PreparedGeometry</code> to be used in a multi-threaded context (which
 * allows extracting maximum benefit from the prepared state).
 *
 * @author Martin Davis
 */
public interface PreparedGeometry {
	 ...
	 *
	 * <p>
	 * The <code>containsProperly</code> predicate has the following equivalent
	 * definitions:
	 *
	 * <ul>
	 ...
	 * </ul>
	 *
	 * In other words, if the test geometry has any interaction with the boundary of
	 * the target geometry the result of <tt>containsProperly</tt> is
	 * <tt>false</tt>. This is different semantics to the {@link Geometry#contains}
	 * predicate, in which test geometries can intersect the target's boundary and
	 * still be contained.
	 *
	 * <p>
	 * The advantage of using this predicate is that it can be computed efficiently,
	 * since it avoids the need to compute the full topological relationship of the
	 * input boundaries in cases where they intersect.
	 *
	 * <p>
	 * An example use case is computing the intersections of a set of geometries
	 * with a large polygonal geometry. Since <tt>intersection</tt> is a fairly slow
	 * operation, it can be more efficient to use <tt>containsProperly</tt> to
	 * filter out test geometries which lie wholly inside the area. In these cases
	 * the intersection is known <i>a priori</i> to be exactly the original test
	 * geometry.
	 *
	 * @param geom
	 *            the Geometry to test
	...
	/**
	 * Tests whether the base {@link Geometry} intersects a given geometry. This
	 * method supports {@link GeometryCollection}s as input
	 *
	 * @param geom
	 *            the Geometry to test
