...
/**
 * Computes the <tt>contains</tt> spatial relationship predicate for a {@link PreparedPolygon}
 * relative to all other {@link Geometry} classes. Uses short-circuit tests and indexing to improve
 * performance.
 *
 * <p>It is not possible to short-circuit in all cases, in particular in the case where the test
 * geometry touches the polygon linework. In this case full topology must be computed.
 *
 * @author Martin Davis
 */
class PreparedPolygonContains extends AbstractPreparedPolygonContains {
  /**
   * Computes the </tt>contains</tt> predicate between a {@link PreparedPolygon} and a {@link
   * Geometry}.
   *
  ...
  /**
   * Computes the full topological <tt>contains</tt> predicate. Used when short-circuit tests are
   * not conclusive.
   *
   * @param geom the test geometry
