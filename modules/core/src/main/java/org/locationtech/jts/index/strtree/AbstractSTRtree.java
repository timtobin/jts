 ...
 * Francisco, 2002.
 *
 * <p>This implementation is based on {@link Boundable}s rather than {@link AbstractNode}s, because
 * the STR algorithm operates on both nodes and data, both of which are treated as Boundables.
 *
...
public abstract class AbstractSTRtree implements Serializable {
  ...
  /** */
  @Serial private static final long serialVersionUID = -3886435814360241337L;
  ...
  /**
   * Constructs an AbstractSTRtree with the specified maximum number of child nodes that a node may
   * have, and all leaf nodes in the tree
   *
   * @param nodeCapacity the maximum number of child nodes in a node
   ...
   * in this tree.
   *
   * <p>The returned {@link List}s contain either {@link Object} items, or Lists which correspond to
   * subtrees of the tree Subtrees which do not contain any items are not included.
   *
  ...
  private boolean removeItem(AbstractNode node, Object item) {
    ...
    for (Object o : node.getChildBoundables()) {
      ...
      if (childBoundable instanceof ItemBoundable boundable) {
        if (boundable.getItem() == item) childToRemove = childBoundable;
      }
    }
