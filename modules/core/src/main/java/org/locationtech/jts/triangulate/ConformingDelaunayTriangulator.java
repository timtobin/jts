 ...
 *
 * <p>A conforming Delaunay triangulation is a true Delaunay triangulation. In it each constraint
 * segment is present as a union of one or more triangulation edges. Constraint segments may be
 * subdivided into two or more triangulation edges by the insertion of additional sites. The
 * additional sites are called Steiner points, and are necessary to allow the segments to be
 * faithfully reflected in the triangulation while maintaining the Delaunay property. Another way of
 * stating this is that in a conforming Delaunay triangulation every constraint segment will be the
 * union of a subset of the triangulation edges (up to tolerance).
 *
 * <p>A Conforming Delaunay triangulation is distinct from a Constrained Delaunay triangulation. A
 * Constrained Delaunay triangulation is not necessarily fully Delaunay, and it contains the
 * constraint segments exactly as edges of the triangulation.
 *
 * <p>A typical usage pattern for the triangulator is:
 *
 * <pre>
...
public class ConformingDelaunayTriangulator {
   ...
   * The initial vertex set should not contain any vertices which appear in the constraint set.
   *
   * @param initialVertices a collection of {@link ConstraintVertex}
  ...
  /**
   * Sets the constraints to be conformed to by the computed triangulation. The constraints must not
   * contain duplicate segments (up to orientation). The unique set of vertices (as {@link
   * ConstraintVertex}es) forming the constraints must also be supplied. Supplying it explicitly
   * allows the ConstraintVertexes to be initialized appropriately (e.g. with external data), and
   * avoids re-computing the unique set if it is already available.
   *
  ...
  /**
   * Sets the {@link ConstraintSplitPointFinder} to be used during constraint enforcement. Different
   * splitting strategies may be appropriate for special situations.
   *
  ...
  /**
   * Gets the <tt>ConstraintVertexFactory</tt> used to create new constraint vertices at split
   * points.
   *
  ...
  /**
   * Sets a custom {@link ConstraintVertexFactory} to be used to allow vertices carrying extra
  ...
  /**
   * Gets the convex hull of all the sites in the triangulation, including constraint vertices. Only
   * valid after the constraints have been enforced.
   *
  ...
  private Coordinate[] getPointArray() {
    Coordinate[] pts = new Coordinate[initialVertices.size() + segVertices.size()];
    int index = 0;
  ...
  private ConstraintVertex createVertex(Coordinate p) {
    ConstraintVertex v;
    if (vertexFactory != null) v = vertexFactory.createVertex(p, null);
    else v = new ConstraintVertex(p);
    return v;
  }
  ...
  private ConstraintVertex createVertex(Coordinate p, Segment seg) {
    ConstraintVertex v;
    if (vertexFactory != null) v = vertexFactory.createVertex(p, seg);
    else v = new ConstraintVertex(p);
  ...
  /**
   * Inserts a site into the triangulation, maintaining the conformal Delaunay property. This can be
   * used to further refine the triangulation if required (e.g. to approximate the medial axis of
   * the constraints, or to improve the grading of the triangulation).
   *
   ...
   *
   * @throws ConstraintEnforcementException if the constraints cannot be enforced
   */
  public void enforceConstraints() {
    ...
    do {
      ...
      count++;
      Debug.println(
          "Iter: " + count + "   Splits: " + splits + "   Current # segments = " + segments.size());
  ...
  private int enforceGabriel(Collection segsToInsert) {
    ...
    /**
     * On each iteration must always scan all constraint (sub)segments, since some constraints may
     * be rebroken by Delaunay triangle flipping caused by insertion of another constraint. However,
     * this process must converge eventually, with no splits remaining to find.
     */
    /**
     * Check whether the inserted point still equals the split pt. This will not be the case if the
     * split pt was too close to an existing site. If the point was snapped, the triangulation will
     * not respect the inserted constraint - this is a failure. This can be caused by:
     *
     * <ul>
     *   <li>An initial site that lies very close to a constraint segment The cure for this is to
     *       remove any initial sites which are close to constraint segments in a preprocessing
     *       phase.
     *   <li>A narrow constraint angle which causing repeated splitting until the split segments are
     *       too small. The cure for this is to either choose better split points or "guard" narrow
     *       angles by cracking the segments equidistant from the corner.
     * </ul>
     */
    for (Object o : segsToInsert) {
      ...
      /**
       * Check whether the inserted point still equals the split pt. This will not be the case if
       * the split pt was too close to an existing site. If the point was snapped, the triangulation
       * will not respect the inserted constraint - this is a failure. This can be caused by:
       *
       * <ul>
       *   <li>An initial site that lies very close to a constraint segment The cure for this is to
       *       remove any initial sites which are close to constraint segments in a preprocessing
       *       phase.
       *   <li>A narrow constraint angle which causing repeated splitting until the split segments
       *       are too small. The cure for this is to either choose better split points or "guard"
       *       narrow angles by cracking the segments equidistant from the corner.
       * </ul>
       */
  ...
  /**
   * Given a set of points stored in the kd-tree and a line segment defined by two points in this
   * set, finds a {@link Coordinate} in the circumcircle of the line segment, if one exists. This is
   * called the Gabriel point - if none exists then the segment is said to have the Gabriel
   * condition. Uses the heuristic of finding the non-Gabriel point closest to the midpoint of the
   * segment.
   *
