 ...
 *
 * <p>
 * A Conforming Delaunay triangulation is distinct from a Constrained Delaunay
 * triangulation. A Constrained Delaunay triangulation is not necessarily fully
 * Delaunay, and it contains the constraint segments exactly as edges of the
 * triangulation.
 *
 * <p>
...
public class ConformingDelaunayTriangulator {
	...
	/**
	 * Creates a Conforming Delaunay Triangulation based on the given unconstrained
	 * initial vertices. The initial vertex set should not contain any vertices
	 * which appear in the constraint set.
	 *
	...
	private int enforceGabriel(Collection segsToInsert) {
		...
		/**
		 * On each iteration must always scan all constraint (sub)segments, since some
		 * constraints may be rebroken by Delaunay triangle flipping caused by insertion
		 * of another constraint. However, this process must converge eventually, with
		 * no splits remaining to find.
		 */
		/**
		 * Check whether the inserted point still equals the split pt. This will not be
		 * the case if the split pt was too close to an existing site. If the point was
		 * snapped, the triangulation will not respect the inserted constraint - this is
		 * a failure. This can be caused by:
		 *
		 * <ul>
		 * <li>An initial site that lies very close to a constraint segment The cure for
		 * this is to remove any initial sites which are close to constraint segments in
		 * a preprocessing phase.
		 * <li>A narrow constraint angle which causing repeated splitting until the
		 * split segments are too small. The cure for this is to either choose better
		 * split points or "guard" narrow angles by cracking the segments equidistant
		 * from the corner.
		 * </ul>
		 */
		for (Object o : segsToInsert) {
			...
			Segment s1 = new Segment(seg.getStartX(), seg.getStartY(), seg.getStartZ(), splitVertex.getX(),
					splitVertex.getY(), splitVertex.getZ(), seg.getData());
			Segment s2 = new Segment(splitVertex.getX(), splitVertex.getY(), splitVertex.getZ(), seg.getEndX(),
	...
	/**
	 * Given a set of points stored in the kd-tree and a line segment defined by two
	 * points in this set, finds a {@link Coordinate} in the circumcircle of the
	 * line segment, if one exists. This is called the Gabriel point - if none
	...
	/**
	 * Gets the convex hull of all the sites in the triangulation, including
	 * constraint vertices. Only valid after the constraints have been enforced.
	 *
	...
	/**
	 * Gets the <tt>ConstraintVertexFactory</tt> used to create new constraint
	 * vertices at split points.
	 *
	...
	/**
	 * Inserts a site into the triangulation, maintaining the conformal Delaunay
	 * property. This can be used to further refine the triangulation if required
	 * (e.g. to approximate the medial axis of the constraints, or to improve the
	...
	/**
	 * Sets the constraints to be conformed to by the computed triangulation. The
	 * constraints must not contain duplicate segments (up to orientation). The
	 * unique set of vertices (as {@link ConstraintVertex}es) forming the
	 * constraints must also be supplied. Supplying it explicitly allows the
	 * ConstraintVertexes to be initialized appropriately (e.g. with external data),
	 * and avoids re-computing the unique set if it is already available.
	 *
	 * @param segments
	 *            a list of the constraint {@link Segment}s
	 * @param segVertices
	 *            the set of unique {@link ConstraintVertex}es referenced by the
	 *            segments
	 */
	 ...
	 * segments.get(i); QuadEdge q = subdiv.locate(s.getStart(), s.getEnd()); if (q
	 * == null) missingSegs.add(s); } return missingSegs; }
	 */

	/**
	 * Sets the {@link ConstraintSplitPointFinder} to be used during constraint
	 * enforcement. Different splitting strategies may be appropriate for special
	 * situations.
	 *
	 * @param splitFinder
	...
	/**
	 * Sets a custom {@link ConstraintVertexFactory} to be used to allow vertices
	 * carrying extra information to be created.
	 *
	 * @param vertexFactory
