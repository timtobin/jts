...
/**
 * Dissolves the linear components from a collection of {@link Geometry}s into a set of
 * maximal-length {@link LineString}s in which every unique segment appears once only. The output
 * linestrings run between node vertices of the input, which are vertices which have either degree
 * 1, or degree 3 or greater.
 *
 * <p>Use cases for dissolving linear components include generalization (in particular, simplifying
 * polygonal coverages), and visualization (in particular, avoiding symbology conflicts when
 * depicting shared polygon boundaries).
 *
 * <p>This class does <b>not</b> node the input lines. If there are line segments crossing in the
 * input, they will still cross in the output.
 *
 * @author Martin Davis
 */
public class LineDissolver {
  ...
  /**
   * Adds a {@link Geometry} to be dissolved. Any number of geometries may be added by calling this
   * method multiple times. Any type of Geometry may be added. The constituent linework will be
  ...
  private void add(LineString lineString) {
    ...
    for (int i = 1; i < seq.size(); i++) {
      DissolveHalfEdge e =
          (DissolveHalfEdge) graph.addEdge(seq.getCoordinate(i - 1), seq.getCoordinate(i));
      // skip zero-length edges
      if (e == null) continue;
      /**
       * Record source initial segments, so that they can be reflected in output when needed (i.e.
       * during formation of isolated rings)
       */
      if (!doneStart) {
        e.setStart();
        doneStart = true;
   ...
   * {@link Coordinate} ordering).
   *
   * <p>Identifying the lowest starting node meets two goals:
   *
   * <ul>
   *   <li>It ensures that isolated input rings are created using the original node and orientation
   *   <li>For isolated rings formed from multiple input linestrings, it provides a canonical node
   *       and orientation for the output (rather than essentially random, and thus hard to test).
   * </ul>
   *
   * @param e
   */
  ...
  /**
   * Builds a line starting from the given edge. The start edge origin is a node (valence = 1 or >=
   * 3), unless it is part of a pure ring. A pure ring has no other incident lines. In this case the
   * start edge may occur anywhere on the ring.
   *
   * <p>The line is built up to the next node encountered, or until the start edge is re-encountered
