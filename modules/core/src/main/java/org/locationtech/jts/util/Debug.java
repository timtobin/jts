...
/**
 * Provides routines to simplify and localize debugging output. Debugging is
 * controlled via a Java system property value. If the system property with the
 * name given in DEBUG_PROPERTY_NAME (currently "jts.debug") has the value "on"
 * or "true" debugging is enabled. Otherwise, debugging is disabled. The system
 * property can be set by specifying the following JVM option:
 *
 * <pre>
 * -Djts.debug=on
 * </pre>
...
public class Debug {
	...
	/**
	 * Adds an object to be watched. A watched object can be printed out at any
	 * time.
	 *
	 * <p>
	...
	/*
	 * public static void println(String str) { if (! debugOn) return;
	 * debug.instancePrint(str); debug.println(); }
	 */
	...
	public static void printTime(String tag) {
		...
		debug.instancePrint(formatField(Stopwatch.getTimeString(time), 10) + " ("
				+ formatField(Stopwatch.getTimeString(elapsedTime), 10) + " ) " + tag);
		debug.println();
