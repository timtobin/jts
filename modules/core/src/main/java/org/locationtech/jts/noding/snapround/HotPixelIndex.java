...
/**
 * An index which creates unique {@link HotPixel}s for provided points, and
 * performs range queries on them. The points passed to the index do not needed
 * to be rounded to the specified scale factor; this is done internally when
 * creating the HotPixels for them.
 *
 * @author mdavis
 */
class HotPixelIndex {
	 ...
	 * HotPixels have an extent, range queries to the index must enlarge the query
	 * range by a suitable value (using the pixel width is safest).
	 */
	...
	/**
	 * Adds a point as a Hot Pixel. If the point has been added already, it is
	 * marked as a node.
	 *
	 * @param p
	 *            the point to add
	...
	public HotPixel add(Coordinate p) {
		...
		/**
		 * Hot Pixels which are added more than once must have more than one vertex in
		 * them and thus must be nodes.
		 */
		if (hp != null) {
			hp.setToNode();
			return hp;
	...
	public void add(Coordinate[] pts) {
		/**
		 * Shuffle the points before adding. This avoids having long monontic runs of
		 * points causing an unbalanced KD-tree, which would create performance and
		 * robustness issues.
		 */
	...
	public void addNodes(List<Coordinate> pts) {
		/**
		 * Node points are not shuffled, since they are added after the vertex points,
		 * and hence the KD-tree should be reasonably balanced already.
		 */
	...
	/**
	 * Visits all the hot pixels which may intersect a segment (p0-p1). The visitor
	 * must determine whether each hot pixel actually intersects the segment.
	 *
	 * @param p0
	...
	/**
	 * Utility class to shuffle an array of {@link Coordinate}s using the
	 * Fisher-Yates shuffle algorithm
	 *
	 * @see <a href=
	 *      "https://en.wikipedia.org/wiki/Fisher%E2%80%93Yates_shuffle">Fihser-Yates
	 *      shuffle</a>
	 */
