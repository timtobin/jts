...
/**
 * Computes topology-preserving simplified hulls of polygonal geometry. Both outer and inner hulls
 * can be computed. Outer hulls contain the input geometry and are larger in area. Inner hulls are
 * contained by the input geometry and are smaller in area. In both the hull vertices are a subset
 * of the input vertices. The hull construction attempts to minimize the area difference with the
 * input geometry. Hulls are generally concave if the input is. Computed hulls are
 * topology-preserving: they do not contain any self-intersections or overlaps, so the result
 * polygonal geometry is valid.
 *
 * <p>Polygons with holes and MultiPolygons are supported. The result has the same geometric type
 * and structure as the input.
 *
 * <p>The number of vertices in the computed hull is determined by a target parameter. Two
 * parameters are supported:
 *
 * <ol>
 *   <li><b>Vertex Number fraction:</b> the fraction of the input vertices retained in the result.
 *       Value 1 produces the original geometry. Smaller values produce less concave results. For
 *       outer hulls, value 0 produces the convex hull (with triangles for any holes). For inner
 *       hulls, value 0 produces a triangle (if no holes are present).
 *   <li><b>Area Delta ratio:</b> the ratio of the change in area to the input area. Value 0
 *       produces the original geometry. Larger values produce less concave results.
 * </ol>
 *
 * The algorithm ensures that the result does not cause the target parameter to be exceeded. This
 * allows computing outer or inner hulls with a small area delta ratio as an effective way of
 * removing narrow gores and spikes.
 *
 * @author Martin Davis
 */
public class PolygonHullSimplifier {
  ...
  /**
   * Computes a topology-preserving simplified hull of a polygonal geometry, with hull shape
   * determined by a target parameter specifying the fraction of the input vertices retained in the
   * result. Larger values compute less concave results. A value of 1 produces the convex hull; a
   * value of 0 produces the original geometry. Either outer or inner hulls can be computed.
   *
  ...
  /**
   * Computes a topology-preserving simplified hull of a polygonal geometry, with hull shape
   * determined by a target parameter specifying the ratio of maximum difference in area to original
   * area. Larger values compute less concave results. A value of 0 produces the original geometry.
  ...
  /**
   * Creates a new instance to compute a simplified hull of a polygonal geometry. An outer or inner
   * hull is computed depending on the value of <code>isOuter</code>.
   *
  ...
  /**
   * Sets the target fraction of input vertices which are retained in the result. The value should
   * be in the range [0,1].
   *
  ...
  /**
   * Sets the target maximum ratio of the change in area of the result to the input area. The value
  ...
  public Geometry getResult() {
    ...
    if (inputGeom instanceof MultiPolygon polygon1) {
      /**
       * Only outer hulls where there is more than one polygon can potentially overlap. Shell outer
       * hulls could overlap adjacent shell hulls or hole hulls surrounding them; hole outer hulls
       * could overlap contained shell hulls.
       */
  ...
  /**
   * Computes hulls for MultiPolygon elements for the cases where hulls might overlap.
   *
  ...
  private Polygon computePolygon(Polygon poly) {
    ...
    /**
     * For a single polygon overlaps are only possible for inner hulls and where holes are present.
     */
  ...
  /**
   * Create all ring hulls for the rings of a polygon, so that all are in the hull index if
   * required.
   *
  ...
  private RingHull createRingHull(
      LinearRing ring, boolean isOuter, double areaTotal, RingHullIndex hullIndex) {
