...
public abstract class EdgeEndStar {
  ...
  public void computeLabelling(GeometryGraph[] geomGraph) {
     ...
     * geometry.
     *
     * <p>Note that the edge cannot be on the BOUNDARY of the geometry, since then there would have
     * been a parallel edge from the Geometry at this node also labelled BOUNDARY and this edge
     * would have been labelled in the previous step.
     *
     * <p>This code causes a problem when dimensional collapses are present, since it may try and
     * determine the location of a node where a dimensional collapse has occurred. The point should
     * be considered to be on the EXTERIOR of the polygon, but locate() will return INTERIOR, since
     * it is passed the original Geometry, not the collapsed version.
     *
     * <p>If there are incident edges which are Line edges labelled BOUNDARY, then they must be
     * edges resulting from dimensional collapses. In this case the other edges can be labelled
     * EXTERIOR for this Geometry.
     *
     * <p>MD 8/11/01 - NOT TRUE! The collapsed edges may in fact be in the interior of the Geometry,
  ...
  private int getLocation(int geomIndex, Coordinate p, GeometryGraph[] geom) {
    ...
    if (ptInAreaLocation[geomIndex] == Location.NONE) {
      ptInAreaLocation[geomIndex] =
          SimplePointInAreaLocator.locate(p, geom[geomIndex].getGeometry());
    }
