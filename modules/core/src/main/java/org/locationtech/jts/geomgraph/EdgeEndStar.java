...
public abstract class EdgeEndStar {
	...
	public void computeLabelling(GeometryGraph[] geomGraph) {
		 ...
		 * this case, to label the edge for that geometry we must test whether the edge
		 * is in the interior of the geometry. To do this it suffices to determine
		 * whether the node for the edge is in the interior of an area. If so, the edge
		 * has location INTERIOR for the geometry. In all other cases (e.g. the node is
		 * on a line, on a point, or not on the geometry at all) the edge has the
		 * location EXTERIOR for the geometry.
		 *
		 * <p>
		 ...
		 * may try and determine the location of a node where a dimensional collapse has
		 * occurred. The point should be considered to be on the EXTERIOR of the
		 * polygon, but locate() will return INTERIOR, since it is passed the original
		 * Geometry, not the collapsed version.
		 *
		 * <p>
		 * If there are incident edges which are Line edges labelled BOUNDARY, then they
		 * must be edges resulting from dimensional collapses. In this case the other
		 * edges can be labelled EXTERIOR for this Geometry.
		 *
		 * <p>
		 * MD 8/11/01 - NOT TRUE! The collapsed edges may in fact be in the interior of
		 * the Geometry, which means the other edges should be labelled INTERIOR for
		 * this Geometry. Not sure how solve this... Possibly labelling needs to be
		 * split into several phases: area label propagation, symLabel merging, then
		 * finally null label resolution.
		 */
