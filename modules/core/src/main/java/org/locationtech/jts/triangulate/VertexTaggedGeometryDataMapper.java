...
/**
 * Creates a map between the vertex {@link Coordinate}s of a set of
 * {@link Geometry}s, and the parent geometry, and transfers the source geometry
 * data objects to geometry components tagged with the coordinates.
 *
 * <p>
 * This class can be used in conjunction with {@link VoronoiDiagramBuilder} to
 * transfer data objects from the input site geometries to the constructed
 * Voronoi polygons.
 *
 * @author Martin Davis
...
public class VertexTaggedGeometryDataMapper {
	...
	/**
	 * Input is assumed to be a multiGeometry in which every component has its
	 * userData set to be a Coordinate which is the key to the output data. The
	 * Coordinate is used to determine the output data object to be written back
	 * into the component.
	 *
	 * @param targetGeom
	 */
