 ...
 *
 * <p>This class allows the {@link Geometry} predicate methods to be switched between the original
 * {@link RelateOp} algorithm and the modern {@link RelateNG} codebase via a system property <code>
 * jts.relate</code>.
 *
 * <ul>
...
class GeometryRelate {
  ...
  static void setRelateImpl(String relateImplCode) {
    if (relateImplCode == null) return;
    // set flag explicitly since current value may not be default
    isRelateNG = RELATE_NG_DEFAULT;

    if (RELATE_PROPERTY_VALUE_NG.equalsIgnoreCase(relateImplCode)) isRelateNG = true;
  }
