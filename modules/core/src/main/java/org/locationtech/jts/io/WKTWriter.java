...
public class WKTWriter {
	...
	/**
	 * Generates the WKT for a <tt>LINESTRING</tt> specified by a
	 ...
	 * dimension (2 to 4). The output follows the following rules:
	 *
	 * <ul>
	 ...
	 */
	private void appendCoordinate(CoordinateSequence seq, EnumSet<Ordinate> outputOrdinates, int i, Writer writer,
			OrdinateFormat formatter) throws IOException {
	 ...
	 *            the output writer to append to
	 * @param formatter
	 *            the <code>DecimalFormatter</code> to use to convert from a precise
	...
	private void appendGeometryCollectionTaggedText(GeometryCollection geometryCollection,
			EnumSet<Ordinate> outputOrdinates, boolean useFormatting, int level, Writer writer,
			OrdinateFormat formatter) throws IOException {
	 ...
	 *            the formatter to use for writing ordinate values.
	 */
	private void appendGeometryCollectionText(GeometryCollection geometryCollection, EnumSet<Ordinate> outputOrdinates,
		...
		if (geometryCollection.getNumGeometries() == 0) {
			...
			for (int i = 0; i < geometryCollection.getNumGeometries(); i++) {
				...
				appendGeometryTaggedText(geometryCollection.getGeometryN(i), outputOrdinates, useFormatting, level2,
						writer, formatter);
			}
			writer.write(")");
		}
	 ...
	 *            coordinate to an external coordinate
	 */
	private void appendGeometryTaggedText(Geometry geometry, EnumSet<Ordinate> outputOrdinates, boolean useFormatting,
	 ...
	 *            coordinate to an external coordinate
	 */
	private void appendLineStringTaggedText(LineString lineString, EnumSet<Ordinate> outputOrdinates,
			boolean useFormatting, int level, Writer writer, OrdinateFormat formatter) throws IOException {
	 ...
	 *            coordinate to an external coordinate
	 */
	private void appendLinearRingTaggedText(LinearRing linearRing, EnumSet<Ordinate> outputOrdinates,
			boolean useFormatting, int level, Writer writer, OrdinateFormat formatter) throws IOException {
	 ...
	 *            coordinate to an external coordinate
	 */
	private void appendMultiLineStringTaggedText(MultiLineString multiLineString, EnumSet<Ordinate> outputOrdinates,
			boolean useFormatting, int level, Writer writer, OrdinateFormat formatter) throws IOException {
	 ...
	 *            flag indicating that
	 * @param level
	 *            the indentation level //@param indentFirst flag indicating that
	 *            the first {@code
	...
	private void appendMultiLineStringText(MultiLineString multiLineString, EnumSet<Ordinate> outputOrdinates,
			boolean useFormatting, int level, /* boolean indentFirst, */
			Writer writer, OrdinateFormat formatter) throws IOException {
	 ...
	 *            the formatter to use for writing ordinate values.
	 */
	private void appendMultiPointText(MultiPoint multiPoint, EnumSet<Ordinate> outputOrdinates, boolean useFormatting,
			int level, Writer writer, OrdinateFormat formatter) throws IOException {
	 ...
	 *            coordinate to an external coordinate
	 */
	private void appendMultiPolygonTaggedText(MultiPolygon multiPolygon, EnumSet<Ordinate> outputOrdinates,
			boolean useFormatting, int level, Writer writer, OrdinateFormat formatter) throws IOException {
	 ...
	 *            the formatter to use for writing ordinate values.
	 */
	private void appendMultiPolygonText(MultiPolygon multiPolygon, EnumSet<Ordinate> outputOrdinates,
			boolean useFormatting, int level, Writer writer, OrdinateFormat formatter) throws IOException {
	 ...
	 */
	private void appendPointTaggedText(Point point, EnumSet<Ordinate> outputOrdinates, boolean useFormatting, int level,
			Writer writer, OrdinateFormat formatter) throws IOException {
	 ...
	 *            coordinate to an external coordinate
	 */
	private void appendPolygonTaggedText(Polygon polygon, EnumSet<Ordinate> outputOrdinates, boolean useFormatting,
			int level, Writer writer, OrdinateFormat formatter) throws IOException {
	 ...
	 *            the indentation level
	 * @param indentFirst
	 *            flag indicating that the first {@code Coordinate} of the sequence
	 *            should be indented for better visibility
	 * @param writer
	 *            the output writer to append to
	 * @param formatter
	 *            the formatter to use for writing ordinate values.
	 */
	private void appendPolygonText(Polygon polygon, EnumSet<Ordinate> outputOrdinates, boolean useFormatting, int level,
			boolean indentFirst, Writer writer, OrdinateFormat formatter) throws IOException {
		if (polygon.isEmpty()) {
			writer.write(WKTConstants.EMPTY);
		} else {
			if (indentFirst)
	...
	private void appendSequenceText(CoordinateSequence seq, EnumSet<Ordinate> outputOrdinates, boolean useFormatting,
			int level, boolean indentFirst, Writer writer, OrdinateFormat formatter) throws IOException {
		if (seq.size() == 0) {
			writer.write(WKTConstants.EMPTY);
		} else {
			if (indentFirst)
	 ...
	 *
	 * <p>
	 * If none/{@code null} is assigned, the precision model of the
	 * {@link Geometry#getFactory()} is used.
	 *
	 * <p>
	 * Note: The precision model is applied to all ordinate values, not just x and
	 * y.
	 *
	 * @param precisionModel
	 *            the flag indicating if {@link Coordinate#z}/{} is actually a
	 *            measure value.
	 */
