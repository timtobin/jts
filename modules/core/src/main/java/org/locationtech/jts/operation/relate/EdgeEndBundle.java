...
/**
 * A collection of {@link EdgeEnd}s which obey the following invariant: They
 * originate at the same node and have the same direction.
 *
 * @version 1.7
 */
public class EdgeEndBundle extends EdgeEnd {
	...
	public EdgeEndBundle(BoundaryNodeRule boundaryNodeRule, EdgeEnd e) {
		...
		insert(e);
		/*
		 * if (boundaryNodeRule != null) this.boundaryNodeRule = boundaryNodeRule; else
	 ...
	 * edgeStubs can be either on the boundary (e.g. Polygon edge) OR in the
	 * interior (e.g. segment of a LineString) of their parent Geometry. In
	 * addition, GeometryCollections use a {@link BoundaryNodeRule} to determine
	 * whether a segment is on the boundary or not. Finally, in GeometryCollections
	 * it can occur that an edge is both on the boundary and in the interior (e.g. a
	 * LineString segment lying on top of a Polygon edge.) In this case the Boundary
	 * is given precedence. <br>
