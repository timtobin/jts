 ...
 * constraints. A concave hull is a concave or convex polygon containing all the
 * input polygons, whose vertices are a subset of the vertices in the input. A
 * given set of polygons has a sequence of hulls of increasing concaveness,
 * determined by a numeric target parameter. The computed hull "fills the gap"
 * between the polygons, and does not intersect their interior.
 *
 * <p>
 * The concave hull is constructed by removing the longest outer edges of the
 * constrained Delaunay Triangulation of the space between the polygons, until
 * the target criterion parameter is reached.
 *
 * <p>
 * The target criteria are:
 *
 * <ul>
 * <li><b>Maximum Edge Length</b> - the length of the longest edge between the
 * polygons is no larger than this value.
 * <li><b>Maximum Edge Length Ratio</b> - determine the Maximum Edge Length as a
 * fraction of the difference between the longest and shortest edge lengths
 * between the polygons. This provides a scale-free parameter. A value of 1
 * produces the convex hull; a value of 0 produces the original polygons.
 * </ul>
 *
 ...
 *
 * <p>
 * Instead of the complete hull, the "fill area" between the input polygons can
 * be computed using {@link #getFill()}.
 *
 * <p>
 * The input polygons must form a valid {@link MultiPolygon} (i.e. they must be
 * non-overlapping and non-edge-adjacent). If needed, a set of
 * possibly-overlapping Polygons can be converted to a valid MultiPolygon by
 * using {@link Geometry#union()};
 *
 * <p>
...
public class ConcaveHullOfPolygons {
	...
	private static double computeTargetEdgeLength(List<Tri> triList, Coordinate[] frameCorners,
	...
	/**
	 * Computes a concave fill area between a set of polygons, using the target
	...
	/**
	 * Computes a concave fill area between a set of polygons, using the target
	 * criterion of maximum edge length ratio.
	 *
	 * @param polygons
	 *            the input polygons
	 * @param lengthRatio
	...
	/**
	 * Computes a concave hull of set of polygons using the target criterion of
	 * maximum edge length.
	 *
	 * @param polygons
	 *            the input polygons
	...
	/**
	 * Computes a concave hull of set of polygons using the target criterion of
	 * maximum edge length, and allowing control over whether the hull boundary is
	 * tight and can contain holes.
	 *
	 * @param polygons
	 *            the input polygons
	 * @param maxLength
	 *            the target maximum edge length
	 * @param isTight
	 *            true if the hull should be tight to the outside of the polygons
	 * @param isHolesAllowed
	 *            true if holes are allowed in the hull polygon
	 * @return the concave hull
	 */
	public static Geometry concaveHullByLength(Geometry polygons, double maxLength, boolean isTight,
	...
	/**
	 * Computes a concave hull of set of polygons using the target criterion of
	 * maximum edge length ratio, and allowing control over whether the hull
	 * boundary is tight and can contain holes.
	 *
	 * @param polygons
	 *            the input polygons
	 * @param lengthRatio
	 ...
	 * @return the concave hull
	 */
	public static Geometry concaveHullByLengthRatio(Geometry polygons, double lengthRatio, boolean isTight,
			boolean isHolesAllowed) {
		ConcaveHullOfPolygons hull = new ConcaveHullOfPolygons(polygons);
		hull.setMaximumEdgeLengthRatio(lengthRatio);
		hull.setHolesAllowed(isHolesAllowed);
	...
	/**
	 * Creates a rectangular "frame" around the input polygons, with the input
	 * polygons as holes in it. The frame is large enough that the constrained
	 * Delaunay triangulation of it should contain the convex hull of the input as
	 * edges. The frame corner triangles can be removed to produce a triangulation
	 * of the space around and between the input polygons.
	 *
	 * @param polygonsEnv
	 * @param polygonRings
	 * @param geomFactory
	...
	/**
	 * Get the tri vertex index of some point in a list, or -1 if none are vertices.
	 *
	 * @param tri
	 *            the tri to test for containing a point
	 * @param pts
	 *            the points to test
	...
	/**
	 * Records the edge index of the longest border edge for border tris, so it can
	 * be tested for length and possible removal.
	 */
	...
	/**
	 * Adds an adjacent tri to the current border. The adjacent edge is recorded as
	 * the border edge for the tri. Note that only edges adjacent to another tri can
	 * become border edges. Since constraint-adjacent edges do not have an adjacent
	 * tri, they can never be on the border and thus will not be removed due to
	 * being shorter than the length threshold. The tri containing them may still be
	 * removed via another edge, however.
	 *
	 * @param tri
	 *            the tri adjacent to the tri to be added to the border
	 * @param index
	 *            the index of the adjacent tri
	 */
	...
	/**
	 * Gets the concave fill, which is the area between the input polygons, subject
	 * to the concaveness control parameter.
	 *
	 * @return the concave fill
	 */
	...
	private boolean isHoleSeedTri(Tri tri) {
		if (isBorderTri(tri))
			return false;
		for (int i = 0; i < 3; i++) {
			if (tri.hasAdjacent(i) && tri.getLength(i) > maxEdgeLength)
				return true;
		}
		return false;
	}
	...
	/**
	 * Tests whether a triangle touches a single polygon at all vertices. If so, it
	 * is a candidate for removal if the hull polygon is being kept tight to the
	 * outer boundary of the input polygons. Tris which touch more than one polygon
	 * are called "bridging".
	 *
	 * @param tri
	...
	private Set<Tri> removeFrameCornerTris(List<Tri> tris, Coordinate[] frameCorners) {
		...
		for (Tri tri : tris) {
			...
			if (isFrameTri) {
				/**
				 * Frame tris are adjacent to at most one border tri, which is opposite the
				 * frame corner vertex. Or, the opposite tri may be another frame tri, which is
				 * not added as a border tri.
				 */
	 ...
	 * <li>Larger values produce less concave results. Above a certain large value
	 * the result is the convex hull of the input.
	 * <p>
	 * The edge length ratio provides a scale-free parameter which is intended to
	 * produce similar concave results for a variety of inputs.
	 *
	 * @param edgeLength
	 ...
	 * edge lengths in the Delaunay Triangulation of the area between the input
	 * polygons. (Roughly speaking, it is a fraction of the difference between the
	 * shortest and longest distances between the input polygons.) It is a value in
	 * the range 0 to 1.
	 *
	 * <ul>
	...
	/**
	 * Sets whether the boundary of the hull polygon is kept tight to the outer
	 * edges of the input polygons.
	 *
	 * @param isTight
