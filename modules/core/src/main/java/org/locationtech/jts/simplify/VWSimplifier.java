...
/**
 * Simplifies a {@link Geometry} using the Visvalingam-Whyatt area-based
 * algorithm. Ensures that any polygonal geometries returned are valid. Simple
 * lines are not guaranteed to remain simple after simplification. All geometry
 * types are handled. Empty and point geometries are returned unchanged. Empty
 * geometry components are deleted.
 *
 * <p>
 * The simplification tolerance is specified as a distance. This is converted to
 * an area tolerance by squaring it.
 *
 * <p>
 * Note that in general this algorithm does not preserve topology - e.g.
 * polygons can be split, collapse to lines or disappear holes can be created or
 * disappear, and lines can cross.
 *
 * <h3>Known Bugs</h3>
 *
 * <ul>
...
public class VWSimplifier {
	 ...
	 * <li>in some pathological cases the topology fixing operation may either fail
	 * or run for too long
	 * </ul>
	 *
	...
	static class VWTransformer extends GeometryTransformer {
		...
		/**
		 * Simplifies a LinearRing. If the simplification results in a degenerate ring,
