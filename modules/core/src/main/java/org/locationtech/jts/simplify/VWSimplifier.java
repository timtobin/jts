...
/**
 * Simplifies a {@link Geometry} using the Visvalingam-Whyatt area-based algorithm. Ensures that any
 * polygonal geometries returned are valid. Simple lines are not guaranteed to remain simple after
 ...
 *
 * <p>The simplification tolerance is specified as a distance. This is converted to an area
 * tolerance by squaring it.
 *
 * <p>Note that in general this algorithm does not preserve topology - e.g. polygons can be split,
...
public class VWSimplifier {
  ...
  static class VWTransformer extends GeometryTransformer {
    ...
    private Geometry createValidArea(Geometry rawAreaGeom) {
      ...
      if (isEnsureValidTopology && !rawAreaGeom.isValid()) return rawAreaGeom.buffer(0.0);
      return rawAreaGeom;
    }
  }
}
