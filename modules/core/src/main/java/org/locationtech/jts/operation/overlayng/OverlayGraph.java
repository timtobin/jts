...
/**
 * A planar graph of edges, representing the topology resulting from an overlay
 * operation. Each source edge is represented by a pair of {@link OverlayEdge}s,
 * with opposite (symmetric) orientation. The pair of OverlayEdges share the
 * edge coordinates and a single {@link OverlayLabel}.
 *
 * @author Martin Davis
 */
class OverlayGraph {
	...
	/**
	 * Adds a new edge to this graph, for the given linework and topology
	 * information. A pair of {@link OverlayEdge}s with opposite (symmetric)
	 * orientation is added, sharing the same {@link OverlayLabel}.
	 *
	 * @param pts
	 *            the edge vertices
	 * @param label
	...
	/**
	 * Gets the set of edges in this graph. Only one of each symmetric pair of
	 * OverlayEdges is included. The opposing edge can be found by using
	 * {@link OverlayEdge#sym()}.
	 *
	...
	/**
	 * Gets the collection of edges representing the nodes in this graph. For each
	 * star of edges originating at a node a single representative edge is included.
	 * The other edges around the node can be found by following the next and prev
	 * links.
	 *
	...
	/**
	 * Inserts a single half-edge into the graph. The sym edge must also be
	 * inserted.
	 *
	 * @param e
	...
	private void insert(OverlayEdge e) {
		...
		/**
		 * If the edge origin node is already in the graph, insert the edge into the
		 * star of edges around the node. Otherwise, add a new node for the origin.
		 */
