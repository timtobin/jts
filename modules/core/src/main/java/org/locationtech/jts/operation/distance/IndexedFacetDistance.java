 ...
 * {@link Geometry}s using a Branch-and-Bound algorithm. The Branch-and-Bound
 * algorithm operates over a traversal of R-trees built on the target and the
 * query geometries.
 *
 * <p>
 ...
 *
 * <ul>
 * <li>Performance is dramatically improved due to the use of the R-tree index
 * and the pruning due to the Branch-and-Bound approach
 * <li>The spatial index on the target geometry is cached which allow reuse in
 * an repeated query situation.
 * </ul>
 *
 * Using this technique is usually much more performant than using the
 * brute-force {@link Geometry#distance(Geometry)} when one or both input
 * geometries are large, or when evaluating many distance computations against a
 * single geometry.
 *
 * <p>
...
public class IndexedFacetDistance {
	 ...
	 *
	 * <p>
	 * For geometries with many segments or points, this can be faster than using a
	 * simple distance algorithm.
	 *
	 * @param g1
	 *            a geometry
	 * @param g2
	 ...
	 *
	 * <p>
	 * Distances will be computed to all facets of the input geometry. The facets of
	 * the geometry are the discrete segments and points contained in its
	 * components. In the case of {@link Lineal} and {@link Puntal} inputs, this is
	 * equivalent to computing the conventional distance. In the case of
	 * {@link Polygonal} inputs, this is equivalent to computing the distance to the
	 * polygon boundaries.
	 *
	 * @param geom
	...
	/**
	 * Tests whether the base geometry lies within a specified distance of the given
	 * geometry.
	 *
	 * @param g
	 ...
	 *
	 * @param g
	 *            the geometry to compute the nearest location to
	 ...
	 *
	 * @param g
	 *            the geometry to compute the nearest point to
