 ...
 * built on the target and the query geometries.
 *
 * <p>This approach provides the following benefits:
 *
 * <ul>
 *   <li>Performance is dramatically improved due to the use of the R-tree index and the pruning due
 *       to the Branch-and-Bound approach
 *   <li>The spatial index on the target geometry is cached which allow reuse in an repeated query
 *       situation.
 * </ul>
 *
 * Using this technique is usually much more performant than using the brute-force {@link
 * Geometry#distance(Geometry)} when one or both input geometries are large, or when evaluating many
 * distance computations against a single geometry.
 *
...
public class IndexedFacetDistance {
   ...
   *
   * <p>For geometries with many segments or points, this can be faster than using a simple distance
   * algorithm.
   *
   * @param g1 a geometry
   * @param g2 a geometry
   ...
   *
   * <p>Distances will be computed to all facets of the input geometry. The facets of the geometry
   * are the discrete segments and points contained in its components. In the case of {@link Lineal}
   * and {@link Puntal} inputs, this is equivalent to computing the conventional distance. In the
   * case of {@link Polygonal} inputs, this is equivalent to computing the distance to the polygon
   * boundaries.
   *
  ...
  public double distance(Geometry g) {
    ...
    Object[] obj = cachedTree.nearestNeighbour(tree2, FACET_SEQ_DIST);
  ...
  private static Coordinate[] toPoints(GeometryLocation[] locations) {
    ...
    Coordinate[] nearestPts =
        new Coordinate[] {locations[0].getCoordinate(), locations[1].getCoordinate()};
    return nearestPts;
  }
