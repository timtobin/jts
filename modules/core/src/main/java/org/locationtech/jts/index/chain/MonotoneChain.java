 ...
 * searching of intersections. They have the following properties:
 *
 * <ol>
 *   <li>the segments within a monotone chain never intersect each other
 *   <li>the envelope of any contiguous subset of the segments in a monotone chain is equal to the
 ...
 * chain for intersection.
 *
 * <p>Property 2 allows an efficient binary search to be used to find the intersection points of two
 * monotone chains. For many types of real-world data, these properties eliminate a large number of
 * segment comparisons, producing substantial speed gains.
 *
 * <p>One of the goals of this implementation of MonotoneChains is to be as space and time efficient
 * as possible. One design choice that aids this is that a MonotoneChain is based on a subarray of a
 * list of points. This means that new arrays of points (potentially very large) do not have to be
 * allocated.
 *
 * <p>MonotoneChains support the following kinds of queries:
 *
 * <ul>
 *   <li>Envelope select: determine all the segments in the chain which intersect a given envelope
 *   <li>Overlap: determine all the pairs of segments in two chains whose envelopes overlap
 * </ul>
 *
 ...
 * are thread-safe.
 *
 * <p>MonotoneChains support being assigned an integer id value to provide a total ordering for a
 * set of chains. This can be used during some kinds of processing to avoid redundant comparisons
 * (i.e. by comparing only chains where the first id is less than the second).
 *
 * <p>MonotoneChains support using an tolerance distance for overlap tests. This allows reporting
 * overlap in situations where intersection snapping is being used. If this is used the chain
 * envelope must be computed providing an expansion distance using {@link #getEnvelope(double)}.
 *
 * @version 1.7
 */
public class MonotoneChain {
  ...
  public Envelope getEnvelope(double expansionDistance) {
    if (env == null) {
      ...
      if (expansionDistance > 0.0) env.expandBy(expansionDistance);
    }
    return env;
  }
   ...
   * process them.
   *
   * <p>The monotone chain search algorithm attempts to optimize performance by not calling the
   * select action on chain segments which it can determine are not in the search envelope. However,
   * it *may* call the select action on segments which do not intersect the search envelope. This
   * saves on the overhead of checking envelope intersection each time, since clients may be able to
  ...
  /**
   * Determines the line segments in two chains which may overlap, and passes them to an overlap
   * action.
   *
   * <p>The monotone chain search algorithm attempts to optimize performance by not calling the
   * overlap action on chain segments which it can determine do not overlap. However, it *may* call
   * the overlap action on segments which do not actually interact. This saves on the overhead of
  ...
  public void computeOverlaps(
      MonotoneChain mc, double overlapTolerance, MonotoneChainOverlapAction mco) {
