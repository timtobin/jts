 ...
 * ({@link MonotoneChainSelectAction} and {@link MonotoneChainOverlapAction}) to
 * return the results for queries. This has time and space advantages, since it
 * is not necessary to build lists of instantiated objects to represent the
 * segments returned by the query. Queries made in this manner are thread-safe.
 *
 * <p>
 ...
 *
 * <p>
 * MonotoneChains support using an tolerance distance for overlap tests. This
 ...
 * expansion distance using {@link #getEnvelope(double)}.
 *
 * @version 1.7
 */
public class MonotoneChain {
	...
	/**
	 * Determines the line segments in two chains which may overlap, and passes them
	 * to an overlap action.
	 *
	 * <p>
	...
	/**
	 * Determines the line segments in two chains which may overlap, using an
	 * overlap distance tolerance, and passes them to an overlap action.
	 *
	 * @param mc
	 *            the chain to compare to
	...
	/**
	 * Uses an efficient mutual binary search strategy to determine which pairs of
	 * chain segments may overlap, and calls the given overlap action on them.
	 *
	 * @param start0
	 ...
	 *            the overlap action to execute on selected segments
	 */
	private void computeOverlaps(int start0, int end0, MonotoneChain mc, int start1, int end1, double overlapTolerance,
	...
	/**
	 * Gets the index of the end of the monotone chain in the underlying array of
	 * points.
	 *
	...
	/**
	 * Gets the index of the start of the monotone chain in the underlying array of
	 * points.
	 *
	...
	/**
	 * Tests whether the envelope of a section of the chain overlaps (intersects)
	 * the envelope of a section of another target chain. This test is efficient due
	 * to the monotonicity property of the sections (i.e. the envelopes can be are
	 * determined from the section endpoints rather than a full scan).
	 *
	 * @param start0
	 *            the start index of this chain section
	 * @param end0
	...
	/**
	 * Sets the id of this chain. Useful for assigning an ordering to a set of
