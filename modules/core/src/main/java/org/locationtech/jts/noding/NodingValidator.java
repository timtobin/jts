...
public class NodingValidator {
	...
	private void checkCollapse(Coordinate p0, Coordinate p1, Coordinate p2) {
		if (p0.equals(p2))
			throw new RuntimeException(
					"found non-noded collapse at " + fact.createLineString(new Coordinate[]{p0, p1, p2}));
	}

	/**
	 * Checks if a segment string contains a segment pattern a-b-a (which implies a
	 * self-intersection)
	 */
	...
	/**
	 * Checks for intersections between an endpoint of a segment string and an
	 * interior vertex of another segment string
	 */
	...
	/**
	 * Checks all pairs of segments for intersections at an interior point of a
	 * segment
	 */
	private void checkInteriorIntersections() {
		for (Object string : segStrings) {
			SegmentString ss0 = (SegmentString) string;
			for (Object segString : segStrings) {
	...
	public void checkValid() {
		// MD - is this call required? Or could it be done in the Interior Intersection
		// code?
	...
	/**
	 * @return true if there is an intersection point which is not an endpoint of
	 *         the segment p0-p1
	 */
