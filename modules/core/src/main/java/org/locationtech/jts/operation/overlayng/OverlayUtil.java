...
class OverlayUtil {
	...
	/**
	 * Computes a clipping envelope for overlay input geometries. The clipping
	 * envelope encloses all geometry line segments which might participate in the
	 * overlay, with a buffer to account for numerical precision (in particular,
	 * rounding due to a precision model. The clipping envelope is used in both the
	 * {@link RingClipper} and in the {@link LineLimiter}.
	 *
	 * <p>
	 * Some overlay operations (i.e. {@link OverlayNG#UNION and
	 * OverlayNG#SYMDIFFERENCE} cannot use clipping as an optimization, since the
	 * result envelope is the full extent of the two input geometries. In this case
	 * the returned envelope is <code>null</code> to indicate this.
	 *
	 * @param opCode
	 *            the overlay op code
	 * @param inputGeom
	 ...
	 * @return an envelope for clipping and line limiting, or null if no clipping is
	 *         performed
	 */
	static Envelope clippingEnvelope(int opCode, InputGeometry inputGeom, PrecisionModel pm) {
		...
		Envelope clipEnv = RobustClipEnvelopeComputer.getEnvelope(inputGeom.getGeometry(0), inputGeom.getGeometry(1),
				resultEnv);
	 ...
	 * operation for given inputs. The operations which have a result envelope
	 * smaller than the extent of the inputs are:
	 *
	 * <ul>
	 * <li>{@link OverlayNG#INTERSECTION}: result envelope is the intersection of
	 * the input envelopes
	 * <li>{@link OverlayNG#DIFERENCE}: result envelope is the envelope of the A
	 * input geometry
	 * </ul>
	 *
	...
	/**
	 * Determines a safe geometry envelope for clipping, taking into account the
	 * precision model being used.
	 *
	 * @param env
	 *            a geometry envelope
	 * @param pm
	 ...
	 * of the input geometries (such as whether one or both are empty, or their
	...
	/**
	 * Tests if the geometry envelopes are disjoint, or empty. The disjoint test
	 * must take into account the precision model being used, since geometry
	 * coordinates may shift under rounding.
	 *
	 * @param a
	 *            a geometry
	 * @param b
	...
	/**
	 * Tests for disjoint envelopes adjusting for rounding caused by a fixed
	 * precision model. Assumes envelopes are non-empty.
	 *
	 * @param envA
	 *            an envelope
	 * @param envB
	...
	/**
	 * Computes the dimension of the result of applying the given operation to
	 * inputs with the given dimensions. This assumes that complete collapse does
	 * not occur.
	 *
	 * <p>
	 ...
	 *
	 * <ul>
	 * <li>{@link OverlayNG#INTERSECTION} - result has the dimension of the lowest
	 * input dimension
	 * <li>{@link OverlayNG#UNION} - result has the dimension of the highest input
	 * dimension
	 * <li>{@link OverlayNG#DIFFERENCE} - result has the dimension of the left-hand
	 * input
	 * <li>{@link OverlayNG#SYMDIFFERENCE} - result has the dimension of the highest
	 * input dimension (since the Symmetric Difference is the Union of the
	 * Differences).
	 * </ul>
	 *
	 * @param opCode
	 ...
	 */
	static Geometry createResultGeometry(List<Polygon> resultPolyList, List<LineString> resultLineList,
			List<Point> resultPointList, GeometryFactory geometryFactory) {
		List<Geometry> geomList = new ArrayList<>();

		// TODO: for mixed dimension, return collection of Multigeom for each dimension
		// (breaking
		// change)
	...
	/**
	 * Round the key point if precision model is fixed. Note: return value is only
	 * copied if rounding is performed.
	 *
	 * @param pt
	 *            the Point to round
	...
	/**
	 * Rounds a coordinate if precision model is fixed. Note: return value is only
	 * copied if rounding is performed.
	 *
	 * @param p
	...
	/**
	 * A heuristic check for overlay result correctness comparing the areas of the
	 * input and result. The heuristic is necessarily coarse, but it detects some
	 * obvious issues. (e.g. https://github.com/locationtech/jts/issues/798)
	 *
	 * <p>
	 * <b>Note:</b> - this check is only safe if the precision model is floating. It
	 * should also be safe for snapping noding if the distance tolerance is
	 * reasonably small. (Fixed precision models can lead to collapse causing result
	 * area to expand.)
	 *
	 * @param geom0
	 *            input geometry 0
	...
	/**
	 * Tests if the area of a difference is greater than the minimum possible
	 * difference area. This is a heuristic which will only detect gross overlay
	 * errors.
	 *
	 * @param areaA
	 *            the area of A
