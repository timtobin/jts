 ...
 * segment strings are of maximal length in the output arrangement).
 *
 * <p>The snap tolerance should be chosen to be as small as possible while still producing a correct
 * result. It probably only needs to be small enough to eliminate "nearly-coincident" segments, for
 * which intersection points cannot be computed accurately. This implies a factor of about 10e-12
 * smaller than the magnitude of the segment coordinates.
 *
 * <p>With an appropriate snap tolerance this algorithm appears to be very robust. So far no failure
 * cases have been found, given a small enough snap tolerance.
 *
 * <p>The correctness of the output is not verified by this noder. If required this can be done by
 * {@link org.locationtech.jts.noding.ValidatingNoder}.
 *
 * @version 1.17
 */
public class SnappingNoder implements Noder {
  ...
  /**
   * Seeds the snap index with a small set of vertices chosen quasi-randomly using a low-discrepancy
   * sequence. Seeding the snap index KdTree induces a more balanced tree. This prevents monotonic
   * runs of vertices unbalancing the tree and causing poor query performance.
   *
   ...
   *
   * <p>Also adds the intersection nodes to the segments.
   *
