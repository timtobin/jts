...
/**
 * Nodes a set of segment strings snapping vertices and intersection points
 * together if they lie within the given snap tolerance distance. Vertices take
 * priority over intersection points for snapping. Input segment strings are
 * generally only split at true node points (i.e. the output segment strings are
 * of maximal length in the output arrangement).
 *
 * <p>
 ...
 * So far no failure cases have been found, given a small enough snap tolerance.
 *
 * <p>
 * The correctness of the output is not verified by this noder. If required this
 * can be done by {@link org.locationtech.jts.noding.ValidatingNoder}.
 *
 * @version 1.17
 */
public class SnappingNoder implements Noder {
	...
	/**
	 * Seeds the snap index with a small set of vertices chosen quasi-randomly using
	 * a low-discrepancy sequence. Seeding the snap index KdTree induces a more
	 * balanced tree. This prevents monotonic runs of vertices unbalancing the tree
	 * and causing poor query performance.
	 *
	 * @param segStrings
	 *            the segStrings to be noded
	 */
	...
	private Collection snapIntersections(List<NodedSegmentString> inputSS) {
		...
		/**
		 * Use an overlap tolerance to ensure all possible snapped intersections are
		 * found
		 */
