...
/**
 * The root node of a single {@link Bintree}. It is centred at the origin, and does not have a
 * defined extent.
 *
 * @version 1.7
 */
public class Root extends NodeBase {
  ...
  public void insert(Interval itemInterval, Object item) {
    ...
    /**
     * the item must be contained in one interval, so insert it into the tree for that interval
     * (which may not yet exist)
     */
    Node node = subnode[index];
    /**
     * If the subnode doesn't exist or this item is not contained in it, have to expand the tree
    ...
    /**
     * At this point we have a subnode which exists and must contain contains the env for the item.
  ...
  /**
   * insert an item which is known to be contained in the tree rooted at the given Node. Lower
   * levels of the tree will be created if necessary to hold the item.
   */
  private void insertContained(Node tree, Interval itemInterval, Object item) {
    Assert.isTrue(tree.getInterval().contains(itemInterval));
    /**
     * Do NOT create a new node for zero-area intervals - this would lead to infinite recursion.
     * Instead, use a heuristic of simply returning the smallest existing node containing the query
     */
    boolean isZeroArea = IntervalSize.isZeroWidth(itemInterval.getMin(), itemInterval.getMax());
    NodeBase node;
    if (isZeroArea) node = tree.find(itemInterval);
