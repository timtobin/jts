...
public class Root extends NodeBase {
	...
	public void insert(Interval itemInterval, Object item) {
		...
		/**
		 * the item must be contained in one interval, so insert it into the tree for
		 * that interval (which may not yet exist)
		 */
		...
		/**
		 * At this point we have a subnode which exists and must contain contains the
	 ...
	 * Node. Lower levels of the tree will be created if necessary to hold the item.
	 */
	private void insertContained(Node tree, Interval itemInterval, Object item) {
		 ...
		 * existing node containing the query
		 */
