...
/**
 * Writes a {@link Geometry} into Well-Known Binary format. Supports use of an
 * {@link OutStream}, which allows easy use with arbitrary byte stream sinks.
 *
 * <p>
 * The WKB format is specified in the OGC
 * <A HREF="http://portal.opengeospatial.org/files/?artifact_id=829"><i>Simple
 * Features for SQL specification</i></a> (section 3.3.2.6).
 *
 * <p>
 * There are a few cases which are not specified in the standard. The
 * implementation uses a representation which is compatible with other common
 * spatial systems (notably, PostGIS).
 *
 * <ul>
 * <li>{@link LinearRing}s are written as {@link LineString}s
 * <li>Empty geometries are output as follows:
 * <ul>
 * <li><b>Point</b>: a <code>WKBPoint</code> with <code>NaN</code> ordinate
 * values
 * <li><b>LineString</b>: a <code>WKBLineString</code> with zero points
 * <li><b>Polygon</b>: a <code>WKBPolygon</code> with zero rings
 * <li><b>Multigeometries</b>: a <code>WKBMulti</code> geometry of appropriate
 * type with zero elements
 * <li><b>GeometryCollections</b>: a <code>WKBGeometryCollection</code> with
 * zero elements
 * </ul>
 * </ul>
 *
 * <p>
 * This implementation supports the <b>Extended WKB</b> standard. Extended WKB
 * allows writing 3-dimensional coordinates and the geometry SRID value. The
 * presence of 3D coordinates is indicated by setting the high bit of the
 * <tt>wkbType</tt> word. The presence of a SRID is indicated by setting the
 * third bit of the <tt>wkbType</tt> word. EWKB format is upward-compatible with
 * the original SFS WKB format.
 *
 * <p>
 * SRID output is optimized, if specified. Only the top-level geometry has the
 * SRID included. This assumes that all geometries in a collection have the same
 * SRID as the collection (which is the JTS convention).
 *
 * <p>
 * This class supports reuse of a single instance to read multiple geometries.
 ...
 *
 * <p>
 * <i>The specification uses a syntax language similar to that used in the C
 * language. Bitfields are specified from high-order to low-order bits.</i>
 *
 * <p>
 *
 * <blockquote>
...
public class WKBWriter {
	...
	/**
	 * Creates a writer that writes {@link Geometry}s with output dimension = 2 and
	...
	/**
	 * Creates a writer that writes {@link Geometry}s with the given dimension (2 or
	 * 3) for output coordinates and {@link ByteOrderValues#BIG_ENDIAN} byte order.
	 * If the input geometry has a small coordinate dimension, coordinates will be
	 * padded with {@link Coordinate#NULL_ORDINATE}.
	 *
	 ...
	 * Creates a writer that writes {@link Geometry}s with the given dimension (2 or
	 * 3) for output coordinates and {@link ByteOrderValues#BIG_ENDIAN} byte order.
	 * This constructor also takes a flag to control whether srid information will
	 * be written. If the input geometry has a smaller coordinate dimension,
	...
	/**
	 * Creates a writer that writes {@link Geometry}s with the given dimension (2 or
	 * 3) for output coordinates and byte order If the input geometry has a small
	 * coordinate dimension, coordinates will be padded with
	 * {@link Coordinate#NULL_ORDINATE}.
	 *
	 * @param outputDimension
	 *            the coordinate dimension to output (2 or 3)
	 * @param byteOrder
	...
	/**
	 * Creates a writer that writes {@link Geometry}s with the given dimension (2 to
	 * 4) for output coordinates and byte order. This constructor also takes a flag
	 * to control whether srid information will be written. If the input geometry
	 * has a small coordinate dimension, coordinates will be padded with
	 * {@link Coordinate#NULL_ORDINATE}. The output follows the following rules:
	 *
	 * <ul>
	 * <li>If the specified <b>output dimension is 3</b> and the <b>z is measure
	 * flag is set to true</b>, the Z value of coordinates will be written if it is
	 * present (i.e. if it is not <code>Double.NaN</code>)
	 * <li>If the specified <b>output dimension is 3</b> and the <b>z is measure
	 * flag is set to false</b>, the Measure value of coordinates will be written if
	 * it is present (i.e. if it is not <code>Double.NaN</code>)
	 * <li>If the specified <b>output dimension is 4</b>, the Z value of coordinates
	 * will be written even if it is not present when the Measure value is present.
	 * The Measure value of coordinates will be written if it is present (i.e. if it
	...
	private void writeCoordinateSequence(CoordinateSequence seq, EnumSet<Ordinate> outputOrdinates, boolean writeSize,
	...
	private void writeLineString(LineString line, EnumSet<Ordinate> outputOrdinates, OutStream os) throws IOException {
		writeByteOrder(os);
	...
	private void writeNaNs(EnumSet<Ordinate> outputOrdinates, OutStream os) throws IOException {
	...
	private void writePolygon(Polygon poly, EnumSet<Ordinate> outputOrdinates, OutStream os) throws IOException {
		...
		for (int i = 0; i < poly.getNumInteriorRing(); i++) {
			writeCoordinateSequence(poly.getInteriorRingN(i).getCoordinateSequence(), outputOrdinates, true, os);
		}
	}
}
