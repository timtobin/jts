...
/**
 * Writes a {@link Geometry} into Well-Known Binary format. Supports use of an {@link OutStream},
 * which allows easy use with arbitrary byte stream sinks.
 *
 * <p>The WKB format is specified in the OGC <A
 * HREF="http://portal.opengeospatial.org/files/?artifact_id=829"><i>Simple Features for SQL
 * specification</i></a> (section 3.3.2.6).
 *
 * <p>There are a few cases which are not specified in the standard. The implementation uses a
 * representation which is compatible with other common spatial systems (notably, PostGIS).
 *
 * <ul>
 *   <li>{@link LinearRing}s are written as {@link LineString}s
 *   <li>Empty geometries are output as follows:
 *       <ul>
 *         <li><b>Point</b>: a <code>WKBPoint</code> with <code>NaN</code> ordinate values
 *         <li><b>LineString</b>: a <code>WKBLineString</code> with zero points
 *         <li><b>Polygon</b>: a <code>WKBPolygon</code> with zero rings
 *         <li><b>Multigeometries</b>: a <code>WKBMulti</code> geometry of appropriate type with
 *             zero elements
 *         <li><b>GeometryCollections</b>: a <code>WKBGeometryCollection</code> with zero elements
 *       </ul>
 * </ul>
 *
 * <p>This implementation supports the <b>Extended WKB</b> standard. Extended WKB allows writing
 * 3-dimensional coordinates and the geometry SRID value. The presence of 3D coordinates is
 * indicated by setting the high bit of the <tt>wkbType</tt> word. The presence of a SRID is
 * indicated by setting the third bit of the <tt>wkbType</tt> word. EWKB format is upward-compatible
 * with the original SFS WKB format.
 *
 * <p>SRID output is optimized, if specified. Only the top-level geometry has the SRID included.
 * This assumes that all geometries in a collection have the same SRID as the collection (which is
 * the JTS convention).
 *
 * <p>This class supports reuse of a single instance to read multiple geometries. This class is not
 ...
 *
 * <p><i>The specification uses a syntax language similar to that used in the C language. Bitfields
 * are specified from high-order to low-order bits.</i>
 *
 * <p>
 *
 * <blockquote>
...
public class WKBWriter {
  ...
  /**
   * Creates a writer that writes {@link Geometry}s with the given dimension (2 or 3) for output
   * coordinates and {@link ByteOrderValues#BIG_ENDIAN} byte order. This constructor also takes a
   * flag to control whether srid information will be written. If the input geometry has a smaller
  ...
  /**
   * Creates a writer that writes {@link Geometry}s with the given dimension (2 or 3) for output
   * coordinates and byte order If the input geometry has a small coordinate dimension, coordinates
   * will be padded with {@link Coordinate#NULL_ORDINATE}.
   *
   * @param outputDimension the coordinate dimension to output (2 or 3)
  ...
  /**
   * Creates a writer that writes {@link Geometry}s with the given dimension (2 to 4) for output
   * coordinates and byte order. This constructor also takes a flag to control whether srid
   * information will be written. If the input geometry has a small coordinate dimension,
   * coordinates will be padded with {@link Coordinate#NULL_ORDINATE}. The output follows the
   * following rules:
   *
   * <ul>
  ...
  public void write(Geometry geom, OutStream os) throws IOException {
    ...
    if (geom instanceof Point) writePoint((Point) geom, actualOutputOrdinates, os);
    // LinearRings will be written as LineStrings
    else if (geom instanceof LineString)
      writeLineString((LineString) geom, actualOutputOrdinates, os);
    else if (geom instanceof Polygon) writePolygon((Polygon) geom, actualOutputOrdinates, os);
    else if (geom instanceof MultiPoint)
      writeGeometryCollection(
          WKBConstants.wkbMultiPoint, (MultiPoint) geom, actualOutputOrdinates, os);
    else if (geom instanceof MultiLineString)
      writeGeometryCollection(
          WKBConstants.wkbMultiLineString, (MultiLineString) geom, actualOutputOrdinates, os);
    else if (geom instanceof MultiPolygon)
      writeGeometryCollection(
          WKBConstants.wkbMultiPolygon, (MultiPolygon) geom, actualOutputOrdinates, os);
    else if (geom instanceof GeometryCollection)
      writeGeometryCollection(
          WKBConstants.wkbGeometryCollection, (GeometryCollection) geom, actualOutputOrdinates, os);
    else {
      Assert.shouldNeverReachHere("Unknown Geometry type");
    }
  }

  private void writePoint(Point pt, EnumSet<Ordinate> outputOrdinates, OutStream os)
      throws IOException {
    writeByteOrder(os);
  ...
  private void writePolygon(Polygon poly, EnumSet<Ordinate> outputOrdinates, OutStream os)
      throws IOException {
    writeByteOrder(os);
    ...
    writeCoordinateSequence(
        poly.getExteriorRing().getCoordinateSequence(), outputOrdinates, true, os);
    for (int i = 0; i < poly.getNumInteriorRing(); i++) {
      writeCoordinateSequence(
          poly.getInteriorRingN(i).getCoordinateSequence(), outputOrdinates, true, os);
    }
  }

  private void writeGeometryCollection(
      int geometryType, GeometryCollection gc, EnumSet<Ordinate> outputOrdinates, OutStream os)
      throws IOException {
    writeByteOrder(os);
  ...
  private void writeGeometryType(
      int geometryType, EnumSet<Ordinate> outputOrdinates, Geometry g, OutStream os)
      throws IOException {
    int ordinals = 0;
  ...
  private void writeCoordinateSequence(
      CoordinateSequence seq, EnumSet<Ordinate> outputOrdinates, boolean writeSize, OutStream os)
      throws IOException {
  ...
  private void writeCoordinate(
      CoordinateSequence seq, boolean hasZ, boolean hasM, int index, OutStream os)
      throws IOException {
