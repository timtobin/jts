...
/**
 * A transformer to reduce the precision of geometry in a topologically valid way. Repeated points
 * are removed. If geometry elements collapse below their valid length, they may be removed by
 * specifying <code>isRemoveCollapsed</code as <code>true</code>.
 *
 * @author mdavis
 */
class PrecisionReducerTransformer extends GeometryTransformer {
  ...
  protected CoordinateSequence transformCoordinates(
    ...
    /**
     * Check if the removal of repeated points collapsed the coordinate list to an invalid size for
     * the type of the parent geometry. It is not necessary to check for Point collapses, since the
     * coordinate list can never collapse to less than one point. If the size is invalid, return the
     * full-size coordinate array first computed, or null if collapses are being removed. (This may
    ...
    /**
     * Handle collapse. If specified return null so parent geometry is removed or empty, otherwise
