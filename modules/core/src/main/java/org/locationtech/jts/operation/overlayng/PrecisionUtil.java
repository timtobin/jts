...
/**
 * Functions for computing precision model scale factors that ensure robust
 * geometry operations. In particular, these can be used to automatically
 * determine appropriate scale factors for operations using limited-precision
 * noding (such as {@link OverlayNG}).
 *
 * <p>
 * WARNING: the <code>inherentScale</code> and <code>robustScale</code>
 * functions can be very slow, due to the method used to determine number of
 * decimal places of a number. These are not recommended for production use.
 *
 * @author Martin Davis
 */
public class PrecisionUtil {
	/**
	 * A number of digits of precision which leaves some computational "headroom" to
	 * ensure robust evaluation of certain double-precision floating point geometric
	 * operations.
	 *
	 * <p>
	 ...
	 * factor for rounding which preserves <b>all</b> digits of precision
	 * (significant digits) present in the geometry ordinates.
	 *
	 * <p>
	...
	/**
	 * Computes the inherent scale of two geometries. The inherent scale is the
	 * scale factor for rounding which preserves <b>all</b> digits of precision
	 * (significant digits) present in the geometry ordinates.
	 *
	 * <p>
	...
	/**
	 * Computes the inherent scale of a number. The inherent scale is the scale
	 * factor for rounding which preserves <b>all</b> digits of precision
	 * (significant digits) present in the numeric value. In other words, it is the
	 * scale factor which does not change the numeric value when rounded:
	 *
	 * <pre>
	...
	/**
	 * Determines the maximum magnitude (absolute value) of the bounds of an of an
	 * envelope. This is equal to the largest ordinate value which must be
	 * accommodated by a scale factor.
	 *
	 * @param env
	 *            an envelope
	...
	/**
	 * Determines the number of decimal places represented in a double-precision
	 * number (as determined by Java). This uses the Java double-precision print
	 * routine to determine the number of decimal places, This is likely not optimal
	 * for performance, but should be accurate and portable.
	 *
	 * @param value
	 *            a numeric value
	...
	private static int numberOfDecimals(double value) {
		/**
		 * Ensure that scientific notation is NOT used (it would skew the number of
		 * fraction digits)
		 */
	...
	/**
	 * Computes the scale factor which will produce a given number of digits of
	 * precision (significant digits) when used to round the given number.
	 *
	 * <p>
	 * For example: to provide 5 decimal digits of precision for the number 123.456
	 * the precision scale factor is 100; for 3 digits of precision the scale factor
	 * is 1; for 2 digits of precision the scale factor is 0.1.
	 *
	 * <p>
	 * Rounding to the scale factor can be performed with
	 * {@link PrecisionModel#round}
	 *
	 * @param value
	 *            a number to be rounded
	 * @param precisionDigits
	 *            the number of digits of precision required
	 * @return scale factor which provides the required number of digits of
	 *         precision
	...
	/**
	 * Determines a precision model to use for robust overlay operations for one
	 * geometry. The precision scale factor is chosen to maximize output precision
	 * while avoiding round-off issues.
	 *
	 * <p>
	 * NOTE: this is a heuristic determination, so is not guaranteed to eliminate
	 * precision issues.
	 *
	 * <p>
	 * WARNING: this is very slow.
	...
	/**
	 * Determines a precision model to use for robust overlay operations. The
	 * precision scale factor is chosen to maximize output precision while avoiding
	 * round-off issues.
	 *
	 * <p>
	 * NOTE: this is a heuristic determination, so is not guaranteed to eliminate
	...
	/**
	 * Determines a scale factor which maximizes the digits of precision and is safe
	 * to use for overlay operations. The robust scale is the minimum of the
	...
	/*
	 * // this doesn't work private static int BADnumDecimals(double value) { double
	 * val = Math.abs(value); double frac = val - Math.floor(val); int numDec = 0;
	 * while (frac > 0 && numDec < MAX_PRECISION_DIGITS) { double mul10 = 10 * frac;
	 * frac = mul10 - Math.floor(mul10); numDec ++; } return numDec; }
	 */
	...
	private static double robustScale(double inherentScale, double safeScale) {
		/**
		 * Use safe scale if lower, since it is important to preserve some precision for
		 * robustness
		 */
	...
	/**
	 * Computes a safe scale factor for a geometry. A safe scale factor ensures that
	 * the rounded ordinates have no more than {@link #MAX_ROBUST_DP_DIGITS} digits
	 * of precision.
	 *
	 * @param geom
	 *            a geometry
	...
	/**
	 * Computes a safe scale factor for two geometries. A safe scale factor ensures
	 * that the rounded ordinates have no more than {@link #MAX_ROBUST_DP_DIGITS}
	 * digits of precision.
	 *
	 * @param a
	 *            a geometry
	...
	/**
	 * Computes a safe scale factor for a numeric value. A safe scale factor ensures
	 * that rounded number has no more than {@link #MAX_ROBUST_DP_DIGITS} digits of
	 * precision.
	 *
	 * @param value
	 *            a numeric value
