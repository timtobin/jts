...
public class PointBuilder {
	...
	private void extractNonCoveredResultNodes(int opCode) {
		...
		for (Object o : op.getGraph().getNodes()) {
			...
			if (n.isInResult())
				continue;
			// if an incident edge is in the result, then the node coordinate is included
			// already
	 ...
	 *
	 * <p>
	 * A node is covered if it is contained in another element Geometry with higher
	 * dimension (e.g. a node point might be contained in a polygon, in which case
	 * the point can be eliminated from the result).
	 *
	 * @param n
	 *            the node to test
	 */
