 ...
 * {@link Geometry}. The output is KML fragments which can be substituted
 * wherever the KML <i>Geometry</i> abstract element can be used.
 *
 * <p>
 * Output elements are indented to provide a nicely-formatted representation. An
 * output line prefix and maximum number of coordinates per line can be
 * specified.
 *
 * <p>
 * The Z ordinate value output can be forced to be a specific value. The
 * <code>extrude</code> and <code>altitudeMode</code> modes can be set. If set,
 * the corresponding sub-elements will be output.
 */
public class KMLWriter {
	...
	/**
	 * The KML standard value <code>relativeToGround</code> for use in
	 * {@link #setAltitudeMode(String)}.
	 */
	...
	/**
	 * Writes a Geometry as KML to a string, using a specified Z value, precision,
	 * extrude flag, and altitude mode code.
	 *
	 * @param geometry
	 *            the geometry to write
	 * @param z
	 ...
	 * @return a string containing the KML geometry representation
	 */
	public static String writeGeometry(Geometry geometry, double z, int precision, boolean extrude,
			String altitudeMode) {
		KMLWriter writer = new KMLWriter();
		writer.setZ(z);
	...
	/**
	 * Sets a tag string which is prefixed to every emitted text line. This can be
	 * used to indent the geometry text in a containing document.
	 *
	 * @param linePrefix
	 *            the tag string
	 */
	...
	/**
	 * Sets the Z value to be output for all coordinates. This overrides any Z value
	 * present in the Geometry coordinates.
	 *
	 * @param zVal
	...
	/**
	 * Appends the KML representation of a {@link Geometry} to a
	 * {@link StringBuffer}.
	 *
	 * @param geometry
	 *            the geometry to write
	...
	private void writeLinearRing(LinearRing lr, String attributes, boolean writeModifiers, int level,
			StringBuffer buf) {
