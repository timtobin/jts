...
/**
 * Writes {@link Geometry}s into Java2D {@link Shape} objects of the appropriate type. This supports
 * rendering geometries using Java2D. The ShapeWriter allows supplying a {@link PointTransformation}
 * class, to transform coordinates from model space into view space. This is useful if a client is
 * providing its own transformation logic, rather than relying on Java2D <tt>AffineTransform</tt>s.
 *
 * <p>The writer supports removing duplicate consecutive points (via the {@link
 * #setRemoveDuplicatePoints(boolean)} method) as well as true <b>decimation</b> (via the {@link
 * #setDecimation(double)} method. Enabling one of these strategies can substantially improve
 * rendering speed for large geometries. It is only necessary to enable one strategy. Using
 * decimation is preferred, but this requires determining a distance below which input geometry
 * vertices can be considered unique (which may not always be feasible). If neither strategy is
 * enabled, all vertices of the input <tt>Geometry</tt> will be represented in the output
 * <tt>Shape</tt>.
 *
 * <p>
 */
public class ShapeWriter {
  ...
  /**
   * If true, decimation will be used to reduce the number of vertices by removing consecutive
   * duplicates.
   */
  ...
  /**
   * Creates a new ShapeWriter with a specified point transformation and the default point shape
   * factory.
   *
   * @param pointTransformer a transformation from model to view space to use
   */
  ...
  /**
   * Sets whether duplicate consecutive points should be eliminated. This can reduce the size of the
   * generated Shapes and improve rendering speed, especially in situations where a transform
   * reduces the extent of the geometry.
   *
  ...
  /**
   * Sets the decimation distance used to determine whether vertices of the input geometry are
   * considered to be duplicate and thus removed. The distance is axis distance, not Euclidean
   * distance. The distance is specified in the input geometry coordinate system (NOT the
   * transformed output coordinate system).
   *
   * <p>When rendering to a screen image, a suitably small distance should be used to avoid obvious
   * rendering defects. A distance equivalent to the equivalent of 1.5 pixels or less is recommended
   * (and perhaps even smaller to avoid any chance of visible artifacts).
   *
   * <p>The default distance is 0.0, which disables decimation.
   *
  ...
  /**
   * Creates a {@link Shape} representing a {@link Geometry}, according to the specified
   * PointTransformation and PointShapeFactory (if relevant).
   *
   * <p>Note that Shapes do not preserve information about which elements in heterogeneous
   * collections are 1D and which are 2D. For example, a GeometryCollection containing a ring and a
   * disk will render as two disks if Graphics.fill is used, or as two rings if Graphics.draw is
   * used. To avoid this issue use separate shapes for the components.
   *
  ...
  private void appendRing(PolygonShape poly, Coordinate[] coords) {
    ...
    /**
     * Don't include closing point. Ring path will be closed explicitly, which provides a more
