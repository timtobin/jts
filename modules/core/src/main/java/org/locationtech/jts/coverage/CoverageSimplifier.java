...
/**
 * Simplifies the boundaries of the polygons in a polygonal coverage while
 * preserving the original coverage topology. An area-based simplification
 * algorithm (similar to Visvalingam-Whyatt simplification) is used to provide
 * high-quality results. Also supports simplifying just the inner edges in a
 * coverage, which allows simplifying "patches" without affecting their
 * boundary.
 *
 * <p>
 * The amount of simplification is determined by a tolerance value, which is a
 * non-negative quantity. It equates roughly to the maximum distance by which a
 * simplified line can change from the original. (In fact, it is the square root
 * of the area tolerance used in the Visvalingam-Whyatt algorithm.)
 *
 * <p>
 ...
 * <li>Node points (inner vertices shared by three or more polygons, or boundary
 * vertices shared by two or more) are not changed
 * <li>Polygons maintain their line-adjacency (edges are never removed)
 * <li>Rings are simplified to a minimum of 4 vertices, to better preserve their
 * shape
 * <lI>Rings smaller than the area tolerance are removed where possible. This
 * applies to both holes and "islands" (multipolygon elements which are disjoint
 * or touch another polygon at a single vertex). At least one polygon is
 * retained for each input geometry (the one with largest area).
 * </ul>
 *
 * This class supports simplification using different distance tolerances for
 * inner and outer edges of the coverage (including no simplfication using a
 * tolerance of 0.0). This allows, for example, inner simplification, which
 * simplifies only edges of the coverage which are adjacent to two polygons.
 * This allows partial simplification of a coverage, since a simplified subset
 * of a coverage still matches the remainder of the coverage.
 *
 * <p>
 * The class allows specifying a separate tolerance for each element of the
 * input coverage.
 *
 * <p>
 * The input coverage should be valid according to {@link CoverageValidator}.
 * Invalid coverages may be simplified, but the result will likely still be
 * invalid.
 *
 * <h3>FUTURE WORK</h3>
 *
 * Support geodetic data by computing true geodetic area, and accepting
...
public class CoverageSimplifier {
	...
	private static TPVWSimplifier.Edge[] createEdges(List<CoverageEdge> covEdges, double toleranceInner,
	...
	/**
	 * Simplifies the boundaries of a set of polygonal geometries forming a
	 * coverage, preserving the coverage topology.
	 *
	 * @param coverage
	...
	/**
	 * Simplifies the boundaries of a set of polygonal geometries forming a
	 * coverage, preserving the coverage topology, using a separate tolerance for
	 * each element of the coverage. Coverage edges are simplified using the lowest
	 * tolerance of each adjacent element.
	 *
	 * @param coverage
	 *            a set of polygonal geometries forming a coverage
	 * @param tolerance
	 *            the simplification tolerances (one per input element)
	 * @return the simplified coverage polygons
	 */
	...
	/**
	 * Simplifies the inner boundaries of a set of polygonal geometries forming a
	 * coverage, preserving the coverage topology. Edges which form the exterior
	 * boundary of the coverage are left unchanged.
	 *
	 * @param coverage
	 *            a set of polygonal geometries forming a coverage
	 * @param tolerance
	...
	/**
	 * Simplifies the outer boundaries of a set of polygonal geometries forming a
	 * coverage, preserving the coverage topology. Edges in the interior of the
	...
	/**
	 * Sets the factor applied to the area tolerance to determine if small rings
	 * should be removed. Larger values cause more rings to be removed. A value of 0
	 * prevents rings from being removed.
	 *
	 * @param removableSizeFactor
	...
	/**
	 * Sets the weight influencing how smooth the simplification should be. The
	 * weight must be between 0 and 1. Larger values increase the smoothness of the
	 * simplified edges.
	 *
	 * @param smoothWeight
	...
	/**
	 * Computes the simplified coverage using separate distance tolerances for inner
	 * and outer edges, preserving the coverage topology.
	 *
	 * @param toleranceInner
	 *            the distance tolerance for inner edges
	 * @param toleranceOuter
	 *            the distance tolerance for outer edges
	 * @return the simplified coverage polygons
	 */
	...
	/**
	 * Computes the simplified coverage using separate distance tolerances for each
	 * coverage element, preserving the coverage topology.
	 *
	 * @param tolerances
	 *            the distance tolerances for the coverage elements
	 * @return the simplified coverage polygons
	 */
