...
/**
 * Computes the {@link LinearLocation} of the point on a linear {@link Geometry}
 * nearest a given {@link Coordinate}. The nearest point is not necessarily
 * unique; this class always computes the nearest point closest to the start of
 * the geometry.
 */
class LocationIndexOfPoint {
	...
	/**
	 * Find the nearest {@link LinearLocation} along the linear {@link Geometry} to
	 * a given {@link Coordinate} after the specified minimum
	 * {@link LinearLocation}. If possible the location returned will be strictly
	 * greater than the <code>minLocation</code>. If this is not possible, the value
	 * returned will equal <code>minLocation</code>. (An example where this is not
	 * possible is when minLocation = [end of line] ).
	 *
	 * @param inputPt
	 *            the coordinate to locate
	 * @param minIndex
	...
	/**
	 * Computes the fraction of distance (in <tt>[0.0, 1.0]</tt>) that a point
	 * occurs along a line segment. If the point is beyond either ends of the line
	 * segment, the closest fractional value (<tt>0.0</tt> or <tt>1.0</tt>) is
	 * returned.
	 *
	 * @param seg
	 *            the line segment to use
	 * @param inputPt
	 *            the point
	 ...
	 * seg, Coordinate inputPt) { double segFrac = seg.projectionFactor(inputPt); if
	 * (segFrac < 0.0) segFrac = 0.0; else if (segFrac > 1.0) segFrac = 1.0; return
	 * segFrac; }
	 */
}
