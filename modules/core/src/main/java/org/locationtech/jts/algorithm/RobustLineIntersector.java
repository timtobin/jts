...
public class RobustLineIntersector extends LineIntersector {
	...
	/**
	 * Gets the Z value of the first argument if present, otherwise the value of the
	 * second argument.
	 *
	 * @param p
	 *            a coordinate, possibly with Z
	 * @param q
	 *            a coordinate, possibly with Z
	...
	/**
	 * Gets the Z value of a coordinate if present, or interpolates it from the
	 * segment it lies on. If the segment Z values are not fully populate NaN is
	 * returned.
	 *
	 * @param p
	 *            a coordinate, possibly with Z
	 * @param p1
	 *            a segment endpoint, possibly with Z
	 * @param p2
	 *            a segment endpoint, possibly with Z
	...
	/**
	 * Interpolates a Z value for a point along a line segment between two points.
	 * The Z value of the interpolation point (if any) is ignored. If either segment
	 * point is missing Z, returns NaN.
	 *
	 * @param p
	 *            a coordinate
	 * @param p1
	...
	/**
	 * Interpolates a Z value for a point along two line segments and computes their
	 * average. The Z value of the interpolation point (if any) is ignored. If one
	 * segment point is missing Z that segment is ignored if both segments are
	 * missing Z, returns NaN.
	 *
	 * @param p
	 *            a coordinate
	 * @param p1
	...
	/*
	 * private void checkDD(Coordinate p1, Coordinate p2, Coordinate q1, Coordinate
	 * q2, Coordinate intPt) { Coordinate intPtDD = CGAlgorithmsDD.intersection(p1,
	 * p2, q1, q2); boolean isIn = isInSegmentEnvelopes(intPtDD); Debug.println(
	 * "DD in env = " + isIn + "  --------------------- " + intPtDD); if
	 * (intPt.distance(intPtDD) > 0.0001) { Debug.println("Distance = " +
	 * intPt.distance(intPtDD)); } }
	 */
	...
	protected int computeIntersect(Coordinate p1, Coordinate p2, Coordinate q1, Coordinate q2) {
		...
		if (Pq1 == 0 || Pq2 == 0 || Qp1 == 0 || Qp2 == 0) {
			 ...
			 * 46.76654261437082 ) and LINESTRING ( -48.51001596420236 -22.063180333403878,
			 * 19.850257749638203 46.29709338043669 )
			 *
			 * <p>
