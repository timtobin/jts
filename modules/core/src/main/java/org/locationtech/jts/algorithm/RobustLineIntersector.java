...
public class RobustLineIntersector extends LineIntersector {
  ...
  /**
   * This method computes the actual value of the intersection point. It is rounded to the precision
   * model if being used.
   */
  ...
  */
  ...
  /**
   * Computes a segment intersection. Round-off error can cause the raw computation to fail,
   * (usually due to the segments being approximately parallel). If this happens, a reasonable
  ...
  /**
   * Gets the Z value of the first argument if present, otherwise the value of the second argument.
   *
  ...
  /**
   * Gets the Z value of a coordinate if present, or interpolates it from the segment it lies on. If
   * the segment Z values are not fully populate NaN is returned.
   *
   * @param p a coordinate, possibly with Z
  ...
  /**
   * Interpolates a Z value for a point along a line segment between two points. The Z value of the
   * interpolation point (if any) is ignored. If either segment point is missing Z, returns NaN.
   *
  ...
  /**
   * Interpolates a Z value for a point along two line segments and computes their average. The Z
   * value of the interpolation point (if any) is ignored. If one segment point is missing Z that
   * segment is ignored if both segments are missing Z, returns NaN.
   *
   * @param p a coordinate
   * @param p1 a segment endpoint, possibly with Z
   * @param p2 a segment endpoint, possibly with Z
