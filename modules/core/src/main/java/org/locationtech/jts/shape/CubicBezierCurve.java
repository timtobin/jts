...
/**
 * Creates a curved geometry by replacing the segments of the input with Cubic
 * Bezier Curves. The Bezier control points are determined from the segments of
 * the geometry and the alpha control parameter controlling curvedness, and the
 * optional skew parameter controlling the shape of the curve at vertices. The
 * Bezier Curves are created to be C2-continuous (smooth) at each input vertex.
 *
 * <p>
...
public class CubicBezierCurve {
	...
	/**
	 * Creates a control point aimed at the control point at the opposite end of the
	 * segment.
	 *
	 * <p>
	...
	/**
	 * Creates a geometry of linearized Cubic Bezier Curves defined by the segments
	 * of the input and a list (or lists) of control points.
	 *
	 * <p>
	 * Typically the control point geometry is a {@link LineString} or
	 * {@link MultiLineString} containing an element for each line or ring in the
	 * input geometry. The list of control points for each linear element must
	 * contain two vertices for each segment (and thus <code>2 * npts - 2
	 * </code>).
	 *
	 * @param geom
	...
	/**
	 * Creates a geometry of linearized Cubic Bezier Curves defined by the segments
	 * of the input and a parameter controlling how curved the result should be.
	 *
	 * @param geom
	 *            the geometry defining the curve
	 * @param alpha
	 *            curvedness parameter (0 is linear, 1 is round, >1 is increasingly
	 *            curved)
	 * @return the linearized curved geometry
	 */
	 ...
	 * Creates a geometry of linearized Cubic Bezier Curves defined by the segments
	 * of the input and a parameter controlling how curved the result should be,
	 * with a skew factor affecting the curve shape at each vertex.
	 *
	 * @param geom
	 *            the geometry defining the curve
	 * @param alpha
	 ...
	 *            curved)
	 * @param skew
	 *            the skew parameter (0 is none, positive skews towards longer side,
	 *            negative towards shorter
	 * @return the linearized curved geometry
	 */
	...
	/**
	 * Gets the interpolation parameters for a Bezier curve approximated by a given
	 * number of vertices.
	 *
	 * @param n
	 *            number of vertices
	...
	/**
	 * Creates control points for each vertex of curve. The control points are
	 * collinear with each vertex, thus providing C1-continuity. By default the
	 * control vectors are the same length, which provides C2-continuity (same
	 * curvature on each side of vertex. The alpha parameter controls the length of
	 * the control vectors. Alpha = 0 makes the vectors zero-length, and hence
	 * flattens the curves. Alpha = 1 makes the curve at right angles roughly
	 * circular. Alpha > 1 starts to distort the curve and may introduce
	 * self-intersections.
	 *
	 * <p>
	 * The control point array contains a pair of coordinates for each input
	 * segment.
	 *
	 * @param coords
	 * @param isRing
	 * @param alpha
	 *            determines the curviness
	...
	/**
	 * Gets a linear geometry containing the generated control points for the Bezier
	 * curve defined by the segments of the input and a parameter controlling how
	 * curved the result should be, with a skew factor affecting the curve shape at
	 * each vertex.
	 *
	 * @param geom
	 *            the geometry defining the curve
	...
	/**
	 * Sets the end control points for a line. Produce a symmetric curve for the
	 * first and last segments by using mirrored control points for start and end
	 * vertex.
	 *
	 * @param coords
	 * @param ctrl
	...
	/**
	 * Creates a new instance producing a Bezier curve defined by a geometry, and a
	 ...
	 *
	 * @param geom
	 *            geometry defining curve
	 * @param controlPoints
	 *            the geometry containing the control points
	 */
	...
	/**
	 * Creates a new instance producing a Bezier curve defined by a geometry and an
	 * alpha curvedness value.
	 *
	 * @param geom
	 *            geometry defining curve
	 * @param alpha
	 *            curvedness parameter (0 = linear, 1 = round, 2 = distorted)
	 */
	...
	/**
	 * Creates a new instance producing a Bezier curve defined by a geometry, an
	 * alpha curvedness value, and a skew factor.
	 *
	 * @param geom
	 *            geometry defining curve
	 * @param alpha
	 ...
	 * @param curve
	 *            array to hold generated points
	 */
	private void cubicBezier(final Coordinate p0, final Coordinate p1, final Coordinate ctrl1, final Coordinate ctrl2,
			double[][] param, Coordinate[] curve) {
