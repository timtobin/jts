...
/**
 * Creates a curved geometry by replacing the segments of the input with Cubic Bezier Curves. The
 * Bezier control points are determined from the segments of the geometry and the alpha control
 * parameter controlling curvedness, and the optional skew parameter controlling the shape of the
 * curve at vertices. The Bezier Curves are created to be C2-continuous (smooth) at each input
 * vertex.
 *
 * <p>Alternatively, the Bezier control points can be supplied explicitly.
 *
 * <p>The result is not guaranteed to be valid, since large alpha values may cause
 * self-intersections.
 */
public class CubicBezierCurve {

  /**
   * Creates a geometry of linearized Cubic Bezier Curves defined by the segments of the input and a
   * parameter controlling how curved the result should be.
   *
  ...
  /**
   * Creates a geometry of linearized Cubic Bezier Curves defined by the segments of the input and a
   * parameter controlling how curved the result should be, with a skew factor affecting the curve
   * shape at each vertex.
   *
   * @param geom the geometry defining the curve
   * @param alpha curvedness parameter (0 is linear, 1 is round, >1 is increasingly curved)
   * @param skew the skew parameter (0 is none, positive skews towards longer side, negative towards
   *     shorter
   * @return the linearized curved geometry
   */
  ...
  /**
   * Gets a linear geometry containing the generated control points for the Bezier curve defined by
   * the segments of the input and a parameter controlling how curved the result should be, with a
   * skew factor affecting the curve shape at each vertex.
   *
   * @param geom the geometry defining the curve
   * @param alpha curvedness parameter (0 is linear, 1 is round, >1 is increasingly curved)
  ...
  /**
   * Creates a geometry of linearized Cubic Bezier Curves defined by the segments of the input and a
   * list (or lists) of control points.
   *
   * <p>Typically the control point geometry is a {@link LineString} or {@link MultiLineString}
   * containing an element for each line or ring in the input geometry. The list of control points
   * for each linear element must contain two vertices for each segment (and thus <code>2 * npts - 2
   * </code>).
   *
   * @param geom the geometry defining the curve
  ...
  /**
   * Creates a new instance producing a Bezier curve defined by a geometry and an alpha curvedness
   * value.
   *
  ...
  /**
   * Creates a new instance producing a Bezier curve defined by a geometry, an alpha curvedness
   * value, and a skew factor.
   *
   * @param geom geometry defining curve
   * @param alpha curvedness parameter (0 is linear, 1 is round, >1 is increasingly curved)
   * @param skew the skew parameter (0 is none, positive skews towards longer side, negative towards
  ...
  /**
   * Creates a new instance producing a Bezier curve defined by a geometry, and a list (or lists) of
   * control points.
   *
   * <p>Typically the control point geometry is a {@link LineString} or {@link MultiLineString}
   * containing an element for each line or ring in the input geometry. The list of control points
  ...
  private LineString bezierLine(LineString ls) {
    ...
    if (ls.getNumPoints() <= 2) return (LineString) ls.copy();
  ...
  private CoordinateList bezierCurve(Coordinate[] coords, boolean isRing) {
    ...
    // -- can't curve a single segment
    if (coords.length <= 2) return curvePts;
  ...
  private void addCurve(
    ...
    if (len < minSegmentLength) {
      ...
      cubicBezier(p0, p1, ctrl0, crtl1, interpolationParam, bezierCurvePts);
  ...
  /**
   * Creates control points for each vertex of curve. The control points are collinear with each
   * vertex, thus providing C1-continuity. By default the control vectors are the same length, which
   * provides C2-continuity (same curvature on each side of vertex. The alpha parameter controls the
   * length of the control vectors. Alpha = 0 makes the vectors zero-length, and hence flattens the
   * curves. Alpha = 1 makes the curve at right angles roughly circular. Alpha > 1 starts to distort
   * the curve and may introduce self-intersections.
   *
   * <p>The control point array contains a pair of coordinates for each input segment.
   *
   * @param coords
   * @param isRing
   ...
   */
  private static Coordinate[] controlPoints(
      Coordinate[] coords, boolean isRing, double alpha, double skew) {
    int N = coords.length;
    int start = 1;
    int end = N - 1;
    if (isRing) {
  ...
  /**
   * Sets the end control points for a line. Produce a symmetric curve for the first and last
   * segments by using mirrored control points for start and end vertex.
   *
   * @param coords
   * @param ctrl
   */
   ...
   *
   * <p>Produces overly flat results, so not used currently.
   *
   * @param c
   * @param p1
   * @param p0
  ...
  private static Coordinate mirrorControlPoint(Coordinate c, Coordinate p0, Coordinate p1) {
    ...
    return reflectPointInLine(
        c, new Coordinate(midx, midy), new Coordinate(midx + vrotx, midy + vroty));
  }
  ...
  /**
   * Gets the interpolation parameters for a Bezier curve approximated by a given number of
   * vertices.
   *
