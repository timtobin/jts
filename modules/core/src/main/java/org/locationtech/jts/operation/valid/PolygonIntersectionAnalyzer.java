...
/**
 * Finds and analyzes intersections in and between polygons, to determine if
 * they are valid.
 *
 * <p>
 * The {@link SegmentString}s which are analyzed can have {@link PolygonRing}s
 * attached. If so they will be updated with intersection information to support
 * further validity analysis which must be done after basic intersection
...
class PolygonIntersectionAnalyzer implements SegmentIntersector {
	...
	/**
	 * Tests if two segments in a closed {@link SegmentString} are adjacent. This
	 * handles determining adjacency across the start/end of the ring.
	 *
	 * @param ringSS
	 *            the segment string
	 * @param segIndex0
	...
	private static boolean isAdjacentInRing(SegmentString ringSS, int segIndex0, int segIndex1) {
		...
		if (delta <= 1)
			return true;
		/**
		 * A string with N vertices has maximum segment index of N-2. If the delta is at
	...
	/**
	 * For a segment string for a ring, gets the coordinate previous to the given
	 * index (wrapping if the index is 0)
	 *
	 * @param ringSS
	 *            the ring segment string
	 * @param segIndex
	 *            the segment index
	 ...
	 *            true if inverted rings are valid.
	 */
	...
	private int findInvalidIntersection(SegmentString ss0, int segIndex0, SegmentString ss1, int segIndex1) {
		...
		/**
		 * Check for an intersection in the interior of both segments. Collinear
		 * intersections by definition contain an interior intersection.
		 */
		...
		/**
		 * Now know there is exactly one intersection, at a vertex of at least one
		 * segment.
		 */
		 ...
		 * (since they are not collinear). This is valid.
		 */
		...
		/**
		 * Under OGC semantics, rings cannot self-intersect. So the intersection is
		 * invalid.
		 *
		 * <p>
		 * The return of RING_SELF_INTERSECTION is to match the previous IsValid
		 * semantics.
		 */
		...
		/**
		 * Optimization: don't analyze intPts at the endpoint of a segment. This is
		 * because they are also start points, so don't need to be evaluated twice. This
		 * simplifies following logic, by removing the segment endpoint case.
		 */
		...
		/**
		 * If the rings are in the same polygon then record the touch to support
		 * connected interior checking.
		 *
		 * <p>
		 * Also check for an invalid double-touch situation, if the rings are different.
		 */
		boolean isDoubleTouch = addDoubleTouch(ss0, ss1, intPt);
		if (isDoubleTouch && !isSameSegString) {
			hasDoubleTouch = true;
			doubleTouchLocation = intPt;
			// TODO: for poly-hole or hole-hole touch, check if it has bad topology. If so
			// return invalid
			// code
		}
	...
	public void processIntersections(SegmentString ss0, int segIndex0, SegmentString ss1, int segIndex1) {
		...
		/**
		 * Ensure that invalidCode is only set once, since the short-circuiting in
		 * {@link SegmentIntersector} is not guaranteed to happen immediately.
		 */
