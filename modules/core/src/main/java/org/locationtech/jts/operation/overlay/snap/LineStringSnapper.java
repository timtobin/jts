...
public class LineStringSnapper {
	 ...
	 * @return the index of the snapped segment or -1 if no segment snaps to the
	 *         snap point
	 */
	 ...
	 *
	 * <p>
	 * For each distinct snap vertex, at most one source segment is snapped to. This
	 * prevents "cracking" multiple segments at the same point, which would likely
	 * cause topology collapse when being used on polygonal linework.
	 *
	 * @param srcCoords
	...
	private void snapSegments(CoordinateList srcCoords, Coordinate[] snapPts) {
		...
		for (int i = 0; i < distinctPtCount; i++) {
			...
			/**
			 * If a segment to snap to was found, "crack" it at the snap pt. The new pt is
			 * inserted immediately into the src segment list, so that subsequent snapping
			 * will take place on the modified segments. Duplicate points are not added.
			 */
			if (index >= 0) {
	...
	/**
	 * Snaps the vertices and segments of the source LineString to the given set of
	 * snap vertices.
	 *
	 * @param snapPts
