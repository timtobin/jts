 ...
 * distance tolerance is used to control where snapping is performed.
 *
 * <p>The implementation handles empty geometry and empty snap vertex sets.
 *
 * @author Martin Davis
 * @version 1.7
 */
public class LineStringSnapper {
  ...
  /**
   * Creates a new snapper using the points in the given {@link LineString} as source snap points.
   *
  ...
  private void snapVertices(CoordinateList srcCoords, Coordinate[] snapPts) {
    ...
    for (int i = 0; i < end; i++) {
      ...
      if (snapVert != null) {
        ...
        if (i == 0 && isClosed) srcCoords.set(srcCoords.size() - 1, new Coordinate(snapVert));
      }
    }
  }
  ...
  private Coordinate findSnapForVertex(Coordinate pt, Coordinate[] snapPts) {
    for (Coordinate snapPt : snapPts) {
      ...
      if (pt.equals2D(snapPt)) return null;
      if (pt.distance(snapPt) < snapTolerance) return snapPt;
    }
    return null;
  }
   ...
   * vertex. A single input segment may be snapped several times to different snap vertices.
   *
   * <p>For each distinct snap vertex, at most one source segment is snapped to. This prevents
   * "cracking" multiple segments at the same point, which would likely cause topology collapse when
   * being used on polygonal linework.
   *
   ...
   *
   * <p>Only a single segment is selected for snapping. This prevents multiple segments snapping to
   * the same snap vertex, which would almost certainly cause invalid geometry to be created. (The
   * heuristic approach to snapping used here is really only appropriate when snap pts snap to a
   * unique spot on the src geometry.)
   *
   * <p>Also, if the snap vertex occurs as a vertex in the src coordinate list, no snapping is
   * performed.
   *
   * @param snapPt the point to snap to
   * @param srcCoords the source segment coordinates
   * @return the index of the snapped segment or -1 if no segment snaps to the snap point
   */
  private int findSegmentIndexToSnap(Coordinate snapPt, CoordinateList srcCoords) {
    ...
    for (int i = 0; i < srcCoords.size() - 1; i++) {
       ...
       *
       * <p>If the snap pt is already in the src list, don't snap at all.
       */
      if (p0.equals2D(snapPt) || p1.equals2D(snapPt)) {
        if (allowSnappingToSourceVertices) continue;
        else return -1;
      }
