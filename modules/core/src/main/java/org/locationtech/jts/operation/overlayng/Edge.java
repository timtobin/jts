 ...
 *   <li>Due to topology collapse caused by snapping or rounding of polygonal geometries.
 * </ul>
 *
...
class Edge {
   ...
   *   <li>If input is an Area and the edge is on the boundary (which may include some collapses),
   *       edge is marked as an AREA edge and side locations are assigned
   *   <li>If input is an Area and the edge is collapsed (depth delta = 0), the label is set to
   *       COLLAPSE. The location will be determined later by evaluating the final graph topology.
   *   <li>If input is a Line edge is set to a LINE edge. For line edges the line location is not
  ...
  private static void initLabel(
      OverlayLabel lbl, int geomIndex, int dim, int depthDelta, boolean isHole) {
