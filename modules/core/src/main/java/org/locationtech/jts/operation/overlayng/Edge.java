 ...
 * edges which have the same linework (although possibly different
 * orientations). In this case the topology information is derived from the
 * merging of the information in the source edges. Merged edges can occur in the
 * following situations
 *
 * <ul>
 * <li>Due to coincident edges of polygonal or linear geometries.
 * <li>Due to topology collapse caused by snapping or rounding of polygonal
 * geometries.
 * </ul>
 *
...
class Edge {
	 ...
	 * <li>If input is an Area and the edge is on the boundary (which may include
	 * some collapses), edge is marked as an AREA edge and side locations are
	 * assigned
	 * <li>If input is an Area and the edge is collapsed (depth delta = 0), the
	 * label is set to COLLAPSE. The location will be determined later by evaluating
	 * the final graph topology.
	 * <li>If input is a Line edge is set to a LINE edge. For line edges the line
	 * location is not significant (since there is no parent area for which to
	 * determine location).
	 * </ul>
	 *
	 * @param lbl
	...
	/**
	 * Tests if the given point sequence is a collapsed line. A collapsed edge has
	...
	/**
	 * Tests whether the edge is part of a shell in the given geometry. This is only
	 * the case if the edge is a boundary.
	 *
	 * @param geomIndex
	...
	public void merge(Edge edge) {
		/**
		 * Marks this as a shell edge if any contributing edge is a shell. Update hole
		 * status first, since it depends on edge dim
		 */
		...
		/*
		 * if (aDepthDelta > 1) { Debug.println(this); }
		 */
	}

	/**
	 * Compares two coincident edges to determine whether they have the same or
	 * opposite direction.
	 *
	 * @param edge1
	 *            an edge
