...
public class GeometryStrategies {
   ...
   *
   * <p>The implementation of this class are intended to be used as static function points in C.
   * These strategies should be associated with an element when the element begins. The strategy is
   * utilized at the end of the element to create an object of value to the user.
   *
   * <p>In this case all the objects are either java.lang.* or JTS Geometry objects
   *
   * @author David Zwiers, Vivid Solutions.
   */
  ...
  private static HashMap loadStrategies() {
    ...
    // point
    strats.put(
        GMLConstants.GML_POINT.toLowerCase(),
        (ParseStrategy)
            (arg, gf) -> {
    ...
    // linestring
    strats.put(
        GMLConstants.GML_LINESTRING.toLowerCase(),
        (ParseStrategy)
            (arg, gf) -> {
              // one child, either a coord
              // or a coordinate sequence
    ...
    // linearring
    strats.put(
        GMLConstants.GML_LINEARRING.toLowerCase(),
        (ParseStrategy)
            (arg, gf) -> {
              // one child, either a coord
              // or a coordinate sequence
    ...
    // polygon
    strats.put(
        GMLConstants.GML_POLYGON.toLowerCase(),
        (ParseStrategy)
            (arg, gf) -> {
              // one child, either a coord
              // or a coordinate sequence
              ...
              List t =
                  arg.children.size() > 1 ? arg.children.subList(1, arg.children.size()) : null;
    ...
    // box
    strats.put(
        GMLConstants.GML_BOX.toLowerCase(),
        (ParseStrategy)
            (arg, gf) -> {
              // one child, either a coord
              // or a coordinate sequence
              ...
              if (arg.children.size() < 1 || arg.children.size() > 2)
                throw new SAXException(
                    "Cannot create a box without either two coords or one coordinate sequence");
              ...
              if (arg.children.size() == 1) {
                ...
                box =
                    new Envelope(
                        (Coordinate) arg.children.get(0), (Coordinate) arg.children.get(1));
              }
    ...
    // multi-point
    strats.put(
        GMLConstants.GML_MULTI_POINT.toLowerCase(),
        (ParseStrategy)
            (arg, gf) -> {
              // one child, either a coord
              // or a coordinate sequence
    ...
    // multi-linestring
    strats.put(
        GMLConstants.GML_MULTI_LINESTRING.toLowerCase(),
        (ParseStrategy)
            (arg, gf) -> {
              // one child, either a coord
              // or a coordinate sequence
              ...
              if (arg.children.size() < 1)
                throw new SAXException(
                    "Cannot create a multi-linestring without atleast one linestring");
    ...
    // multi-poly
    strats.put(
        GMLConstants.GML_MULTI_POLYGON.toLowerCase(),
        (ParseStrategy)
            (arg, gf) -> {
              // one child, either a coord
              // or a coordinate sequence
    ...
    // multi-geom
    strats.put(
        GMLConstants.GML_MULTI_GEOMETRY.toLowerCase(),
        (ParseStrategy)
            (arg, gf) -> {
              // one child, either a coord
              // or a coordinate sequence
              ...
              if (arg.children.size() < 1)
                throw new SAXException(
                    "Cannot create a multi-polygon without atleast one geometry");
    ...
    // coordinates
    strats.put(
        GMLConstants.GML_COORDINATES.toLowerCase(),
        new ParseStrategy() {
          ...
          public Object parse(Handler arg, GeometryFactory gf) throws SAXException {
            ...
            if (arg.attrs.getIndex("decimal") >= 0) decimal = arg.attrs.getValue("decimal");
            ...
            if (arg.attrs.getIndex("cs") >= 0) coordSeperator = arg.attrs.getValue("cs");
            ...
            if (arg.attrs.getIndex("ts") >= 0) toupleSeperator = arg.attrs.getValue("ts");
            ...
            t = t.replaceAll("\\s", " ");
            /**
             * Remove spaces after commas, for when they are used as separators (default). This
             * prevents coordinates being split by the tuple separator
             */
            t = t.replaceAll("\\s*,\\s*", ",");
            ...
            if (touples.length == 0)
              throw new SAXException(
                  "Cannot create a coordinate sequence without a touple to parse");
            ...
            for (int i = numNonNullTouples; i < touples.length; i++) touples[i] = null;

            // null touples now at end of array
            if (numNonNullTouples == 0)
              throw new SAXException(
                  "Cannot create a coordinate sequence without a non-null touple to parse");

            int dim = StringUtil.split(touples[0], coordSeperator).length;
            CoordinateSequence cs =
                gf.getCoordinateSequenceFactory().create(numNonNullTouples, dim);
            ...
            for (int i = 0; i < numNonNullTouples; i++) {
              ...
              for (int j = 0; j < coords.length && j < dim; j++) {
                if (coords[j] != null && !coords[j].trim().isEmpty()) {
                  double ordinate =
                      Double.parseDouble(
                          replaceDec ? coords[j].replaceAll(decimal, ".") : coords[j]);
    ...
    // coord
    strats.put(
        GMLConstants.GML_COORD.toLowerCase(),
        (ParseStrategy)
            (arg, gf) -> {
              // one child, either a coord
              // or a coordinate sequence
              ...
              if (axis.length > 2) c.setZ(axis[2].doubleValue());

              return c;
            });

    ParseStrategy coord_child =
        (arg, gf) -> {
