...
/**
 * Writes {@link Geometry}s as XML fragments in GML2 format. Allows specifying the XML prefix,
 * namespace and srsName of the emitted GML. Also allows adding custom root elements to support GML
 * extensions such as KML. With appropriate settings for prefix (none) and custom root elements this
 * class can be used to write out geometry in KML format.
 *
 * <p>An example of the output that can be generated is:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>This class does not rely on any external XML libraries.
 *
...
public class GMLWriter {
  ...
  /**
   * Specifies the namespace prefix to write on each GML tag. A null or blank prefix may be used to
   * indicate no prefix.
   *
   * <p>The default is to write <tt>gml:</tt> as the namespace prefix.
   *
  ...
  /**
   * Sets the value of the <tt>srsName</tt> attribute to be written into the root geometry tag. If
   * the value is <tt>null</tt> or blank no srsName attribute will be written. The provided value
   * must be a valid XML attribute value - it will not be XML-escaped.
   *
   * <p>The default is not to write the <tt>srsName</tt> attribute.
   *
   ...
   * geometries. Useful in XML-aware environments which parse the geometries before use, such as
   * XSLT.
   *
   * @param emitNamespace true if the GML namespace prefix declaration should be written in the root
   *     geometry element
   */
   ...
   * element. The text contained in the string sequence should form valid XML markup. The specified
   * strings are written one per line immediately after the root geometry tag line.
   *
   * <p>For instance, this is useful for adding KML-specific geometry parameters such as
   * <tt>&lt;extrude&gt;</tt>
   *
  ...
  // <gml:LineString><gml:coordinates>1195123.37289257,381985.763974674
  // 1195120.22369473,381964.660533343
  // 1195118.14929823,381942.597718511</gml:coordinates></gml:LineString>
  ...
  // <gml:LinearRing><gml:coordinates>1226890.26761027,1466433.47430292
  // 1226880.59239079,1466427.03208053...></coordinates></gml:LinearRing>
  ...
  private void writeMultiLineString(MultiLineString mls, Writer writer, int level)
      throws IOException {
    startLine(level, writer);
  ...
  private void writeGeometryCollection(GeometryCollection gc, Writer writer, int level)
      throws IOException {
    startLine(level, writer);
  ...
  private void startGeomTag(String geometryName, Geometry g, Writer writer) throws IOException {
    writer.write("<" + ((prefix == null || prefix.isEmpty()) ? "" : prefix + ":"));
