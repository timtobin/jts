...
/**
 * Writes {@link Geometry}s as XML fragments in GML2 format. Allows specifying
 * the XML prefix, namespace and srsName of the emitted GML. Also allows adding
 * custom root elements to support GML extensions such as KML. With appropriate
 * settings for prefix (none) and custom root elements this class can be used to
 * write out geometry in KML format.
 *
 * <p>
...
public class GMLWriter {
	...
	/**
	 * Creates a writer which may emit the GML namespace prefix declaration in the
	 * geometry root element.
	 *
	 * @param emitNamespace
	 *            true if the GML namespace prefix declaration should be written in
	 *            the geometry root element
	 */
	...
	/**
	 * Specifies a list of custom elements which are written after the opening tag
	 * of the root element. The text contained in the string sequence should form
	 * valid XML markup. The specified strings are written one per line immediately
	 * after the root geometry tag line.
	 *
	 * <p>
	 * For instance, this is useful for adding KML-specific geometry parameters such
	 * as <tt>&lt;extrude&gt;</tt>
	 *
	...
	/**
	 * Determines whether a GML namespace declaration will be written in the opening
	 * tag of geometries. Useful in XML-aware environments which parse the
	 * geometries before use, such as XSLT.
	 *
	 * @param emitNamespace
	 *            true if the GML namespace prefix declaration should be written in
	 *            the root geometry element
	 */
	...
	/**
	 * Specifies the namespace prefix to write on each GML tag. A null or blank
	 * prefix may be used to indicate no prefix.
	 *
	 * <p>
	 * The default is to write <tt>gml:</tt> as the namespace prefix.
	 *
	 * @param prefix
	 *            the namespace prefix to use (<tt>null</tt> or blank if none)
	 */
	...
	/**
	 * Sets the value of the <tt>srsName</tt> attribute to be written into the root
	 * geometry tag. If the value is <tt>null</tt> or blank no srsName attribute
	 * will be written. The provided value must be a valid XML attribute value - it
	 * will not be XML-escaped.
	 *
	 * <p>
