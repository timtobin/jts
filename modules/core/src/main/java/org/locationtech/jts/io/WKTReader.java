 ...
 * implementation. In particular, the <code>
 * GeometryFactory</code> determines the <code>PrecisionModel</code> and
 * <code>SRID</code> that is used.
 *
 * <p>
 ...
 * coordinate dimension Z, M or ZM as modifiers (e.g. POINT Z) or in the name of
 * the geometry type (e.g. LINESTRINGZM). If the coordinate dimension is
 ...
 * XY geometry by calling {@link #setIsOldJtsCoordinateSyntaxAllowed(boolean)}.
 *
 * <p>
 * A reader can be set to ensure the input is structurally valid by calling
 * {@link #setFixStructure(boolean)}. This ensures that geometry can be
 * constructed without errors due to missing coordinates. The created geometry
 * may still be topologically invalid.
 *
 * <h3>Notes:</h3>
 *
 * <ul>
 ...
 * <li>The reader uses <tt>Double.parseDouble</tt> to perform the conversion of
 * ASCII numbers to floating point. This means it supports the Java syntax for
...
public class WKTReader {
	...
	/**
	 * Flag indicating that the old notation of MultiPoint coordinates in JTS is
	 * supported.
	 */
	 ...
	 * @param tokenizer
	 *            the tokenizer
	 * @return {@code true} if the next token is a {@link #L_PAREN}, otherwise
	 *         {@code false}
	 * @throws IOException
	 *             if an I/O error occurs
	 */
	...
	/**
	 * Creates a formatted ParseException reporting that the current token was
	 * unexpected.
	 *
	 * @param expected
	...
	private static String tokenString(StreamTokenizer tokenizer) {
		return switch (tokenizer.ttype) {
			...
			case StreamTokenizer.TT_EOF -> "End-of-Stream";
			case StreamTokenizer.TT_WORD -> "'" + tokenizer.sval + "'";
			default -> "'" + (char) tokenizer.ttype + "'";
		};
	}
	...
	/**
	 * Reads a <code>Coordinate</Code> from a stream using the given
	 * {@link StreamTokenizer}.
	 *
	 * <p>
	 * All ordinate values are read, but -depending on the
	 * {@link CoordinateSequenceFactory} of the underlying {@link GeometryFactory}-
	 * not necessarily all can be handled. Those are silently dropped.
	 *
	 * @param tokenizer
	 *            the tokenizer to use
	 * @param ordinateFlags
	 *            a bit-mask defining the ordinates to read.
	 * @param tryParen
	 *            a value indicating if a starting {@link #L_PAREN} should be
	 *            probed.
	 * @return a {@link Coordinate} of appropriate dimension containing the read
	 *         ordinate values
	 * @throws IOException
	 *             if an I/O error occurs
	 * @throws ParseException
	 ...
	 * @param ordinateFlags
	 *            a bit-mask defining the ordinates to read.
	 * @return a {@link CoordinateSequence} of length 1 containing the read ordinate
	 *         values
	 * @throws IOException
	 *             if an I/O error occurs
	 * @throws ParseException
	...
	private CoordinateSequence getCoordinateSequence(StreamTokenizer tokenizer, EnumSet<Ordinate> ordinateFlags,
			int minSize, boolean isRing) throws IOException, ParseException {
	...
	/**
	 * Reads a <code>CoordinateSequence</Code> from a stream using the given
	 * {@link StreamTokenizer} for an old-style JTS MultiPoint (Point coordinates
	 ...
	 *            a bit-mask defining the ordinates to read.
	 * @param tryParen
	 *            a value indicating if a starting {@link #L_PAREN} should be probed
	 *            for each coordinate.
	 * @param isReadEmptyOrOpener
	 *            indicates if an opening paren or EMPTY should be scanned for
	 * @return a {@link CoordinateSequence} of length 1 containing the read ordinate
	 *         values
	 * @throws IOException
	 *             if an I/O error occurs
	 * @throws ParseException
	 *             if an unexpected token was encountered S
	 */
	 ...
	 * <tt>NaN</tt> values are handled correctly, and the case of the "NaN" symbol
	 * is not significant.
	 *
	 * @param tokenizer
	 *            tokenizer over a stream of text in Well-known Text
	...
	private boolean isTypeName(StreamTokenizer tokenizer, String type, String typeName) throws ParseException {
		...
		boolean isValidMod = modifiers.length() <= 2 && (modifiers.isEmpty() || modifiers.equals(WKTConstants.Z)
				|| modifiers.equals(WKTConstants.M) || modifiers.equals(WKTConstants.ZM));
		if (!isValidMod) {
	 ...
	 * @return a <code>GeometryCollection</code> specified by the next token in the
	 *         stream
	 * @throws ParseException
	 *             if the coordinates used to create a <code>Polygon</code> shell
	 *             and holes do not form closed linestrings, or if an unexpected
	...
	private Geometry readGeometryTaggedText(StreamTokenizer tokenizer, String type, EnumSet<Ordinate> ordinateFlags)
		...
		// if we can create a sequence with the required dimension everything is ok,
		// otherwise
		// we need to take a different coordinate sequence factory.
		// It would be good to not have to try/catch this but if the
		...
		try {
			...
			geometryFactory = new GeometryFactory(geometryFactory.getPrecisionModel(), geometryFactory.getSRID(),
					csFactoryXYZM);
		}
	 ...
	 *             if an I/O error occurs
	 * @throws ParseException
	 *             if the coordinates used to create the <code>LinearRing</code> do
	 *             not form a closed linestring, or if an unexpected token was
	 *             encountered
	 */
	private LinearRing readLinearRingText(StreamTokenizer tokenizer, EnumSet<Ordinate> ordinateFlags)
			throws IOException, ParseException {
		return geometryFactory
				.createLinearRing(getCoordinateSequence(tokenizer, ordinateFlags, LinearRing.MINIMUM_VALID_SIZE, true));
	}
	...
	private MultiPoint readMultiPointText(StreamTokenizer tokenizer, EnumSet<Ordinate> ordinateFlags)
		...
		// and parse it if
		// present
		// MD 2009-02-21 - this is only provided for backwards compatibility for a few
		// versions
		if (isAllowOldJtsMultipointSyntax) {
			String nextWord = lookAheadWord(tokenizer);
			if (!nextWord.equals(L_PAREN) && !nextWord.equals(WKTConstants.EMPTY)) {
	 ...
	 *             if an unexpected token was encountered
	 */
	private MultiPolygon readMultiPolygonText(StreamTokenizer tokenizer, EnumSet<Ordinate> ordinateFlags)
			throws IOException, ParseException {
		String nextToken = getNextEmptyOrOpener(tokenizer);
		if (nextToken.equals(WKTConstants.EMPTY)) {
	 ...
	 * @throws ParseException
	 *             if the coordinates used to create the <code>Polygon</code> shell
	 *             and holes do not form closed linestrings, or if an unexpected
	 *             token was encountered.
	 * @throws IOException
	 *             if an I/O error occurs
	 ...
	 * so that the geometry can be constructed without error. This involves adding
	 * coordinates if the input coordinate sequence is shorter than required.
	 *
	 * @param isFixStructure
	 *            true if the input structure should be fixed
	 ...
	 * though no Z or M ordinate indicator is present. The default value is
	 * {@link #ALLOW_OLD_JTS_COORDINATE_SYNTAX}.
	 *
	 * @param value
	 *            a boolean value
	 */
	...
	/**
	 * Sets a flag indicating, that point coordinates in a MultiPoint geometry must
	 * not be enclosed in paren. The default value is
	 * {@link #ALLOW_OLD_JTS_MULTIPOINT_SYNTAX}
	 *
	 * @param value
	 *            a boolean value
	 */
	...
	/**
	 * Computes the required dimension based on the given ordinate values. It is
	 * assumed that {@link Ordinate#X} and {@link Ordinate#Y} are included.
	 *
	 * @param ordinateFlags
	 *            the ordinate bit-mask
	 * @return the number of dimensions required to store ordinates for the given
	 *         bit-mask.
	 */
