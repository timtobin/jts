...
class TopologyComputer {
  ...
  private static final String MSG_GEOMETRY_DIMENSION_UNEXPECTED =
      "Unexpected combination of geometry dimensions";
   ...
   * linework, or may have lines lying in the boundary of an area. This causes the coordinates of
   * nodes created by crossing segments to be computed explicitly. This ensures that node locations
   * match in situations where a self-crossing and mutual crossing occur at the same logical
   * location. The canonical example is a self-crossing line tested against a single segment
   * identical to one of the crossed segments.
   *
   * <p>Currently, requiring self-noding prevents noder caching. So it it important to limit the
   * cases which require self-noding. Currently self-noding is required for:
   *
   * <ul>
   ...
   * </ul>
   *
   * Note that linear B inputs do not require self-noding in all cases. In particular, if A is
   * polygonal then predicates with linear B do not require self-noding.
   *
   * @return true if self-noding is required
   */
  public boolean isSelfNodingRequired() {
    if (!predicate.requireSelfNoding()) return false;

    if (geomA.isSelfNodingRequired()) return true;

    // -- if B is a mixed GC with A and L require full noding
    if (geomB.hasAreaAndLine()) return true;
  ...
  private void updateDim(int locA, int locB, int dimension) {
    // System.out.println(Location.toLocationSymbol(locA) + "/" + Location.toLocationSymbol(locB) +
    // ": " + dimension);
  ...
  public void addPointOnGeometry(boolean isPointA, int locTarget, int dimTarget, Coordinate pt) {
    ...
    if (getGeometry(!isPointA).isEmpty()) return;
   ...
   * @param dimTarget the dimension of the interacting target geometry element, (if any), or the
   *     dimension of the target
   * @param pt the line end coordinate
   */
  public void addLineEndOnGeometry(
      boolean isLineA, int locLineEnd, int locTarget, int dimTarget, Coordinate pt) {
    // -- record topology at line end point
    updateDim(isLineA, locLineEnd, locTarget, Dimension.P);

    // -- an empty geometry has no points to infer entries from
    if (getGeometry(!isLineA).isEmpty()) return;
   ...
   * provided by {@link RelatePointLocator}.
   *
   * <p>Note that in a GeometryCollection containing overlapping or adjacent polygons, the area
   * vertex location may be INTERIOR instead of BOUNDARY.
   *
  ...
  public void addAreaVertex(
      boolean isAreaA, int locArea, int locTarget, int dimTarget, Coordinate pt) {
  ...
  private void evaluateNode(NodeSections nodeSections) {
    ...
    boolean isAreaInteriorA =
        geomA.isNodeInArea(p, nodeSections.getPolygonal(RelateGeometry.GEOM_A));
    boolean isAreaInteriorB =
        geomB.isNodeInArea(p, nodeSections.getPolygonal(RelateGeometry.GEOM_B));
