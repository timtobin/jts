...
class TopologyComputer {
	...
	public void addAreaVertex(boolean isAreaA, int locArea, int locTarget, int dimTarget, Coordinate pt) {
		if (locTarget == Location.EXTERIOR) {
			 ...
			 * polygonal geometries. For GCs, the vertex may be either on boundary or in
			 * interior (i.e. of overlapping or adjacent polygons)
			 */
	...
	private void addAreaVertexOnLine(boolean isAreaA, int locArea, int locTarget, Coordinate pt) {
		...
		/**
		 * If an area vertex intersects a line, all we know is the intersection at that
		 * point. e.g. the line may or may not be collinear with the area boundary, and
		 * the line may or may not intersect the area interior. Full topology is
	...
	/**
	 * Updates topology for an area vertex (in Interior or on Boundary) intersecting
	 * a point. Note that because the largest dimension of intersecting target is
	 * determined, the intersecting point is not part of any other target geometry,
	...
	private void addLineEndOnArea(boolean isLineA, int locLineEnd, int locArea, Coordinate pt) {
		if (locArea != Location.BOUNDARY) {
			/**
			 * When a line end is in an Area INTERIOR or EXTERIOR some length of the source
			 * Line Interior AND the Exterior of the line is also in that location of the
			 * target. NOTE: this assumes the line end is NOT also in an Area of a mixed-dim
			 * GC
			 */
	 ...
	 * @param dimTarget
	 *            the dimension of the interacting target geometry element, (if
	 *            any), or the dimension of the target
	 * @param pt
	...
	private void addLineEndOnLine(boolean isLineA, int locLineEnd, int locLine, Coordinate pt) {
		/**
		 * When a line end is in the EXTERIOR of a Line, some length of the source Line
		 * INTERIOR is also in the target Line EXTERIOR. This works for zero-length
		 * lines as well.
		 */
	...
	public void addPointOnGeometry(boolean isPointA, int locTarget, int dimTarget, Coordinate pt) {
		...
		switch (dimTarget) {
			...
			case Dimension.L :
				/**
				 * Because zero-length lines are handled, a point lying in the exterior of the
				 * line target may imply either P or L for the Exterior interaction
				 */
	...
	private void evaluateNodeEdges(RelateNode node) {
		...
		for (RelateEdge e : node.getEdges()) {
			...
			updateDim(e.location(RelateGeometry.GEOM_A, Position.ON), e.location(RelateGeometry.GEOM_B, Position.ON),
					Dimension.L);
		}
	}
	 ...
	 * geometries which may have self-crossing linework, or may have lines lying in
	 * the boundary of an area. This causes the coordinates of nodes created by
	 * crossing segments to be computed explicitly. This ensures that node locations
	 * match in situations where a self-crossing and mutual crossing occur at the
	 * same logical location. The canonical example is a self-crossing line tested
	 * against a single segment identical to one of the crossed segments.
	 *
	 * <p>
	 * Currently, requiring self-noding prevents noder caching. So it it important
	 * to limit the cases which require self-noding. Currently self-noding is
	 * required for:
	 *
	 * <ul>
	 * <li>A geoms which require self-noding (lines or GCs, except for
	 * single-polygon GCs)
	 * <li>B geoms which are mixed A/L GCs
	 * </ul>
	 *
	 * Note that linear B inputs do not require self-noding in all cases. In
	 * particular, if A is polygonal then predicates with linear B do not require
	 * self-noding.
	 *
	...
	/**
	 * Updates topology for an AB Area-Area crossing node. Sections cross at a node
	 * if (a) the intersection is proper (i.e. in the interior of two segments) or
	 * (b) if non-proper then whether the linework crosses is determined by the
	 * geometry of the segments on either side of the node. In these situations the
	 * area geometry interiors intersect (in dimension 2).
	 *
	 * @param a
	...
	private void updateAreaAreaCross(NodeSection a, NodeSection b) {
		...
		if (isProper || PolygonNodeTopology.isCrossing(a.nodePt(), a.getVertex(0), a.getVertex(1), b.getVertex(0),
				b.getVertex(1))) {
	...
	private void updateDim(int locA, int locB, int dimension) {
		// System.out.println(Location.toLocationSymbol(locA) + "/" +
