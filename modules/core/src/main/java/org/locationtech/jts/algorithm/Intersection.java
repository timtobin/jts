 ...
 *
 * <p>In general it is not possible to compute the intersection point of two lines exactly, due to
 * numerical roundoff. This is particularly true when the lines are nearly parallel. These routines
 * uses numerical conditioning on the input values to ensure that the computed value is very close
 * to the correct value.
 *
 * <p>The Z-ordinate is ignored, and not populated.
 *
 * @author Martin Davis
 */
public class Intersection {
   ...
   * @return the intersection point between the lines, if there is one, or null if the lines are
  ...
  /**
   * Compute intersection of two lines, using a floating-point algorithm. This is less accurate than
   * {@link CGAlgorithmsDD#intersection(Coordinate, Coordinate, Coordinate, Coordinate)}. It has
   * caused spatial predicate failures in some cases. This is kept for testing purposes.
   *
   * @param p1 an endpoint of line 1
   * @param p2 an endpoint of line 1
   * @param q1 an endpoint of line 2
   ...
   *     parallel or collinear
   */
  private static Coordinate intersectionFP(
      Coordinate p1, Coordinate p2, Coordinate q1, Coordinate q2) {
  ...
  /**
   * Computes the intersection point of a line and a line segment (if any). There will be no
   ...
   */
  public static Coordinate lineSegment(
      Coordinate line1, Coordinate line2, Coordinate seg1, Coordinate seg2) {
    ...
    /**
     * The segment intersects the line. The full line-line intersection is used to compute the
     * intersection point.
     */
    ...
    /**
     * Due to robustness failure it is possible the intersection computation will return null. In
     * this case choose the closest point
     */
    ...
    if (dist1 < dist2) return seg1.copy();
    return seg2;
  }
}
