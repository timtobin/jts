 ...
 *
 * <p>
 * An analogy for the Fréchet distance taken from <a href=
 * "http://www.kr.tuwien.ac.at/staff/eiter/et-archive/cdtr9464.pdf">Computing
 * Discrete Fréchet Distance</a>
 *
 * <pre>
 ...
 *
 * <p>
 * Its metric is better than the Hausdorff distance because it takes the
 * directions of the curves into account. It is possible that two curves have a
 * small Hausdorff but a large Fréchet distance.
 *
 * <p>
 * This implementation is base on the following optimized Fréchet distance
 * algorithm:
 *
 * <pre>
 ...
 *
 * @see <a href="https://en.wikipedia.org/wiki/Fr%C3%A9chet_distance">Fréchet
 *      distance</a>
 * @see <a href=
 *      "http://www.kr.tuwien.ac.at/staff/eiter/et-archive/cdtr9464.pdf">Computing
 *      Discrete Fréchet Distance</a>
 * @see <a href=
 *      "https://hal.archives-ouvertes.fr/hal-02110055/document">Distance
 *      discrète de Fréchet optimisée</a>
 * @see <a href=
...
public class DiscreteFrechetDistance {
	 ...
	 * using the <a
	 * href=https://en.wikipedia.org/wiki/Bresenham%27s_line_algorithm>Bresenham
	 * line algorithm</a>.
	 *
	 * @param numCols
	 ...
	 *
	 * @param coords0
	 *            an array of {@code Coordinate}s.
	 * @param coords1
	 *            an array of {@code Coordinate}s.
	 * @param diagonal
	 *            an array of alternating col/row index values for the diagonal of
	 *            the distance matrix
	 * @param distances
	 *            the distance matrix
	 * @param distanceToPair
	 *            a lookup for coordinate pairs based on a distance
	 */
	...
	/**
	 * Computes the Discrete Fréchet Distance between two {@link Geometry}s using a
	 * {@code Cartesian} distance computation function.
	 *
	 * @param g0
	 *            the 1st geometry
	 * @param g1
	...
	/**
	 * A matrix implementation that adheres to the <a href=
	...
	static final class CsrMatrix extends MatrixStorage {
		...
		/**
		 * Ensures that the column index vector (ci) and value vector (v) are
		 * sufficiently large.
		 *
		 * @param required
		 *            the number of items to store in the matrix
		 */
	...
	abstract static class MatrixStorage {
		...
		/**
		 * Gets a flag indicating if the matrix has a set value, e.g. one that is
		 * different than {@link MatrixStorage#defaultValue}.
		 *
		 * @param i
		 *            the row index
		 * @param j
		 ...
		 *            the column index
		 * @param value
		 *            The matrix value to set at i, j
		 */
		 ...
		 * StringBuilder("["); for (int i = 0; i < this.numRows; i++) { sb.append('[');
		 * for(int j = 0; j < this.numCols; j++) { if (j > 0) sb.append(", ");
		 * sb.append(String.format(java.util.Locale.ROOT, "%8.4f", get(i, j))); }
		 * sb.append(']'); if (i < this.numRows - 1) sb.append(",\n"); } sb.append(']');
		 * return sb.toString(); }
		 */
	}
	...
	static final class RectMatrix extends MatrixStorage {
		...
		/**
		 * Creates an instance of this matrix using the given number of rows and
		 * columns. A default value can be specified
		 *
		 * @param numRows
		 *            the number of rows
		 * @param numCols
