...
/**
 * A semi-static spatial index for points which occur in a spatially-coherent
 * sequence. In particular, this is suitable for indexing the vertices of a
 * {@link LineString} or {@link Polygon} ring.
 *
 * <p>
 * The index is constructed in a batch fashion on a given sequence of
 * coordinates. Coordinates can be removed via the {@link #remove(int)} method.
 *
 * <p>
 * Note that this index queries only the individual points of the input
 * coordinate sequence, <b>not</b> any line segments which might be lie between
 * them.
 *
 * <p>
 * The input coordinate array is read-only, and is not changed when vertices are
 * removed.
 *
 * @author Martin Davis
 */
public class VertexSequencePackedRtree {
	...
	/**
	 * Number of items/nodes in a parent node. Determined empirically. Performance
	 * is not too sensitive to this.
	 */
	...
	/**
	 * Creates a new tree over the given sequence of coordinates. The sequence
	 * should be spatially coherent to provide query performance.
	 *
	 * @param pts
	...
	/**
	 * Computes the level offsets. This is the position in the <tt>bounds</tt> array
	 * of each level.
	 *
	 * <p>
	 * The levelOffsets array includes a sentinel value of offset[0] = 0. The top
	 * level is always of size 1, and so also indicates the total number of bounds.
	 *
	...
	/**
	 * Queries the index to find all items which intersect an extent. The query
	 * result is a list of the indices of input coordinates which intersect the
	 * extent.
	 *
	 * @param queryEnv
	 *            the query extent
	...
	private void queryItemRange(Envelope queryEnv, int itemIndex, IntArrayList resultList) {
		for (int i = 0; i < nodeCapacity; i++) {
			...
			if (!isRemoved[index] && queryEnv.contains(p))
	...
	/**
	 * Removes the input item at the given index from the spatial index. This does
	 * not change the underlying coordinate array.
	 *
	 * @param index
