...
/**
 * Models a polygon lying in a plane in 3-dimensional Cartesian space. The
 * polygon representation is supplied by a {@link Polygon}, containing
 * coordinates with XYZ ordinates. 3D polygons are assumed to lie in a single
 * plane. The plane best fitting the polygon coordinates is computed and is
 * represented by a {@link Plane3D}.
 *
 * @author mdavis
 */
public class PlanarPolygon3D {
	...
	/**
	 * Computes an average normal vector from a list of polygon coordinates. Uses
	 * Newell's method, which is based on the fact that the vector with components
	 * equal to the areas of the projection of the polygon onto the Cartesian axis
	 * planes is normal.
	 *
	 * @param seq
	 *            the sequence of coordinates for the polygon
	...
	/**
	 * Computes a point which is the average of all coordinates in a sequence. If
	 * the sequence lies in a single plane, the computed point also lies in the
	 * plane.
	 *
	 * @param seq
	...
	/**
	 * Finds a best-fit plane for the polygon, by sampling a few points from the
	 * exterior ring.
	 *
	 * <p>
	 * The algorithm used is Newell's algorithm: - a base point for the plane is
	 * determined from the average of all vertices - the normal vector is determined
	 * by computing the area of the projections on each of the axis planes
	 *
	 * @param poly
	 *            the polygon to determine the plane for
