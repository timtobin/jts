...
/**
 * Computes the {@link LinearLocation} for a given length along a linear
 * {@link Geometry}. Negative lengths are measured in reverse from end of the
 * linear geometry. Out-of-range values are clamped.
 *
 * <p>
 * <b>Note:</b><br>
 * This class is intended for internal use only, and it might be made
 * package-private in a future version of this library
 */
public class LengthLocationMap {
	...
	/**
	 * Computes the length for a given {@link LinearLocation} on a linear
	 * {@link Geometry}.
	 *
	 * @param linearGeom
	 *            the linear geometry to use
	 * @param loc
	 *            the {@link LinearLocation} index of the location
	...
	/**
	 * Computes the {@link LinearLocation} for a given length along a linear
	 * {@link Geometry}, with control over how the location is resolved at component
	 * endpoints.
	 *
	 * @param linearGeom
	 *            the linear geometry to use
	...
	/**
	 * Compute the {@link LinearLocation} corresponding to a length. Negative
	 * lengths are measured in reverse from end of the linear geometry. Out-of-range
	 * values are clamped. Ambiguous indexes are resolved to the lowest possible
	 * location value.
	 *
	 * @param length
	 *            the length index
	 ...
	 * Compute the {@link LinearLocation} corresponding to a length. Negative
	 * lengths are measured in reverse from end of the linear geometry. Out-of-range
	 * values are clamped. Ambiguous indexes are resolved to the lowest or highest
	 * possible location value, depending on the value of <tt>resolveLower</tt>
	 *
	 * @param length
	 *            the length index
	 * @return the corresponding LinearLocation
	...
	private LinearLocation getLocationForward(double length) {
		...
		while (it.hasNext()) {
			...
			/*
			 * Special handling is required for the situation when the length references
			 * exactly to a component endpoint. In this case, the endpoint location of the
			 * current component is returned, rather than the startpoint location of the
			 * next component. This produces consistent behaviour with the project method.
			 */
			if (it.isEndOfLine()) {
