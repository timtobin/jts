 ...
 * a given <code>Geometry</code>. This class supports labels for relationships to two <code>Geometry
 * </code>s, which is sufficient for algorithms for binary operations.
 *
 * <p>Topology graphs support the concept of labeling nodes and edges in the graph. The label of a
 * node or edge specifies its topological relationship to one or more geometries. (In fact, since
 * JTS operations have only two arguments labels are required for only two geometries). A label for
 * a node or edge has one or two elements, depending on whether the node or edge occurs in one or
 * both of the input <code>Geometry</code>s. Elements contain attributes which categorize the
 * topological location of the node or edge relative to the parent <code>Geometry</code>; that is,
 * whether the node or edge is in the interior, boundary or exterior of the <code>Geometry</code>.
 * Attributes have a value from the set <code>{Interior, Boundary, Exterior}</code>. In a node each
 * element has a single attribute <code>&lt;On&gt;</code>. For an edge each element has a triplet of
 * attributes <code>&lt;Left, On, Right&gt;</code>.
 *
