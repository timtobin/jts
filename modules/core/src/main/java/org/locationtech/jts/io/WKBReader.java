...
/**
 * Reads a {@link Geometry}from a byte stream in Well-Known Binary format. Supports use of an {@link
 * InStream}, which allows easy use with arbitrary byte stream sources.
 *
 * <p>This class reads the format describe in {@link WKBWriter}. It partially handles the
 * <b>Extended WKB</b> format used by PostGIS, by parsing and storing optional SRID values. If a
 * SRID is not specified in an element geometry, it is inherited from the parent's SRID. The default
 * SRID value is 0.
 *
 * <p>Although not defined in the WKB specification, empty points are handled if they are
 * represented as a Point with <code>NaN</code> X and Y ordinates.
 *
 * <p>The reader repairs structurally-invalid input (specifically, LineStrings and LinearRings which
 * contain too few points have vertices added, and non-closed rings are closed).
 *
 * <p>The reader handles most errors caused by malformed or malicious WKB data. It checks for
 * obviously excessive values of the fields <code>numElems</code>, <code>numRings</code>, and <code>
 * numCoords</code>. It also checks that the reader does not read beyond the end of the data
 * supplied. A {@link ParseException} is thrown if this situation is detected.
 *
 ...
 *
 * <p>As of version 1.15, the reader can read geometries following the OGC 06-103r4 Simple Features
 * Access 1.2.1 specification, which aligns with the ISO 19125 standard. This format is used by
 * Spatialite and Geopackage.
 *
 * <p>The difference between PostGIS EWKB format and the new ISO/OGC specification is that Z and M
 * coordinates are detected with a bit mask on the higher byte in the former case (0x80 for Z and
 * 0x40 for M) while new OGC specification use specific int ranges for 2D geometries, Z geometries
 * (2D code+1000), M geometries (2D code+2000) and ZM geometries (2D code+3000).
 *
 * <p>Note that the {@link WKBWriter} is not changed and still writes the PostGIS EWKB geometry
 * format.
 *
...
public class WKBReader {
  ...
  private LineString readLineString(EnumSet<Ordinate> ordinateFlags)
  ...
  private LinearRing readLinearRing(EnumSet<Ordinate> ordinateFlags)
      throws IOException, ParseException {
    int size = readNumField(FIELD_NUMCOORDS);
