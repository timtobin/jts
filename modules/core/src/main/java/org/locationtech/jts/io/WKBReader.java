 ...
 * Supports use of an {@link InStream}, which allows easy use with arbitrary
 * byte stream sources.
 *
 * <p>
 * This class reads the format describe in {@link WKBWriter}. It partially
 * handles the <b>Extended WKB</b> format used by PostGIS, by parsing and
 * storing optional SRID values. If a SRID is not specified in an element
 * geometry, it is inherited from the parent's SRID. The default SRID value is
 * 0.
 *
 * <p>
 * Although not defined in the WKB specification, empty points are handled if
 * they are represented as a Point with <code>NaN</code> X and Y ordinates.
 *
 * <p>
 * The reader repairs structurally-invalid input (specifically, LineStrings and
 * LinearRings which contain too few points have vertices added, and non-closed
 * rings are closed).
 *
 * <p>
 * The reader handles most errors caused by malformed or malicious WKB data. It
 * checks for obviously excessive values of the fields <code>numElems</code>,
 * <code>numRings</code>, and <code>
 * numCoords</code>. It also checks that the reader does not read beyond the end
 * of the data supplied. A {@link ParseException} is thrown if this situation is
 * detected.
 *
 * <p>
 ...
 * Simple Features Access 1.2.1 specification, which aligns with the ISO 19125
 * standard. This format is used by Spatialite and Geopackage.
 *
 * <p>
 * The difference between PostGIS EWKB format and the new ISO/OGC specification
 * is that Z and M coordinates are detected with a bit mask on the higher byte
 * in the former case (0x80 for Z and 0x40 for M) while new OGC specification
 * use specific int ranges for 2D geometries, Z geometries (2D code+1000), M
 * geometries (2D code+2000) and ZM geometries (2D code+3000).
 *
 * <p>
 * Note that the {@link WKBWriter} is not changed and still writes the PostGIS
 * EWKB geometry format.
 *
...
public class WKBReader {
	...
	private Geometry read(InStream is, int maxCoordNum) throws IOException, ParseException {
		/**
		 * This puts an upper bound on the allowed value in coordNum fields. It avoids
		 * OOM exceptions due to malformed input.
		 */
	...
	private CoordinateSequence readCoordinateSequenceLineString(int size, EnumSet<Ordinate> ordinateFlags)
			throws IOException, ParseException {
	...
	private Geometry readGeometry(int SRID) throws IOException, ParseException {
		...
		/**
		 * To get geometry type mask out EWKB flag bits, and use only low 3 digits of
		 * type word. This supports both EWKB and ISO/OGC.
		 */
		...
		// or are in the 1000 range (Z) or in the 3000 range (ZM) of geometry type
		// (ISO/OGC 06-103r4)
		boolean hasZ = ((typeInt & 0x80000000) != 0 || (typeInt & 0xffff) / 1000 == 1
				|| (typeInt & 0xffff) / 1000 == 3);
		// geometries with M coordinates have the 0x40 flag (postgis EWKB)
		// or are in the 1000 range (M) or in the 3000 range (ZM) of geometry type
		// (ISO/OGC 06-103r4)
		boolean hasM = ((typeInt & 0x40000000) != 0 || (typeInt & 0xffff) / 1000 == 2
				|| (typeInt & 0xffff) / 1000 == 3);
