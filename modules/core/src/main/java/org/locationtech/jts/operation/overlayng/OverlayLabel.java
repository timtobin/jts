 ...
 * cases.
 *
 * <p>In an {@link OverlayGraph} a single label is shared between the two oppositely-oriented {@
 * link OverlayEdge}s of a symmetric pair. Accessors for orientation-sensitive information are
 * parameterized by the orientation of the containing edge.
 *
 ...
 *         <li><code>locLeft, locRight</code> : the locations of the edge sides for the Area
 *         <li><code>locLine</code> : INTERIOR
 *         <li><code>isHole</code> : whether the edge is in a shell or a hole (the ring role)
 *       </ul>
 *   <li>A <b>Collapsed</b> edge of an input Area (formed by merging two or more parent edges)
 *       <ul>
 ...
 *         <li><code>isHole</code> : <code>true</code> if all parent edges are in holes; <code>false
 *             </code> if some parent edge is in a shell
 *       </ul>
 *   <li>A <b>Line</b> edge from an input line
 *       <ul>
 *         <li><code>dim</code> = DIM_LINE
 *         <li><code>locLine</code> : the location of the edge relative to the Line. Initialized to
 *             LOC_UNKNOWN to simplify logic.
 *       </ul>
 *   <li>An edge which is <b>Not Part</b> of an input geometry (and thus must be part of the other
 *       geometry).
 *       <ul>
 ...
 *
 * <ul>
 *   <li>an edge cannot be both a Collapse edge and a Line edge in the same input geometry, because
 *       input geometries must be homogeneous in dimension.
 *   <li>an edge may be an Boundary edge in one input geometry and a Line or Collapse edge in the
 *       other input.
 * </ul>
 *
 * @author Martin Davis
...
class OverlayLabel {
   ...
   *
   * <p>This is used to set the locations for linear edges encountered during area label
   * propagation.
   *
  ...
  /**
   * Tests if a line edge is inside a source geometry (i.e. it has location {@link
   * Location#INTERIOR}).
   *
   * @param index the index of the input geometry
  ...
  private String locationString(int index, boolean isForward) {
    ...
    if (isKnown(index)) buf.append(dimensionSymbol(index == 0 ? aDim : bDim));
    if (isCollapse(index)) {
