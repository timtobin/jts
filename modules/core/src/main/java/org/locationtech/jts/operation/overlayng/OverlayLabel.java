...
/**
 * A structure recording the topological situation for an edge in a topology
 * graph used during overlay processing. A label contains the topological
 * {@link Location}s for one or two input geometries to an overlay operation. An
 * input geometry may be either a Line or an Area. The label locations for each
 * input geometry are populated with the {@Location}s for the edge
 * {@link Position}s when they are created or once they are computed by
 ...
 *
 * <p>
 * In an {@link OverlayGraph} a single label is shared between the two
 * oppositely-oriented {@ link OverlayEdge}s of a symmetric pair. Accessors for
 * orientation-sensitive information are parameterized by the orientation of the
 * containing edge.
 *
 * <p>
 * For each input geometry (0 and 1), the label records that an edge is in one
 * of the following states (identified by the <code>dim</code> field). Each
 * state has additional information about the edge topology.
 *
 * <ul>
 * <li>A <b>Boundary</b> edge of an Area (polygon)
 * <ul>
 * <li><code>dim</code> = DIM_BOUNDARY
 * <li><code>locLeft, locRight</code> : the locations of the edge sides for the
 * Area
 * <li><code>locLine</code> : INTERIOR
 * <li><code>isHole</code> : whether the edge is in a shell or a hole (the ring
 * role)
 * </ul>
 * <li>A <b>Collapsed</b> edge of an input Area (formed by merging two or more
 * parent edges)
 * <ul>
 * <li><code>dim</code> = DIM_COLLAPSE
 * <li><code>locLine</code> : the location of the edge relative to the effective
 * input Area (a collapsed spike is EXTERIOR, a collapsed gore or hole is
 * INTERIOR)
 * <li><code>isHole</code> : <code>true</code> if all parent edges are in holes;
 * <code>false
 *             </code> if some parent edge is in a shell
 * </ul>
 * <li>A <b>Line</b> edge from an input line
 * <ul>
 * <li><code>dim</code> = DIM_LINE
 * <li><code>locLine</code> : the location of the edge relative to the Line.
 * Initialized to LOC_UNKNOWN to simplify logic.
 * </ul>
 * <li>An edge which is <b>Not Part</b> of an input geometry (and thus must be
 * part of the other geometry).
 * <ul>
 * <li><code>dim</code> = NOT_PART
 * </ul>
 * </ul>
 *
 * Note that:
 *
 * <ul>
 * <li>an edge cannot be both a Collapse edge and a Line edge in the same input
 * geometry, because input geometries must be homogeneous in dimension.
 * <li>an edge may be an Boundary edge in one input geometry and a Line or
 * Collapse edge in the other input.
 * </ul>
 *
 * @author Martin Davis
 */
class OverlayLabel {
	...
	/**
	 * The dimension for an edge which is a collapsed part of an input Area geometry
	 * boundary. A collapsed edge represents two or more line segments which have
	 * the same endpoints. They usually are caused by edges in valid polygonal
	 * geometries having their endpoints become identical due to precision
	 * reduction.
	 */
	...
	/**
	 * Initializes the label for an edge which is the collapse of part of the
	 * boundary of an Area input geometry. The location of the collapsed edge
	 * relative to the parent area geometry is initially unknown. It must be
	 ...
	 *
	 * <p>
	 * This is used to set the locations for linear edges encountered during area
	 * label propagation.
	 *
	 * @param index
	 *            source to update
	...
	/**
	 * Sets the location for a collapsed edge (the Line position) for an input
	 * geometry, depending on the ring role recorded in the label. If the input
	 * geometry edge is from a shell, the location is {@link Location#EXTERIOR}, if
	 * it is a hole it is {@link Location#INTERIOR}.
	 *
	 * @param index
	 *            the index of the input geometry
	 */
	...
	/**
	 * Tests whether a label is for an edge which is not part of a given input
	 * geometry.
	 *
	 * @param index
	 *            the index of the source geometry
	...
	/**
	 * Tests if a label is for an edge which is in the boundary of either source
	 * geometry.
	 *
	...
	/**
	 * Tests if a label is for an edge which is in the boundary of both source
	 * geometries.
	 *
	...
	/**
	 * Tests if the label is a collapsed edge of one area and is a (non-collapsed)
	 * boundary edge of the other area.
	 *
	...
	public boolean isBoundaryTouch() {
		return isBoundaryBoth() && getLocation(0, Position.RIGHT, true) != getLocation(1, Position.RIGHT, true);
	}

	/**
	 * Tests if a label is for an edge which is in the boundary of a source
	 * geometry. Collapses are not reported as being in the boundary.
	 *
	 * @param index
	 *            the index of the input geometry
	...
	/**
	 * Tests whether a label is for an edge which is a boundary of one geometry and
	...
	/**
	 * Tests if a line edge is inside a source geometry (i.e. it has location
	...
	/**
	 * Tests if a label is a Collapse has location {@link Location#INTERIOR}, to at
	 * least one source geometry.
	 *
	...
	/**
	 * Tests if a label is a Collapse and NotPart with location
	 * {@link Location#INTERIOR} for the other geometry.
	 *
	...
	/**
	 * Gets the location for a {@link Position} of an edge of a source for an edge
	...
	/**
	 * Gets the location for this label for either a Boundary or a Line edge. This
	 * supports a simple determination of whether the edge should be included as a
	 * result edge.
	 *
	 * @param index
	 *            the source index
