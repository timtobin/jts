 ...
 *   <li>Output may contain more nodes than in the input (in particular, sequences of coincident
 *       line segments are noded at each vertex
 * </ol>
 *
 * Various strategies are possible for how to merge graph edges into lines.
 *
 * <ul>
 *   <li>This implementation uses the simplest approach of maintaining all nodes arising from noding
 *       (which includes all nodes in the input, and possibly others). This matches the current JTS
 *       overlay output semantics.
 *   <li>Another option is to fully merge output lines from node to node. For rings a node point is
 *       chosen arbitrarily. It would also be possible to output LinearRings, if the input is a
 *       LinearRing and is unchanged. This will require additional info from the input linework.
 * </ul>
 *
 * @author Martin Davis
 */
class LineBuilder {
   ...
   * result line.
   *
   * <p>Note that the logic here relies on the semantic that for intersection lines are only
   * returned if there is no result area components.
   *
  ...
  private boolean isResultLine(OverlayLabel lbl) {
     ...
     *
     * <p>This logic is only used if not including collapse lines in result.
     */
     ...
     *
     * <p>For Intersection, a line edge interior to an area is included.
     */
  ...
  private void addResultLinesForNodes() {
    ...
    for (OverlayEdge edge : edges) {
       ...
       * edges.
       *
       * <p>This will find all lines originating at nodes
       */
   ...
   *
   * <p>The direction of the linework is preserved as far as possible. Specifically, the direction
   * of the line is determined by the start edge direction. This implies that if all edges are
   * reversed, the created line will be reversed to match. This ensures the orientation of linework
   * is faithful to the input in the case of polygon-line overlay. However, this does not provide a
   * consistent orientation in the case of line-line intersection(where A and B might have different
   * orientations). (Other more complex strategies would be possible. E.g. using the direction of
   * the majority of segments, or preferring the direction of the A edges.)
   *
   * @param node
   * @return
   */
