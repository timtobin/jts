...
/**
 * Finds and builds overlay result lines from the overlay graph. Output linework
 * has the following semantics:
 *
 * <ol>
 * <li>Linework is fully noded
 * <li>Nodes in the input are preserved in the output
 * <li>Output may contain more nodes than in the input (in particular, sequences
 ...
 *
 * <ul>
 * <li>This implementation uses the simplest approach of maintaining all nodes
 * arising from noding (which includes all nodes in the input, and possibly
 * others). This matches the current JTS overlay output semantics.
 * <li>Another option is to fully merge output lines from node to node. For
 * rings a node point is chosen arbitrarily. It would also be possible to output
 * LinearRings, if the input is a LinearRing and is unchanged. This will require
 * additional info from the input linework.
 * </ul>
 *
 * @author Martin Davis
 */
class LineBuilder {
	...
	/**
	 * Determines the effective location for a line, for the purpose of overlay
	 * operation evaluation. Line edges and Collapses are reported as INTERIOR so
	 * they may be included in the result if warranted by the effect of the
	 * operation on the two edges. (For instance, the intersection of a line edge
	 * and a collapsed boundary is included in the result).
	 *
	 * @param lbl
	 *            label of line
	 * @param geomIndex
	...
	/**
	 * Allow lines created by area topology collapses to appear in the result. True
	 * provides the original JTS semantics.
	 */
	private boolean isAllowCollapseLines = !OverlayNG.STRICT_MODE_DEFAULT;

	/**
	 * Indicates whether intersections are allowed to produce heterogeneous results
	 * including proper boundary touches. This does not control inclusion of touches
	 * along collapses. True provides the original JTS semantics.
	 */
	...
	/**
	 * Creates a builder for linear elements which may be present in the overlay
	 * result.
	 *
	 * @param inputGeom
	 ...
	 */
	public LineBuilder(InputGeometry inputGeom, OverlayGraph graph, boolean hasResultArea, int opCode,
	...
	private void addResultLinesForNodes() {
		...
		for (OverlayEdge edge : edges) {
			...
			/**
			 * Choose line start point as a node. Nodes in the line graph are degree-1 or
			 * degree >= 3 edges.
			 *
			 * <p>
	 ...
	 * original overlay semantics. It is also faster.
	 */
	...
	private void addResultLinesRings() {
		// TODO: an ordering could be imposed on the endpoints to make this more
		// repeatable

		// TODO: preserve input LinearRings if possible? Would require marking them as
		// such
		Collection<OverlayEdge> edges = graph.getEdges();
		for (OverlayEdge edge : edges) {
			if (!edge.isInResultLine())
	 ...
	 *
	 * <p>
	 * The direction of the linework is preserved as far as possible. Specifically,
	 * the direction of the line is determined by the start edge direction. This
	 * implies that if all edges are reversed, the created line will be reversed to
	 * match. This ensures the orientation of linework is faithful to the input in
	 * the case of polygon-line overlay. However, this does not provide a consistent
	 * orientation in the case of line-line intersection(where A and B might have
	 * different orientations). (Other more complex strategies would be possible.
	 * E.g. using the direction of the majority of segments, or preferring the
	 * direction of the A edges.)
	 *
	 * @param node
	 * @return
	 */
	...
	/**
	 * Checks if the topology indicated by an edge label determines that this edge
	 * should be part of a result line.
	 *
	 * <p>
	 * Note that the logic here relies on the semantic that for intersection lines
	...
	private boolean isResultLine(OverlayLabel lbl) {
		/**
		 * Omit edge which is a boundary of a single geometry (i.e. not a collapse or
		 * line edge as well). These are only included if part of a result area. This is
		 * a short-circuit for the most common area edge case
		 */
		if (lbl.isBoundarySingleton())
			return false;

		/**
		 * Omit edge which is a collapse along a boundary. I.e a result line edge must
		 * be from a input line OR two coincident area boundaries.
		 *
		 * <p>
		...
		/**
		 * Omit edge which is a collapse interior to its parent area. (E.g. a narrow
		 * gore, or spike off a hole)
		 */
		if (lbl.isInteriorCollapse())
			return false;

		/**
		 * For ops other than Intersection, omit a line edge if it is interior to the
		 * other area.
		 *
		 * <p>
		...
		if (opCode != OverlayNG.INTERSECTION) {
			...
			/**
			 * If there is a result area, omit line edge inside it. It is sufficient to
			 * check against the input area rather than the result area, because if line
			 * edges are present then there is only one input area, and the result area must
			 * be the same as the input area.
			 */
	...
	private void markResultLines() {
		...
		for (OverlayEdge edge : edges) {
			/**
			 * If the edge linework is already marked as in the result, it is not included
			 * as a line. This occurs when an edge either is in a result area or has already
			 * been included as a line.
			 */
