...
/**
 * Computes the boundary of a {@link Geometry}. Allows specifying the
 * {@link BoundaryNodeRule} to be used. This operation will always return a
 * {@link Geometry} of the appropriate dimension for the boundary (even if the
 * input geometry is empty). The boundary of zero-dimensional geometries
...
public class BoundaryOp {
	...
	/**
	 * Computes a geometry representing the boundary of a geometry, using an
	 * explicit {@link BoundaryNodeRule}.
	 *
	 * @param g
	 *            the input geometry
	 * @param bnRule
	 ...
	 * <li>For linear geometries the existence of the boundary is determined by the
	...
	public static boolean hasBoundary(Geometry geom, BoundaryNodeRule boundaryNodeRule) {
		// Note that this does not handle geometry collections with a non-empty linear
		// element
		if (geom.isEmpty())
			return false;
	...
	public Geometry getBoundary() {
		if (geom instanceof LineString string)
			return boundaryLineString(string);
		if (geom instanceof MultiLineString string)
			return boundaryMultiLineString(string);
	...
	/*
	 * // MD - superseded private Coordinate[]
	 * computeBoundaryFromGeometryGraph(MultiLineString mLine) { GeometryGraph g =
	 * new GeometryGraph(0, mLine, bnRule); Coordinate[] bdyPts =
