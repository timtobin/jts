 ...
 * Unlike objects of type {@link Point} (which contain additional information
 * such as an envelope, a precision model, and spatial reference system
 * information), a <code>Coordinate</code> only contains ordinate values and
 * accessor methods.
 *
 * <p>
 * <code>Coordinate</code>s are two-dimensional points, with an additional
 * Z-ordinate. If an Z-ordinate value is not specified or not defined,
 * constructed coordinates have a Z-ordinate of <code>NaN</code> (which is also
 * the value of <code>NULL_ORDINATE</code>). The standard comparison functions
 * ignore the Z-ordinate. Apart from the basic accessor functions, JTS supports
 ...
 * appropriate for a {@link CoordinateSequence}. Use of {@link #getZ()} and
 * {@link #getM()} accessors, or {@link #getOrdinate(int)} are recommended.
 *
 * @version 1.16
 */
public class Coordinate implements Comparable<Coordinate>, Cloneable, Serializable {
	 ...
	 * used for the value of ordinates for dimensions greater than the defined
	 ...
	 * assumption using {@link CoordinateSequence#getDimension()} and
	...
	/**
	 * Retrieves the value of the Z ordinate, if present. If no Z value is present
	 * returns <tt>NaN</tt>.
	 *
	...
	/**
	 * Retrieves the value of the measure, if present. If no measure value is
	 * present returns <tt>NaN</tt>.
	 *
	 ...
	 *
	 * <p>
	 * The base implementation supports values for the index are {@link #X},
	 * {@link #Y}, and {@link #Z}.
	 *
	 * @param ordinateIndex
	 *            the ordinate index
	 ...
	 *
	 * <p>
	 * The base implementation supported values for the index are {@link #X},
	 * {@link #Y}, and {@link #Z}.
	 *
	 * @param ordinateIndex
	 *            the ordinate index
	...
	/**
	 * Tests if the coordinate has valid X and Y ordinate values. An ordinate value
	 * is valid iff it is finite.
	 *
	 ...
	 * @return true if <code>other</code> is a <code>Coordinate</code> with the same
	 *         values for X and Y.
	 */
	...
	/**
	 * Returns <code>true</code> if <code>other</code> has the same values for the x
	 * and y ordinates. Since Coordinates are 2.5D, this routine ignores the z value
	 ...
	 * @return -1, zero, or 1 as this <code>Coordinate</code> is less than, equal
	...
	/**
	 * Create a new Coordinate of the same type as this Coordinate, but with no
	 * values.
	 *
	...
	public static class DimensionalComparator implements Comparator<Coordinate> {
		/**
		 * Compare two <code>double</code>s, allowing for NaN values. NaN is treated as
		 * being less than any valid number.
		 *
		 * @param a
		 *            a <code>double</code>
		 * @param b
		 *            a <code>double</code>
		 * @return -1, 0, or 1 depending on whether a is less than, equal to or greater
		 *         than b
		 */
		 ...
		 * @param c2
		 *            a {link Coordinate}
		 * @return -1, 0, or 1 depending on whether o1 is less than, equal to, or
		 *         greater than 02
		 */
