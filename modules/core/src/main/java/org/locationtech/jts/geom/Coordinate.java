 ...
 *
 * <p>It is distinct from {@link Point}, which is a subclass of {@link Geometry}. Unlike objects of
 * type {@link Point} (which contain additional information such as an envelope, a precision model,
 * and spatial reference system information), a <code>Coordinate</code> only contains ordinate
 * values and accessor methods.
 *
 * <p><code>Coordinate</code>s are two-dimensional points, with an additional Z-ordinate. If an
 * Z-ordinate value is not specified or not defined, constructed coordinates have a Z-ordinate of
 * <code>NaN</code> (which is also the value of <code>NULL_ORDINATE</code>). The standard comparison
 * functions ignore the Z-ordinate. Apart from the basic accessor functions, JTS supports only
 * specific operations involving the Z-ordinate.
 *
 * <p>Implementations may optionally support Z-ordinate and M-measure values as appropriate for a
 * {@link CoordinateSequence}. Use of {@link #getZ()} and {@link #getM()} accessors, or {@link
...
public class Coordinate implements Comparable<Coordinate>, Cloneable, Serializable {
  ...
  /**
   * The value used to indicate a null or missing ordinate value. In particular, used for the value
   * of ordinates for dimensions greater than the defined dimension of a coordinate.
   */
   ...
   * The z-ordinate.
   *
   * <p>Direct access to this field is discouraged; use {@link #getZ()}.
   */
  public double z;
  ...
  /**
   * Retrieves the value of the Z ordinate, if present. If no Z value is present returns
   * <tt>NaN</tt>.
   *
  ...
  /**
   * Retrieves the value of the measure, if present. If no measure value is present returns
   * <tt>NaN</tt>.
   *
   ...
   *
   * <p>The base implementation supports values for the index are {@link #X}, {@link #Y}, and {@link
   * #Z}.
   *
   ...
   *
   * <p>The base implementation supported values for the index are {@link #X}, {@link #Y}, and
   * {@link #Z}.
   *
   * @param ordinateIndex the ordinate index
  ...
  /**
   * Tests if the coordinate has valid X and Y ordinate values. An ordinate value is valid iff it is
   * finite.
   *
   ...
   * @return <code>true</code> if the x- and y-coordinates are equal; the z-coordinates do not have
   *     to be equal.
   */
  ...
  /**
   * Tests if another Coordinate has the same values for the X and Y ordinates, within a specified
   * tolerance value. The Z ordinate is ignored.
   *
   * @param c a <code>Coordinate</code> with which to do the 2D comparison.
   * @param tolerance the tolerance value to use
   * @return true if <code>other</code> is a <code>Coordinate</code> with the same values for X and
   *     Y.
   */
  ...
  /**
   * Returns <code>true</code> if <code>other</code> has the same values for the x and y ordinates.
   * Since Coordinates are 2.5D, this routine ignores the z value when making the comparison.
   *
   * @param other a <code>Coordinate</code> with which to do the comparison.
   * @return <code>true</code> if <code>other</code> is a <code>Coordinate</code> with the same
   ...
   * correctly.
   *
   * @param o the <code>Coordinate</code> with which this <code>Coordinate</code> is being compared
   * @return -1, zero, or 1 as this <code>Coordinate</code> is less than, equal to, or greater than
   *     the specified <code>Coordinate</code>
   */
  ...
  /**
   * Computes the squared 2-dimensional Euclidean distance to another location. The Z-ordinate is
   * ignored.
   *
   * @param c a point
  ...
  /**
   * Compares two {@link Coordinate}s, allowing for either a 2-dimensional or 3-dimensional
   * comparison, and handling NaN values correctly.
   */
