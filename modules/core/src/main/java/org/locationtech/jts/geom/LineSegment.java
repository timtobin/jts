...
public class LineSegment implements Comparable, Serializable {
	...
	/**
	 * Computes the angle that the vector defined by this segment makes with the
	 * X-axis. The angle will be in the range [ -PI, PI ] radians.
	 *
	 ...
	 * @return a Coordinate which is the closest point on the line segment to the
	 *         point p
	 */
	...
	/**
	 * Compares this object with the specified object for order. Uses the standard
	 * lexicographic ordering for the points in the LineSegment.
	 *
	 * @param o
	 *            the <code>LineSegment</code> with which this
	 *            <code>LineSegment</code> is being compared
	 * @return a negative integer, zero, or a positive integer as this
	 *         <code>LineSegment</code> is less than, equal to, or greater than the
	 *         specified <code>LineSegment</code>
	 */
	...
	/**
	 * Computes the perpendicular distance between the (infinite) line defined by
	 * this line segment and a point. If the segment has zero length this returns
	 ...
	 * defined by this line segment and a point. The oriented distance is positive
	 * if the point on the left of the line, and negative if it is on the right. If
	 * the segment has zero length this returns the distance between the segment and
	 * the point.
	 *
	 * @param p
	 *            the point to compute the distance to
	...
	/**
	 * Returns <code>true</code> if <code>other</code> has the same values for its
	 * points.
	 *
	 * @param o
	 *            a <code>LineSegment</code> with which to do the comparison.
	...
	/**
	 * Returns <code>true</code> if <code>other</code> is topologically equal to
	 * this LineSegment (e.g. irrespective of orientation).
	 *
	 * @param other
	 *            a <code>LineSegment</code> with which to do the comparison.
	 * @return <code>true</code> if <code>other</code> is a <code>LineSegment</code>
	...
	/**
	 * Computes the intersection point of the lines of infinite extent defined by
	 * two line segments (if there is one). There may be 0, 1 or an infinite number
	 * of intersection points between two lines. If there is a unique intersection
	 * point, it is returned. Otherwise, <tt>null</tt> is returned. If more
	 ...
	 * @return an intersection point, or <code>null</code> if there is no point of
	 *         intersection or an infinite number of intersection points
	 * @see RobustLineIntersector
	 */
	...
	/**
	 * Puts the line segment into a normalized form. This is useful for using line
	 * segments in maps and indexes when topological equality rather than exact
	 * equality is desired. A segment in normalized form has the first point smaller
	...
	/**
	 * Computes the {@link LineSegment} that is offset from the segment by a given
	 ...
	 *            the distance the point is offset from the segment (positive is to
	...
	/**
	 * Determines the orientation index of a {@link Coordinate} relative to this
	 * segment. The orientation index is as defined in
	...
	/**
	 * Determines the orientation of a LineSegment relative to this segment. The
	 * concept of orientation is specified as follows: Given two line segments A and
	 * L,
	 *
	 * <ul>
	 * <li>A is to the left of a segment L if A lies wholly in the closed half-plane
	 * lying to the left of L
	 * <li>A is to the right of a segment L if A lies wholly in the closed
	 * half-plane lying to the right of L
	 * <li>otherwise, A has indeterminate orientation relative to L. This happens if
	 * A is collinear with L or if A crosses the line determined by L.
	 * </ul>
	 *
	 * @param seg
	...
	/**
	 * Computes the {@link Coordinate} that lies a given fraction along the line
	 * defined by this segment. A fraction of <code>0.0</code> returns the start
	 * point of the segment; a fraction of <code>1.0</code> returns the end point of
	 * the segment. If the fraction is &lt; 0.0 or &gt; 1.0 the point returned will
	 * lie before the start or beyond the end of the segment.
	 *
	...
	/**
	 * Computes the {@link Coordinate} that lies a given fraction along the line
	 * defined by this segment and offset from the segment by a given distance. A
	 * fraction of <code>0.0</code> offsets from the start point of the segment; a
	 * fraction of <code>1.0</code> offsets from the end point of the segment. The
	 * computed point is offset to the left of the line if the offset distance is
	...
	public Coordinate pointAlongOffset(double segmentLengthFraction, double offsetDistance) {
		...
		// the offset point is the seg point plus the offset vector rotated 90 degrees
		// CCW
	...
	/**
	 * Project a line segment onto this line segment and return the resulting line
	 * segment. The returned line segment will be a subset of the target line line
	 * segment. This subset may be null, if the segments are oriented in such a way
	 ...
	 * @return the projected line segment, or <code>null</code> if there is no
	 *         overlap
	 */
	 ...
	 * <tt>p</tt> on the line defined by this segment.
	 *
	 * <p>
	 * The projection factor lies in the range <tt>(-inf, +inf)</tt>. It is
	 * <code>NaN</code> if the line segment has zero length..
	 *
	 * @param p
	...
	public double projectionFactor(Coordinate p) {
		 ...
		 *
		 * <p>
		 * r has the following meaning: r=0 : P = A r=1 : P = B r<0 : P is on the
		 * backward extension of AB r>1 : P is on the forward extension of AB 0<r<1 P is
		 * interior to AB
		 */
		double dx = p1.x - p0.x;
		double dy = p1.y - p0.y;
	...
	/**
	 * Computes the fraction of distance (in <tt>[0.0, 1.0]</tt>) that the
	 * projection of a point occurs along this line segment. If the point is beyond
	 * either ends of the line segment, the closest fractional value (<tt>0.0</tt>
	 * or <tt>1.0</tt>) is returned.
	 *
	 * <p>
	 * Essentially, this is the {@link #projectionFactor} clamped to the range
	 * <tt>[0.0, 1.0]</tt>. If the segment has zero length, 1.0 is returned.
	 *
	 * @param inputPt
	 *            the point
	 * @return the fraction along the line segment the projection of the point
	 *         occurs
	 */
