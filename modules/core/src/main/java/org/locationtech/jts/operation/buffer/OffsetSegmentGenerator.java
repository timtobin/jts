...
class OffsetSegmentGenerator {
  ...
  /**
   * Factor controlling how close offset segments can be to skip adding a fillet or mitre. This
   * eliminates very short fillet segments, reduces the number of offset curve vertices. and
   * improves the robustness of mitre construction.
   */
   ...
   *
   * <p>A closingSegFactor of 0 results in lines to the corner vertex A closingSegFactor of 1
   * results in lines halfway to the corner vertex A closingSegFactor of 80 results in lines 1/81 of
   * the way to the corner vertex (this option is reasonable for the very common default situation
   * of round joins and quadrantSegs >= 8)
   */
  ...
  private void addOutsideTurn(int orientation, boolean addStartPoint) {
    /**
     * Heuristic: If offset endpoints are very close together, (which happens for nearly-parallel
     * segments), use an endpoint as the single offset corner vertex. This eliminates very short
     * single-segment joins and reduces the number of offset curve vertices. This also avoids
     * robustness problems with computing mitre corners for nearly-parallel segments.
     */
  ...
  private void addInsideTurn(int orientation, boolean addStartPoint) {
    ...
    if (li.hasIntersection()) {
       ...
       *
       * <p>In complex buffer cases the closing segment may cut across many other segments in the
  ...
  public void addLineEndCap(Coordinate p0, Coordinate p1) {
    ...
    switch (bufParams.getEndCapStyle()) {
      case BufferParameters.CAP_ROUND:
        ...
        addDirectedFillet(
            p1, angle + Angle.PI_OVER_2, angle - Angle.PI_OVER_2, Orientation.CLOCKWISE, distance);
      ...
      case BufferParameters.CAP_SQUARE:
        ...
        Coordinate squareCapLOffset =
            new Coordinate(
                offsetL.p1.x + squareCapSideOffset.x, offsetL.p1.y + squareCapSideOffset.y);
        Coordinate squareCapROffset =
            new Coordinate(
                offsetR.p1.x + squareCapSideOffset.x, offsetR.p1.y + squareCapSideOffset.y);
  ...
  /**
   * Adds a mitre join connecting two convex offset segments. The mitre is beveled if it exceeds the
   * mitre limit factor. The mitre limit is intended to prevent extremely long corners occurring. If
   * the mitre limit is very small it can cause unwanted artifacts around fairly flat corners. This
   * is prevented by using a simple bevel join in this case. In other words, the limit prevents the
   * corner from getting too long, but it won't force it to be very short/flat.
   *
  ...
  private void addMitreJoin(
    ...
    /**
     * First try a non-beveled join. Compute the intersection point of the lines determined by the
     * offsets. Parallel or collinear lines will return a null point ==> need to be beveled
     *
     * <p>Note: This computation is unstable if the offset segments are nearly collinear. However,
     * this situation should have been eliminated earlier by the check for whether the offset
     * segment endpoints are almost coincident
     */
    ...
    /**
     * In case the mitre limit is very small, try a plain bevel. Use it if it's further than the
     * limit.
     */
  ...
  /**
   * Adds a limited mitre join connecting two convex offset segments. A limited mitre join is
   * beveled at the distance determined by the mitre limit factor, or as a standard bevel join,
  ...
  private void addLimitedMitreJoin(
    ...
    /**
     * If the corner is very flat or the mitre limit is very small the limited bevel segment may not
     * intersect the offsets. In this case just bevel the join.
     */
    addBevelJoin(offset0, offset1);
  }
  ...
  /**
   * Add points for a circular fillet around a convex corner. Adds the start and end points
   *
   ...
   */
  private void addCornerFillet(
      Coordinate p, Coordinate p0, Coordinate p1, int direction, double radius) {
    double dx0 = p0.x - p.x;
    double dy0 = p0.y - p.y;
   ...
   * @param radius the radius of the fillet
   */
  private void addDirectedFillet(
