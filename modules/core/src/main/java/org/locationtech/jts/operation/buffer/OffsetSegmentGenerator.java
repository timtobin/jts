 ...
 * options provided for buffering. This algorithm implements various heuristics
 * to produce smoother, simpler curves which are still within a reasonable
...
class OffsetSegmentGenerator {
	...
	/**
	 * Factor controlling how close curve vertices on inside turns can be to be
	 * snapped
	 */
	...
	/**
	 * Factor controlling how close offset segments can be to skip adding a fillet
	 * or mitre. This eliminates very short fillet segments, reduces the number of
	 * offset curve vertices. and improves the robustness of mitre construction.
	 */
	...
	public OffsetSegmentGenerator(PrecisionModel precisionModel, BufferParameters bufParams, double distance) {
		...
		/**
		 * Non-round joins cause issues with short closing segments, so don't use them.
		 * In any case, non-round joins only really make sense for relatively small
		 * buffer distances.
		 */
	...
	/**
	 * Adds a bevel join connecting two offset segments around a convex corner.
	 *
	 * @param offset0
	...
	private void addCollinear(boolean addStartPoint) {
		/**
		 * This test could probably be done more efficiently, but the situation of exact
		 * collinearity should be fairly rare.
		 */
	...
	/**
	 * Add points for a circular fillet around a convex corner. Adds the start and
	 * end points
	 *
	 * @param p
	 *            base point of curve
	...
	private void addInsideTurn(int orientation, boolean addStartPoint) {
		...
		if (li.hasIntersection()) {
			 ...
			 * offset so large that the offsets segments don't intersect. In this case we
			 * must add a "closing segment" to make sure the buffer curve is continuous,
			 * fairly smooth (e.g. no sharp reversals in direction) and tracks the buffer
			 * correctly around the corner. The curve connects the endpoints of the segment
			 * offsets to points which lie toward the centre point of the corner. The
			 * joining curve will not appear in the final buffer outline, since it is
			 * completely internal to the buffer polygon.
			 *
			 * <p>
			 * In complex buffer cases the closing segment may cut across many other
			 * segments in the generated offset curve. In order to improve the performance
			 * of the noding, the closing segment should be kept as short as possible. (But
			 * not too short, since that would defeat its purpose). This is the purpose of
			 * the closingSegFactor heuristic value.
			 */

			/**
			 * The intersection test above is vulnerable to robustness errors; i.e. it may
			 * be that the offsets should intersect very close to their endpoints, but
			 * aren't reported as such due to rounding. To handle this situation
			...
			if (offset0.p1.distance(offset1.p0) < distance * INSIDE_TURN_VERTEX_SNAP_DISTANCE_FACTOR) {
				...
				if (closingSegLengthFactor > 0) {
					...
					/**
					 * This branch is not expected to be used except for testing purposes. It is
					 * equivalent to the JTS 1.9 logic for closing segments (which results in very
					 * poor performance for large buffer distances)
					 */
					segList.addPt(s1);
				}
	...
	/**
	 * Adds a limited mitre join connecting two convex offset segments. A limited
	 * mitre join is beveled at the distance determined by the mitre limit factor,
	 ...
	 * @param distance
	 *            the offset distance
	 * @param mitreLimitDistance
	 *            the mitre limit distance
	 */
	private void addLimitedMitreJoin(LineSegment offset0, LineSegment offset1, double distance,
		...
		/**
		 * If the corner is very flat or the mitre limit is very small the limited bevel
		 * segment may not intersect the offsets. In this case just bevel the join.
		 */
	...
	public void addLineEndCap(Coordinate p0, Coordinate p1) {
		...
		switch (bufParams.getEndCapStyle()) {
			case BufferParameters.CAP_ROUND :
				...
				addDirectedFillet(p1, angle + Angle.PI_OVER_2, angle - Angle.PI_OVER_2, Orientation.CLOCKWISE,
						distance);
	...
	/**
	 * Adds a mitre join connecting two convex offset segments. The mitre is beveled
	 * if it exceeds the mitre limit factor. The mitre limit is intended to prevent
	 * extremely long corners occurring. If the mitre limit is very small it can
	 * cause unwanted artifacts around fairly flat corners. This is prevented by
	 * using a simple bevel join in this case. In other words, the limit prevents
	 * the corner from getting too long, but it won't force it to be very
	 * short/flat.
	 *
	 * @param offset0
	 *            the first offset segment
	...
	private void addMitreJoin(Coordinate cornerPt, LineSegment offset0, LineSegment offset1, double distance) {
		...
		/**
		 * First try a non-beveled join. Compute the intersection point of the lines
		 * determined by the offsets. Parallel or collinear lines will return a null
		 * point ==> need to be beveled
		 *
		 * <p>
		 * Note: This computation is unstable if the offset segments are nearly
		 * collinear. However, this situation should have been eliminated earlier by the
		 * check for whether the offset segment endpoints are almost coincident
		 */
		...
		/**
		 * In case the mitre limit is very small, try a plain bevel. Use it if it's
	...
	private void addOutsideTurn(int orientation, boolean addStartPoint) {
		/**
		 * Heuristic: If offset endpoints are very close together, (which happens for
		 * nearly-parallel segments), use an endpoint as the single offset corner
		 * vertex. This eliminates very short single-segment joins and reduces the
		 * number of offset curve vertices. This also avoids robustness problems with
		 * computing mitre corners for nearly-parallel segments.
		 */
