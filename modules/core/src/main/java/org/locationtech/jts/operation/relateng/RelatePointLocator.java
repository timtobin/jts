...
/**
 * Locates a point on a geometry, including mixed-type collections. The
 * dimension of the containing geometry element is also determined.
 * GeometryCollections are handled with union semantics; i.e. the location of a
 * point is that location of that point on the union of the elements of the
 * collection.
 *
 * <p>
 ...
 *
 * <ol>
 * <li>For a mixed-dimension (heterogeneous) collection a point may lie on two
 * geometry elements with different dimensions. In this case the location on the
 * largest-dimension element is reported.
 * <li>For a collection with overlapping or adjacent polygons, points on polygon
 ...
 *
 * <p>
 * Supports specifying the {@link BoundaryNodeRule} to use for line endpoints.
 *
 * @author Martin Davis
 */
class RelatePointLocator {
	...
	private PointOnGeometryLocator getLocator(int index) {
		...
		if (locator == null) {
			...
			locator = isPrepared ? new IndexedPointInAreaLocator(polygonal) : new SimplePointInAreaLocator(polygonal);
	...
	/**
	 * Locates a line endpoint, as a {@link DimensionLocation}. In a mixed-dim GC,
	 * the line end point may also lie in an area. In this case the area location is
	 * reported. Otherwise, the dimLoc is either LINE_BOUNDARY or LINE_INTERIOR,
	 * depending on the endpoint valence and the BoundaryNodeRule in place.
	 *
	 * @param p
	 *            the line end point to locate
	...
	/**
	 * Locates a point which is known to be a node of the geometry (i.e. a vertex or
	 * on an edge).
	 *
	 * @param p
	 *            the node point to locate
	 * @param parentPolygonal
	 *            the polygon the point is a node of
	...
	/**
	 * Locates a point which is known to be a node of the geometry, as a
	 * {@link DimensionLocation}.
	 *
	 * @param p
	 *            the point to locate
	 * @param parentPolygonal
	...
	/**
	 * Computes the topological location ({@link Location}) of a single point in a
	...
	private int locateWithDim(Coordinate p, boolean isNode, Geometry parentPolygonal) {
		...
		/**
		 * In a polygonal geometry a node must be on the boundary. (This is not the case
		 * for a mixed collection, since the node may be in the interior of a polygon.)
		 */
