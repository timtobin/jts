 ...
 * collection.
 *
 * <p>Union semantics for GeometryCollections has the following behaviours:
 *
 * <ol>
 ...
 *       reported.
 *   <li>For a collection with overlapping or adjacent polygons, points on polygon element
 *       boundaries may lie in the effective interior of the collection geometry.
 * </ol>
 *
 * Prepared mode is supported via cached spatial indexes.
 *
 * <p>Supports specifying the {@link BoundaryNodeRule} to use for line endpoints.
 *
 * @author Martin Davis
 */
class RelatePointLocator {
  ...
  public int locateLineEndWithDim(Coordinate p) {
    ...
    if (polygons != null) {
      ...
      if (locPoly != Location.EXTERIOR) return DimensionLocation.locationArea(locPoly);
    }
  ...
  /**
   * Computes the topological location ({@link Location}) of a single point in a Geometry, as well
   * as the dimension of the geometry element the point is located in (if not in the Exterior). It
  ...
  private int computeDimLocation(Coordinate p, boolean isNode, Geometry parentPolygonal) {
    ...
    if (lines != null) {
      ...
      if (locLine != Location.EXTERIOR) return DimensionLocation.locationLine(locLine);
    }
    if (points != null) {
      int locPt = locateOnPoints(p);
      if (locPt != Location.EXTERIOR) return DimensionLocation.locationPoint(locPt);
    }
  ...
  private int locateOnLines(Coordinate p, boolean isNode) {
    ...
    if (isNode) return Location.INTERIOR;
