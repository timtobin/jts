...
/**
 * Tests whether a MultiPolygon has any element polygon improperly nested inside
 * another polygon, using a spatial index to speed up the comparisons.
 *
 * <p>
 * The logic assumes that the polygons do not overlap and have no collinear
 * segments. So the polygon rings may touch at discrete points, but they are
 * properly nested, and there are no duplicate rings.
 */
class IndexedNestedPolygonTester {
	/**
	 * Finds a point of a shell segment which lies inside a polygon, if any. The
	 * shell is assumed to touch the polygon only at shell vertices, and does not
	 * cross the polygon.
	 *
	 * @param shell
	 *            the shell to test
	...
	private static Coordinate findIncidentSegmentNestedPoint(LinearRing shell, Polygon poly) {
		...
		/**
		 * Check if the shell is inside a hole (if there are any). If so this is valid.
		 */
		...
		/**
		 * The shell is contained in the polygon, but is not contained in a hole. This
		 * is invalid.
		 */
	 ...
	 *
	 * @param shell
	 *            the test polygon shell
	 * @param possibleOuterPoly
	 *            a polygon which may contain it
	 * @param locator
	 *            the locator for the outer polygon
	...
	private Coordinate findNestedPoint(LinearRing shell, Polygon possibleOuterPoly, IndexedPointInAreaLocator locator) {
		...
		/**
		 * The shell points both lie on the boundary of the polygon. Nesting can be
		 * checked via the topology of the incident edges.
		 */
	 ...
	 * This is invalid. The nested point will be set to reflect this.
	 *
