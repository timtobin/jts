...
/**
 * Tests whether a MultiPolygon has any element polygon improperly nested inside another polygon,
 * using a spatial index to speed up the comparisons.
 *
 * <p>The logic assumes that the polygons do not overlap and have no collinear segments. So the
 * polygon rings may touch at discrete points, but they are properly nested, and there are no
 * duplicate rings.
 */
class IndexedNestedPolygonTester {
  ...
  /**
   * Tests if any polygon is improperly nested (contained) within another polygon. This is invalid.
   ...
   */
  private Coordinate findNestedPoint(
    ...
    /**
     * The shell points both lie on the boundary of the polygon. Nesting can be checked via the
     * topology of the incident edges.
     */
    return findIncidentSegmentNestedPoint(shell, possibleOuterPoly);
  }

  /**
   * Finds a point of a shell segment which lies inside a polygon, if any. The shell is assumed to
   * touch the polygon only at shell vertices, and does not cross the polygon.
   *
  ...
  private static Coordinate findIncidentSegmentNestedPoint(LinearRing shell, Polygon poly) {
    ...
    /** The shell is contained in the polygon, but is not contained in a hole. This is invalid. */
