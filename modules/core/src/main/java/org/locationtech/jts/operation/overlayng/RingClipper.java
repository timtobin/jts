...
/**
 * Clips rings of points to a rectangle. Uses a variant of Cohen-Sutherland
 * clipping.
 *
 * <p>
 ...
 * used as input to the {@link OverlayNG} algorithm (which is able to process
 * coincident linework due to the need to handle topology collapse under
 * precision reduction).
 *
 * <p>
 * Because of the likelihood of creating extraneous line segments along the
 * clipping rectangle sides, this class is not suitable for clipping
 * linestrings.
 *
 * <p>
 * The clipping envelope should be generated using
 * {@link RobustClipEnvelopeComputer}, to ensure that intersecting line segments
 * are not perturbed by clipping. This is required to ensure that the overlay of
 * the clipped geometry is robust and correct (i.e. the same as if clipping was
 * not used).
 *
 * @see LineLimiter
 * @author Martin Davis
