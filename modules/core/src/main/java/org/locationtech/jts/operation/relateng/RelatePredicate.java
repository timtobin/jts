...
/**
 * Creates predicate instances for evaluating OGC-standard named topological
 * relationships. Predicates can be evaluated for geometries using
 * {@link RelateNG}.
 *
 * @author Martin Davis
 */
public interface RelatePredicate {
	...
	/**
	 * Creates a predicate to determine whether a geometry contains another
	 * geometry.
	 *
	 * <p>
	 ...
	 * similar behavior but avoiding this subtle limitation, see {@link #covers}.
	 *
	...
	/**
	 * Creates a predicate to determine whether a geometry is covered by another
	 * geometry.
	 *
	 * <p>
	 ...
	 * geometries. For most cases, <code>covers</code> should be used in preference
	 * to <code>contains</code>. As an added benefit, <code>covers</code> is more
	 ...
	 *
	 * <p>
	 * The SFS defined this predicate only for P/L, P/A, L/L, and L/A cases. To make
	 * the relation symmetric JTS extends the definition to apply to L/P, A/P and
	 * A/L cases as well.
	 *
	 * @return the predicate instance
	 */
	...
	/**
	 * Creates a predicate to determine whether two geometries are topologically
	 * equal.
	 *
	 * <p>
	...
	/**
	 * Creates a predicate to determine whether a geometry overlaps another
	 * geometry.
	 *
	 * <p>
	 ...
	 * <li>The DE-9IM Intersection Matrix for the two geometries matches
	 * <code>[T*T***T**]</code> (for P/P and A/A cases) or <code>[1*T***T**]</code>
	 * (for L/L cases)
	 * </ul>
	 *
	...
	/**
	 * Creates a predicate to determine whether a geometry is within another
	 * geometry.
	 *
	 * <p>
	 ...
	 * points in the boundary of a geometry B, <code>within(B, A) = false</code> (As
	 * a concrete example, take A to be a LineString which lies in the boundary of a
	 * Polygon B.) For a predicate with similar behavior but avoiding this subtle
