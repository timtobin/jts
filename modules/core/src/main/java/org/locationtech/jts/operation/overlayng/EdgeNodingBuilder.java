...
/**
 * Builds a set of noded, unique, labelled Edges from the edges of the two input
 * geometries.
 *
 * <p>
 ...
 * <li>chooses a {@link Noder} based on provided precision model, unless a
...
class EdgeNodingBuilder {
	...
	private static int computeDepthDelta(LinearRing ring, boolean isHole) {
		 ...
		 * interior/exterior labels correctly. JTS canonical orientation is that shells
		 * are CW, holes are CCW.
		 *
		 * <p>
		 * It is important to compute orientation on the original ring, since topology
		 * collapse can make the orientation computation give the wrong answer.
		 */
	...
	/**
	 * Removes any repeated points from a linear component. This is required so that
	 * noding can be computed correctly.
	 *
	 * @param line
	 *            the line to process
	...
	/**
	 * Creates a new builder, with an optional custom noder. If the noder is not
	 * provided, a suitable one will be used based on the supplied precision model.
	 *
	 * @param pm
	...
	/**
	 * Adds a line geometry, limiting it if enabled, and otherwise removing repeated
	 * points.
	 *
	 * @param line
	 *            the line to add
	...
	private void addPolygonRing(LinearRing ring, boolean isHole, int index) {
		...
		// if (pts.length < ring.getNumPoints()) System.out.println("Ring clipped: " +
		// ring.getNumPoints() + " => " + pts.length);
	...
	/**
	 * Creates a set of labelled {Edge}s. representing the fully noded edges of the
	 * input geometries. Coincident edges (from the same or both geometries) are
	 * merged along with their labels into a single unique, fully labelled edge.
	 *
	 * @param geom0
	 *            the first geometry
	 * @param geom1
	...
	/**
	 * If a clipper is present, clip the line to the clip extent. Otherwise, remove
	 * duplicate points from the ring.
	 *
	 * <p>
	 * If clipping is enabled, then every ring MUST be clipped, to ensure that holes
	 * are clipped to be inside the shell. This means it is not possible to skip
	 ...
	 * <li>Floating precision: a conventional nodel (which may be non-robust). In
	...
	/**
	 * Reports whether there are noded edges for the given input geometry. If there
	 * are none, this indicates that either the geometry was empty, or has
	 * completely collapsed (because it is smaller than the noding precision).
	 *
	 * @param geomIndex
	...
	/**
	 * Tests whether a geometry (represented by its envelope) lies completely
	 * outside the clip extent(if any).
	 *
	 * @param env
	...
	/**
	 * Tests whether it is worth limiting a line. Lines that have few vertices or
	 * are covered by the clip extent do not need to be limited.
	 *
	 * @param line
	 *            line to test
	...
	/**
	 * Nodes a set of segment strings and creates {@link Edge}s from the result. The
	 * input segment strings each carry a {@link EdgeSourceInfo} object, which is
	 * used to provide source topology info to the constructed Edges (and then is
	 * discarded).
	 *
	 * @param segStrings
	 * @return
