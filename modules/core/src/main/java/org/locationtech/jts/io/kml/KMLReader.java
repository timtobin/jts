...
/**
 * Constructs a {@link Geometry} object from the OGC KML representation. Works
 * only with KML geometry elements and may also parse attributes within these
 * elements
 */
public class KMLReader {
	 ...
	 *            names of attributes that should be parsed (i.e. extrude,
	 *            altitudeMode, tesselate, etc).
	 */
	...
	public KMLReader(GeometryFactory geometryFactory, Collection<String> attributeNames) {
		...
		this.attributeNames = attributeNames == null ? Collections.emptySet() : new HashSet<>(attributeNames);
	}

	/**
	 * Reads a KML representation of a {@link Geometry} from a {@link String}. If
	 * any attribute names were specified during {@link KMLReader} construction,
	 * they will be stored as {@link Map} in {@link Geometry#setUserData(Object)}
	 *
	 * @param kmlGeometryString
	 *            string that specifies kml representation of geometry
	...
	private KMLCoordinatesAndAttributes parseKMLCoordinatesAndAttributes(XMLStreamReader xmlStreamReader,
			String objectNodeName) throws XMLStreamException, ParseException {
		Coordinate[] coordinates = null;
		Map<String, String> attributes = null;

		while (xmlStreamReader.hasNext()
				&& !(xmlStreamReader.isEndElement() && xmlStreamReader.getLocalName().equals(objectNodeName))) {
	...
	private Geometry parseKMLPoint(XMLStreamReader xmlStreamReader) throws XMLStreamException, ParseException {
		KMLCoordinatesAndAttributes kmlCoordinatesAndAttributes = parseKMLCoordinatesAndAttributes(xmlStreamReader,
				POINT);
	...
	private Geometry parseKMLPolygon(XMLStreamReader xmlStreamReader) throws XMLStreamException, ParseException {
		...
		Polygon polygon = geometryFactory.createPolygon(shell,
				holes == null ? null : holes.toArray(new LinearRing[]{}));
	...
	private void moveToElement(XMLStreamReader xmlStreamReader, String elementName, String endElementName)
			throws XMLStreamException, ParseException {
		boolean elementFound = false;

		while (xmlStreamReader.hasNext()
				&& !(xmlStreamReader.isEndElement() && xmlStreamReader.getLocalName().equals(endElementName))) {
