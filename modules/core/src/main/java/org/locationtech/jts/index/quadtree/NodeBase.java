...
public abstract class NodeBase implements Serializable {
	...
	/**
	 * Gets the index of the subquad that wholly contains the given envelope. If
	 * none does, returns -1.
	 *
	 * @return the index of the subquad that wholly contains the given envelope or
	 *         -1 if no subquad wholly contains the envelope
	 */
	...
	private void visitItems(Envelope searchEnv, ItemVisitor visitor) {
		// would be nice to filter items based on search envelope, but can't until they
		// contain an
		// envelope
		synchronized (items) {
