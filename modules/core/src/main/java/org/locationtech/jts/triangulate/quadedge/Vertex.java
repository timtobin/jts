...
/**
 * Models a site (node) in a {@link QuadEdgeSubdivision}. The sites can be points on a line string
 * representing a linear site.
 *
 * <p>The vertex can be considered as a vector with a norm, length, inner product, cross product,
 * etc. Additionally, point relations (e.g., is a point to the left of a line, the circle defined by
 * this point and two others, etc.) are also defined in this class.
 *
 * <p>It is common to want to attach user-defined data to the vertices of a subdivision. One way to
 * do this is to subclass <tt>Vertex</tt> to carry any desired information.
 *
 * @author David Skea
 * @author Martin Davis
 */
public class Vertex {
  ...
  /**
   * Tests if the vertex is inside the circle defined by the triangle with vertices a, b, c
   ...
   * given tolerance B, the associated triangle is considered skinny. For an equal lateral triangle
   * this value is 0.57735. The ratio is related to the minimum triangle angle theta by:
  ...
  /**
   * For this vertex enclosed in a triangle defined by three vertices v0, v1 and v2, interpolate a z
   * value from the surrounding vertices.
   */
  ...
  /**
   * Interpolates the Z-value (height) of a point enclosed in a triangle whose vertices all have Z
   * values. The containing triangle must not be degenerate (in other words, the three vertices must
   * enclose a non-zero area).
   *
