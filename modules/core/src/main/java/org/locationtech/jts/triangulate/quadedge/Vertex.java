 ...
 * points on a line string representing a linear site.
 *
 * <p>
 ...
 *
 * <p>
 * It is common to want to attach user-defined data to the vertices of a
 * subdivision. One way to do this is to subclass <tt>Vertex</tt> to carry any
 * desired information.
 *
 * @author David Skea
 * @author Martin Davis
...
public class Vertex {
	...
	/**
	 * Interpolates the Z-value (height) of a point enclosed in a triangle whose
	 * vertices all have Z values. The containing triangle must not be degenerate
	 * (in other words, the three vertices must enclose a non-zero area).
	 *
	 * @param p
	 *            the point to interpolate the Z value of
	 * @param v0
	 *            a vertex of a triangle containing the p
	 * @param v1
	 *            a vertex of a triangle containing the p
	 * @param v2
	 *            a vertex of a triangle containing the p
	 ...
	 * skinny. For an equal lateral triangle this value is 0.57735. The ratio is
	 * related to the minimum triangle angle theta by: circumRadius/shortestEdge =
	 * 1/(2sin(theta)).
	 *
	 * @param b
	...
	/**
	 * For this vertex enclosed in a triangle defined by three vertices v0, v1 and
	 * v2, interpolate a z value from the surrounding vertices.
	 */
	...
	public final boolean isCCW(Vertex b, Vertex c) {
		/*
		 * // test code used to check for robustness of triArea boolean isCCW = (b.p.x -
		 * p.x) * (c.p.y - p.y) - (b.p.y - p.y) * (c.p.x - p.x) > 0; //boolean isCCW =
		 * triArea(this, b, c) > 0; boolean isCCWRobust =
		 * CGAlgorithms.orientationIndex(p, b.p, c.p) == CGAlgorithms.COUNTERCLOCKWISE;
		 * if (isCCWRobust != isCCW) System.out.println("CCW failure"); //
		 */
	...
	/**
	 * Tests if the vertex is inside the circle defined by the triangle with
	 * vertices a, b, c (oriented counter-clockwise).
	 *
	 * @param a
