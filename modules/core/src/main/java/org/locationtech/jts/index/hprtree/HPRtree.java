...
/**
 * A Hilbert-Packed R-tree. This is a static R-tree which is packed by using the
 * Hilbert ordering of the tree items.
 *
 * <p>
 ...
 *
 * <ul>
 * <li>The items/nodes of the previous are partitioned into blocks of size
 * <code>nodeCapacity
 *       </code>
 * <li>For each block a layer node is created with range equal to the envelope
 * of the items/nodess in the block
 * </ul>
 *
 * The internal layers are stored using an array to store the node bounds. The
 * link between a node and its children is stored implicitly in the indexes of
 * the array. For efficiency, the offsets to the layers within the node array
 * are pre-computed and stored.
 *
 * <p>
 * NOTE: Based on performance testing, the HPRtree is somewhat faster than the
 * STRtree. It should also be more memory-efficent, due to fewer object
 * allocations. However, it is not clear whether this will produce a significant
 * improvement for use in JTS operations.
 *
 * @see STRtree
 * @author Martin Davis
 */
public class HPRtree implements SpatialIndex {
	...
	private static boolean intersects(double[] bounds, int nodeIndex, Envelope env) {
		boolean isBeyond = (env.getMaxX() < bounds[nodeIndex]) || (env.getMaxY() < bounds[nodeIndex + 1])
				|| (env.getMinX() > bounds[nodeIndex + 2]) || (env.getMinY() > bounds[nodeIndex + 3]);
		return !isBeyond;
	}
	...
	private void computeNodeBounds(int nodeIndex, int blockStart, int nodeMaxIndex) {
		for (int i = 0; i <= nodeCapacity; i++) {
			...
			if (index >= nodeMaxIndex)
				break;
			updateNodeBounds(nodeIndex, nodeBounds[index], nodeBounds[index + 1], nodeBounds[index + 2],
					nodeBounds[index + 3]);
		}
	}
	...
	public Envelope[] getBounds() {
		...
		for (int i = numNodes - 1; i >= 0; i--) {
			int boundIndex = 4 * i;
			bounds[i] = new Envelope(nodeBounds[boundIndex], nodeBounds[boundIndex + 2], nodeBounds[boundIndex + 1],
