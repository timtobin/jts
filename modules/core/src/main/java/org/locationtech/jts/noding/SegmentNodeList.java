...
public class SegmentNodeList {
	...
	public void addSplitEdges(Collection edgeList) {
		...
		// there should always be at least two entries in the list, since the endpoints
		// are nodes
	...
	/**
	 * Extracts the points for a split edge running between two nodes. The extracted
	 * points should contain no duplicate points. There should always be at least
	 * two points extracted (which will be the given nodes).
	 *
	 * @param ei0
	 *            the start node of the split edge
	 * @param ei1
	 *            the end node of the split edge
	...
	private Coordinate[] createSplitEdgePts(SegmentNode ei0, SegmentNode ei1) {
		...
		/**
		 * If the last intersection point is not equal to the its segment start pt, add
	...
	/**
	 * Gets the list of coordinates for the fully noded segment string, including
	 * all original segment string vertices and vertices introduced by nodes in this
	 * list. Repeated coordinates are collapsed.
	 *
