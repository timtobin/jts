...
public class SegmentNodeList {
  ...
  public SegmentNode add(Coordinate intPt, int segmentIndex) {
    SegmentNode eiNew =
        new SegmentNode(edge, intPt, segmentIndex, edge.getSegmentOctant(segmentIndex));
  ...
  private Coordinate[] createSplitEdgePts(SegmentNode ei0, SegmentNode ei1) {
     ...
     *
     * <p>Also ensure that the created edge always has at least 2 points.
     *
     * <p>The check for point equality is 2D only - Z values are ignored
     */
...
class NodeVertexIterator implements Iterator {
  ...
  private void readNextNode() {
    if (nodeIt.hasNext()) nextNode = (SegmentNode) nodeIt.next();
    else nextNode = null;
  }
