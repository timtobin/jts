...
class Rectangle {
   ...
   * forming the opposite, left and right sides of the rectangle. The base segment and side points
   * must be presented so that the rectangle has CW orientation.
   *
   * <p>The rectangle corners are computed as intersections of lines, which generally cannot produce
   * exact values. If a rectangle corner is determined to coincide with a side point the side point
   * value is used to avoid numerical inaccuracy.
   *
   * <p>The first side of the constructed rectangle contains the base segment.
   *
   ...
   */
  public static Polygon createFromSidePts(
    ...
    /**
     * Corners of rectangle are the intersections of the base and opposite, and left and right
     * lines. The rectangle is constructed with CW orientation. The first side of the constructed
     * rectangle contains the base segment.
     *
     * <p>If a corner coincides with a input point the exact value is used to avoid numerical
     * inaccuracy.
     */
    Coordinate p0 =
        rightSidePt.equals2D(baseRightPt)
            ? baseRightPt.copy()
    ...
    Coordinate p3 =
        rightSidePt.equals2D(oppositePt) ? oppositePt.copy() : oppLine.lineIntersection(rightLine);
  ...
  /**
   * Computes the constant C in the standard line equation Ax + By = C from A and B and a point on
   * the line.
   *
