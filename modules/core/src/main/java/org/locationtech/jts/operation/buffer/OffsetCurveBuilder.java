 ...
 *
 * <p>
 * Note: this may not produce correct results if the input contains repeated or
 * invalid points. Repeated points should be removed before calling. See
 * {@link CoordinateArrays#removeRepeatedOrInvalidPoints(Coordinate[])}.
 *
 * @version 1.7
 */
public class OffsetCurveBuilder {
	 ...
	 * assumed <b>not</b> to be closed (the function will not fail for closed lines,
	 * but will generate superfluous line caps).
	 *
	 * @param inputPts
	 *            the vertices of the line to offset
	 * @param distance
	 *            the offset distance
	 * @return a Coordinate array representing the curve or null if the curve is
	 *         empty
	 */
	 ...
	 * @param side
	 *            side the side {@link Position} of the ring on which to construct
	 *            the buffer line
	 * @param distance
	 *            the positive distance at which to create the offset
	 * @return a Coordinate array representing the curve, or null if the curve is
	 *         empty
	 */
	...
	/**
	 * Tests whether the offset curve for line or point geometries at the given
	 * offset distance is empty (does not exist). This is the case if:
	 *
	 * <ul>
