...
/**
 * Computes the raw offset curve for a single {@link Geometry} component (ring, line or point). A
 * raw offset curve line is not noded - it may contain self-intersections (and usually will). The
 * final buffer polygon is computed by forming a topological graph of all the noded raw curves and
 * tracing outside contours. The points in the raw curve are rounded to a given {@link
 * PrecisionModel}.
 *
 * <p>Note: this may not produce correct results if the input contains repeated or invalid points.
 * Repeated points should be removed before calling. See {@link
 * CoordinateArrays#removeRepeatedOrInvalidPoints(Coordinate[])}.
 *
 * @version 1.7
 */
public class OffsetCurveBuilder {
   ...
   * empty (does not exist). This is the case if:
   *
   * <ul>
  ...
  private void computeSingleSidedBufferCurve(
      Coordinate[] inputPts, boolean isRightSide, OffsetSegmentGenerator segGen) {
    double distTol = simplifyTolerance(distance);
  ...
  private void computeOffsetCurve(
      Coordinate[] inputPts, boolean isRightSide, OffsetSegmentGenerator segGen) {
  ...
  private void computeRingBufferCurve(
      Coordinate[] inputPts, int side, OffsetSegmentGenerator segGen) {
