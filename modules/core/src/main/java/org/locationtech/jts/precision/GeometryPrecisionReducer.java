 ...
 *
 * <p>By default the geometry precision model is not changed. This can be overridden by using {@link
 * #setChangePrecisionModel(boolean)}.
 *
 * <h4>Topological Precision Reduction</h4>
 *
 * The default mode of operation ensures the reduced result is topologically valid (i.e. {@link
 * Geometry#isValid()} is true). To ensure this polygonal geometry is reduced in a topologically
 * valid fashion (technically, by using snap-rounding). Note that this may change polygonal geometry
 * structure (e.g. two polygons separated by a distance below the specified precision will be merged
 * into a single polygon). Duplicate vertices are removed. This mode is invoked by the static method
 * {@link #reduce(Geometry, PrecisionModel)}.
 *
 * <p>Normally, collapsed linear components (e.g. lines collapsing to a point) are not included in
 * the result. This behavior can be changed by setting {@link
 * #setRemoveCollapsedComponents(boolean)} to <code>false</code>, or by using the static method
 * {@link #reduceKeepCollapsed(Geometry, PrecisionModel)}.
 *
 * <p>In general input must be valid geometry, or an {@link IllegalArgumentException} will be
 * thrown. However if the invalidity is "mild" or very small then it may be eliminated by precision
 * reduction.
 *
 * <h4>Pointwise Precision Reduction</h4>
 *
 * Alternatively, geometry can be reduced pointwise by using {@link #setPointwise(boolean)}. Linear
 * and point geometry are always reduced pointwise (i.e. without further change to topology or
 * structure), since this does not change validity. Invalid inputs are allowed. Duplicate vertices
 * are preserved. Collapsed components are always included in the result. The result geometry may be
 * invalid.
 *
 * <p>This mode is invoked by the static method {@link #reducePointwise(Geometry, PrecisionModel)}.
 *
 * @version 1.12
 */
public class GeometryPrecisionReducer {
  /**
   * Reduces precision of a geometry, ensuring output geometry is valid. Collapsed linear and
   * polygonal components are removed. Duplicate vertices are removed. The geometry precision model
   * is not changed.
   *
   * <p>Invalid input geometry may cause an error, unless the invalidity is below the scale of the
   * precision reduction.
   *
  ...
  /**
   * Reduces precision of a geometry, ensuring output polygonal geometry is valid, and preserving
   * collapsed linear elements. Duplicate vertices are removed. The geometry precision model is not
   * changed.
   *
   * <p>Invalid input geometry may cause an error, unless the invalidity is below the scale of the
   * precision reduction.
  ...
  /**
   * Reduce precision of a geometry in a pointwise way. All input geometry elements are preserved in
   * the output, including invalid polygons and collapsed polygons and linestrings. The output may
   * not be valid, due to collapse or self-intersection. Duplicate vertices are not removed. The
   * geometry precision model is not changed.
   *
   * <p>Invalid input geometry is allowed.
   *
   * @param g the geometry to reduce
   * @param precModel the precision model to use
   * @return the reduced geometry
   ...
   * {@link PrecisionModel} supplied to specify the precision reduction.
   *
   * <p>The default is to <b>not</b> change the precision model
   *
   * @param changePrecisionModel if <code>true</code> the precision model of the created Geometry
  ...
  private GeometryFactory createFactory(GeometryFactory inputFactory, PrecisionModel pm) {
    GeometryFactory newFactory =
        new GeometryFactory(
            pm, inputFactory.getSRID(), inputFactory.getCoordinateSequenceFactory());
    return newFactory;
  }
}
