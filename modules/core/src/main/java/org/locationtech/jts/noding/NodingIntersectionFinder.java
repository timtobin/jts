...
/**
 * Finds non-noded intersections in a set of {@link SegmentString}s, if any
 * exist.
 *
 * <p>
 ...
 * <li><b>Vertex intersections</b> which occur at vertices in the interior of
 * {@link SegmentString}s (with a segment string endpoint or with another
 * interior vertex)
 * </ul>
 *
 * The finder can be limited to finding only interior intersections by setting
 * {@link #setInteriorIntersectionsOnly(boolean)}.
 *
 * <p>
 * By default only the first intersection is found, but all can be found by
 * setting {@link #setFindAllIntersections(boolean)}
 *
 * @version 1.7
 */
public class NodingIntersectionFinder implements SegmentIntersector {
	/**
	 * Creates a finder which finds all intersections. The intersections are
	...
	/**
	 * Creates a finder which tests if there is at least one intersection. Uses
	 * short-circuiting for efficient performance. The intersection found is
	 * recorded.
	 *
	 * @param li
	 *            a line intersector
	...
	/**
	 * Creates an finder which counts all interior intersections. The intersections
	 * are note recorded to reduce memory usage.
	 *
	 * @param li
	 *            a line intersector
	...
	/**
	 * Creates a finder which finds all interior intersections. The intersections
	...
	/**
	 * Creates an finder which counts all intersections. The intersections are note
	...
	/**
	 * Tests whether a segment in a {@link SegmentString} is an end segment. (either
	 * the first or last).
	 *
	 * @param segStr
	 *            a segment string
	...
	/**
	 * Tests if an intersection occurs between a segmentString interior vertex and
	 * another vertex. Note that intersections between two endpoint vertices are
	 * valid noding, and are not flagged.
	 *
	 * @param p00
	 *            a segment vertex
	 * @param p01
	 ...
	 */
	private static boolean isInteriorVertexIntersection(Coordinate p00, Coordinate p01, Coordinate p10, Coordinate p11,
			boolean isEnd00, boolean isEnd01, boolean isEnd10, boolean isEnd11) {
	...
	/**
	 * Tests if two vertices with at least one in a segmentString interior are
	 * equal.
	 *
	 * @param p0
	 *            a segment vertex
	...
	/**
	 * Sets whether only end segments should be tested for intersection. This is a
	 * performance optimization that may be used if the segments have been
	 * previously noded by an appropriate algorithm. It may be known that any
	 ...
	 * <code>false</code> (the default value) the value of {@link #isDone()} is
	 * <code>true</code> after the first intersection is found.
	 *
	 * <p>
	...
	/**
	 * Sets whether intersection points are recorded. If the only need is to count
	 * intersection points, this can be set to <code>false</code>.
	 *
	 * <p>
