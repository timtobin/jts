 ...
 *
 * <p>Non-noded intersections include:
 *
 * <ul>
 *   <li><b>Interior intersections</b> which lie in the interior of a segment (with another segment
 *       interior or with a vertex or endpoint)
 *   <li><b>Vertex intersections</b> which occur at vertices in the interior of {@link
 *       SegmentString}s (with a segment string endpoint or with another interior vertex)
 * </ul>
 *
 * The finder can be limited to finding only interior intersections by setting {@link
 * #setInteriorIntersectionsOnly(boolean)}.
 *
 * <p>By default only the first intersection is found, but all can be found by setting {@link
...
public class NodingIntersectionFinder implements SegmentIntersector {
   ...
   * points, this can be set to <code>false</code>.
   *
   ...
   */
  private static boolean isInteriorVertexIntersection(
      Coordinate p0, Coordinate p1, boolean isEnd0, boolean isEnd1) {
