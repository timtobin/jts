 ...
 *
 * <p>The <b>Minimum Clearance</b> is a measure of what magnitude of perturbation of the vertices of
 * a geometry can be tolerated before the geometry becomes topologically invalid. The smaller the
 * Minimum Clearance distance, the less vertex perturbation the geometry can tolerate before
 * becoming invalid.
 *
 * <p>The concept was introduced by Thompson and Van Oosterom [TV06], based on earlier work by
 * Milenkovic [Mi88].
 *
 * <p>The Minimum Clearance of a geometry G is defined to be the value <i>r</i> such that "the
 * movement of all points by a distance of <i>r</i> in any direction will guarantee to leave the
 * geometry valid" [TV06]. An equivalent constructive definition [Mi88] is that <i>r</i> is the
 * largest value such:
 *
 * <ol>
 *   <li>No two distinct vertices of G are closer than <i>r</i>
 *   <li>No vertex of G is closer than <i>r</i> to an edge of G of which the vertex is not an
 *       endpoint
 * </ol>
 *
 * The following image shows an example of the Minimum Clearance of a simple polygon.
 *
 * <p><center><img src='doc-files/minClearance.png' alt='minimum clearance'></center>
 *
 * <p>If G has only a single vertex (i.e. is a {@link Point}), the value of the minimum clearance is
 * {@link Double#MAX_VALUE}.
 *
 * <p>If G is a {@link Puntal} or {@link Lineal} geometry, then in fact no amount of perturbation
 * will render the geometry invalid. In this case a Minimum Clearance is still computed based on the
 * vertex and segment distances according to the constructive definition.
 *
 * <p>It is possible for no Minimum Clearance to exist. For instance, a {@link MultiPoint} with all
 * members identical has no Minimum Clearance (i.e. no amount of perturbation will cause the member
 * points to become non-identical). Empty geometries also have no such distance. The lack of a
 * meaningful MinimumClearance distance is detected and suitable values are returned by {@link
 * #getDistance()} and {@link #getLine()}.
 *
 * <p>The computation of Minimum Clearance utilizes the {@link
 * STRtree#nearestNeighbour(ItemDistance)} method to provide good performance even for large inputs.
 *
 * <p>An interesting note is that for the case of {@link MultiPoint}s, the computed Minimum
 * Clearance line effectively determines the Nearest Neighbours in the collection.
 *
 * <h3>References</h3>
 *
 * <ul>
 *   <li>[Mi88] Milenkovic, V. J., <i>Verifiable implementations of geometric algorithms using
 *       finite precision arithmetic</i>. in Artificial Intelligence, 377-401. 1988
 *   <li>[TV06] Thompson, Rod and van Oosterom, Peter, <i>Interchange of Spatial Data-Inhibiting
 *       Factors</i>, Agile 2006, Visegrad, Hungary. 2006
 * </ul>
 *
 * @author Martin Davis
 */
public class MinimumClearance {
  ...
  /**
   * Gets a LineString containing two points which are at the Minimum Clearance distance for the
   * given Geometry.
   *
   * @param g the input geometry
   * @return the value of the minimum clearance distance or <tt>LINESTRING EMPTY</tt> if no Minimum
   *     Clearance distance exists
   */
   ...
   *
   * <p>If no distance exists (e.g. in the case of two identical points) <tt>Double.MAX_VALUE</tt>
   * is returned.
   *
   * @return the value of the minimum clearance distance or <tt>Double.MAX_VALUE</tt> if no Minimum
   *     Clearance distance exists
   */
   ...
   *
   * <p>If no distance could be found (e.g. in the case of two identical points) <tt>LINESTRING
   * EMPTY</tt> is returned.
   *
   * @return the value of the minimum clearance distance or <tt>LINESTRING EMPTY</tt> if no Minimum
   *     Clearance distance exists
   */
  ...
  private void compute() {
    ...
    minClearance = mcd.distance((FacetSequence) nearest[0], (FacetSequence) nearest[1]);
