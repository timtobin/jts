...
public class CoordinateArrays {
	 ...
	 * into a destination Coordinate array. The destination array must be an
	...
	/**
	 * Utility method ensuring array contents are of consistent dimension and
	 * measures.
	 *
	 * <p>
	 * Array is modified in place if required, coordinates are replaced in the array
	 * as required to ensure all coordinates have the same dimension and measures.
	 * The final dimension and measures used are the maximum found when checking the
	 * array.
	 *
	 * @param array
	 *            Modified in place to coordinates of consistent dimension and
	 *            measures.
	 */
	...
	/**
	 * Utility method ensuring array contents are of the specified dimension and
	 * measures.
	 *
	 * <p>
	 * Array is returned unmodified if consistent, or a copy of the array is made
	 * with each inconsistent coordinate duplicated into an instance of the correct
	 ...
	 * (as compared using Coordinate#equals)
	 *
	...
	/**
	 * Returns true if the two arrays are identical, both null, or pointwise equal,
	 * using a user-defined {@link Comparator} for {@link Coordinate} s
	 *
	 * @param coord1
	 *            an array of Coordinates
	 * @param coord2
	 ...
	 * <code>start</code> to <code>end</code> (inclusive). The input indices are
	 * clamped to the array size; If the end index is less than the start index, the
	 * extracted array will be empty.
	 *
	 * @param pts
	 *            the input array
	 * @param start
	...
	/**
	 * Tests whether {@link Coordinate#equals(Object)} returns true for any two
	 * consecutive Coordinates in the given array.
	 *
	 * @param coord
	 *            an array of coordinates
	...
	/**
	 * Returns the index of <code>coordinate</code> in <code>coordinates</code>. The
	 * first position is 0; the second, 1; etc.
	 *
	 * @param coordinate
	...
	/**
	 * Determines whether two {@link Coordinate} arrays of equal length are equal in
	 * opposite directions.
	 *
	 * @param pts1
	 * @param pts2
	 ...
	 * length and closure. Self-intersection is not checked.
	 *
	 * @param pts
	 *            an array of Coordinates
	...
	/**
	 * Orients an array of points to have a specified orientation. The array is not
	 * copied if it already has the required orientation. The points are not copied.
	 *
	 * @param pts
	 *            the array to orient
	 * @param orientCW
	 *            true if the points should be oriented CVW
	 ...
	 * @param pts
	 *            an array of {@link Coordinate}s to test the input points against
	...
	public static Coordinate[] removeNull(Coordinate[] coord) {
		...
		for (Coordinate coordinate : coord) {
			if (coordinate != null)
				newCoord[j++] = coordinate;
		}
		return newCoord;
	}

	/**
	 * If the coordinate array argument has repeated or invalid points, constructs a
	 * new array containing no repeated points. Otherwise, returns the argument.
	 *
	 * @param coord
	 *            an array of coordinates
	...
	public static Coordinate[] removeRepeatedOrInvalidPoints(Coordinate[] coord) {
		if (!hasRepeatedOrInvalidPoints(coord))
			return coord;
		CoordinateList coordList = new CoordinateList();
		for (Coordinate coordinate : coord) {
	...
	/**
	 * Shifts the positions of the coordinates until <code>firstCoordinate</code> is
	 * first.
	 *
	 * @param coordinates
	...
	/**
	 * Shifts the positions of the coordinates until the coordinate at
	 * <code>indexOfFirstCoordinate
	 * </code> is first.
	 *
	 * <p>
	 * If {@code ensureRing} is {@code true}, first and last coordinate of the
	 * returned array are equal.
	 *
	 * @param coordinates
	 *            the array to rearrange
	 * @param indexOfFirstCoordinate
	...
	/**
	 * A {@link Comparator} for {@link Coordinate} arrays in the forward direction
	 * of their coordinates, using lexicographic ordering.
	 */
