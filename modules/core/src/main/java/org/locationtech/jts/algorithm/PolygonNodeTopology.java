...
/**
 * Functions to compute topological information about nodes (ring intersections)
 * in polygonal geometry.
 *
 * @author mdavis
 */
public class PolygonNodeTopology {
	/**
	 * Compares the angles of two vectors relative to the positive X-axis at their
	 * origin. Angles increase CCW from the X-axis.
	 *
	 * @param origin
	 *            the origin of the vectors
	 * @param p
	 *            the endpoint of the vector P
	 * @param q
	 *            the endpoint of the vector Q
	 * @return a negative integer, zero, or a positive integer as this vector P has
	...
	/**
	 * Compares whether an edge p is between or outside the edges e0 and e1, where
	 * the edges all originate at a common origin. The "inside" of e0 and e1 is the
	 * arc which does not include the positive X-axis at the origin. If p is
	 * collinear with an edge 0 is returned.
	 *
	 * @param origin
	 *            the origin
	 * @param p
	 *            the destination point of edge p
	 * @param e0
	 *            the destination point of edge e0
	 * @param e1
	 *            the destination point of edge e1
	 * @return a negative integer, zero or positive integer as the vector P lies
	...
	/**
	 * Tests if an edge p is between edges e0 and e1, where the edges all originate
	 * at a common origin. The "inside" of e0 and e1 is the arc which does not
	 * include the origin. The edges are assumed to be distinct (non-collinear).
	 *
	 * @param origin
	 *            the origin
	 * @param p
	...
	/**
	 * Check if four segments at a node cross. Typically the segments lie in two
	 * different rings, or different sections of one ring. The node is topologically
	 * valid if the rings do not cross. If any segments are collinear, the test
	 * returns false.
	 *
	 * @param nodePt
	 *            the node location
	 * @param a0
	 *            the previous segment endpoint in a ring
	 * @param a1
	 *            the next segment endpoint in a ring
	 * @param b0
	 *            the previous segment endpoint in the other ring
	 * @param b1
	 *            the next segment endpoint in the other ring
	...
	public static boolean isCrossing(Coordinate nodePt, Coordinate a0, Coordinate a1, Coordinate b0, Coordinate b1) {
		...
		/*
		 * boolean isBetween0 = isBetween(nodePt, b0, aLo, aHi); boolean isBetween1 =
		 * isBetween(nodePt, b1, aLo, aHi);
		 *
		 * return isBetween0 != isBetween1;
		 */

		/**
		 * Find positions of b0 and b1. The edges cross if the positions are different.
	...
	/**
	 * Tests whether an segment node-b lies in the interior or exterior of a corner
	 * of a ring formed by the two segments a0-node-a1. The ring interior is assumed
	 * to be on the right of the corner (i.e. a CW shell or CCW hole). The test
	 * segment must not be collinear with the corner segments.
	 *
	 * @param nodePt
	 *            the node location
	 * @param a0
	 *            the first vertex of the corner
	 * @param a1
	 *            the second vertex of the corner
	 * @param b
	 *            the other vertex of the test segment
