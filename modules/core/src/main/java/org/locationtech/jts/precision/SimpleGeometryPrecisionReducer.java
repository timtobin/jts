...
/**
 * Reduces the precision of the coordinates of a {@link Geometry} according to
 * the supplied {@link PrecisionModel}, without attempting to preserve valid
 * topology.
 *
 * <p>
 * In the case of {@link Polygonal} geometries, the topology of the resulting
 * geometry may be invalid if topological collapse occurs due to coordinates
 * being shifted. It is up to the client to check this and handle it if
 * necessary. Collapses may not matter for some uses. An example is simplifying
 * the input to the buffer algorithm. The buffer algorithm does not depend on
 * the validity of the input geometry.
 *
 * @version 1.7
...
public class SimpleGeometryPrecisionReducer {
	/**
	 * Convenience method for doing precision reduction on a single geometry, with
	 * collapses removed and keeping the geometry precision model the same.
	 *
	 * @param g
	 * @param precModel
