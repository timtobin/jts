...
/**
 * Reduces the precision of the coordinates of a {@link Geometry} according to the supplied {@link
 * PrecisionModel}, without attempting to preserve valid topology.
 *
 * <p>In the case of {@link Polygonal} geometries, the topology of the resulting geometry may be
 * invalid if topological collapse occurs due to coordinates being shifted. It is up to the client
 * to check this and handle it if necessary. Collapses may not matter for some uses. An example is
 * simplifying the input to the buffer algorithm. The buffer algorithm does not depend on the
 * validity of the input geometry.
 *
 * @version 1.7
...
public class SimpleGeometryPrecisionReducer {
  ...
  public Geometry reduce(Geometry geom) {
    ...
    if (changePrecisionModel) {
      GeometryFactory newFactory =
          new GeometryFactory(newPrecisionModel, geom.getFactory().getSRID());
  ...
  private class PrecisionReducerCoordinateOperation extends GeometryEditor.CoordinateOperation {
    public Coordinate[] edit(Coordinate[] coordinates, Geometry geom) {
       ...
       * length for the type of the parent geometry. It is not necessary to check for Point
       * collapses, since the coordinate list can never collapse to less than one point. If the
       * length is invalid, return the full-length coordinate array first computed, or null if
       * collapses are being removed. (This may create an invalid geometry - the client must handle
       * this.)
       */
      int minLength = 0;
