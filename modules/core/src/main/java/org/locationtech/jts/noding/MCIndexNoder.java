...
/**
 * Nodes a set of {@link SegmentString}s using a index based on
 * {@link MonotoneChain}s and a {@link SpatialIndex}. The {@link SpatialIndex}
 * used should be something that supports envelope (range) queries efficiently
 * (such as a <code>Quadtree</code>} or {@link HPRtree} (which is the default
 * index provided).
 *
 * <p>
...
public class MCIndexNoder extends SinglePassNoder {
	...
	/**
	 * Creates a new noder with a given {@link SegmentIntersector} and an overlap
	 * tolerance distance to expand intersection tests with.
	 *
	 * @param si
	 *            the segment intersector
