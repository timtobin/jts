...
/**
 * Computes an offset curve from a geometry. An offset curve is a linear geometry which is offset a
 * given distance from the input. If the offset distance is positive the curve lies on the left side
 * of the input; if it is negative the curve is on the right side. The curve(s) have the same
 * direction as the input line(s). The result for a zero offset distance is a copy of the input
 * linework.
 *
 * <p>The offset curve is based on the boundary of the buffer for the geometry at the offset
 * distance (see {@link BufferOp}. The normal mode of operation is to return the sections of the
 * buffer boundary which lie on the raw offset curve (obtained via {@link #rawOffset(LineString,
 * double)}. The offset curve will contain multiple sections if the input self-intersects or has
 * close approaches. The computed sections are ordered along the raw offset curve. Sections are
 * disjoint. They never self-intersect, but may be rings.
 *
 * <ul>
 *   <li>For a {@link LineString} the offset curve is a linear geometry ({@link LineString} or
 *       {@link MultiLineString}).
 *   <li>For a {@link Point} or {@link MultiPoint} the offset curve is an empty {@link LineString}.
 *   <li>For a {@link Polygon} the offset curve is the boundary of the polygon buffer (which may be
 *       a {@link MultiLineString}.
 *   <li>For a collection the output is a {@link MultiLineString} containing the offset curves of
 *       the elements.
 * </ul>
 *
 * In "joined" mode (see {@link #setJoined(boolean)} the sections computed for each input line are
 * joined into a single offset curve line. The joined curve may self-intersect. At larger offset
 * distances the curve may contain "flat-line" artifacts in places where the input self-intersects.
 *
 * <p>Offset curves support setting the number of quadrant segments, the join style, and the mitre
 * limit (if applicable) via the {@link BufferParameters}.
 *
 * @author Martin Davis
 */
public class OffsetCurve {
  ...
  /**
   * A QuadSegs minimum value that will prevent generating unwanted offset curve artifacts near end
   * caps.
   */
  ...
  /**
   * Computes the offset curve of a geometry at a given distance, with specified quadrant segments,
   * join style and mitre limit.
   *
   * @param geom a geometry
   * @param distance the offset distance (positive for left, negative for right)
   ...
   * @return the offset curve
   */
  public static Geometry getCurve(
  ...
  /**
   * Computes the offset curve of a geometry at a given distance, joining curve sections into a
   * single line for each input line.
   *
   * @param geom a geometry
   * @param distance the offset distance (positive for left, negative for right)
  ...
  /**
   * Creates a new instance for computing an offset curve for a geometry at a given distance. with
   * default quadrant segments ({@link BufferParameters#DEFAULT_QUADRANT_SEGMENTS}) and join style
   ...
   * setting the quadrant segments and join style and mitre limit via {@link BufferParameters}.
   *
   * @param geom the geometry to offset
   * @param distance the offset distance (positive for left, negative for right)
  ...
  public OffsetCurve(Geometry geom, double distance, BufferParameters bufParams) {
    ...
    if (bufParams != null) {
      /**
       * Prevent using a very small QuadSegs value, to avoid offset curve artifacts near the end
       * caps.
       */
  ...
  /**
   * Computes a single curve line for each input linear component, by joining curve sections in
   * order along the raw offset curve. The default mode is to compute separate curve sections.
   *
  ...
  private Geometry computePolygonCurve(Polygon poly, double distance) {
    Geometry buffer;
    if (bufferParams == null) buffer = BufferOp.bufferOp(poly, distance);
    else {
  ...
  /**
   * Gets the raw offset curve for a line at a given distance. The quadrant segments, join style and
   * mitre limit can be specified via {@link BufferParameters}.
   *
   * <p>The raw offset line may contain loops and other artifacts which are not present in the true
   * offset curve.
   *
   ...
   */
  public static Coordinate[] rawOffset(
      LineString line, double distance, BufferParameters bufParams) {
    Coordinate[] pts = line.getCoordinates();
    Coordinate[] cleanPts = CoordinateArrays.removeRepeatedOrInvalidPoints(pts);
    OffsetCurveBuilder ocb =
        new OffsetCurveBuilder(line.getFactory().getPrecisionModel(), bufParams);
  ...
  private List<OffsetCurveSection> computeSections(LineString lineGeom, double distance) {
    ...
    /**
     * Note: If the raw offset curve has no narrow concave angles or self-intersections it could be
     * returned as is. However, this is likely to be a less frequent situation, and testing
     * indicates little performance advantage, so not doing this.
     */
  ...
  private static Polygon getBufferOriented(
      LineString geom, double distance, BufferParameters bufParams) {
  ...
  /**
   * Extracts the largest polygon by area from a geometry. Used here to avoid issues with non-robust
   * buffer results which have spurious extra polygons.
   *
   * @param geom a geometry
   * @return the polygon element of largest area
   */
  private static Polygon extractMaxAreaPolygon(Geometry geom) {
    if (geom.getNumGeometries() == 1) return (Polygon) geom;
  ...
  private void computeCurveSections(
      Coordinate[] bufferRingPts, Coordinate[] rawCurve, List<OffsetCurveSection> sections) {
  ...
  /**
   * Matches the segments in a buffer ring to the raw offset curve to obtain their match positions
   * (if any).
   *
  ...
  private int matchSegments(
      ...
      Coordinate[] bufferPts,
      double[] rawCurvePos) {
    Envelope matchEnv = new Envelope(raw0, raw1);
    matchEnv.expandBy(matchDistance);
    MatchCurveSegmentAction matchAction =
        new MatchCurveSegmentAction(
            raw0, raw1, rawCurveIndex, matchDistance, bufferPts, rawCurvePos);
    bufferSegIndex.query(matchEnv, matchAction);
    return matchAction.getBufferMinIndex();
  }

  /**
   * An action to match a raw offset curve segment to segments in a buffer ring and record the
   * matched segment locations(s) along the raw curve.
   *
   * @author Martin Davis
   */
  private static class MatchCurveSegmentAction extends MonotoneChainSelectAction {
    ...
    public MatchCurveSegmentAction(
        Coordinate raw0,
        Coordinate raw1,
        int rawCurveIndex,
        double matchDistance,
        Coordinate[] bufferRingPts,
        double[] rawCurveLoc) {
      this.raw0 = raw0;
      this.raw1 = raw1;
    ...
    public void select(MonotoneChain mc, int segIndex) {
      /**
       * Generally buffer segments are no longer than raw curve segments, since the final buffer
       * line likely has node points added. So a buffer segment may match all or only a portion of a
       * single raw segment. There may be multiple buffer ring segs that match along the raw
       * segment.
       *
       * <p>HOWEVER, in some cases the buffer construction may contain a matching buffer segment
       * which is slightly longer than a raw curve segment. Specifically, at the endpoint of a
       * closed line with nearly parallel end segments - the closing fillet line is very short so is
       * heuristically removed in the buffer. In this case, the buffer segment must still be
       * matched. This produces closed offset curves, which is technically an anomaly, but only
       * happens in rare cases.
       */
      double frac =
          segmentMatchFrac(
              bufferRingPts[segIndex], bufferRingPts[segIndex + 1], raw0, raw1, matchDistance);
      // -- no match
      if (frac < 0) return;
    ...
    private double segmentMatchFrac(
  ...
  /**
   * This is only called when there is at least one ring segment matched (so rawCurvePos has at
   * least one entry != NOT_IN_CURVE). The start index of the first section must be provided. This
   * is intended to be the section with lowest position along the raw curve.
   *
  ...
  private int findSectionStart(double[] loc, int end) {
    ...
    do {
      ...
      if (isJoined) {
        /**
         * Start section at next gap in raw curve. Only needed for joined curve, since otherwise
  ...
  private int findSectionEnd(double[] loc, int start, int firstStartIndex) {
    ...
    do {
      ...
      if (isJoined) {
        /**
         * End section at gap in raw curve. Only needed for joined curve, since otherwise contigous
         * buffer segments can be in same section
         */
