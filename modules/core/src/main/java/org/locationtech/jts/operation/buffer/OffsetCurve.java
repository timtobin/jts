...
/**
 * Computes an offset curve from a geometry. An offset curve is a linear
 * geometry which is offset a given distance from the input. If the offset
 * distance is positive the curve lies on the left side of the input; if it is
 * negative the curve is on the right side. The curve(s) have the same direction
 * as the input line(s). The result for a zero offset distance is a copy of the
 * input linework.
 *
 * <p>
 * The offset curve is based on the boundary of the buffer for the geometry at
 * the offset distance (see {@link BufferOp}. The normal mode of operation is to
 * return the sections of the buffer boundary which lie on the raw offset curve
 * (obtained via {@link #rawOffset(LineString, double)}. The offset curve will
 * contain multiple sections if the input self-intersects or has close
 * approaches. The computed sections are ordered along the raw offset curve.
 ...
 * <li>For a {@link Point} or {@link MultiPoint} the offset curve is an empty
 * {@link LineString}.
 * <li>For a {@link Polygon} the offset curve is the boundary of the polygon
 * buffer (which may be a {@link MultiLineString}.
 * <li>For a collection the output is a {@link MultiLineString} containing the
 * offset curves of the elements.
 * </ul>
 *
 * In "joined" mode (see {@link #setJoined(boolean)} the sections computed for
 * each input line are joined into a single offset curve line. The joined curve
 * may self-intersect. At larger offset distances the curve may contain
 * "flat-line" artifacts in places where the input self-intersects.
 *
 * <p>
 * Offset curves support setting the number of quadrant segments, the join
 * style, and the mitre limit (if applicable) via the {@link BufferParameters}.
 *
 * @author Martin Davis
 */
public class OffsetCurve {
	...
	/**
	 * The nearness tolerance for matching the the raw offset linework and the
	 * buffer curve.
	 */
	private static final int MATCH_DISTANCE_FACTOR = 10000;

	/**
	 * A QuadSegs minimum value that will prevent generating unwanted offset curve
	 * artifacts near end caps.
	 */
	...
	/**
	 * Extracts the largest polygon by area from a geometry. Used here to avoid
	 * issues with non-robust buffer results which have spurious extra polygons.
	 *
	 * @param geom
	 *            a geometry
	...
	private static Polygon getBufferOriented(LineString geom, double distance, BufferParameters bufParams) {
		...
		// -- for negative distances (Right of input) reverse buffer direction to match
		// offset curve
		if (distance < 0) {
	 ...
	 *            a geometry
	 * @param distance
	 *            the offset distance (positive for left, negative for right)
	...
	/**
	 * Computes the offset curve of a geometry at a given distance, with specified
	 * quadrant segments, join style and mitre limit.
	 *
	 * @param geom
	 *            a geometry
	 * @param distance
	 *            the offset distance (positive for left, negative for right)
	 * @param quadSegs
	 *            the quadrant segments (-1 for default)
	 * @param joinStyle
	 *            the join style (-1 for default)
	 * @param mitreLimit
	 *            the mitre limit (-1 for default)
	 * @return the offset curve
	 */
	...
	/**
	 * Computes the offset curve of a geometry at a given distance, joining curve
	 * sections into a single line for each input line.
	 *
	 * @param geom
	 *            a geometry
	 * @param distance
	...
	/**
	 * Gets the raw offset curve for a line at a given distance, with default buffer
	 * parameters.
	 *
	 * @param line
	 *            the line to offset
	...
	/**
	 * Gets the raw offset curve for a line at a given distance. The quadrant
	 * segments, join style and mitre limit can be specified via
	 * {@link BufferParameters}.
	 *
	 * <p>
	 * The raw offset line may contain loops and other artifacts which are not
	 * present in the true offset curve.
	 *
	 * @param line
	 *            the line to offset
	 * @param distance
	 ...
	 *
	 * @param geom
	 *            a linear geometry which may be a LinearRing
	...
	/**
	 * Creates a new instance for computing an offset curve for a geometry at a
	 * given distance. with default quadrant segments
	 * ({@link BufferParameters#DEFAULT_QUADRANT_SEGMENTS}) and join style
	 * ({@link BufferParameters#JOIN_STYLE}).
	 *
	 * @param geom
	...
	/**
	 * Creates a new instance for computing an offset curve for a geometry at a
	 * given distance. setting the quadrant segments and join style and mitre limit
	...
	public OffsetCurve(Geometry geom, double distance, BufferParameters bufParams) {
		...
		if (bufParams != null) {
			/**
			 * Prevent using a very small QuadSegs value, to avoid offset curve artifacts
			 * near the end caps.
			 */
	...
	private void computeCurveSections(Coordinate[] bufferRingPts, Coordinate[] rawCurve,
			List<OffsetCurveSection> sections) {
		double[] rawPosition = new double[bufferRingPts.length - 1];
		Arrays.fill(rawPosition, NOT_IN_CURVE);
		...
		for (int i = 0; i < rawCurve.length - 1; i++) {
			int minBufferIndexForSeg = matchSegments(rawCurve[i], rawCurve[i + 1], i, bufferSegIndex, bufferRingPts,
					rawPosition);
	...
	private List<OffsetCurveSection> computeSections(LineString lineGeom, double distance) {
		...
		List<OffsetCurveSection> sections = new ArrayList<>();
		if (rawCurve.length == 0) {
			return sections;
		}

		/**
		 * Note: If the raw offset curve has no narrow concave angles or
		 * self-intersections it could be returned as is. However, this is likely to be
		 * a less frequent situation, and testing indicates little performance
		 * advantage, so not doing this.
		 */
	...
	/**
	 * This is only called when there is at least one ring segment matched (so
	 * rawCurvePos has at least one entry != NOT_IN_CURVE). The start index of the
	 * first section must be provided. This is intended to be the section with
	 * lowest position along the raw curve.
	 *
	 * @param ringPts
	 *            the points in a buffer ring
	 * @param rawCurveLoc
	 *            the position of buffer ring segments along the raw curve
	 * @param startIndex
	 *            the index of the start of a section
	 * @param sections
	 *            the list of extracted offset curve sections
	 */
	private void extractSections(Coordinate[] ringPts, double[] rawCurveLoc, int startIndex,
			List<OffsetCurveSection> sections) {
		...
		do {
			...
			OffsetCurveSection section = OffsetCurveSection.create(ringPts, sectionStart, sectionEnd, location,
					lastLoc);
	...
	private int findSectionEnd(double[] loc, int start, int firstStartIndex) {
		...
		do {
			...
			if (isJoined) {
				/**
				 * End section at gap in raw curve. Only needed for joined curve, since
				 * otherwise contigous buffer segments can be in same section
				 */
	...
	private int findSectionStart(double[] loc, int end) {
		...
		do {
			...
			if (isJoined) {
				/**
				 * Start section at next gap in raw curve. Only needed for joined curve, since
				 * otherwise contiguous buffer segments can be in same curve section.
				 */
	...
	/**
	 * Matches the segments in a buffer ring to the raw offset curve to obtain their
	 * match positions (if any).
	 *
	 * @param raw0
	 *            a raw curve segment start point
	 * @param raw1
	 *            a raw curve segment end point
	 * @param rawCurveIndex
	 *            the index of the raw curve segment
	 * @param bufferSegIndex
	 *            the spatial index of the buffer ring segments
	 * @param bufferPts
	 *            the points of the buffer ring
	 * @param rawCurvePos
	 *            the raw curve positions of the buffer ring segments
	 * @return the index of the minimum matched buffer segment
	 */
	private int matchSegments(Coordinate raw0, Coordinate raw1, int rawCurveIndex, SegmentMCIndex bufferSegIndex,
			Coordinate[] bufferPts, double[] rawCurvePos) {
		Envelope matchEnv = new Envelope(raw0, raw1);
		matchEnv.expandBy(matchDistance);
		MatchCurveSegmentAction matchAction = new MatchCurveSegmentAction(raw0, raw1, rawCurveIndex, matchDistance,
				bufferPts, rawCurvePos);
	...
	/**
	 * Computes a single curve line for each input linear component, by joining
	 * curve sections in order along the raw offset curve. The default mode is to
	 * compute separate curve sections.
	 *
	 * @param isJoined
	...
	/**
	 * An action to match a raw offset curve segment to segments in a buffer ring
	...
	private static class MatchCurveSegmentAction extends MonotoneChainSelectAction {
		...
		private final Coordinate[] bufferRingPts;
		private final double matchDistance;
		private double minRawLocation = -1;
		private final Coordinate raw0;
		private final Coordinate raw1;
		private final int rawCurveIndex;
		private final double[] rawCurveLoc;
		private final double rawLen;

		public MatchCurveSegmentAction(Coordinate raw0, Coordinate raw1, int rawCurveIndex, double matchDistance,
				Coordinate[] bufferRingPts, double[] rawCurveLoc) {
			this.raw0 = raw0;
			this.raw1 = raw1;
		...
		private boolean isMatch(Coordinate buf0, Coordinate buf1, Coordinate raw0, Coordinate raw1,
		...
		public void select(MonotoneChain mc, int segIndex) {
			/**
			 * Generally buffer segments are no longer than raw curve segments, since the
			 * final buffer line likely has node points added. So a buffer segment may match
			 * all or only a portion of a single raw segment. There may be multiple buffer
			 * ring segs that match along the raw segment.
			 *
			 * <p>
			 * HOWEVER, in some cases the buffer construction may contain a matching buffer
			 * segment which is slightly longer than a raw curve segment. Specifically, at
			 * the endpoint of a closed line with nearly parallel end segments - the closing
			 * fillet line is very short so is heuristically removed in the buffer. In this
			 * case, the buffer segment must still be matched. This produces closed offset
			 * curves, which is technically an anomaly, but only happens in rare cases.
			 */
			double frac = segmentMatchFrac(bufferRingPts[segIndex], bufferRingPts[segIndex + 1], raw0, raw1,
					matchDistance);
			// -- no match
			if (frac < 0)
				return;
