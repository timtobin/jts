...
/**
 * Finds coverage segments which occur in only a single coverage element. In a
 * valid coverage, these are exactly the line segments which lie on the boundary
 * of the coverage.
 *
 * <p>
 * In an invalid coverage, segments might occur in 3 or more elements. This
...
class CoverageBoundarySegmentFinder implements CoordinateSequenceFilter {
	...
	public void filter(CoordinateSequence seq, int i) {
		...
		LineSegment seg = createSegment(seq, i);
		/**
		 * Records segments with an odd number of occurrences. In a valid coverage each
		 * segment can occur only 1 or 2 times. This does not detect invalid situations,
		 * where a segment might occur 3 or more times.
		 */
