 ...
 * coordinates in any way. In particular, they are not rounded to the supplied
 * <tt>PrecisionModel</tt>. It is assumed that input Coordinates meet the given
 * precision.
 *
 * <p>
...
public class GeometryFactory implements Serializable {
	 ...
	 * CoordinateSequence implementation, a double-precision floating PrecisionModel
	...
	/**
	 * Constructs a GeometryFactory that generates Geometries having the given
	 * {@link PrecisionModel} and the default CoordinateSequence implementation.
	 *
	...
	/**
	 * Constructs a GeometryFactory that generates Geometries having the given
	 * {@link PrecisionModel} and spatial-reference ID, and the default
	 ...
	 * <code>
	 * GeometryCollection</code> to contain the <code>Geometry</code>s in it. For
	 * example:<br>
	 *
	 * <ul>
	 * <li>If <code>geomList</code> contains a single <code>Polygon</code>, the
	 * <code>Polygon</code> is returned.
	 * <li>If <code>geomList</code> contains several <code>Polygon</code>s, a
	 * <code>MultiPolygon
	 *       </code> is returned.
	 * <li>If <code>geomList</code> contains some <code>Polygon</code>s and some
	 * <code>LineString
	 ...
	 * @return a <code>Geometry</code> of the "smallest", "most type-specific" class
	 *         that can contain the elements of <code>geomList</code> .
	 */
	...
	/**
	 * Creates an empty atomic geometry of the given dimension. If passed a
	 * dimension of -1 will create an empty {@link GeometryCollection}.
	 *
	 * @param dimension
	 *            the required dimension (-1, 0, 1 or 2)
	...
	/**
	 * Creates a deep copy of the input {@link Geometry}. The
	 * {@link CoordinateSequenceFactory} defined for this factory is used to copy
	 * the {@link CoordinateSequence}s of the input geometry.
	 *
	 * <p>
	 * This is a convenient way to change the <tt>CoordinateSequence</tt> used to
	 * represent a geometry, or to change the factory used for a geometry.
	 *
	 * <p>
	 * {@link Geometry#copy()} can also be used to make a deep copy, but it does not
	 * allow changing the CoordinateSequence type.
	 *
	 * @return a deep copy of the input geometry, using the CoordinateSequence type
	 *         of this factory
	 * @see Geometry#copy()
	 */
	 ...
	 *
	 * @param geometries
	 *            an array of Geometries, each of which may be empty but not null,
	 *            or null
	 ...
	 * CoordinateSequence creates an empty LineString.
	 *
	 * @param coordinates
	...
	/**
	 * Creates a LineString using the given Coordinates. A null or empty array
	...
	/**
	 * Creates a {@link LinearRing} using the given {@link CoordinateSequence}. A
	 * null or empty array creates an empty LinearRing. The points must form a
	...
	/**
	 * Creates a {@link LinearRing} using the given {@link Coordinate}s. A null or
	 * empty array creates an empty LinearRing. The points must form a closed and
	 * simple linestring.
	 *
	 * @param coordinates
	 *            an array without null elements, or an empty array, or null
	...
	/**
	 * Creates a MultiLineString using the given LineStrings; a null or empty array
	 * will create an empty MultiLineString.
	 *
	 * @param lineStrings
	...
	/**
	 * Creates a {@link MultiPoint} using the points in the given
	 * {@link CoordinateSequence}. A <code>
	...
	/**
	 * Creates a {@link MultiPoint} using the given {@link Coordinate}s. A null or
	 * empty array will create an empty MultiPoint.
	 *
	 * @param coordinates
	 ...
	 * array will create an empty MultiPoint.
	 *
	 * @param point
	...
	/**
	 * Creates a MultiPolygon using the given Polygons; a null or empty array will
	 * create an empty Polygon. The polygons must conform to the assertions
	 * specified in the <A HREF="http://www.opengis.org/techno/specs.htm">OpenGIS
	...
	/**
	 * Creates a Point using the given Coordinate. A null Coordinate creates an
	 * empty Geometry.
	 *
	 * @param coordinate
	 ...
	 *            <code>null</code> or an empty <code>LinearRing</code> if the empty
	 ...
	 *            <code>null</code> or empty <code>LinearRing</code> s if the empty
	 *            geometry is to be created.
	 * @throws IllegalArgumentException
	...
	/**
	 * Returns the PrecisionModel that Geometries created by this factory will be
	 * associated with.
	 *
	 ...
	 * @return an empty <code>Point</code> (for null <code>Envelope</code>s), a
	 *         <code>Point</code> (when min x = max x and min y = max y) or a
