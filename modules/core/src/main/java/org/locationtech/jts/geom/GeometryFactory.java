 ...
 *
 * <p>Note that the factory constructor methods do <b>not</b> change the input coordinates in any
 * way. In particular, they are not rounded to the supplied <tt>PrecisionModel</tt>. It is assumed
 * that input Coordinates meet the given precision.
 *
...
public class GeometryFactory implements Serializable {
  @Serial private static final long serialVersionUID = -6820524753094095635L;
  ...
  /**
   * Creates a {@link MultiPoint} using the given {@link Point}s. A null or empty array will create
   * an empty MultiPoint.
   *
   * @param point an array of Points (without null elements), or an empty array, or <code>null
   *     </code>
  ...
  /**
   * Creates a deep copy of the input {@link Geometry}. The {@link CoordinateSequenceFactory}
   * defined for this factory is used to copy the {@link CoordinateSequence}s of the input geometry.
   *
   * <p>This is a convenient way to change the <tt>CoordinateSequence</tt> used to represent a
   * geometry, or to change the factory used for a geometry.
   *
   * <p>{@link Geometry#copy()} can also be used to make a deep copy, but it does not allow changing
   * the CoordinateSequence type.
   *
