...
/**
 * Computes the length index of the point on a linear {@link Geometry} nearest a
 * given {@link Coordinate}. The nearest point is not necessarily unique; this
 * class always computes the nearest point closest to the start of the geometry.
 */
class LengthIndexOfPoint {
	 ...
	 * {@link Coordinate} after the specified minimum index. If possible the
	 * location returned will be strictly greater than the <code>minLocation</code>.
	 * If this is not possible, the value returned will equal <code>
	 * minLocation</code>. (An example where this is not possible is when
