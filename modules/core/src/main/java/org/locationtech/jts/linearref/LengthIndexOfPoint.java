...
/**
 * Computes the length index of the point on a linear {@link Geometry} nearest a given {@link
 * Coordinate}. The nearest point is not necessarily unique; this class always computes the nearest
 * point closest to the start of the geometry.
 */
class LengthIndexOfPoint {
  ...
  /**
   * Finds the nearest index along the linear {@link Geometry} to a given {@link Coordinate} after
   * the specified minimum index. If possible the location returned will be strictly greater than
   * the <code>minLocation</code>. If this is not possible, the value returned will equal <code>
   * minLocation</code>. (An example where this is not possible is when minLocation = [end of line]
   * ).
   *
   * @param inputPt the coordinate to locate
