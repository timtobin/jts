 ...
 *
 * <p>There are three possible outcomes when determining whether two line segments intersect:
 *
 * <ul>
 *   <li>{@link #NO_INTERSECTION} - the segments do not intersect
 *   <li>{@link #POINT_INTERSECTION} - the segments intersect in a single point
 *   <li>{@link #COLLINEAR_INTERSECTION} - the segments are collinear and they intersect in a line
 *       segment
 * </ul>
 *
 * For segments which intersect in a single point, the point may be either an endpoint or in the
 * interior of each segment. If the point lies in the interior of both segments, this is termed a
 * <i>proper intersection</i>. The method {@link #isProper()} test for this situation.
 *
 * <p>The intersection point(s) may be computed in a precise or non-precise manner. Computing an
 * intersection point precisely involves rounding it via a supplied {@link PrecisionModel}.
 *
 * <p>LineIntersectors do not perform an initial envelope intersection test to determine if the
 * segments are disjoint. This is because this class is likely to be used in a context where
...
public abstract class LineIntersector {
  ...
  /**
   * Computes the "edge distance" of an intersection point p along a segment. The edge distance is a
   * metric of the point along the edge. The metric used is a robust and easy to compute metric
   * function. It is <b>not</b> equivalent to the usual Euclidean metric. It relies on the fact that
   * either the x or the y ordinates of the points in the edge are unique, depending on whether the
   * edge is longer in the horizontal or vertical direction.
   *
   * <p>NOTE: This function may produce incorrect distances for inputs where p is not precisely on
   * p1-p2 (E.g. p = (139,9) p1 = (139,10), p2 = (280,1) produces distance 0.0, which is incorrect.
   *
   * <p>My hypothesis is that the function is safe to use for points which are the result of
   * <b>rounding</b> points which lie on the line, but not safe to use for <b>truncated</b> points.
   */
  ...
  /**
   * Computes the intersection of the lines p1-p2 and p3-p4. This function computes both the boolean
   * value of the hasIntersection test and the (approximate) value of the intersection point itself
   * (if there is one).
   */
  ...
  */
  ...
  /**
   * Computes the index (order) of the intIndex'th intersection point in the direction of a
