 ...
 * of {@link #rot()} operations. Quadedges in a subdivision are linked together
 * via their <tt>next</tt> references. The linkage between the quadedge quartets
...
public class QuadEdge {
	...
	/**
	 * Splices two edges together or apart. Splice affects the two edge rings around
	 * the origins of a and b, and, independently, the two edge rings around the
	 * left faces of <tt>a</tt> and <tt>b</tt>. In each case, (i) if the two rings
	 * are distinct, Splice will combine them into one, or (ii) if the two are the
	 * same ring, Splice will break it into two separate pieces. Thus, Splice can be
	 * used both to attach the two edges together, and to break them apart.
	 *
	 * @param a
	 *            an edge to splice
	 * @param b
	...
	/**
	 * Marks this quadedge as being deleted. This does not free the memory used by
	 * this quadedge quartet, but indicates that this edge no longer participates in
	 * a subdivision.
	 */
	public void delete() {
		rot = null;
	 ...
	 * @param qe
	 *            a quadedge
	 * @return true if the quadedges are based on the same line segment regardless
	 *         of orientation
	 */
	...
	/**
	 * Tests if this quadedge and another have the same line segment geometry with
	...
	/**
	 * Gets the primary edge of this quadedge and its <tt>sym</tt>. The primary edge
	 * is the one for which the origin and destination coordinates are ordered
	...
	/**
	 * Converts this edge to a WKT two-point <tt>LINESTRING</tt> indicating the
