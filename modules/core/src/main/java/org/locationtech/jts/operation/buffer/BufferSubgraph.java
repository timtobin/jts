...
class BufferSubgraph implements Comparable {
	...
	private void add(Node node, Stack nodeStack) {
		...
		for (Iterator i = node.getEdges().iterator(); i.hasNext();) {
			...
			/**
			 * NOTE: this is a depth-first traversal of the graph. This will cause a large
			 * depth of recursion. It might be better to do a breadth-first traversal.
			 */
	...
	public void findResultEdges() {
		/**
		 * Select edges which have an interior depth on the RHS and an exterior depth on
		 * the LHS. Note that because of weird rounding effects there may be edges which
		 * have negative depths! Negative depths count as "outside".
		 */
		for (Object o : dirEdgeList) {
			DirectedEdge de = (DirectedEdge) o;
			/**
	 ...
	 * "saveDirEdges"; private static int saveCount = 0; public void saveDirEdges()
	 * { GeometryFactory fact = new GeometryFactory(); for (Iterator it =
	 * dirEdgeList.iterator(); it.hasNext(); ) { DirectedEdge de = (DirectedEdge)
	 * it.next(); double dx = de.getDx(); double dy = de.getDy(); Coordinate p0 =
	 * de.getCoordinate(); double ang = Math.atan2(dy, dx); Coordinate p1 = new
	 * Coordinate( p0.x + .4 * Math.cos(ang), p0.y + .4 * Math.sin(ang)); //
	 * DebugFeature.add(SAVE_DIREDGES, // fact.createLineString(new Coordinate[] {
	 * p0, p1 } ), // de.getDepth(Position.LEFT) + "/" + de.getDepth(Position.RIGHT)
	 * // ); } String filepath = "x:\\jts\\testBuffer\\dirEdges" + saveCount++ +
	 * ".jml"; DebugFeature.saveFeatures(SAVE_DIREDGES, filepath); }
	 */
}
