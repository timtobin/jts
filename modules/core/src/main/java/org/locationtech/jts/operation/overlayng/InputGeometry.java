...
/**
 * Manages the input geometries for an overlay operation. The second geometry is
 * allowed to be null, to support for instance precision reduction.
 *
 * @author Martin Davis
 */
class InputGeometry {
	...
	/**
	 * Gets the index of an input which is an area, if one exists. Otherwise returns
	 * -1. If both inputs are areas, returns the index of the first one (0).
	 *
	...
	/**
	 * Tests if an input geometry has edges. This indicates that topology needs to
	...
	/**
	 * Determines the location within an area geometry. This allows disconnected
	 * edges to be fully located.
	 *
	 * @param geomIndex
	...
	public int locatePointInArea(int geomIndex, Coordinate pt) {
		...
		// this check is required because IndexedPointInAreaLocator can't handle empty
		// polygons
