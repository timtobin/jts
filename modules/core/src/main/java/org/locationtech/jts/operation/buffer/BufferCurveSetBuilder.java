...
public class BufferCurveSetBuilder {
  ...
  /**
   * Computes orientation of a ring using a signed-area orientation test. For invalid
   * (self-crossing) rings this ensures the largest enclosed area is taken to be the interior of the
   * ring. This produces a more sensible result when used for repairing polygonal geometry via
   * buffer-by-zero. For buffer use the lower robustness of orientation-by-area doesn't matter,
   * since narrow or flat rings produce an acceptable offset curve for either orientation.
   *
  ...
  /**
   * Computes the set of raw offset curves for the buffer. Each offset curve has an attached {@link
   * Label} indicating its left and right location.
   *
  ...
  private void addCurve(Coordinate[] coord, int leftLoc, int rightLoc) {
    ...
    SegmentString e =
        new NodedSegmentString(coord, new Label(0, Location.BOUNDARY, leftLoc, rightLoc));
    curveList.add(e);
  }
  ...
  private void addPoint(Point p) {
    ...
    if (coord.length >= 1 && !coord[0].isValid()) return;
  ...
  private void addLineString(LineString line) {
     ...
     *
     * <p>Singled-sided buffers currently treat rings as if they are lines.
     */
  ...
  private void addPolygon(Polygon p) {
    ...
    if (distance < 0.0 && isRingFullyEroded(shell, false, distance)) return;
    ...
    for (int i = 0; i < p.getNumInteriorRing(); i++) {
      ...
      if (distance > 0.0 && isRingFullyEroded(hole, true, distance)) continue;
  ...
  /**
   * Adds an offset curve for one side of a polygon ring. The side and left and right topological
   * location arguments are provided as if the ring is oriented CW. If the ring is in the opposite
   * orientation, the left and right locations are interchanged and the side is flipped.
   *
   ...
   */
  private void addPolygonRingSide(
      Coordinate[] coord, double offsetDistance, int side, int cwLeftLoc, int cwRightLoc) {
  ...
  /**
   * Add both sides of a linear ring. Checks for erosion of the hole side.
   *
  ...
  private void addLinearRingSides(Coordinate[] coord, double distance) {
    ...
    boolean isHoleComputed =
        !isRingFullyEroded(coord, CoordinateArrays.envelope(coord), true, distance);
  ...
  private void addRingSide(
      Coordinate[] coord, double offsetDistance, int side, int leftLoc, int rightLoc) {
    Coordinate[] curve = curveBuilder.getRingCurve(coord, side, offsetDistance);
    /**
     * If the offset curve has inverted completely it will produce an unwanted artifact in the
     * result, so skip it.
     */
  ...
  /**
   * Tests whether the offset curve for a ring is fully inverted. An inverted ("inside-out") curve
   * occurs in some specific situations involving a buffer distance which should result in a
   * fully-eroded (empty) buffer. It can happen that the sides of a small, convex polygon produce
   * offset segments which all cross one another to form a curve with inverted orientation. This
   * happens at buffer distances slightly greater than the distance at which the buffer should
   * disappear. The inverted curve will produce an incorrect non-empty buffer (for a shell) or an
   * incorrect hole (for a hole). It must be discarded from the set of offset curves used in the
   * buffer. Heuristics are used to reduce the number of cases which area checked, for efficiency
   * and correctness.
   *
   * <p>See https://github.com/locationtech/jts/issues/472
   *
   ...
   */
  private static boolean isRingCurveInverted(
      Coordinate[] inputRing, double distance, Coordinate[] curveRing) {
    if (distance == 0.0) return false;
    /** Only proper rings can invert. */
    if (inputRing.length <= 3) return false;
    /**
     * Heuristic based on low chance that a ring with many vertices will invert. This low limit
     * ensures this test is fairly efficient.
     */
    if (inputRing.length >= MAX_INVERTED_RING_SIZE) return false;

    /**
     * Don't check curves which are much larger than the input. This improves performance by
     * avoiding checking some concave inputs (which can produce fillet arcs with many more vertices)
     */
    if (curveRing.length > INVERTED_CURVE_VERTEX_FACTOR * inputRing.length) return false;

    /**
     * If curve contains points which are on the buffer, it is not inverted and can be included in
     * the raw curves.
     */
  ...
  /**
   * Tests if there are points on the raw offset curve which may lie on the final buffer curve (i.e.
   * they are (approximately) at the buffer distance from the input ring). For efficiency this only
   * tests a limited set of points on the curve.
   *
   * @param inputRing
   * @param distance
   * @param curveRing
   ...
   */
  private static boolean hasPointOnBuffer(
      Coordinate[] inputRing, double distance, Coordinate[] curveRing) {
   ...
   *
   * <p>The ringCoord is assumed to contain no repeated points. It may be degenerate (i.e. contain
   * only 1, 2, or 3 points). In this case it has no area, and hence has a minimum diameter of 0.
   *
   * @param ringCoord
  ...
  private static boolean isRingFullyEroded(LinearRing ring, boolean isHole, double bufferDistance) {
    return isRingFullyEroded(
        ring.getCoordinates(), ring.getEnvelopeInternal(), isHole, bufferDistance);
  }

  private static boolean isRingFullyEroded(
      Coordinate[] ringCoord, Envelope ringEnv, boolean isHole, double bufferDistance) {
    // degenerate ring has no area
    if (ringCoord.length < 4) return true;

    // important test to eliminate inverted triangle bug
    // also optimizes erosion test for triangles
    if (ringCoord.length == 4) return isTriangleErodedCompletely(ringCoord, bufferDistance);

    boolean isErodable = (isHole && bufferDistance > 0) || (!isHole && bufferDistance < 0);

    if (isErodable) {
      // -- if envelope is narrower than twice the buffer distance, ring is eroded
      double envMinDimension = Math.min(ringEnv.getHeight(), ringEnv.getWidth());
      if (2 * Math.abs(bufferDistance) > envMinDimension) return true;
    }
    return false;
  }
   ...
   *
   * <p>This test is important, since it removes a problematic case where the buffer distance is
  ...
  private static boolean isTriangleErodedCompletely(
      Coordinate[] triangleCoord, double bufferDistance) {
