...
public class BufferCurveSetBuilder {
	...
	/**
	 * Tests if there are points on the raw offset curve which may lie on the final
	 * buffer curve (i.e. they are (approximately) at the buffer distance from the
	 * input ring). For efficiency this only tests a limited set of points on the
	 * curve.
	 *
	 * @param inputRing
	 * @param distance
	 * @param curveRing
	 * @return true if the curve contains points lying at the required buffer
	 *         distance
	 */
	...
	/**
	 * Tests whether the offset curve for a ring is fully inverted. An inverted
	 * ("inside-out") curve occurs in some specific situations involving a buffer
	 * distance which should result in a fully-eroded (empty) buffer. It can happen
	 * that the sides of a small, convex polygon produce offset segments which all
	 * cross one another to form a curve with inverted orientation. This happens at
	 * buffer distances slightly greater than the distance at which the buffer
	 * should disappear. The inverted curve will produce an incorrect non-empty
	 * buffer (for a shell) or an incorrect hole (for a hole). It must be discarded
	 * from the set of offset curves used in the buffer. Heuristics are used to
	 * reduce the number of cases which area checked, for efficiency and
	 * correctness.
	 *
	 * <p>
	 ...
	 *            the buffer distance
	 * @param curveRing
	 *            the generated offset curve ring
	...
	private static boolean isRingCurveInverted(Coordinate[] inputRing, double distance, Coordinate[] curveRing) {
		 ...
		 */
		if (inputRing.length >= MAX_INVERTED_RING_SIZE)
			return false;

		/**
		 * Don't check curves which are much larger than the input. This improves
		 * performance by avoiding checking some concave inputs (which can produce
		 * fillet arcs with many more vertices)
		 */
		if (curveRing.length > INVERTED_CURVE_VERTEX_FACTOR * inputRing.length)
			return false;
	...
	/**
	 * Tests whether a ring buffer is eroded completely (is empty) based on simple
	 * heuristics.
	 *
	 * <p>
	...
	public BufferCurveSetBuilder(Geometry inputGeom, double distance, PrecisionModel precisionModel,
	...
	private void addCurve(Coordinate[] coord, int leftLoc, int rightLoc) {
		...
		SegmentString e = new NodedSegmentString(coord, new Label(0, Location.BOUNDARY, leftLoc, rightLoc));
		curveList.add(e);
	}
	...
	private void addLineString(LineString line) {
		...
		/**
		 * Rings (closed lines) are generated with a continuous curve, with no end arcs.
		 * This produces better quality linework, and avoids noding issues with arcs
		 * around almost-parallel end segments. See JTS #523 and #518.
		 *
		 * <p>
	 ...
	 *            ring vertices
	 * @param distance
	 *            offset distance (must be non-zero positive)
	 */
	private void addLinearRingSides(Coordinate[] coord, double distance) {
		/*
		 * (f "hole" side will be eroded completely, avoid generating it. This prevents
		 * hole artifacts (e.g. https://github.com/libgeos/geos/issues/1223)
		 */
	...
	private void addPolygon(Polygon p) {
		...
		for (int i = 0; i < p.getNumInteriorRing(); i++) {
			...
			addPolygonRingSide(holeCoord, offsetDistance, Position.opposite(offsetSide), Location.INTERIOR,
					Location.EXTERIOR);
		}
	}

	/**
	 * Adds an offset curve for one side of a polygon ring. The side and left and
	 * right topological location arguments are provided as if the ring is oriented
	 * CW. If the ring is in the opposite orientation, the left and right locations
	 * are interchanged and the side is flipped.
	 *
	 * @param coord
	 ...
	 * @param side
	 *            the side {@link Position} of the ring on which to construct the
	 *            buffer line
	 * @param cwLeftLoc
	 ...
	 */
	private void addPolygonRingSide(Coordinate[] coord, double offsetDistance, int side, int cwLeftLoc,
			int cwRightLoc) {
	...
	private void addRingSide(Coordinate[] coord, double offsetDistance, int side, int leftLoc, int rightLoc) {
		...
		/**
		 * If the offset curve has inverted completely it will produce an unwanted
		 * artifact in the result, so skip it.
		 */
	...
	/**
	 * Computes orientation of a ring using a signed-area orientation test. For
	 * invalid (self-crossing) rings this ensures the largest enclosed area is taken
	 * to be the interior of the ring. This produces a more sensible result when
	 * used for repairing polygonal geometry via buffer-by-zero. For buffer use the
	 * lower robustness of orientation-by-area doesn't matter, since narrow or flat
	 * rings produce an acceptable offset curve for either orientation.
	 *
	 * @param coord
	 *            the ring coordinates
