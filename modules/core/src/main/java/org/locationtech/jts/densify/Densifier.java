 ...
 * geometry will be <b>no longer</b> than the given distance tolerance (that is,
 * all segments in the output will have length less than or equal to the
 * distance tolerance). The coordinates created during densification respect the
 * input geometry's {@link PrecisionModel}.
 *
 * <p>
 * By default polygonal results are processed to ensure they are valid. This
 * processing is costly, and it is very rare for results to be invalid.
 * Validation processing can be disabled by calling the
 * {@link #setValidate(boolean)} method.
 *
 * <p>
 * <b>Note:</b> At some future point this class will offer a variety of
...
public class Densifier {
	...
	/**
	 * Sets the distance tolerance for the densification. All line segments in the
	 * densified geometry will be no longer than the distance tolerance. The
	 * distance tolerance must be positive.
	 *
	 * @param distanceTolerance
