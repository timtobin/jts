 ...
 * lies inside the polygon (i.e. they does not enclose hole edges). This allows {@link RelateNode}
 * to use simple area-additive semantics for adding edges and propagating edge locations.
 *
 * <p>The input node sections are assumed to have canonical orientation (CW shells and CCW holes).
 * The arrangement of shells and holes must be topologically valid. Specifically, the node sections
 * must not cross or be collinear.
 *
 * <p>This supports multiple shell-shell touches (including ones containing holes), and hole-hole
...
class PolygonNodeConverter {
  ...
  private static int convertShellAndHoles(
