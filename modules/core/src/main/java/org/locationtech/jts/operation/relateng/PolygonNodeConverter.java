 ...
 * model. In the "self-touch" model the converted NodeSection corners enclose
 * areas which all lies inside the polygon (i.e. they does not enclose hole
 * edges). This allows {@link RelateNode} to use simple area-additive semantics
...
class PolygonNodeConverter {
	...
	private static NodeSection createSection(NodeSection ns, Coordinate v0, Coordinate v1) {
		return new NodeSection(ns.isA(), Dimension.A, ns.id(), 0, ns.getPolygonal(), ns.isNodeAtVertex(), v0,
				ns.nodePt(), v1);
	}
