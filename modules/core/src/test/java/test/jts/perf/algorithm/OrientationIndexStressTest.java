 ...
 * i.e. it should produce the same result for the 3 possible permutations of the
 * input coordinates which have the same orientation:
 *
 * <p>
 * p0-p1 / p2 p1-p2 / p0 p2-p0 / p1
 *
 * <p>
 * Also, the reverse orientations should themselves be consistent, and be
 * opposite in sign to the forward orientation.
 *
 * <p>
 * The robust implementation uses DoubleDouble arithmetic and a filter to
 * improve computation time. It is compared to the simple Floating-Point
 * orientation computation, which is not robust.
 *
 * @author mdavis
 */
public class OrientationIndexStressTest {
	...
	boolean isConsistent(String tag, Coordinate p0, Coordinate p1, Coordinate p2, OrientationFunction orientFunc) {
		...
		if (isVerbose) {
			...
			System.out.format("%s %c%c%c %c%c%c %s ", tag, orientSym(orient0), orientSym(orient1), orientSym(orient2),
					orientSym(orientRev0), orientSym(orientRev1), orientSym(orientRev2), consistentInd);
		}
		return isConsistent;
	}

	boolean isConsistentDD(Coordinate p0, Coordinate p1, Coordinate p2) {
		return isConsistent("DD", p0, p1, p2, Orientation::index);
	}

	boolean isConsistentFP(Coordinate p0, Coordinate p1, Coordinate p2) {
		return isConsistent("FP", p0, p1, p2, NonRobustCGAlgorithms::orientationIndex);
	}
		...
		for (int i = 1; i <= MAX_RUN; i++) {
			...
			if (i % 1000 == 0) {
				System.out.printf("Num tests: %,d  DD fail = %,d (%d%%)  FP fail = %,d (%d%%)\n", i, failDD,
						(int) (100 * failDD / (double) i), failFP, (int) (100 * failFP / (double) i));
			}
		}
	}
	...
	private void runTest(int i) {
		...
		/*
		 * LineSegment ls = new LineSegment(p0, p1); Coordinate p2 =
		 * ls.pointAlongOffset(0.4, 1);
		 */
		...
		if (isVerbose) {
			System.out.println(WKTWriter.toLineString(p0, p1) + " - " + WKTWriter.toPoint(p2));
		}
	}
