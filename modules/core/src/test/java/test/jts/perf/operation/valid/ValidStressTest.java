...
/**
 * Stress-tests {@link IsValidOp} by running it on an invalid MultiPolygon with many intersections.
 * In JTS 1.14 and earlier this takes a very long time to run, since all intersections are computed
 * before the invalid result is returned. In fact it is only necessary to detect a single
 * intersection in order to determine invalidity, and this provides much faster performance.
 *
 * @author mdavis
 */
...
/**
 * Creates comb-like geometries. Crossed combs provide a geometry with a very high ratio of
...
class Comb {
  ...
  public static MultiPolygon crossedComb(Envelope env, int size, GeometryFactory geomFact) {
    ...
    AffineTransformation trans =
        AffineTransformation.rotationInstance(0.5 * Math.PI, centre.x, centre.y);
