...
/**
 * A utility that reads a WKB file of geometries, and outputs an XML file of
 * test cases where pairs of polygons fail for OverlayNG union with a floating
 * noder.
 *
 * <p>
...
public class OverlayCaseDumper {
	...
	private List<Geometry> filter(List<Geometry> geoms) {
		...
		for (Geometry geom : geoms) {
			if (geom.getNumPoints() > MAX_POINTS)
				continue;
			filt.add(geom);
		}
		return filt;
