...
/**
 * A utility that reads a WKB file of geometries, and outputs an XML file of test cases where pairs
 * of polygons fail for OverlayNG union with a floating noder.
 *
 * <p>Usage: OverlayCaseDumper infile [ outfile ]
 *
 * @author mdavis
 */
public class OverlayCaseDumper {
  ...
  private void parseArgs(String[] args) {
    if (args.length < 1) throw new IllegalArgumentException("Input filename is required");
    inFilename = args[0];
  ...
  private void doIntersections(List<Geometry> geoms) {
    ...
    for (int i = 0; i < geoms.size(); i++) {
      ...
      for (int j = i + 1; j < geoms.size(); j++) {
        ...
        if (!geom.getEnvelopeInternal().intersects(geom1.getEnvelopeInternal())) continue;

        // skip duplicates to avoid repetition
        if (prevGeom0 != null && prevGeom0.equalsExact(geom)) continue;
        if (prevGeom1 != null && prevGeom1.equalsExact(geom1)) continue;

        prevGeom0 = geom;
        prevGeom1 = geom1;

        boolean isDumped = doIntersection(geom, geom1);
        if (isDumped) caseCount++;
        if (caseCount >= MAX_CASES) return;
      }
    }
  }
