...
/**
 * Runs overlay operations on pairs of random polygonal geometries to see if
 * errors occur.
 *
 * <p>
 * For OverlayNG a spectrum of scale factors is used. Using a Floating precision
 * model can be optionally specified.
 *
 * @author mdavis
 */
public class RandomPolygonOverlayFuzzer {
	...
	static final double[] SCALES = new double[]{
			// 0, // floating PM
	...
	private void overlayNG(Geometry poly1, Geometry poly2, double scale) {
		...
		Geometry inter = OverlayNG.overlay(poly1, poly2, OverlayNG.INTERSECTION, pm);
		Geometry symDiff = OverlayNG.overlay(poly1, poly2, OverlayNG.SYMDIFFERENCE, pm);
		Geometry union = OverlayNG.overlay(onlyPolys(inter), onlyPolys(symDiff), OverlayNG.UNION, pm);
	}
