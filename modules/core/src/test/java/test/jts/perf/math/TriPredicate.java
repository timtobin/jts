 ...
 * extended-precision versions are provided, which are more robust (i.e. they produce correct
 * answers in more cases). These are used in triangulation algorithms.
 *
 * @author Martin Davis
 */
public class TriPredicate {
  /**
   * Tests if a point is inside the circle defined by the points a, b, c. This test uses simple
  ...
  /**
   * Tests if a point is inside the circle defined by the points a, b, c. This test uses robust
   * computation.
   *
   * @param a a vertex of the triangle
   * @param b a vertex of the triangle
  ...
  /**
   * Tests if a point is inside the circle defined by the points a, b, c. The computation uses
  ...
  public static boolean isInCircleDD(Coordinate a, Coordinate b, Coordinate c, Coordinate p) {
    ...
    DD aTerm = (ax.multiply(ax).add(ay.multiply(ay))).multiply(triAreaDD(bx, by, cx, cy, px, py));
    DD bTerm = (bx.multiply(bx).add(by.multiply(by))).multiply(triAreaDD(ax, ay, cx, cy, px, py));
    DD cTerm = (cx.multiply(cx).add(cy.multiply(cy))).multiply(triAreaDD(ax, ay, bx, by, px, py));
  ...
  public static boolean isInCircleDD2(Coordinate a, Coordinate b, Coordinate c, Coordinate p) {
    DD aTerm = (DD.sqr(a.x).selfAdd(DD.sqr(a.y))).selfMultiply(triAreaDD2(b, c, p));
    DD bTerm = (DD.sqr(b.x).selfAdd(DD.sqr(b.y))).selfMultiply(triAreaDD2(a, c, p));
    DD cTerm = (DD.sqr(c.x).selfAdd(DD.sqr(c.y))).selfMultiply(triAreaDD2(a, b, p));
    ...
    if (nonRobustInCircle != isInCircleDD || nonRobustInCircle != isInCircleCC) {
      System.out.println(
          "inCircle robustness failure (double result = "
              ...
              + ")");
      System.out.println(
          WKTWriter.toLineString(new CoordinateArraySequence(new Coordinate[] {a, b, c, p})));
      System.out.println(
          "Circumcentre = "
              + WKTWriter.toPoint(circumCentre)
              + " radius = "
              + a.distance(circumCentre));
      System.out.println(
          "p radius diff a = " + Math.abs(p.distance(circumCentre) / a.distance(circumCentre) - 1));
      System.out.println(
          "p radius diff b = " + Math.abs(p.distance(circumCentre) / b.distance(circumCentre) - 1));
      System.out.println(
          "p radius diff c = " + Math.abs(p.distance(circumCentre) / c.distance(circumCentre) - 1));
      System.out.println();
    }
  }
}
