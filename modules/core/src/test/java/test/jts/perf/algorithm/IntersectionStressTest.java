...
/**
 * Stress test for accuracy of various line intersection implementations. The
 * test is to compute the intersection point of pairs of line segments with
 * realistically large ordinate values, for angles of incidence which become
 * increasingly close to parallel. The measure of accuracy is the sum of the
 * distances of the computed point from the two lines.
 *
 * <p>
 * The intersection algorithms are:
 *
 * <ul>
 * <li>DP - a basic double-precision (DP) implementation, with no attempt at
 * reducing the effects of numerical round-off
 * <li>DP-Cond - a DP implementation in which the inputs are conditioned by
 * translating them to around the origin
 * <li>DP-CB - a DP implementation using the
 * {@link org.locationtech.jts.precision.CommonBitsRemover} functionality
 * <li>DD - an implementation using extended-precision
 * {@link org.locationtech.jts.math.DD} arithmetic
 * </ul>
 *
 * <h2>Results</h2>
 *
 ...
 * <li>DP-CB accuracy is better than DP, but degrades significantly as angle
 * becomes closer to parallel
...
public class IntersectionStressTest {
	...
	private void doIntersectionTest(int i, double incidentAngleFactor) {
		...
		if (verbose) {
			System.out.println(
					i + ":  Lines: " + WKTWriter.toLineString(p1, p2) + "  -  " + WKTWriter.toLineString(q1, q2));
		}
	...
	private void printStats(String tag, Coordinate intPt, Coordinate p1, Coordinate p2, Coordinate q1, Coordinate q2) {
		...
		if (verbose) {
			System.out.println(
					tag + " : " + WKTWriter.toPoint(intPt) + " -- Dist P = " + distP + "    Dist Q = " + distQ);
		}
	}
	...
	/**
	 * Run tests for a given incident angle factor. The angle between the segments
	 * is <code>
	 * PI * incidentAngleFactor</code>. A factor closer to 1 means the segments are
	 * more nearly parallel. A factor of 1 means they are parallel.
	 *
	 * @param incidentAngleFactor
	 *            the factor of PI between the two segments
	 */
