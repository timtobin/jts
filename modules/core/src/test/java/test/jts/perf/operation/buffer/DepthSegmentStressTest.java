...
class DepthSegment implements Comparable {
	...
	/**
	 * A comparison operation which orders segments left to right along some
	 * horizontal line. If segments don't touch the same line, or touch at the same
	 * point, they are compared in their Y extent.
	 *
	 * <p>
	...
	public int compareTo(Object obj) {
		...
		if (upwardSeg.maxX() < otherSeg.minX())
			return -1;
		/**
		 * The segments Y ranges should intersect, since they intersect same stabbing
		 * line. But check for disjoint in Y and provide a result based on Y ordering
		 */
		...
		/**
		 * Check if any segment point is left or right of the other segment in its Y
		 * extent.
		 */
		...
		/**
		 * If point checks in Y range are indeterminate, segments (probably?) touch at a
		 * point and lie above and below that point, or on same line
		 */
 ...
 * Generates sets of random non-crossing segments, extracts lists of segments
 * along stabbing lines, and checks the DepthSegment compare method for if it is
