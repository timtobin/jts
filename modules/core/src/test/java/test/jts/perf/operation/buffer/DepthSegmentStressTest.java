...
/**
 * Stress tests DepthSegment to determine if the compare contract is maintained. Generates sets of
 * random non-crossing segments, extracts lists of segments along stabbing lines, and checks the
 * DepthSegment compare method for if it is antisymmetric and transitive.
 *
 * @author Martin Davis
 */
public class DepthSegmentStressTest {
  ...
  private void reportError(String validityCheck, DepthSegment seg1, DepthSegment seg2) {
  ...
  static List<DepthSegment> query(List<DepthSegment> segs, double y) {
    ...
    for (DepthSegment ds : segs) {
      if (y >= ds.minY() && y <= ds.maxY()) result.add(ds);
    }
    return result;
  }
 ...
 *
 * <p>A copy of ubgraphDepthLocater.DepthSegment, which is private.
 */
class DepthSegment implements Comparable {
  ...
  /**
   * A comparison operation which orders segments left to right along some horizontal line. If
   * segments don't touch the same line, or touch at the same point, they are compared in their Y
   * extent.
   *
   * <p>The definition of the ordering is:
   *
   * <ul>
  ...
  public int compareTo(Object obj) {
    ...
    if (upwardSeg.minX() > otherSeg.maxX()) return 1;
    if (upwardSeg.maxX() < otherSeg.minX()) return -1;
    /**
     * The segments Y ranges should intersect, since they intersect same stabbing line. But check
     * for disjoint in Y and provide a result based on Y ordering
     */
    if (upwardSeg.minY() > otherSeg.maxY()) return 1;
    if (upwardSeg.maxY() < otherSeg.minY()) return -1;

    /** Check if any segment point is left or right of the other segment in its Y extent. */
    ...
    /**
     * If point checks in Y range are indeterminate, segments (probably?) touch at a point and lie
     * above and below that point, or on same line
     */
    if (upwardSeg.maxY() > otherSeg.maxY()) return 1;
    if (upwardSeg.maxY() < otherSeg.maxY()) return -1;

    // -- check for both horizontal
    if (upwardSeg.isHorizontal() && otherSeg.isHorizontal()) {
      if (upwardSeg.minX() < otherSeg.minX()) return -1;
      if (upwardSeg.minX() > otherSeg.minX()) return 1;
    }
