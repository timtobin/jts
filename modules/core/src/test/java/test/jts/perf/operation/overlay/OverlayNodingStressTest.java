...
/**
 * Tests Noding checking during overlay. Intended to show that noding check
 * failures due to robustness problems do not occur very often (i.e. that the
 * heuristic is not triggering so often that a large performance penalty would
 * be incurred.)
 *
 * <p>
 * The class generates test geometries for input to overlay which contain almost
 * parallel lines - this should cause noding failures relatively frequently.
 *
 * <p>
 * Can also be used to check that the cross-snapping heuristic fix for
 * robustness failures works well. If snapping ever fails to fix a case, an
 * exception is thrown. It is expected (and has been observed) that
 * cross-snapping works extremely well on this dataset.
 *
 * @version 1.7
 */
public class OverlayNodingStressTest {
	...
	public void testNoding() {
		...
		System.out.println("Test count = " + iterLimit + "  Noding failure count = " + failureCount);
	}
}
...
class RotatedRectangleFactory {
	...
	public Polygon createRectangle(double length, double width, double angle, Coordinate base) {
		...
		Coordinate[] pts = new Coordinate[]{new Coordinate(base.x + posx + widthOffsetx, base.y + posy + widthOffsety),
