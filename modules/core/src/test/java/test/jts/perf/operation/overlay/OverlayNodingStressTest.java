...
/**
 * Tests Noding checking during overlay. Intended to show that noding check failures due to
 * robustness problems do not occur very often (i.e. that the heuristic is not triggering so often
 * that a large performance penalty would be incurred.)
 *
 * <p>The class generates test geometries for input to overlay which contain almost parallel lines -
 * this should cause noding failures relatively frequently.
 *
 * <p>Can also be used to check that the cross-snapping heuristic fix for robustness failures works
 * well. If snapping ever fails to fix a case, an exception is thrown. It is expected (and has been
 * observed) that cross-snapping works extremely well on this dataset.
 *
 * @version 1.7
 */
public class OverlayNodingStressTest {
  ...
  public Geometry[] generateGeometryAccum(double angle1, double angle2) {
    ...
    geomCount++;
    if (geomCount >= BATCH_SIZE) geomCount = 0;
    if (geomCount == 0) baseAccum = null;

    if (baseAccum == null) baseAccum = rr1;
    else {
      // this line can be used to test for the presence of noding failures for
      // non-tricky cases
      // Geometry star = rr2;
