...
/** Discrete Fréchet Distance computation using a simple O(n^2) algorithm. */
public class DiscreteFrechetDistanceSimple {
			...
			for (int j = 0; j < coords1.length; j++) {
				...
				if (i > 0 && j > 0) {
					distances[i][j] = Math.max(
							Math.min(Math.min(distances[i - 1][j], distances[i - 1][j - 1]), distances[i][j - 1]),
							distance);
				} else if (i > 0) {
	...
	/*
	 * // For debugging purposes only private static String toString(int numRows,
	 * int numCols, double[][] sparse) {
	 *
	 * StringBuilder sb = new StringBuilder("["); for (int i = 0; i < numRows; i++)
	 * { sb.append('['); for(int j = 0; j < numCols; j++) { if (j > 0)
	 * sb.append(", "); sb.append(String.format("%8.4f", sparse[i][j])); }
	 * sb.append(']'); if (i < numRows - 1) sb.append(",\n"); } sb.append(']');
	 * return sb.toString(); }
	 */
