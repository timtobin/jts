...
/**
 * Tests performance of {@link IndexedFacetDistance} versus standard {@link DistanceOp} using a grid
 * of points to a target set of lines
 *
 * @author Martin Davis
 */
public class TestPerfDistanceLinesPoints {
  ...
  public void test(Geometry[] pts, Geometry target) {
    if (verbose) {
      System.out.println();
      System.out.println(
          "Query points = " + pts.length + "     Target points = " + target.getNumPoints());
    }
    test(pts, target, true);
  ...
  void computeDistance(Geometry[] pts, Geometry geom) {
    ...
    if (USE_INDEXED_DIST) bbd = new IndexedFacetDistance(geom);
    for (Geometry pt : pts) {
      if (USE_INDEXED_DIST) {
      ...
      for (int j = 0; j < nPtsSide; j++) {
        pts[index++] =
            geomFact.createPoint(
                new Coordinate(extent.getMinX() + i * xinc, extent.getMinY() + j * yinc));
      }
    }
    return pts;
  }
