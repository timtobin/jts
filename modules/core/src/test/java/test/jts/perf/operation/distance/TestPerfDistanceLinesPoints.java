 ...
 * {@link DistanceOp} using a grid of points to a target set of lines
 *
 * @author Martin Davis
 */
public class TestPerfDistanceLinesPoints {
	...
	Geometry createLine(double extent, int nSegs) {
		Coordinate[] pts = new Coordinate[]{new Coordinate(0, 0), new Coordinate(0, extent),
				new Coordinate(extent, extent), new Coordinate(extent, 0)};
	...
	Geometry[] createPoints(Envelope extent, int nPtsSide) {
		...
		for (int i = 0; i < nPtsSide; i++) {
			for (int j = 0; j < nPtsSide; j++) {
				pts[index++] = geomFact
						.createPoint(new Coordinate(extent.getMinX() + i * xinc, extent.getMinY() + j * yinc));
			}
		}
		return pts;
	}
