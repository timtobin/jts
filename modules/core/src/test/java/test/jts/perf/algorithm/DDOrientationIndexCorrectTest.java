...
public class DDOrientationIndexCorrectTest extends GeometryTestCase {
	...
	/**
	 * This test captures a situation where the DD orientation apparently fails.
	 *
	 * <p>
	 * According to the stated decimal representation the two orientations should be
	 * equal, but they are not.
	 *
	 * <p>
	 * Even more disturbingly, the orientationIndexFilter handles this case, without
	 * dropping through to the actual DD code. The result is the same, however.
	 */
	public void testPointCloseToLine() {
		Coordinate[] pts = {new Coordinate(2.4829102, 48.8726807), new Coordinate(2.4832535, 48.8737106),
				new Coordinate(2.4830818249999997, 48.873195575)};
		int orientDD = runDD("Orginal case", pts);
		// System.out.println("DD - Alt: " + orientDD);

		Coordinate[] ptsScale = {new Coordinate(24829102, 488726807), new Coordinate(24832535, 488737106),
				new Coordinate(24830818.249999997, 488731955.75)};
		int orientSC = runDD("Scaled case", ptsScale);
		// System.out.println("DD - Alt: " + orientDD);

		/**
		 * Same arrangement as above, but translated by removing digits before decimal
		 * point to reduce numeric precision
		 */
		Coordinate[] ptsLowPrec = {new Coordinate(0.4829102, 0.8726807), new Coordinate(0.4832535, 0.8737106),
