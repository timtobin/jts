...
/**
 * Robustness test of Point-In-Ring algorithms. The input to each test is a
 * triangle, and a point interpolated along the side. Almost always the point
 * will not lie exactly on the side. The robustness test consists of comparing
 * the result of computing Point-In-Ring and the result of determining the
 * orientation of the point relative to the side. The test fails if these are
 * not consistent.
 *
 * <p>
 * The stress test reveals that the original {@link RayCrossingCounter} has
 * inconsistencies with the
 * {@link org.locationtech.jts.algorithm.CGAlgorithmsDD#orientationIndex(Coordinate, Coordinate, Coordinate)}
 * orientation index computation (which is now the standard in JTS, due to its
 * improved robustness). The {@link RayCrossingCounter} implementation is
 * consistent, as expected.
 *
 * <p>
 * Note that the inconsistency does not indicate which algorithm is "more
 * correct" - just that they produce different results. However, it is highly
 * likely that the original RCC algorithm is <b>not</b> as robust as RCCDD,
 * since it uses a series of double-precision arithmetic operations.
 *
 * @author Martin Davis
 */
public class PointInRingRobustnessTest extends GeometryTestCase {
	...
	/**
	 * Winding Number algorithm included for comparison purposes. This is almost
	 * equivalent to the RayCrossing algorithm, except that it checks all crossing
	 * segments instead of just the ones to the right of the point.
	 *
	 * <p>
	 ...
	 *            point to test
	 * @param ring
	 *            coordinates of ring (with same first and last point)
	...
	private void checkRandomTriangles(int sideLen, int numTris, int numEdgePts) {
		for (int i = 0; i < numTris; i++) {
			...
			Coordinate[] triPts = new Coordinate[]{start, new Coordinate(randomInt(sideLen), randomInt(sideLen)),
					new Coordinate(randomInt(sideLen), randomInt(sideLen)), new Coordinate(start)};
	...
	private void checkRightTriangles(double maxHeight, double width, int numEdgePts) {
		for (int height = 0; height < maxHeight; height++) {
			Coordinate[] triPts = new Coordinate[]{new Coordinate(0, 0), new Coordinate(0, height),
					new Coordinate(width, 0), new Coordinate(0, 0)};
			checkTriangleEdgePoints(triPts, numEdgePts);
		}
	}
	...
	boolean checkTriangleConsistent(Coordinate[] triPts, double lenFraction) {
		...
		// boolean isPointInRing = Location.INTERIOR ==
		// RayCrossingCounter.locatePointInRing(pt,
		// triPts);
		...
		if (!isConsistent) {
			...
			failureCount++;
			System.out.println("Inconsistent: " + "PIR=" + isPointInRing + " Orient=" + orientation + "  Pt: "
					+ WKTWriter.toPoint(pt) + "  seg: " + WKTWriter.toLineString(triPts[1], triPts[2]) + "  tri: "
					+ toPolygon(triPts));
		}
		return isConsistent;
	}
