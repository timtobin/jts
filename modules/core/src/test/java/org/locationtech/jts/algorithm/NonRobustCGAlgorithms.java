...
/**
 * Non-robust versions of various fundamental Computational Geometric
 * algorithms, <b>FOR TESTING PURPOSES ONLY!</b>. The non-robustness is due to
 * rounding error in floating point computation.
 *
 * @version 1.7
 */
public class NonRobustCGAlgorithms {
	...
	public static double distanceLineLine(Coordinate A, Coordinate B, Coordinate C, Coordinate D) {
		 ...
		 * conditions: If 0<=r<=1 & 0<=s<=1, intersection exists r<0 or r>1 or s<0 or
		 * s>1 line segments do not intersect If the denominator in eqn 1 is zero, AB &
		...
		if ((r_bot == 0) || (s_bot == 0)) {
			return Math.min(Distance.pointToSegment(A, C, D), Math.min(Distance.pointToSegment(B, C, D),
	 ...
	 *             if the ring is degenerate (does not contain 3 different points)
	 */
	public static boolean isCCW(Coordinate[] ring) {
		 ...
		 * the lines lie along the x axis in opposite directions (2) the line lie on top
		 * of one another (2) should never happen, so we're going to ignore it! (Might
		 * want to assert this) (1) is handled by checking if next is left of prev ==>
		 * CCW
		 */
		if (disc == 0.0) {
	 ...
	 * a vector specified by <code>p1-p2</code>. The orientation of a point relative
	 * to a directed line segment indicates which way you turn to get to q after
