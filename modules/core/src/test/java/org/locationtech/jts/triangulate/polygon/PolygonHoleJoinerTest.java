...
public class PolygonHoleJoinerTest extends GeometryTestCase {
  /**
   * A failing case for hole joining with two holes that touch at leftmost vertex Fails due to
   * PolygonHoleJoiner joining holes with same leftmost vertex so that the result linework
   * self-crosses. Note that input is normalized.
   */
  @Test
  ...
  /**
   * A failing case revealing that joining a hole by a zero-length cut was introducing duplicate
   * vertices.
   */
  @Test
