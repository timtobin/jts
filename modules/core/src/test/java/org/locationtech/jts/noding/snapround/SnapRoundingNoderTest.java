...
public class SnapRoundingNoderTest extends GeometryTestCase {
	...
	/**
	 * This test will fail if the diagonals of hot pixels are not checked. Note that
	 * the nearby vertex is far enough from the long segment to avoid being snapped
	 * as an intersection.
	 */
	@Test
	...
	/**
	 * Currently fails, perhaps due to intersection lying right on a grid cell
	 * corner? Fixed by ensuring intersections are forced into segments
	 */
	@Test
	...
	/**
	 * Tests that MCIndexNoder tolerance is set correctly. See
	 * https://trac.osgeo.org/geos/ticket/1127 and
	 * https://github.com/libgeos/geos/pull/504
	 */
	@Test
	...
	/**
	 * An A vertex lies very close to a B segment. The vertex is snapped across the
	 * segment, but the segment is not noded. FIXED by adding intersection detection
	 * for near vertices to segments
	 */
	@Test
	...
	/**
	 * Looks like a very short line is stretched between two grid points, and for
	 * some reason the node at one end is not inserted in a line snapped to it
	 */
	@Test
	...
	/**
	 * Currently fails, does not node correctly. Fixed by NOT rounding lines
	 * extracted by Overlay
	 */
	@Test
	...
	/**
	 * A vertex lies near interior of horizontal segment. Both are moved by
	 * rounding, and vertex ends up coincident with segment, but node is not
	 * created. This is very subtle, since because the segment is horizontal the
	 * vertex lies exactly on it and thus still reports as valid geometry (although
	 * a noding check reports failure). This is caused by the indexing used in
	 * Snap-rounding using exact envelopes. What is needed is a small expansion
	 * amount to ensure segments within snap distance are tested (in MCIndexNoder)
	 */
	@Test
