...
public class OverlayNGFloatingNoderTest extends GeometryTestCase {
	 ...
	 * <li>This causes the clipped A and B edges to become disjoint (whereas in the
	 * original geometry they intersected).
	 * <li>Both edge rings are thus determined to be disconnected during overlay
	 * labeling.
	 * <li>For the overlay labeling for the disconnected edge in geometry B, the
	 * chosen edge coordinate has its location computed as inside the original A
	 * polygon. This is because the chosen coordinate happens to be the one that the
	 * clipped edge crossed over.
	 * <li>This causes the (clipped) B edge ring to be labelled as Interior to the A
	 * polygon.
	 * <li>The B edge ring thus is computed as being in the intersection, and the
	 * entire ring is output, producing a much larger polygon than is correct.
	 * </ol>
	 *
	 ...
	 * <li>Improve choosing a point for disconnected edge location; i.e. by finding
	 * one that is far from the other geometry edges. However, this still creates a
	 * result which may not reflect the actual input topology.
	 * </ol>
	 *
	 * The chosen fix is the first above - improve clipping by choosing a larger
	 * clipping envelope.
	 *
	 * <p>
	 * NOTE: When clipping is improved to avoid perturbing intersecting segments,
	 * the floating overlay now reports a TopologyException. This is reported as an
	 * empty geometry to allow tests to pass.
	 */
