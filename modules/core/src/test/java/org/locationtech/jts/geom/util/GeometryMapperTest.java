...
public class GeometryMapperTest extends GeometryTestCase {
  ...
  static GeometryMapper.MapOp KEEP_LINE =
      geom -> {
        if (geom instanceof Point) {
          return geom.getFactory().createEmpty(1);
        }
        if (geom instanceof LineString) return geom;
        return null;
      };
  ...
  public void testFlatMapInputEmpty() throws ParseException {
    checkFlatMap(
        "GEOMETRYCOLLECTION( POINT EMPTY, LINESTRING EMPTY)", 1, KEEP_LINE, "LINESTRING EMPTY");
  }
