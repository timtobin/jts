 ...
 *
 * <p>
 * This version is non-robust, since it uses relies on double-precision
 * arithmetic.
 *
 * @author Martin Davis
...
public class NonRobustRayCrossingCounter {
	...
	public void countSegment(Coordinate p1, Coordinate p2) {
		/**
		 * For each segment, check if it crosses a horizontal ray running from the test
		...
		if (((p1.y > p.y) && (p2.y <= p.y)) || ((p2.y > p.y) && (p1.y <= p.y))) {
			...
			/**
			 * The translated segment straddles the x-axis. Compute the sign of the ordinate
			 * of intersection with the x-axis. (y2 != y1, so denominator will never be 0.0)
			 */
	 ...
	 * this method is <tt>true</tt>, no further segments need be supplied, since the
	 * result will never change again.
	 *
