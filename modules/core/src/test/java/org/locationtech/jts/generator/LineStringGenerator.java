...
public class LineStringGenerator extends GeometryGenerator {
   ...
   *
   * <p>Note: this requires the number of points to be greater than 2.
   *
   * @see #getNumberPoints()
  ...
  /**
   * As the user increases the number of points, the probability of creating a random valid
   * linestring decreases. Please take not of this when selecting the generation style, and the
   * number of points.
   *
  ...
  private static void fillVert(
      double x, double dx, double y, double dy, Coordinate[] coords, GeometryFactory gf) {
  ...
  private static void fillArc(
      double x, double dx, double y, double dy, Coordinate[] coords, GeometryFactory gf) {
    if (coords.length == 2) throw new IllegalStateException("Too few points for Arc");
