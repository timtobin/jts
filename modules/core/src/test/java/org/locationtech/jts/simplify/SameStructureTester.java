...
/**
 * Test if two geometries have the same structure (but not necessarily the same coordinate sequences
 * or adjacencies).
 */
public class SameStructureTester {
  ...
  public static boolean isSameStructure(Geometry g1, Geometry g2) {
    ...
    else if (g1 instanceof Polygon polygon) return isSameStructurePolygon(polygon, (Polygon) g2);
    else if (g1 instanceof LineString string)
      return isSameStructureLineString(string, (LineString) g2);
    else if (g1 instanceof Point point) return isSameStructurePoint(point, (Point) g2);
  ...
  private static boolean isSameStructureCollection(GeometryCollection g1, GeometryCollection g2) {
    ...
    for (int i = 0; i < g1.getNumGeometries(); i++) {
      if (!isSameStructure(g1.getGeometryN(i), g2.getGeometryN(i))) return false;
    }
    return true;
  }
