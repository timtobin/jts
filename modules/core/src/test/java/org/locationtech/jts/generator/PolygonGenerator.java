...
public class PolygonGenerator extends GeometryGenerator {
	...
	/**
	 * Creates polygons whose points will not be rectangular when there are more
	 * than 4 points
	 */
	...
	/**
	 * As the user increases the number of points, the probability of creating a
	 * random valid polygon decreases. Please take not of this when selecting the
	 * generation style, and the number of points.
	 *
	 * <p>
	 ...
	 *             When the alg is not valid or the number of points is invalid
	 * @throws NullPointerException
	 *             when either the Geometry Factory, or the Bounding Box are
	 *             undefined.
	 */
	...
	private static Polygon createArc(double x, double dx, double y, double dy, int nholes, int npoints,
			GeometryFactory gf) {
	...
	private static LinearRing createTri(double cx, double cy, int startAngle, int endAngle, double radius,
			GeometryFactory gf) {
	...
	private static Polygon createBox(double x, double dx, double y, double dy, int nholes, int npoints,
			GeometryFactory gf) {
		// make outer ring first
		...
		for (int i = 0; i < nrow; i++) {
			for (int j = 0; j < ncol; j++) {
				if (cindex < nholes) {
					...
					inner[cindex++] = createBox(spx + x + j * (ddx + spx), ddx, spy + y + i * (ddy + spy), ddy, pts,
							gf);
				}
			}
		}
	 ...
	 *            The generationAlgorithm to set.
	 */
