...
public class CoordinateSequencesTest {
	...
	private static final double[][] ordinateValues = {{75.76, 77.43}, {41.35, 90.75}, {73.74, 41.67}, {20.87, 86.49},
			{17.49, 93.59}, {67.75, 80.63}, {63.01, 52.57}, {32.9, 44.44}, {79.36, 29.8}, {38.17, 88.0}, {19.31, 49.71},
			{57.03, 19.28}, {63.76, 77.35}, {45.26, 85.15}, {51.71, 50.38}, {92.16, 19.85}, {64.18, 27.7},
			{64.74, 65.1}, {80.07, 13.55}, {55.54, 94.07}};

	private static void checkCoordinateAt(CoordinateSequence seq1, int pos1, CoordinateSequence seq2, int pos2,
			int dim) {
		...
		for (int j = 2; j < dim; j++) {
			assertEquals(seq1.getOrdinate(pos1, j), seq2.getOrdinate(pos2, j),
	...
	private static CoordinateSequence createCircle(CoordinateSequenceFactory factory, int dimension, Coordinate center,
			double radius) {
	...
	/**
	 * @deprecated only use to update in conjunction with
	 *             {@link this.ttestCreateRandomOrdinates}
	 */
	...
	private static void doTestCopy(CoordinateSequenceFactory factory, int dimension) {
		...
		if (sequence.size() <= 7) {
			System.out.println("sequence has a size of " + sequence.size()
					+ ". Execution of this test needs a sequence " + "with more than 6 coordinates.");
			return;
		}
	...
	private static void doTestIsRing(CoordinateSequenceFactory factory, int dimension) {
		...
		CoordinateSequence noRing = createCircularString(factory, dimension, new Coordinate(), 5, 0.1, 22);
	...
	private static void doTestMinCoordinateIndex(CoordinateSequenceFactory factory, int dimension) {
		...
		if (sequence.size() <= 6) {
			System.out.println("sequence has a size of " + sequence.size()
					+ ". Execution of this test needs a sequence " + "with more than 5 coordinates.");
			return;
		}
	...
	private static void doTestScroll(CoordinateSequenceFactory factory, int dimension) {
		...
		// arrange
		CoordinateSequence sequence = createCircularString(factory, dimension, new Coordinate(20, 20), 7d, 0.1, 22);
	...
	private static void doTestScrollRing(CoordinateSequenceFactory factory, int dimension) {
		...
		// arrange
		// System.out.println("Testing '" + factory.getClass().getSimpleName() + "' with
		// dim="
		// +dimension );
	...
	/**
	 * Method used to create a {@link #ordinateValues}. Usage: remove first 't' and
	 * run as unit test. Note: When parameters are changed, some unit tests may need
	 * to be changed, too.
	 *
	 * <p>
	 * This is especially true for the {@link #testMinCoordinateIndex()} test, which
	 * assumes that the coordinates in the sequence are all within an envelope of
	 * [Env(10, 100, 10, 100)]. .
	 *
	 * @deprecated only use to update {@link #ordinateValues}
	 */
	public void ttestCreateRandomOrdinates() {
		CoordinateSequence sequence = createRandomTestSequence(CoordinateArraySequenceFactory.instance(), 20, 2,
				new Random(7), new Envelope(10, 100, 10, 100), new PrecisionModel(100));
