...
public class OverlayNGTest extends OverlayNGTestCase {
	...
	/**
	 * Failure due to B hole collapsing and edges being labeled Exterior. They are
	 * coincident with an A hole edge, but because labeled E are not included in
	 * Intersection result. This occurred because of a very subtle instance field
	 * update sequence bug in Edge.mergeEdge.
	 */
	 ...
	 * to a segment by a nearby vertex, and the snap vertex "jumped" across another
	 * segment. This is because the nearby snap intersection tolerance in
	 * SnapIntersectionAdder was too large (FACTOR = 10).
	 *
	 * <p>
	 * FIXED by reducing the tolerance factor to 100.
	 *
	 * <p>
	 * However, it may be that there is no safe tolerance level? Perhaps there can
	 * always be situations where a snap intersection will jump across a segment?
	 */
	 ...
	 * Labelling marks a single collapsed edge as B:i. Edge is only connected to two
	 * other edges both marked B:e. B:i edge is included in area result edges, and
	 * faild because it does not form a ring.
	 *
	 * <p>
	 * Perhaps a fix is to ignore connected single Bi edges which do not form a
	 * ring? This may be dangerous since it may hide other labelling problems?
	 *
	 * <p>
	...
	/**
	 * Probably due to B collapsing completely and disconnected edges being located
	 * incorrectly in B interior. Have seen other cases of this as well. Also - a B
	 * edge is marked as a Hole, which is incorrect.
	 *
	 * <p>
	...
	/**
	 * Fails because polygon A collapses totally, but one L edge is still labelled
	 * with location A:iL due to being located inside original A polygon by PiP test
	 * for incomplete edges. That edge is then marked as in-result-area, but it is
	 * the only edge marked in-result, so result ring can't be formed because ring
	 * is incomplete
	 */
	...
	/**
	 * Fails because current isResultAreaEdge does not accept L edges as result area
	 * boundary
	 */
	...
	/**
	 * Fails because A holes collapse to L edges, and are not computed as in Int of
	 * A, so are not included as result area edges.
	 */
	...
	/**
	 * Fails because edge of B is computed as Interior to A because it is checked
	 * against full precision input, rather than collapsed linework.
	 *
	 * <p>
	 ...
	 * with a 3-pt edge. The EdgeMerger doesn't check that merged edges are
	 * identical, so merges the 3pt edge into the 2-pt edge. FIXED by better noding.
	 */
	...
	/**
	 * Tests that overlay can handle polygons with flat topology collapse along top,
	 * courtesy of improved Orientation.isCCW algorithm. See also
	...
	/**
	 * Note this result is different to old overlay, because the top-right diagonal
	 * line gets snapped to the vertex above it.
	 */
