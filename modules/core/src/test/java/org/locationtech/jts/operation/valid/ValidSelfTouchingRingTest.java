...
/**
 * Tests that IsValidOp validates polygons with Self-Touching Rings (inverted
 * shells or exverted holes). Mainly tests that configuring {@link IsValidOp} to
 * allow validating the STR validates polygons with this condition, and does not
 * validate polygons with other kinds of self-intersection (such as ones with
 * Disconnected Interiors). Includes some basic tests to confirm that other
 * invalid cases remain detected correctly, but most of this testing is left to
...
public class ValidSelfTouchingRingTest extends GeometryTestCase {
	...
	/**
	 * Tests a geometry with both a shell self-touch and a hole self-touch. This is
	 * valid if STR is allowed, but invalid in OGC
	 */
	@Test
	 ...
	 * {@link #testShellAndHoleSelfTouch} but using a shell-hole touch and a
	 * hole-hole touch. This is valid in OGC.
	 */
	@Test
	...
	/**
	 * Tests an overlapping hole condition, where one of the holes is created by a
	 * shell self-touch. This is never valid.
	 */
	@Test
