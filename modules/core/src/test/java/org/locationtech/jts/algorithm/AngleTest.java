...
public class AngleTest {
	...
	public void testInteriorAngle_randomTriangles() {
		...
		for (int i = 0; i < 100; i++) {
			...
			Polygon triangle = geometryFactory.createPolygon(CoordinateSequences.ensureValidRing(
					coordinateSequenceFactory, coordinateSequenceFactory.create(threeRandomPoints.getCoordinates())));
			...
			double sumOfInteriorAngles = Angle.interiorAngle(c[0], c[1], c[2]) + Angle.interiorAngle(c[1], c[2], c[0])
					+ Angle.interiorAngle(c[2], c[0], c[1]);
			assertEquals(Math.PI, sumOfInteriorAngles, 0.01,
					i + ": The sum of the angles of a triangle is not equal to two right angles for points: "
							+ Arrays.toString(c));
		}
	}
