...
public class NormalizeTest {
  ...
  public void testNormalizeMultiPoint() throws Exception {
    MultiPoint m = (MultiPoint) reader.read("MULTIPOINT(30 20, 10 10, 20 20, 30 30, 20 10)");
    m.normalize();
    MultiPoint expectedValue =
        (MultiPoint) reader.read("MULTIPOINT(10 10, 20 10, 20 20, 30 20, 30 30)");
    assertEqualsExact(expectedValue, m);
    MultiPoint unexpectedValue =
        (MultiPoint) reader.read("MULTIPOINT(20 10, 20 20, 30 20, 30 30, 10 10)");
  ...
  public void testNormalizeLineString1() throws Exception {
    ...
    l.normalize();
    LineString expectedValue =
        (LineString) reader.read("LINESTRING (20 20, 160 40, 160 100, 100 120, 60 60)");
  ...
  public void testNormalizeLineString3() throws Exception {
    ...
    l.normalize();
    LineString expectedValue =
        (LineString) reader.read("LINESTRING (80 80, 160 80, 80 160, 140 160, 200 240)");
    assertEqualsExact(expectedValue, l);
  }
  ...
  public void testNormalizeLineString5() throws Exception {
    LineString l =
        (LineString)
            reader.read("LINESTRING (200 340, 140 240, 140 160, 60 240, 140 240, 200 340)");
    l.normalize();
    LineString expectedValue =
        (LineString)
            reader.read("LINESTRING (200 340, 140 240, 60 240, 140 160, 140 240, 200 340)");
    assertEqualsExact(expectedValue, l);
  }
  ...
  public void testNormalizeStringNoSideEffect() throws Exception {
    LineString l = (LineString) reader.read("LINESTRING (200 240, 140 160, 80 160, 160 80, 80 80)");
    LineString ref = (LineString) reader.read("LINESTRING (200 240, 140 160)");
    LineString seg =
        l.getFactory()
            .createLineString(new Coordinate[] {l.getCoordinates()[0], l.getCoordinates()[1]});
  ...
  public void testNormalizeGeometryCollection() throws Exception {
    ...
    actualValue.normalize();
    GeometryCollection expectedValue =
        (GeometryCollection)
            reader.read(
