 ...
 *
 * @see <a href="https://github.com/TWKB/Specification/blob/master/twkb.md">TWKB
 *      specification</a>
 */
class TWKBHeader {
	...
	public String toString() {
		return "TWKBHeader{" + "geometryType=" + geometryType + ", xyPrecision=" + xyPrecision + ", hasBBOX=" + hasBBOX
				+ ", hasSize=" + hasSize + ", hasIdList=" + hasIdList + ", isEmpty=" + isEmpty + ", hasZ=" + hasZ
				+ ", hasM=" + hasM + ", zPrecision=" + zPrecision + ", mPrecision=" + mPrecision + ", geometryBodySize="
				+ geometryBodySize + '}';
	}
	...
	enum GeometryType {
		...
		// held as a class variable cause calling values() on a tight loop has a
		// non-depreciable
		// performance impact
	...
	/**
	 * Size of encoded geometry body, iif size_flag == 1, defaults to {@code -1} if
	 * {@link #hasSize} ({@code == false}
	 *
	 * <p>
	...
	public int getPrecision(final int dimensionIndex) {
		return switch (dimensionIndex) {
			...
			default ->
				throw new IllegalArgumentException("Dimension index shall be between 0 and 3: " + dimensionIndex);
		};
	}
}
