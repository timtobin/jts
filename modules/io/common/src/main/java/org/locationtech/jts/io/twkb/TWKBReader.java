 ...
 *
 * <p>
 * The current TWKB specification is <a href=
 * 'https://github.com/TWKB/Specification/blob/master/twkb.md'>https://github.com/TWKB/Specification/blob/master/twkb.md</a>.
 *
 * <p>
 */
public class TWKBReader {
	...
	private static TWKBHeader readHeader(DataInput in) throws IOException {
		...
		return new TWKBHeader().setGeometryType(geometryType).setXyPrecision(precision).setHasZ(hasZ)
				.setZPrecision(zprecision).setHasM(hasM).setMPrecision(mprecision).setHasIdList(hasIdList)
				.setEmpty(isEmpty).setHasSize(hasSize).setHasBBOX(hasBBOX).setGeometryBodySize(geometryBodySize);
	}
	...
	private static Geometry readGeometryBody(GeometryFactory factory, TWKBHeader header, DataInput in)
		...
		return switch (geometryType) {
			case POINT -> readPoint(factory, in, header);
			case LINESTRING -> readLineString(factory, in, header, new long[header.getDimensions()]);
			case POLYGON -> readPolygon(factory, in, header, new long[header.getDimensions()]);
			case MULTIPOINT -> readMultiPoint(factory, in, header);
			case MULTILINESTRING -> readMultiLineString(factory, in, header);
			case MULTIPOLYGON -> readMultiPolygon(factory, in, header);
			case GEOMETRYCOLLECTION -> readGeometryCollection(factory, in, header);
			default -> throw new IllegalStateException();
		};
	}

	private static Point readPoint(GeometryFactory factory, DataInput in, TWKBHeader header) throws IOException {
	...
	private static LineString readLineString(GeometryFactory factory, DataInput in, TWKBHeader header, long[] prev)
			throws IOException {
		CoordinateSequence coordinates = readCoordinateSequence(factory, in, header, prev);
		return factory.createLineString(coordinates);
	}

	private static LinearRing readLinearRing(GeometryFactory factory, DataInput in, TWKBHeader header, long[] prev)
			throws IOException {
	...
	private static Polygon readPolygon(GeometryFactory factory, DataInput in, TWKBHeader header, long[] prev)
			throws IOException {
	...
	private static MultiLineString readMultiLineString(GeometryFactory factory, DataInput in, TWKBHeader header)
			throws IOException {
		final int nmembers = Varint.readUnsignedVarInt(in);
		if (header.hasIdList()) {
			skipIdList(nmembers, in);
	...
	private static Geometry readGeometryCollection(GeometryFactory factory, DataInput in, TWKBHeader header)
			throws IOException {
	...
	private static void readIdList(int nmembers, /* Nullable */ long[] target, DataInput in) throws IOException {
	...
	private static CoordinateSequence readCoordinateSequence(GeometryFactory factory, DataInput in, TWKBHeader header,
	...
	private static CoordinateSequence readCoordinateSequence(GeometryFactory factory, DataInput in, int size,
	...
	private static CoordinateSequence createCoordinateSequence(GeometryFactory factory, int size,
		...
		CoordinateSequence sequence = factory.getCoordinateSequenceFactory().create(size, dim, measures);
		if (sequence.getDimension() != dim) {
			throw new IllegalStateException(
					"Provided CoordinateSequenceFactory does not support the required dimension. Requested " + header
							+ ", returned " + sequence.getDimension());
		}
