...
public class TWKBWriter {
	...
	private TWKBHeader paramsHeader = new TWKBHeader().setXyPrecision(7).setZPrecision(0).setMPrecision(0)
			.setHasBBOX(false).setHasSize(false);
	 ...
	 *
	 * <p>
	 * A positive retaining information to the right of the decimal place, negative
	...
	private void writeCoordinateSequence(CoordinateSequence coordinateSequence, DataOutput out, TWKBHeader header,
			long[] prev, int minNPoints) throws IOException {

		final int dimensions = header.getDimensions();
		long[] delta = new long[dimensions];
		int nPoints = 0;
		int nPointsRemaining = coordinateSequence.size();
		// Real number of points can't be determined beforehand, since duplicated points
		// may be removed,
	...
	private void writeMultiPoint(MultiPoint geom, DataOutput out, TWKBHeader header) throws IOException {
		...
		CoordinateSequence seq = geom.getFactory().getCoordinateSequenceFactory().create(geom.getCoordinates());
	...
	private long writeOrdinate(double ordinate, long previousOrdinateValue, int precision, DataOutput out)
			throws IOException {
		long preciseOrdinate = makePrecise(ordinate, precision);
