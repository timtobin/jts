...
/**
 * Repeats a function a given number of times. If the function has a single
 * numeric argument, the argument will be multiplied by the repeat counter for
 * every call, and the function results will be accumulated into a collection to
...
public class RepeaterGeometryFunction implements GeometryFunction {
	...
	public static boolean isRepeatable(GeometryFunction fun) {
		if (!(fun.getReturnType() == Geometry.class))
			return false;
		...
		if (paramType.length < repeatArgIndex + 1)
			return true;
