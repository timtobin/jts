...
/**
 * A {@link GeometryFunction} which calls a static {@link Method}.
 *
 * @author Martin Davis
 */
public class StaticMethodGeometryFunction extends BaseGeometryFunction {
	...
	public static StaticMethodGeometryFunction createFunction(Method method) {
		...
		return new StaticMethodGeometryFunction(category, funcName, description, paramNames, paramTypes, returnType,
				method);
	}
	...
	/**
	 * Java doesn't permit accessing the original code parameter names,
	 * unfortunately.
	 *
	 * @param method
	 * @return
	...
	private static String paramNamePrimary(Class<?> clz) {
		...
		if (ClassUtil.isDouble(clz))
			return PARAM_NAME_DISTANCE;
	...
	public StaticMethodGeometryFunction(String category, String name, String description, String[] parameterNames,
