...
/**
 * Maintains the information associated with mapping the model view to the
 * screen
 *
 * @author Martin Davis
 */
public class Viewport implements PointTransformation {
	...
	/**
	 * Snaps scale to nearest multiple of 2, 5 or 10. This ensures that model
	 * coordinates entered via the geometry view don't carry more precision than the
	 * zoom level warrants.
	 *
	 * @param scaleRaw
	 * @return
	...
	private Envelope computeEnvelopeInModel() {
		return new Envelope(originInModel.getX(), originInModel.getX() + getWidthInModel(), originInModel.getY(),
