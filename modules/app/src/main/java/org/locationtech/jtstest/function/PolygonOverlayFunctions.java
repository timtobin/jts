...
public class PolygonOverlayFunctions {
	 ...
	 * @param g1
	 * @param g2
	 *            a geometry to overlay (may be null)
	 * @param noder
	...
	private static Geometry computeOverlay(Geometry g1, Geometry g2, Noder noder) {
		...
		/**
		 * If the input contained polygons, use PIP to find polygons which have a
		 * parent. Otherwise just return all resultants (to support providing just lines
		 * as input)
		 */
	...
	/**
	 * Finds parentage of a set of overlay resultants. Currently just finds set of
	 * resultants which have at least one parent . This effectively removes holes
	 * from the result set.
	 *
	 * @author mdavis
	 */
		...
		public List<Polygon> findParents(List<Polygon> resultants) {
			...
			for (Polygon res : resultants) {
				...
				for (Geometry cand : candidates) {
					...
					if (isParent) {
						/**
						 * For now, keep resultants which have at least one parent. This could be
						 * enhanced to record all parents of a resultant.
						 */
						polys.add(res);
						break;
					}
