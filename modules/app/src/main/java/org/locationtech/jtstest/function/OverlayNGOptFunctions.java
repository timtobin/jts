...
public class OverlayNGOptFunctions {
	...
	/**
	 * Using auto slows things down quite a bit (due to need to scan to find scale
	 * factor), so not recommended.
	 *
	 * @param a
	 * @param b
	 * @return
	...
	public static Geometry intersectionSR(Geometry a, Geometry b, @Metadata(title = "Grid Scale") double scaleFactor) {
		Geometry intFast = fastCoversIntersection(a, b);
		if (intFast != null)
			return intFast;
		return OverlayNG.overlay(a, b, INTERSECTION, new PrecisionModel(scaleFactor));
