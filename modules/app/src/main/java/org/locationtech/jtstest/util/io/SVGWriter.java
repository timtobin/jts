...
public class SVGWriter {
   ...
   * @param writer the output writer to append to
   */
  private void appendSequencePath(
  ...
  private void appendSequencePoints(
      CoordinateSequence seq, int level, boolean doIndent, Writer writer) throws IOException {
    if (seq.size() == 0) {
      // writer.write("EMPTY");
    } else {
  ...
  private void appendPolygonPath(Polygon polygon, int level, boolean indentFirst, Writer writer)
    ...
    for (int i = 0; i < polygon.getNumInteriorRing(); i++) {
      writer.write(" ");
      appendSequencePath(
          polygon.getInteriorRingN(i).getCoordinateSequence(), level + 1, true, writer);
    }
    writer.write("' />\n");
  }
  ...
  private void appendMultiPoint(MultiPoint multiPoint, int level, Writer writer)
    ...
    if (multiPoint.isEmpty()) {
      ...
      for (int i = 0; i < multiPoint.getNumGeometries(); i++) {
        ...
        appendPoint(
            multiPoint.getGeometryN(i).getCoordinate(),
            level2,
            writer,
   ...
   * @param writer the output writer to append to
   */
  private void appendMultiLineString(
