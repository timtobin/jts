...
public class CreateRandomShapeFunctions {
	...
	public static Geometry haltonPointsWithBases(Geometry g, int nPts, @Metadata(title = "Base 1") int basei,
	...
	public static Geometry randomSegments(Geometry g, int nPts) {
		...
		for (int i = 0; i < nPts; i++) {
			...
			lines.add(geomFact.createLineString(new Coordinate[]{new Coordinate(x0, y0), new Coordinate(x1, y1)}));
		}
	 ...
	 * <a href=
	 * 'http://extremelearning.com.au/unreasonable-effectiveness-of-quasirandom-sequences'>Roberts
	 * recurrences</a> are based on the generalized Golden Ratio (for the 2D case,
	 * Phi2). They have excellent low-discrepancy characteristics. This mean they
	 * are non-periodic and have less clustering than random points or Halton
	 * points.
	 *
	 * @param geom
	 * @param nPts
	...
	/**
	 * Truchet tiling created from a base (lower-left) tile defined by from set of
	 * lines. The tiles are copied to a square grid of size nSide, and rotated by a
	 * random multiple of 90 degrees. The tile line endpoints are snapped to a
	 * precision grid to make them align. If tiling is to be polygonized lines
	 * should be noded and snapped to edge points.
	 *
	 * @param tileLines
	 *            set of lines defining base tile
	 * @param nSide
	 *            number of tiles per side of tiling
	 * @return lines for tiling.
	 */
	@Metadata(description = "Create Truchet tiling from lines defining lower left tile")
	public static Geometry truchetTiling(Geometry tileLines, @Metadata(title = "Grid side cell #") int nSide,
		...
		for (int i = 0; i < nSide; i++) {
			for (int j = 0; j < nSide; j++) {
				...
				AffineTransformation trans = AffineTransformation.rotationInstance(nPi2 * Math.PI / 2.0, centre.getX(),
						centre.getY());
