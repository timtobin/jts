...
public class JTSTestBuilderToolBar {
	...
	public JToolBar getToolBar() {
		 ...
		 * --------------------------------------------------
		 */
		JButton previousButton = createButton(AppStrings.TIP_PREV, leftIcon, e -> {
			boolean isZoom = 0 == (e.getModifiers() & ActionEvent.CTRL_MASK);
			controller().caseMoveTo(-1, isZoom);
		});
		JButton nextButton = createButton(AppStrings.TIP_NEXT, rightIcon, e -> {
			boolean isZoom = 0 == (e.getModifiers() & ActionEvent.CTRL_MASK);
			controller().caseMoveTo(1, isZoom);
		});
		JButton newButton = createButton(AppStrings.TIP_CASE_ADD_NEW, plusIcon, e -> controller().caseCreateNew());
		JButton copyButton = createButton(AppStrings.TIP_CASE_DUP, copyCaseIcon, e -> controller().caseCopy());
		JButton deleteButton = createButton(AppStrings.TIP_CASE_DELETE, deleteIcon, e -> controller().caseDelete());
		JButton oneToOneButton = createButton(AppStrings.TIP_ZOOM_1_1, zoomOneToOneIcon,
				e -> controller().zoomOneToOne());
		JButton zoomToInputButton = createButton("Zoom To Input", zoomToInputIcon, e -> controller().zoomToInput());
		...
		infoButton = createToggleButton(AppStrings.TIP_INFO, infoIcon, e -> controller().modeInfo());
		zoomButton = createToggleButton(AppStrings.TIP_ZOOM, zoomIcon, e -> controller().modeZoomIn());
		panButton = createToggleButton(AppStrings.TIP_PAN, panIcon, e -> controller().modePan());

		btnEditVertex = createToggleButton(AppStrings.TIP_MOVE_VERTEX, moveVertexIcon,
				e -> controller().modeEditVertex());

		btnMove = createToggleButton(AppStrings.TIP_MOVE, AppIcons.MOVE, e -> controller().modeMove());
		...
		JToggleButton selectComponentButton = createToggleButton(AppStrings.TIP_SELECT_ELEMENTS,
				new ImageIcon(this.getClass().getResource("Select.png")), e -> controller().modeSelectComponent());

		deleteVertexButton = createToggleButton(AppStrings.TIP_DELETE_VERTEX_ELEMENT,
				new ImageIcon(this.getClass().getResource("DeleteVertex.png")), e -> controller().modeDeleteVertex());
