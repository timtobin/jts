...
public class LayerStylePanel extends JPanel {
	...
	private static int getPaletteType(JComboBox comboPal) {
		...
		if (palName.equalsIgnoreCase(paletteNames[1]))
			paletteType = Palette.TYPE_VARY;
		if (palName.equalsIgnoreCase(paletteNames[2]))
			paletteType = Palette.TYPE_SPECTRUM;
		if (palName.equalsIgnoreCase(paletteNames[3]))
			paletteType = Palette.TYPE_SPECTRUM_RANDOM;
		return paletteType;
	}
	...
	private static int getVertexSymbol(JComboBox combo) {
		...
		for (int i = 0; i < vertexSymbolNames.length; i++) {
			if (name.equalsIgnoreCase(vertexSymbolNames[i]))
				return i;
		}
	...
	private static void setPaletteType(JComboBox comboPal, int paletteType) {
		int index = 0;
		if (paletteType == Palette.TYPE_VARY)
			index = 1;
		if (paletteType == Palette.TYPE_SPECTRUM)
			index = 2;
		if (paletteType == Palette.TYPE_SPECTRUM_RANDOM)
			index = 3;
	...
	private GridBagConstraints gbc(int x, int y, int align, double weightX) {
		return new GridBagConstraints(x, y, 1, 1, weightX, 1, // weights
				align, GridBagConstraints.NONE, new Insets(2, 2, 2, 2), 2, 0);
	}
	...
	private JPanel stylePanel() {
		...
		btnVertexColor = ColorControl.create(this, "Vertex", AppColors.GEOM_VIEW_BACKGROUND, clr -> {
			if (layer == null)
				return;
		...
		btnLineColor = ColorControl.create(this, "Line", AppColors.GEOM_VIEW_BACKGROUND, clr -> {
		...
		addRow("Line", cbStroked, btnLineColor, btnVertexSynch, sliderLineAlpha, spinnerLineWidth, cbDashed, cbOffset,
				spinOffsetSize);
		...
		btnFillColor = ColorControl.create(this, "Fill", AppColors.GEOM_VIEW_BACKGROUND, clr -> {
		...
		btnLabelColor = ColorControl.create(this, "Label", AppColors.GEOM_VIEW_BACKGROUND, clr -> {
			if (layer == null)
				return;
	 ...
	 * addRow(title, c1, c2, null, null); } private void xaddRow(String title,
	 * JComponent c1, JComponent c2, JComponent c3) { addRow(title, c1, c2, c3,
	 * null, null); } private void xaddRow(String title, JComponent c1, JComponent
	 * c2, JComponent c3, JComponent c4) { addRow(title, c1, c2, c3, c4, null); }
	 */
	...
	private void uiInit() throws Exception {
		...
		JButton btnReset = SwingUtil.createButton(AppIcons.CLEAR, "Reset style to default", arg0 -> {
			if (layer == null)
				return;
			layer.resetStyle();
			updateStyleControls();
