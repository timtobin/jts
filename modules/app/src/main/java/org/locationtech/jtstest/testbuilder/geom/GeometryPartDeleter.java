 ...
 * If the box completely contains one or more components (including polygon
 * holes), those components are deleted and the operation stops. Otherwise if
 * the box contains a subset of vertices from a component, those vertices are
 * deleted. When deleting vertices only <i>one</i> component of the geometry is
 * modified (the first one found which has vertices in the box).
 *
 * @author Martin Davis
 */
public class GeometryPartDeleter {
	...
	public static Geometry deleteComponentsAndVertices(Geometry geom, Envelope env,
			boolean deleteIntersectingComponents) {
		...
		if (deleteIntersectingComponents)
			return geom;
