...
public class HtmlWriter {
	...
	private Object actualResult(TestCaseEdit testCaseEdit, String opName, String first, String second)
			throws Exception {
		try {
			...
			if (second == null) {
				...
				target = new Object[]{testCaseEdit.getGeometry(second.equalsIgnoreCase("A") ? 0 : 1)};
				targetClasses = new Class[]{geometryClass};
			}
	...
	private void createGifFile(String filenameNoPath, Geometry a, Geometry b, Geometry result, boolean showingAB,
			int imageWidth, int imageHeight, boolean zoomToFullExtent) {
	...
	private BooleanResult expectedPredicateResult(TestCaseEdit testCaseEdit, String opName, String first,
		...
		for (Test test : trTestCase.getTests()) {
			if (test.getOperation().equalsIgnoreCase(opName) && test.getGeometryIndex().equalsIgnoreCase(first)
	...
	private String html(Testable testable, int runSkey, int caseSkey) throws IOException {
		...
		String html = "<HTML>" + StringUtil.newLine + "<HEAD>" + StringUtil.newLine + "<TITLE>"
				+ StringUtil.escapeHTML(testName(testCaseEdit, caseSkey)) + "</TITLE>" + StringUtil.newLine
				+ "<link REL='STYLESHEET' HREF='../jts.css' TYPE='Text/css'>" + StringUtil.newLine + "</HEAD>"
				+ StringUtil.newLine + "<BODY>" + StringUtil.newLine + "<div class='testTitle'>"
				+ StringUtil.escapeHTML(testName(testCaseEdit, caseSkey)) + "</div>" + StringUtil.newLine + "<P>"
				+ StringUtil.newLine;
	...
	private String htmlForAB(TestCaseEdit testCaseEdit, int runSkey, int caseSkey) throws IOException {
		String wktHtml = "<span class=wktA>"
				+ (testCaseEdit.getGeometry(0) == null ? " " : testCaseEdit.getGeometry(0).toText()) + "</span>" + "<P>"
				+ "<span class=wktB>"
				+ (testCaseEdit.getGeometry(1) == null ? " " : testCaseEdit.getGeometry(1).toText()) + "</span>";
		String html = StringUtil.newLine + "<TABLE BORDER=0>" + StringUtil.newLine + "  <TR>" + StringUtil.newLine
				+ htmlImageHtmlTextTable("Run" + runSkey + AppStrings.LABEL_TEST_CASE + caseSkey + ".gif", wktHtml, 0)
				+ "  </TR>" + StringUtil.newLine + "</TABLE>" + StringUtil.newLine;
	...
	private String htmlForBinaryPredicates(TestCaseEdit testCaseEdit, int caseSkey) {
		...
		if (testCaseEdit.getGeometry(1) != null) {
			...
			html = "<h2>Binary Predicates</h2>" + StringUtil.newLine + "<TABLE WIDTH=50% BORDER=1>" + StringUtil.newLine
					+ html + "</TABLE>" + StringUtil.newLine;
		}
		return html;
	}

	private String htmlForPredicateTest(TestCaseEdit testCaseEdit, int caseSkey, String opName, String first,
			String second) {
		...
		return "  <TR>" + StringUtil.newLine + "    <TD class=methodTitle>" + opName + "</TD>" + StringUtil.newLine
				+ "    <TD class=resultFalse>" + actualResultString + "</TD>" + StringUtil.newLine + "  </TR>"
				+ StringUtil.newLine;
	}
	...
	private String htmlForRelateTest(TestCaseEdit testCaseEdit, int caseSkey) {
		...
		return "  <TR>" + StringUtil.newLine + "    <TD class=methodTitle rowspan=9>relate</TD>" + StringUtil.newLine
				+ "    <TD rowspan=9>" + actualValue + "</TD>" + StringUtil.newLine + "  </TR>" + StringUtil.newLine;
	}
	...
	private String htmlForSpatialFunctionTest(TestCaseEdit testCaseEdit, int runSkey, int caseSkey,
		...
		String actualResultString;
		try {
			Geometry actualResult = (Geometry) actualResult(testCaseEdit, geometryOpName, first, second);
			...
			actualResultString = htmlImageHtmlTextTable(filenameNoPath,
					"<SPAN class=wktR>" + actualResult.toText() + "</SPAN>", 0);
			createGifFile(filenameNoPath, testCaseEdit.getGeometry(0), testCaseEdit.getGeometry(1), actualResult,
		...
		return "  <TR>" + StringUtil.newLine + "    <TD class=methodTitle>" + geometryOpName + "</TD>"
				+ StringUtil.newLine + actualResultString + StringUtil.newLine + "  </TR>" + StringUtil.newLine;
	}

	private String htmlForTests(TestCaseEdit testCaseEdit, int runSkey, int caseSkey) {
		return htmlForBinaryPredicates(testCaseEdit, caseSkey);
	}

	private String htmlImageHtmlTextTable(String imageFilename, String html, int border) {
		return "    <TD>" + StringUtil.newLine + "      <IMG BORDER=\"1\" SRC=\"" + imageFilename + "\" WIDTH="
				+ IMAGE_WIDTH + " HEIGHT=" + IMAGE_HEIGHT + ">" + StringUtil.newLine + "    </TD>" + StringUtil.newLine
				+ "    <TD>" + StringUtil.newLine + html + StringUtil.newLine + "    </TD>" + StringUtil.newLine;
	}
	...
	private String htmlTitle(PrecisionModel precisionModel) {
		String html = "Precision Model: scale=" + precisionModel.getScale() + StringUtil.newLine;
		html = "<div class='precisionModel'>" + html + "</div>";
		return html;
	}

	private String indexHtml(List runs, Map runMap, PrecisionModel precisionModel) {
		StringBuilder html = new StringBuilder("<HTML>" + StringUtil.newLine + "<HEAD>" + StringUtil.newLine
				+ "<TITLE>JTS Test Suite Index</TITLE>" + StringUtil.newLine
				+ "<link REL='STYLESHEET' HREF='../jts.css' TYPE='Text/css'>" + StringUtil.newLine
				+ "<script LANGUAGE=\"JavaScript\">" + StringUtil.newLine + "  function LoadDetailFrame() {"
				+ StringUtil.newLine + "        testNumber = document.main_form.test_combo.selectedIndex;"
				+ StringUtil.newLine + "        testHtmlFile = document.main_form.test_combo.options[testNumber].value;"
				+ StringUtil.newLine + "        parent.detail.location.href=testHtmlFile;" + StringUtil.newLine
				+ "        document.main_form.test_combo.blur();" + StringUtil.newLine + "  }" + StringUtil.newLine
				...
				+ StringUtil.newLine);
		int runSkey = 0;
		for (Object object : runs) {
			String runDescription = (String) object;
			runSkey++;
			...
			int caseSkey = 0;
			for (Object o : testables) {
				Testable testable = (Testable) o;
				caseSkey++;
				html.append("              document.main_form.test_combo.length = ").append(caseSkey).append(";")
						.append(StringUtil.newLine);
				html.append("              document.main_form.test_combo.options[").append(caseSkey - 1)
						.append("].text  = \"").append(StringUtil.escapeHTML(testName(testable, caseSkey)))
						.append("\";").append(StringUtil.newLine);
				html.append("              document.main_form.test_combo.options[").append(caseSkey - 1)
						.append("].value  = 'Run").append(runSkey).append("Case").append(caseSkey).append(".html';")
						.append(StringUtil.newLine);
			}
			html.append("        LoadDetailFrame();");
			html.append("  }");
		}
		html.append("  }").append(StringUtil.newLine).append("</script>").append(StringUtil.newLine).append("</HEAD>")
				.append(StringUtil.newLine).append("<BODY>").append(StringUtil.newLine)
				.append("<h1>JTS Validation Suite</h1>").append(StringUtil.newLine).append(htmlTitle(precisionModel))
				.append("<p>").append(StringUtil.newLine).append("<FORM id=\"main_form\" name=\"main_form\">")
				.append(StringUtil.newLine);

		html.append("<select id=run_combo name=run_combo size='1' style='width:30%' onChange='onRunChange()'>")
				.append(StringUtil.newLine);
		runSkey = 0;
		for (Object run : runs) {
			String runDescription = (String) run;
			runSkey++;
			html.append("<OPTION VALUE='Run").append(runSkey).append("'>")
					.append(StringUtil.escapeHTML(runName(runDescription, runSkey))).append("</OPTION>")
					.append(StringUtil.newLine);
		}
		html.append("</select>").append(StringUtil.newLine);

		html.append("<select id=test_combo name=test_combo size='1' style='width:60%' onChange='LoadDetailFrame()'>")
				.append(StringUtil.newLine);
		...
		for (Object o : testables) {
			Testable testable = (Testable) o;
			caseSkey++;
			html.append("<OPTION VALUE='Run1Case").append(caseSkey).append(".html'>")
					.append(StringUtil.escapeHTML(testName(testable, caseSkey))).append("</OPTION>")
					.append(StringUtil.newLine);
		}
		html.append("</select>").append(StringUtil.newLine);

		html.append("</FORM>").append(StringUtil.newLine).append("</BODY>").append(StringUtil.newLine).append("</HTML>")
	...
	private MapAndList runMapAndRuns(TestCaseList testCaseList) {
		...
		for (Object o : testCaseList.getList()) {
			TestCaseEdit testCaseEdit = (TestCaseEdit) o;
			...
			if (testable instanceof TestRunnerTestCaseAdapter adapter) {
				...
				runDescription = runDescription.contains(".")
						? runDescription.substring(0, runDescription.indexOf("."))
						: runDescription;
	 ...
	 * Geometry spatialFunction, boolean showingAB, int imageWidth, int imageHeight,
	 * boolean zoomToFullExtent) throws FileNotFoundException, IOException {
	 ...
	 * GeometryBuilder builderSpatialFunction = spatialFunction != null ?
	 * GeometryBuilder.create(spatialFunction) : null; createGifFile(filenameNoPath,
	 * builderA, builderB, builderSpatialFunction, showingAB, imageWidth,
	 * imageHeight, zoomToFullExtent); }
	 */
	...
	private String testName(Testable testable, int caseSkey) {
		...
		if ((name == null || name.isEmpty()) && testable instanceof TestCaseEdit edit) {
	...
	private String testTopHtml() {
		return "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.0 Frameset//EN\"\"http://www.w3.org/TR/REC-html40/frameset.dtd\">"
				+ StringUtil.newLine + "<HTML>" + StringUtil.newLine + "<HEAD>" + StringUtil.newLine + "<TITLE>"
				+ StringUtil.newLine + "JTS Validation Suite" + StringUtil.newLine + "</TITLE>" + StringUtil.newLine
				+ "</HEAD>" + StringUtil.newLine + "<FRAMESET rows=\"120px,*\" framespacing=0 frameborder=0>"
				+ StringUtil.newLine
	...
	public void write(File outputDirectory, TestCaseList testCaseList, PrecisionModel precisionModel)
			throws IOException {
		if (busyDialog != null) {
		...
		for (Object run : runs) {
			...
			for (Object o : testables) {
				...
				caseSkey++;
				if (busyDialog != null) {
					busyDialog.setDescription("Saving .html and .gif files: " + caseSkey + " of "
							+ testCaseList.getList().size() + " tests");
				}
				createHtmlFile("Run" + runSkey + AppStrings.LABEL_TEST_CASE + caseSkey + ".html",
						html(testable, runSkey, caseSkey));
			}
		}
	}
