...
public class HtmlWriter {
  ...
  public void write(File outputDirectory, TestCaseList testCaseList, PrecisionModel precisionModel)
    ...
    for (Iterator i = runs.iterator(); i.hasNext(); ) {
      ...
      for (Iterator m = testables.iterator(); m.hasNext(); ) {
        ...
        caseSkey++;
        if (busyDialog != null) {
          busyDialog.setDescription(
              "Saving .html and .gif files: "
                  + caseSkey
                  + " of "
                  + testCaseList.getList().size()
                  + " tests");
        }
        createHtmlFile(
            "Run" + runSkey + AppStrings.LABEL_TEST_CASE + caseSkey + ".html",
  ...
  private String html(Testable testable, int runSkey, int caseSkey) throws IOException {
    ...
    String html =
        "<HTML>"
            ...
            + StringUtil.newLine
            + "<TITLE>"
            + StringUtil.escapeHTML(testName(testCaseEdit, caseSkey))
            + "</TITLE>"
            + StringUtil.newLine
  ...
  private String htmlForSpatialFunctionTest(
      TestCaseEdit testCaseEdit,
      int runSkey,
      int caseSkey,
      String geometryOpName,
      String first,
      String second) {
    ...
    try {
      ...
      String filenameNoPath =
          "Run" + runSkey + AppStrings.LABEL_TEST_CASE + caseSkey + geometryOpName + "Actual";
      if (first != null) {
      ...
      actualResultString =
          htmlImageHtmlTextTable(
              filenameNoPath, "<SPAN class=wktR>" + actualResult.toText() + "</SPAN>", 0);
      createGifFile(
          filenameNoPath,
          testCaseEdit.getGeometry(0),
          testCaseEdit.getGeometry(1),
          actualResult,
  ...
  private String htmlForRelateTest(TestCaseEdit testCaseEdit, int caseSkey) {
    ...
    try {
      actualValue =
          insertParagraphs(
              testCaseEdit.getGeometry(0).relate(testCaseEdit.getGeometry(1)).toString());
    } catch (Exception e) {
      actualValue =
    ...
    String html =
        "  <TR>"
            + StringUtil.newLine
            + "    <TD class=methodTitle rowspan=9>relate</TD>"
            + StringUtil.newLine
            + "    <TD rowspan=9>"
            + actualValue
            + "</TD>"
  ...
  private String htmlForPredicateTest(
      TestCaseEdit testCaseEdit, int caseSkey, String opName, String first, String second) {
    String actualResultString;
    try {
  ...
  private BooleanResult expectedPredicateResult(
      TestCaseEdit testCaseEdit, String opName, String first, String second) {
  ...
  private String htmlForAB(TestCaseEdit testCaseEdit, int runSkey, int caseSkey)
      throws IOException {
    String wktHtml =
        "<span class=wktA>"
    ...
    String html =
        StringUtil.newLine
            ...
            + StringUtil.newLine
            + htmlImageHtmlTextTable(
                "Run" + runSkey + AppStrings.LABEL_TEST_CASE + caseSkey + ".gif", wktHtml, 0)
            + "  </TR>"
            + StringUtil.newLine
            + "</TABLE>"
            + StringUtil.newLine;
    createGifFile(
        "Run" + runSkey + AppStrings.LABEL_TEST_CASE + caseSkey + ".gif",
        testCaseEdit.getGeometry(0),
        testCaseEdit.getGeometry(1),
        null,
        true,
  ...
  private MapAndList runMapAndRuns(TestCaseList testCaseList) {
    ...
    for (Iterator i = testCaseList.getList().iterator(); i.hasNext(); ) {
      ...
      if (testable instanceof TestRunnerTestCaseAdapter adapter) {
        org.locationtech.jtstest.testrunner.TestCase testRunnerTestCase =
            adapter.getTestRunnerTestCase();
        String runDescription = testRunnerTestCase.getTestRun().getTestFile().getName();
        runDescription =
            runDescription.indexOf(".") > -1
  ...
  private String indexHtml(List runs, Map runMap, PrecisionModel precisionModel) {
    ...
    for (Iterator i = runs.iterator(); i.hasNext(); ) {
      ...
      for (Iterator m = testables.iterator(); m.hasNext(); ) {
        ...
        caseSkey++;
        html +=
            "              document.main_form.test_combo.length = "
                + caseSkey
                + ";"
                + StringUtil.newLine;
        html +=
            "              document.main_form.test_combo.options["
                + (caseSkey - 1)
                + "].text  = \""
    ...
    html +=
        "<select id=run_combo name=run_combo size='1' style='width:30%' onChange='onRunChange()'>"
            + StringUtil.newLine;
    runSkey = 0;
    ...
    html +=
        "<select id=test_combo name=test_combo size='1' style='width:60%' onChange='LoadDetailFrame()'>"
            + StringUtil.newLine;
