...
public class LineSegmentFunctions {
	public static Geometry intersection(Geometry g1, Geometry g2) {
		...
		return switch (ri.getIntersectionNum()) {
			case 0 ->
				// no intersection => return empty point
				g1.getFactory().createPoint((Coordinate) null);
			case 1 ->
				// return point
				g1.getFactory().createPoint(ri.getIntersection(0));
			case 2 ->
				// return line
				g1.getFactory().createLineString(new Coordinate[]{ri.getIntersection(0), ri.getIntersection(1)});
			default -> null;
		};
	}
