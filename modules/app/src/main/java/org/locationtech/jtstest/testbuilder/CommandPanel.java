...
public class CommandPanel extends JPanel {
	...
	public String getCommandName() {
		...
		if (commandIndex <= 0)
			commandIndex = commandLog.size() + 1;
	...
	void jbInit() throws Exception {
		...
		txtCmd.getDocument().addDocumentListener(new DocumentListener() {
			...
			public void save() {
				/**
				 * If the change occurred via internal setting, don't copy to the save buffer.
				 */
		...
		JButton btnRun = SwingUtil.createButton(AppIcons.EXECUTE, "Run Command", e -> doRun());

		JButton btnPaste = SwingUtil.createButton(AppIcons.PASTE, "Paste Command", e -> {
			setCommandText(getPaste());
			update();
		});

		JButton btnClear = SwingUtil.createButton(AppIcons.CUT, "Clear Command", e -> {
			setCommandText("");
			update();
		});

		JButton btnPrev = SwingUtil.createButton(AppIcons.LEFT, "Previous Command", e -> {
			if (historyIndex == -1) {
		...
		JButton btnNext = SwingUtil.createButton(AppIcons.RIGHT, "Next Command", e -> {
			if (historyIndex == -1)
				return;
		...
		/*
		 * Box btnPanel = Box.createVerticalBox(); btnPanel.setPreferredSize(new
		 * java.awt.Dimension(30, 30)); //btnPanel.add(btnRun); this.add(btnPanel,
		 * BorderLayout.EAST);
		 */
	...
	/**
	 * Record the command in history, but only if it is different to ones already
	 * there
	 *
	 * @param cmd
	 */
	public void saveCommand(String cmd) {
		if (commandLog.contains(cmd))
			return;
		commandLog.add(cmd);
	}
