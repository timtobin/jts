...
/**
 * A CLI to run JTS TestBuilder operations. Allows easier execution of JTS functions on test data
...
public class JTSOpCmd {
  ...
  private static CommandLine createCmdLine() {
    ...
    commandLine
        .addOptionSpec(new OptionSpec(CommandOptions.GEOMFUNC, OptionSpec.NARGS_ONE_OR_MORE))
  ...
  JTSOpRunner.OpParams parseArgs(String[] args) throws ParseException, ClassNotFoundException {
    ...
    cmdArgs.outputFile =
        commandLine.hasOption(CommandOptions.OUTPUT)
            ? commandLine.getOptionArg(CommandOptions.OUTPUT, 1)
            : null;

    cmdArgs.repeat =
        commandLine.hasOption(CommandOptions.REPEAT)
    ...
    cmdArgs.filterOp =
        cmdArgs.isFilter ? parseFilterOp(commandLine.getOptionArg(CommandOptions.WHERE, 0)) : 0;
    cmdArgs.filterVal =
        cmdArgs.isFilter ? commandLine.getOptionArgAsNum(CommandOptions.WHERE, 1) : 0;

    cmdArgs.eachA = commandLine.hasOption(CommandOptions.EACHA);
    cmdArgs.eachB = commandLine.hasOption(CommandOptions.EACHB);

    boolean isVerbose =
        commandLine.hasOption(CommandOptions.VERBOSE) || commandLine.hasOption(CommandOptions.V);
  ...
  private String[] parseOpArg(String arg) {
    ...
    if (arg.contains("(")) throw new CommandError(ERR_INVALID_ARG_PARAM, arg);
  ...
  private String[] parseMacroArgs(String macroTerm) {
    ...
    if (indexLeft < 0 || indexRight <= 0) throw new CommandError(ERR_INVALID_ARG_PARAM, macroTerm);
