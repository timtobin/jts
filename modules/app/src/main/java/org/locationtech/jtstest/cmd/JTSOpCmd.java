...
/**
 * A CLI to run JTS TestBuilder operations. Allows easier execution of JTS
 * functions on test data for debugging purposes.
 *
 * <p>
 * Examples:
 *
...
public class JTSOpCmd {
	...
	static final String[] helpDoc = new String[]{"", "Usage: jtsop - CLI for JTS operations",
			"           [ -a  <wkt> | <wkb> | stdin | <filename.ext> ]",
			"           [ -b  <wkt> | <wkb> | stdin | <filename.ext> ]",
			"           [ -ab <wkt> | <wkb> | stdin | <filename.ext> ]", "           [ -limit N ]",
			"           [ -offset N ]", "           [ -collect ]", "           [ -eacha ]", "           [ -eachb ]",
			"           [ -index ]", "           [ -repeat N ]",
			"           [ -where (eq | ne | ge | gt | le | lt) N ]", "           [ -validate ]",
			"           [ -explode", "           [ -srid SRID ]",
			"           [ -f ( txt | wkt | wkb | geojson | gml | svg ) ]", "           [ -q", "           [ -time ]",
			"           [ -v, -verbose ]", "           [ -o filename ]", "           [ -help ]",
			"           [ -geomfunc classname ]", "           [ -op ]", "           [ op [ args... ]]",
			"           op       name of the operation (in format Category.op)",
			"           args     one or more scalar arguments to the operation",
			"           - To run over multiple arguments use v1,v2,v3 OR val(v1,v2,v3,..)", "", "===== Input options:",
			...
			"===== Operation options:", "  -collect        execute op on collection of A geometries",
			"  -eacha          execute op on each element of A", "  -eachb          execute op on each element of B",
			"  -index          index the B geometries", "  -repeat         repeat the operation N times",
			...
			"  -op             separator to delineate operation arguments", "===== Output options:",
			"  -srid           sets the SRID on output geometries", "  -explode        output atomic geometries",
			"  -f              output format to use.  Default is txt/wkt",
			"  -q              quiet mode - result is not output", "  -o filename     write result output to filename",
			"===== Logging options:", "  -time           display execution time",
	...
	/**
	 * Detects various syntaxes for values: - (1,2,3) - val(1,2,3) - 1,2,3
	 *
	 * @param arg
	 * @return
	 */
	...
	JTSOpRunner.OpParams parseArgs(String[] args) throws ParseException, ClassNotFoundException {
		...
		cmdArgs.filterOp = cmdArgs.isFilter ? parseFilterOp(commandLine.getOptionArg(CommandOptions.WHERE, 0)) : 0;
		cmdArgs.filterVal = cmdArgs.isFilter ? commandLine.getOptionArgAsNum(CommandOptions.WHERE, 1) : 0;

		cmdArgs.eachA = commandLine.hasOption(CommandOptions.EACHA);
		cmdArgs.eachB = commandLine.hasOption(CommandOptions.EACHB);

		boolean isVerbose = commandLine.hasOption(CommandOptions.VERBOSE) || commandLine.hasOption(CommandOptions.V);
		...
		// This allows applying a binary op with a fixed LHS to a limited set of RHS
		// geoms
	...
	private int parseFilterOp(String opStr) {
		if ("eq".equalsIgnoreCase(opStr))
			return FilterGeometryFunction.OP_EQ;
		if ("ne".equalsIgnoreCase(opStr))
			return FilterGeometryFunction.OP_NE;
		if ("ge".equalsIgnoreCase(opStr))
			return FilterGeometryFunction.OP_GE;
		if ("gt".equalsIgnoreCase(opStr))
			return FilterGeometryFunction.OP_GT;
		if ("le".equalsIgnoreCase(opStr))
			return FilterGeometryFunction.OP_LE;
		if ("lt".equalsIgnoreCase(opStr))
			return FilterGeometryFunction.OP_LT;
