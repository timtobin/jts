...
public class GeometryPainter {
	...
	private static void paint(Geometry geometry, ShapeWriter converter, Graphics2D g, Color lineColor,
			Color fillColor) {
	...
	private static void paint(Geometry geometry, ShapeWriter converter, Graphics2D g, Color lineColor, Color fillColor,
		...
		if (geometry instanceof GeometryCollection gc) {
			 ...
			 * filled and non-filled (1D) elements correctly
			 */
		...
		if (geometry instanceof Point) {
			/*
			 * BasicStroke ptStroke = createPointStroke(stroke); g.setStroke(ptStroke);
			 * g.setColor(lineColor); g.draw(shape);
			 */
			return;
		}
		...
		if (lineColor != null) {
			...
			try {
				...
				/*
				 * if (geometry instanceof Polygon) { Shape polyShell = converter.toShape(
				 * ((Polygon)geometry).getExteriorRing()); g.setStroke(new BasicStroke(2));
				 * g.draw(polyShell); }
				 */
			} catch (Throwable ex) {
