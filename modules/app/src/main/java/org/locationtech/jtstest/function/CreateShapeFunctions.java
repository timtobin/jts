...
public class CreateShapeFunctions {
  ...
  public static Geometry fontGlyphSerifPointSize(
      Geometry g, String text, @Metadata(title = "Point size") int pointSize) {
    return fontGlyph(g, text, new Font(FontGlyphReader.FONT_SERIF, Font.PLAIN, pointSize));
  }

  public static Geometry fontGlyph(
      Geometry g, String text, @Metadata(title = "Font name") String fontName) {
  ...
  public static Geometry fontGlyphSansSerif(Geometry g, String text) {
    return fontGlyph(
        g, text, new Font(FontGlyphReader.FONT_SANSSERIF, Font.PLAIN, DEFAULT_POINTSIZE));
  }

  public static Geometry fontGlyphMonospaced(Geometry g, String text) {
    return fontGlyph(
        g, text, new Font(FontGlyphReader.FONT_MONOSPACED, Font.PLAIN, DEFAULT_POINTSIZE));
  }
  ...
  public static Geometry supercircle(
      Geometry g,
      @Metadata(title = "Point count") int nPts,
      @Metadata(title = "Power") double pow) {
    GeometricShapeFactory gsf = new GeometricShapeFactory();
    gsf.setNumPoints(nPts);
    if (g != null) gsf.setEnvelope(g.getEnvelopeInternal());
    else gsf.setEnvelope(new Envelope(0, 1, 0, 1));
  ...
  public static Geometry ellipseRotate(
      Geometry g, int nPts, @Metadata(title = "Angle") double ang) {
    GeometricShapeFactory gsf = new GeometricShapeFactory();
    gsf.setNumPoints(nPts);
    gsf.setRotation(ang);
    if (g != null) gsf.setEnvelope(g.getEnvelopeInternal());
  ...
  public static Geometry sineStar(
      Geometry g,
      @Metadata(title = "Arm count") int nArms,
      @Metadata(title = "Point count") int nPts) {
    Envelope env = FunctionsUtil.getEnvelopeOrDefault(g);
    GeometryFactory geomFact = FunctionsUtil.getFactoryOrDefault(g);
  ...
  public static Geometry spiral(
      Geometry geom,
      @Metadata(title = "Num Cycles") int nCycles,
      @Metadata(title = "Quadrant Segs") int quadrantSegs) {
    ...
    for (int i = 1; i <= nCycles; i++) {
      Coordinate[] inCycle =
          genSpiralCycle(centre, i * pitch - pitch / 2, (i + 1) * pitch - pitch / 2, quadrantSegs);
  ...
  private static Coordinate[] genSpiralCycle(
      Coordinate centre, double radiusStart, double radiusEnd, int quadrantSegs) {
  ...
  public static Geometry bezierCurve(
      Geometry geom, @Metadata(title = "Alpha (curveness)") double alpha) {
  ...
  public static Geometry bezierCurveSkew(
      Geometry geom,
      @Metadata(title = "Alpha (curveness)") double alpha,
      @Metadata(title = "Skew factor") double skew) {
  ...
  public static Geometry bezierControl(
      Geometry geom, @Metadata(title = "Alpha (curveness)") double alpha) {
  ...
  public static Geometry bezierControlSkew(
      Geometry geom,
      @Metadata(title = "Alpha (curveness)") double alpha,
      @Metadata(title = "Skew factor") double skew) {
  ...
  public static Geometry nGon(Geometry g, @Metadata(title = "Num sides") int sides) {
    Envelope env = FunctionsUtil.getEnvelopeOrDefault(g);
    Coordinate centre = env.centre();
