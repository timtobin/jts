...
public class LayerRenderer implements Renderer {
	...
	private void render(Graphics2D g, Viewport viewport, Geometry geometry, Layer layer) throws Exception {
	...
	private void renderCustomFill(Graphics2D g, Viewport viewport, Geometry gc, Layer layer) throws Exception {
		...
		/**
		 * Render each element separately. Otherwise it is not possible to render both
		 * filled and non-filled (1D) elements correctly. This also allows varying
		 * styling and cancellation.
		 */
	...
	private void renderGeom(Graphics2D g, Viewport viewport, Geometry geometry, Style style) throws Exception {
		if (!viewport.intersectsInModel(geometry.getEnvelopeInternal()))
			return;
