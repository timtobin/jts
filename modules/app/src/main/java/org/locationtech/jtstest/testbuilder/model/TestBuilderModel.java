...
public class TestBuilderModel {
  ...
  public GeometryFactory getGeometryFactory() {
    if (geometryFactory == null) geometryFactory = new GeometryFactory(getPrecisionModel());
  ...
  public LayerList getLayersAll() {
    LayerList layers =
        LayerList.create(layerListTop, layerList, getLayersFloating(), layerListBase);
    return layers;
  }
  ...
  public LayerList getLayersFloating() {
    ...
    if (layerSelect.hasGeometry()) list.addBottom(layerSelect);
    return list;
  }
  ...
  public Layer getLayerIndicators() {
    ...
    if (ind == null) ind = layerListBase.find(AppStrings.LYR_INDICATORS);
    if (ind == null) {
  ...
  private void initLayers() {
    ...
    if (geomEditModel != null)
      layerList
          .getLayer(LayerList.LYR_RESULT)
          .setSource(new ResultGeometryContainer(geomEditModel));
    ...
    lyrR.setGeometryStyle(
        new BasicStyle(AppColors.GEOM_RESULT_LINE_CLR, AppColors.GEOM_RESULT_FILL_CLR));

    layerSelect.setGeometryStyle(
  ...
  private TestCaseList createTestCaseList(File xmlTestFile) {
    ...
    for (Iterator i = testRun.getTestCases().iterator(); i.hasNext(); ) {
      org.locationtech.jtstest.testrunner.TestCase testCase =
          (org.locationtech.jtstest.testrunner.TestCase) i.next();
  ...
  public static class CaseList {
    ...
    public void prevCase() {
      if (tcIndex > 0) tcIndex--;
    }

    public void nextCase() {
      if (tcIndex < tcList.size() - 1) tcIndex++;
    }
    ...
    public void deleteCase() {
      ...
      if (tcIndex >= tcList.size()) tcIndex = tcList.size() - 1;
    }
  }
  ...
  */
  ...
  public void clearSelection() {
    if (layerSelect.getSource() != null) layerSelect.getSource().clear();
  }
}
