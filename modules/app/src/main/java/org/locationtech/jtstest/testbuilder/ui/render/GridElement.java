...
public class GridElement {
	 ...
	 *
	 * private int gridMagnitudeModel() { double pixelSizeModel =
	 * viewport.toModel(1); double pixelSizeModelLog = Math.log10(pixelSizeModel);
	 * int gridMag = (int) Math.ceil(pixelSizeModelLog);
	 *
	 * // Check if grid size is too small and if so increase it one magnitude double
	 * gridSizeModel = Math.pow(10, gridMag); double gridSizeView =
	 * viewport.toView(gridSizeModel); // System.out.println("\ncand gridSizeView= "
	 * + gridSizeView); if (gridSizeView <= MIN_GRID_PIXELS ) gridMag += 1;
	 *
	...
	private void drawAxes(Graphics2D g) {
		...
		if (vOriginX >= 0.0 && vOriginX <= viewport.getWidthInView()) {
			g.draw(new Line2D.Double(vOriginX, 0, vOriginX, viewport.getHeightInView()));
		}
	...
	private void drawDottedGrid(Graphics2D g) {
		 ...
		 * BasicStroke(1, // Width of stroke BasicStroke.CAP_SQUARE, // End cap style
		 * BasicStroke.JOIN_MITER, // Join style 10, // Miter limit new float[] {4, 6},
		 * // Dash pattern 0); // Dash phase g.setStroke(strokeMid);
		 *
		 * drawGridLines(g, basePt10View.getX() - gridSize10View/2, basePt10View.getY()
		 * + gridSize10View/2, gridSize10View);
		 */
		...
		/** Minor Grid Only display if dots are sparse enough */
		if (gridSizeView >= 4) {
		...
		drawGridLines(g, basePt10View.getX() - gridSize10View / 2, 0, gridSize10View / 2);
	}
	...
	private void drawGridLines(Graphics2D g, double minx, double maxy, double gridSizeInView) {
		...
		/**
		 * Can't draw right to edges of panel, because Swing inset border occupies that
		 * space.
		 */
	...
	private void drawScaleBar(Graphics2D g) {
		...
		for (int i = 0; i < 10; i += 2) {
			...
			// Y axis
			// g.draw(new Line2D.Double(x, y - i * gridSizeView, x, y - (i+1) *
			// gridSizeView));
		}
