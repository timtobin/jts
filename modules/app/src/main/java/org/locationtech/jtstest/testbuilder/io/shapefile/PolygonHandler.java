	...
	private ArrayList<ArrayList<LinearRing>> assignHolesToShells(ArrayList<LinearRing> shells,
		...
		for (LinearRing testHole : holes) {
			...
			for (LinearRing shell : shells) {
				...
				if (nShells <= 1 || PointLocation.isInRing(testHolePt, coordList) || pointInList(testHolePt, coordList))
					isContained = true;
	...
	boolean pointInList(Coordinate testPoint, Coordinate[] pointList) {
		...
		for (t = 0; t < numpoints; t++) {
			...
			if ((testPoint.x == p.x) && (testPoint.y == p.y)
					&& ((testPoint.getZ() == p.getZ()) || (!(testPoint.getZ() == testPoint.getZ()))) // nan test; x!=x
			// if x is nan
			) {
				return true;
			}
 ...
 * drag-n-drop for reading shp files
 *
 ...
 *
 * Revision 1.1 2002/08/27 21:04:58 dblasby original
 *
