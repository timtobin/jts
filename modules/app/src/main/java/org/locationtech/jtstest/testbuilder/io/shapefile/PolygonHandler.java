...
public class PolygonHandler implements ShapeHandler {
  ...
  boolean pointInList(Coordinate testPoint, Coordinate[] pointList) {
    ...
    for (t = 0; t < numpoints; t++) {
      ...
      if ((testPoint.x == p.x)
          && (testPoint.y == p.y)
          && ((testPoint.getZ() == p.getZ())
              || (!(testPoint.getZ() == testPoint.getZ()))) // nan test; x!=x if x is nan
      ) {
        return true;
      }
    }
  ...
  public Geometry read(
    ...
    if (shapeType != myShapeType) {
      throw new InvalidShapefileException(
          "PolygonHandler.read() - got shape type "
              + shapeType
              + " but was expecting "
              + myShapeType);
    }
    ...
    if (myShapeType >= 15) {
      ...
      if (myShapeType == 15) {
        // polyZ (with M)
        fullLength =
            22 + (2 * numParts) + (8 * numPoints) + 8 + (4 * numPoints) + 8 + (4 * numPoints);
      } else {
        // polyM (with M)
    ...
    for (int i = 0; i < shells.size(); i++) {
      polygons[i] =
          geometryFactory.createPolygon(
              ...
              (LinearRing[])
                  ((ArrayList<LinearRing>) holesForShells.get(i)).toArray(new LinearRing[0]));
    }
  ...
  private ArrayList<ArrayList<LinearRing>> assignHolesToShells(
      ArrayList<LinearRing> shells, ArrayList<LinearRing> holes) {
    ...
    for (int i = 0; i < holes.size(); i++) {
      ...
      for (int j = 0; j < nShells; j++) {
        ...
        if (nShells <= 1
            ...
            || pointInList(testHolePt, coordList)) isContained = true;
