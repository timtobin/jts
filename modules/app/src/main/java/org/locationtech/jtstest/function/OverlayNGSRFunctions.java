...
public class OverlayNGSRFunctions {
	...
	public static Geometry difference(Geometry a, Geometry b, @Metadata(title = "Grid Scale") double scaleFactor) {
		return OverlayNG.overlay(a, b, DIFFERENCE, new PrecisionModel(scaleFactor));
	}

	public static Geometry differenceBA(Geometry a, Geometry b, @Metadata(title = "Grid Scale") double scaleFactor) {
	...
	public static Geometry intersection(Geometry a, Geometry b, @Metadata(title = "Grid Scale") double scaleFactor) {
	...
	public static Geometry reducePrecision(Geometry a, @Metadata(title = "Grid Scale") double scaleFactor) {

		/**
		 * This ONLY works if the input GeometryCollection is a non-overlapping
		 * polygonal coverage!
		 */
		Geometry homoGeom = extractHomo(a);
		return PrecisionReducer.reducePrecision(homoGeom, new PrecisionModel(scaleFactor));
		/*
		 * // Not sure why this is needed? // Should be part of precision reducer, or
		 * Overlay (strict mode) List components = null; switch (a.getDimension()) {
		 * case 2: components = PolygonExtracter.getPolygons(union); break; case 1:
		 * components = LineStringExtracter.getLines(union); break; } Geometry result =
		 * a.getFactory().buildGeometry(components); return result;
		 */
	}

	public static Geometry symDifference(Geometry a, Geometry b, @Metadata(title = "Grid Scale") double scaleFactor) {
	...
	public static Geometry unaryUnion(Geometry a, @Metadata(title = "Grid Scale") double scaleFactor) {
		return UnaryUnionNG.union(a, new PrecisionModel(scaleFactor));
	}

	public static Geometry union(Geometry a, Geometry b, @Metadata(title = "Grid Scale") double scaleFactor) {
