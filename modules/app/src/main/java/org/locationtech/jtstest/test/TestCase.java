...
public class TestCase implements Testable {
	...
	public TestCase(String name, String description, String wkta, String wktb, String expectedIM,
			String expectedConvexHull, String expectedIntersection, String expectedUnion, String expectedDifference,
			String expectedSymDifference) {
		this(name, description, wkta, wktb, expectedIM, expectedConvexHull, expectedIntersection, expectedUnion,
				expectedDifference, expectedSymDifference, null);
	}

	public TestCase(String name, String description, String wkta, String wktb, String expectedIM,
			String expectedConvexHull, String expectedIntersection, String expectedUnion, String expectedDifference,
			String expectedSymDifference, String expectedBoundary) {
		try {
	...
	public TestCase(TestCase tc) {
		init(tc.name, tc.description, tc.getWellKnownText(0), tc.getWellKnownText(1), tc.expectedIM,
				tc.getExpectedConvexHull(), tc.getExpectedIntersection(), tc.getExpectedUnion(),
				tc.getExpectedDifference(), tc.getExpectedSymDifference(), tc.getExpectedBoundary());
	}
	...
	void init(String name, String description, String wkta, String wktb, String expectedIM, Geometry expectedConvexHull,
			Geometry expectedIntersection, Geometry expectedUnion, Geometry expectedDifference,
			Geometry expectedSymDifference, Geometry expectedBoundary) {
		this.name = name;
	...
	public void runTest() throws ParseException {
		...
		if (expectedIntersection != null) {
			...
			assertEqualsExact(expectedIntersection, result,
					" expected intersection " + expectedIntersection.toText() + " , found " + result.toText());
		}
		if (expectedUnion != null) {
			Geometry result = geom[0].union(geom[1]);
			assertEqualsExact(expectedUnion, result,
		...
		if (expectedDifference != null) {
			...
			assertEqualsExact(expectedDifference, result,
		...
		if (expectedSymDifference != null) {
			...
			assertEqualsExact(expectedSymDifference, result,
					" expected sym difference " + expectedSymDifference.toText() + " , found " + result.toText());
		}
	}
