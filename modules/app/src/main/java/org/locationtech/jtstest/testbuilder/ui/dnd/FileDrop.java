...
public class FileDrop {
  ...
  /**
   * Constructs a {@link FileDrop} with a default light-blue border and, if <var>c</var> is a {@link
   * java.awt.Container}, recursively sets all elements contained within as drop targets, though
  ...
  /**
   * Constructor with a default border and debugging optionally turned on. With Debugging turned on,
   * more status messages will be displayed to <tt>out</tt>. A common way to use this constructor is
   * with <tt>System.out</tt> or <tt>System.err</tt>. A <tt>null</tt> value for the parameter
   * <tt>out</tt> will result in no debugging output.
   *
  ...
  /**
   * Constructor with a default border, debugging optionally turned on and the option to recursively
   * set drop targets. If your component is a <tt>java.awt.Container</tt>, then each of its children
   * components will also listen for drops, though only the parent will change borders. With
   * Debugging turned on, more status messages will be displayed to <tt>out</tt>. A common way to
   * use this constructor is with <tt>System.out</tt> or <tt>System.err</tt>. A <tt>null</tt> value
  ...
  /**
   * Constructor with a specified border and debugging optionally turned on. With Debugging turned
   * on, more status messages will be displayed to <tt>out</tt>. A common way to use this
   * constructor is with <tt>System.out</tt> or <tt>System.err</tt>. A <tt>null</tt> value for the
  ...
  /**
   * Full constructor with a specified border and debugging optionally turned on. With Debugging
   * turned on, more status messages will be displayed to <tt>out</tt>. A common way to use this
   * constructor is with <tt>System.out</tt> or <tt>System.err</tt>. A <tt>null</tt> value for the
   * parameter <tt>out</tt> will result in no debugging output.
   *
   * @param out PrintStream to record debugging info or null for no debugging.
      ...
      dropListener =
          new java.awt.dnd.DropTargetListener() {
            ...
            public void dropActionChanged(java.awt.dnd.DropTargetDragEvent evt) {
              ...
              // Is this an acceptable drag event?
              if (isDragOk(
                  out, evt)) { // evt.acceptDrag( java.awt.dnd.DnDConstants.ACTION_COPY_OR_MOVE );
                evt.acceptDrag(java.awt.dnd.DnDConstants.ACTION_COPY);
                log(out, "FileDrop: event accepted.");
              }
              // end if: drag ok
  ...
  private boolean isDragOk(
    ...
    while (!ok && i < flavors.length) { // Is the flavor a file list?
      if (flavors[i].equals(java.awt.datatransfer.DataFlavor.javaFileListFlavor)) ok = true;
      i++;
