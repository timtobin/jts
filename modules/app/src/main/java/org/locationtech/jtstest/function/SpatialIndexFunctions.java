...
public class SpatialIndexFunctions {
	...
	private static void addGraphEdges(KdNode node, boolean isXLevel, int depth, double x, double xInc,
			List<Geometry> edges, GeometryFactory factory) {
		double xInc2 = xInc / 2;
		KdNode left = node.getLeft();
		if (left != null) {
			double xLeft = x - xInc2;
			Geometry edgeLeft = factory
					.createLineString(new Coordinate[]{new Coordinate(x, -depth), new Coordinate(xLeft, -depth - 1)});
			edges.add(edgeLeft);
		...
		if (right != null) {
			...
			Geometry edgeRight = factory
					.createLineString(new Coordinate[]{new Coordinate(x, -depth), new Coordinate(xRight, -depth - 1)});
			edges.add(edgeRight);
	...
	private static Geometry createSplitLine(Envelope extent, double splitVal, boolean isXLevel,
			GeometryFactory factory) {
	...
	/*
	 * public static Geometry sprTreeBounds(Geometry geom) { Coordinate[] pts =
	 * geom.getCoordinates(); VertexSequencePackedRtree index = new
	 * VertexSequencePackedRtree(pts); Envelope[] bounds = index.getBounds();
	 * Geometry[] polys = new Geometry[bounds.length]; int i = 0; for (Envelope env
	 * : bounds) { polys[i++] = geom.getFactory().toGeometry(env); } return
	 * geom.getFactory().createGeometryCollection(polys); }
	 */
