...
/**
 * A class that gives most of the functionality of DataInputStream, but is endian aware. Uses a real
 * java.io.DataInputStream to actually do the writing.
 */
public class EndianDataInputStream {
  ...
  /**
   * read a byte in BigEndian - the same as LE because its only 1 byte. returns int as per
   * java.io.DataStream
   */
  ...
  /**
   * read a byte in LittleEndian - the same as BE because its only 1 byte. returns int as per
   * java.io.DataStream
   */
