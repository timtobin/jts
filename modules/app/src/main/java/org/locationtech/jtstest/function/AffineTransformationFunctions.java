...
public class AffineTransformationFunctions {
  ...
  public static Geometry transformByBaseline(Geometry g, Geometry destBaseline) {
    ...
    AffineTransformation trans =
        AffineTransformationFactory.createFromBaseLines(src0, src1, dest0, dest1);
    return trans.transform(g);
  }
  ...
  private static AffineTransformation viewportTrans(
      Envelope srcEnv, Envelope viewEnv, boolean isIsotropic) {
    ...
    AffineTransformation trans =
        AffineTransformation.scaleInstance(scaleX, scaleY, centre.x, centre.y);
  ...
  public static Geometry scale(Geometry g, @Metadata(title = "Scale factor") double scale) {
    Coordinate centre = envelopeCentre(g);
    AffineTransformation trans =
        AffineTransformation.scaleInstance(scale, scale, centre.x, centre.y);
    return trans.transform(g);
  }

  public static Geometry scaleXY(
      Geometry g,
      @Metadata(title = "X scaling") double scaleX,
      @Metadata(title = "Y scaling") double scaleY) {
    Coordinate centre = envelopeCentre(g);
    AffineTransformation trans =
        AffineTransformation.scaleInstance(scaleX, scaleY, centre.x, centre.y);
    return trans.transform(g);
  ...
  public static Geometry rotateByPiMultiple(
      Geometry g, @Metadata(title = "Angle (multiple of Pi)") double multipleOfPi) {
    Coordinate centre = envelopeCentre(g);
    AffineTransformation trans =
        AffineTransformation.rotationInstance(multipleOfPi * Math.PI, centre.x, centre.y);
    return trans.transform(g);
  }

  @Metadata(description = "Rotate a geometry around a point by an multiple of Pi radians")
  public static Geometry rotateByPiMultipleAroundPoint(
      Geometry g, Geometry pt, @Metadata(title = "Angle (multiple of Pi)") double multipleOfPi) {
    Coordinate loc;
    if (pt == null) {
    ...
    AffineTransformation trans =
        AffineTransformation.rotationInstance(multipleOfPi * Math.PI, loc.x, loc.y);
    return trans.transform(g);
  }

  @Metadata(description = "Rotate a geometry by an angle in radians")
  public static Geometry rotate(Geometry g, @Metadata(title = "Angle (radians)") double angle) {
    Coordinate centre = envelopeCentre(g);
  ...
  public static Geometry rotateAroundPoint(
      Geometry g, Geometry pt, @Metadata(title = "Angle (radians)") double angle) {
    Coordinate loc;
    if (pt == null) {
      loc = new Coordinate(0, 0);
    } else {
  ...
  public static Geometry translate(
      Geometry g, @Metadata(title = "dX") double dx, @Metadata(title = "dY") double dy) {
