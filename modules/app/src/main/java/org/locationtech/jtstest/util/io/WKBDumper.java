...
public class WKBDumper {
	 ...
	 */
	private void read(InStream is) throws IOException, ParseException {
		dis.setInStream(is);
		readGeometry(0);
	}
	...
	private void readGeometry(int SRID) throws IOException, ParseException {
		...
		// or are in the 1000 range (Z) or in the 3000 range (ZM) of geometry type (OGC
		// 06-103r4)
		...
		// or are in the 1000 range (M) or in the 3000 range (ZM) of geometry type (OGC
		// 06-103r4)
	...
	private void readPoint() throws IOException, ParseException {
		...
		/*
		 * if (Double.isNaN(pts.getX(0)) || Double.isNaN(pts.getY(0))) { //return
		 * factory.createPoint(); }
		 */
	}
