...
class StretchedVertexFinder {
	...
	/**
	 * Creates a finder for a given set of linework, using the specified tolerance
	 * distance and within a given limiting envelope.
	 *
	 * @param linestrings
	 *            the linework to stretch away from
	 * @param tolerance
	 *            the distance tolerance for points to be stretched
	 * @param limitEnv
	 *            the envelope to limit searching to
	 */
	...
	/**
	 * Finds a single near vertex. This is simply the first one found, not
	 * necessarily the nearest. This choice may sub-optimal, resulting in odd result
	 * geometry. It's not clear that this can be done better, however. If there are
	 * several near points, the stretched geometry is likely to be distorted anyway.
	 *
	 * @param targetPt
	 * @param testLine
	 */
	private void findNearVertex(Coordinate targetPt, LineString testLine) {
		...
		for (int i = 0; i < n; i++) {
			...
			// is near segment?
				...
				/**
				 * Check whether pt is near or equal to other segment endpoint. If near, it will
				 * be handled by the near vertex case code. If equal, don't record it at all
				 */
	...
	/**
	 * Determines points that lie close to the test linestrings, and computes a
	 * {link StretchVertex} for each one.
	 *
	 * @param pts
	 *            the points to test for stretching
