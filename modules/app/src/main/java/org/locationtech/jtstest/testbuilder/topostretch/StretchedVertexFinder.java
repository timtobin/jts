...
class StretchedVertexFinder {
  public static List findNear(
      Collection linestrings, double tolerance, Envelope mask, Coordinate[] pts) {
  ...
  /**
   * Creates a finder for a given set of linework, using the specified tolerance distance and within
   * a given limiting envelope.
   *
  ...
  /**
   * Determines points that lie close to the test linestrings, and computes a {link StretchVertex}
   * for each one.
   *
  ...
  private static int geomPointsLen(Coordinate[] pts) {
    ...
    if (CoordinateArrays.isRing(pts)) n = pts.length - 1;
    return n;
  }
  ...
  private void findNearVertices(Coordinate[] pts) {
    ...
    for (int i = 0; i < n; i++) {
      if (limitEnv.intersects(pts[i])) findNearVertex(pts[i]);
    }
  }
  ...
  /**
   * Finds a single near vertex. This is simply the first one found, not necessarily the nearest.
   * This choice may sub-optimal, resulting in odd result geometry. It's not clear that this can be
   * done better, however. If there are several near points, the stretched geometry is likely to be
   * distorted anyway.
   *
   * @param targetPt
   * @param testLine
  ...
  private void findNearVertex(Coordinate targetPt, LineString testLine) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      // is near segment?
        ...
        /**
         * Check whether pt is near or equal to other segment endpoint. If near, it will be handled
         * by the near vertex case code. If equal, don't record it at all
         */
      ...
      if (stretchVert != null) nearVerts.add(stretchVert);
    }
  }
  ...
  private static boolean isPointNearButNotOnSeg(
    ...
    // on segment
    if (Orientation.index(p0, p1, p) == Orientation.COLLINEAR) return false;

    // compute actual distance
    distSeg.p0 = p0;
    distSeg.p1 = p1;
    double segDist = distSeg.distance(p);
    if (segDist > distTol) return false;
    return true;
  }
  ...
  private static double distanceToSeg(Coordinate p, Coordinate p0, Coordinate p1) {
    ...
    if (Orientation.index(p0, p1, p) == Orientation.COLLINEAR) segDist = 0.0;
