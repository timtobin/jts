...
public class CoverageFunctions {
	...
	public static Geometry clean(Geometry coverage, @Metadata(title = "Snap Distance") double snapDistance,
			@Metadata(title = "Max Gap Width") double maxGapWidth) {
	...
	public static Geometry cleanGapWidth(Geometry coverage, @Metadata(title = "Max Gap Width") double maxGapWidth) {
		Geometry[] cov = toGeometryArray(coverage);
		Geometry[] result = CoverageCleaner.cleanGapWidth(cov, maxGapWidth);
		return coverage.getFactory().createGeometryCollection(result);
	}

	public static Geometry cleanMergeMaxArea(Geometry coverage, @Metadata(title = "Max Gap Width") double maxGapWidth) {
		Geometry[] cov = toGeometryArray(coverage);
		Geometry[] result = CoverageCleaner.cleanOverlapGap(cov, CoverageCleaner.MERGE_MAX_AREA, maxGapWidth);
		return coverage.getFactory().createGeometryCollection(result);
	}

	public static Geometry cleanMergeMinArea(Geometry coverage, @Metadata(title = "Max Gap Width") double maxGapWidth) {
		Geometry[] cov = toGeometryArray(coverage);
		Geometry[] result = CoverageCleaner.cleanOverlapGap(cov, CoverageCleaner.MERGE_MIN_AREA, maxGapWidth);
		return coverage.getFactory().createGeometryCollection(result);
	}

	public static Geometry cleanSnap(Geometry coverage, @Metadata(title = "Snap Distance") double snapDistance) {
		Geometry[] cov = toGeometryArray(coverage);
		Geometry[] result = CoverageCleaner.clean(cov, snapDistance, 0);
		return coverage.getFactory().createGeometryCollection(result);
	}

	public static Geometry cleanedGaps(Geometry coverage, @Metadata(title = "Max Gap Width") double maxGapWidth) {
		Geometry[] cov = toGeometryArray(coverage);
		CoverageCleaner cc = new CoverageCleaner(cov);
		cc.setGapMaximumWidth(maxGapWidth);
		cc.clean();
		Geometry[] gaps = GeometryFactory.toGeometryArray(cc.getMergedGaps());
		return coverage.getFactory().createGeometryCollection(gaps);
	}

	public static Geometry cleanedGapsSnap(Geometry coverage, @Metadata(title = "Snap Distance") double snapDistance,
			@Metadata(title = "Max Gap Width") double maxGapWidth) {
		Geometry[] cov = toGeometryArray(coverage);
		CoverageCleaner cc = new CoverageCleaner(cov);
	...
	public static Geometry cleanedOverlaps(Geometry coverage) {
		...
		CoverageCleaner cc = new CoverageCleaner(cov);
		cc.clean();
		Geometry[] overlaps = GeometryFactory.toGeometryArray(cc.getOverlaps());
	...
	public static Geometry findGaps(Geometry geom, @Metadata(title = "Max Gap Width") double maxGapWidth) {
	...
	public static Geometry simplifyInOut(Geometry coverage,
			@Metadata(title = "Inner Distance tol") double toleranceInner,
			@Metadata(title = "Outer Distance tol") double toleranceOuter) {
		Geometry[] cov = toGeometryArray(coverage);
	...
	public static Geometry simplifyRemoveRings(Geometry coverage, @Metadata(title = "Distance tol") double tolerance,
			@Metadata(title = "Removal Size Factor") double factor) {
		Geometry[] cov = toGeometryArray(coverage);
		CoverageSimplifier simplifier = new CoverageSimplifier(cov);
	...
	public static Geometry simplifySharp(Geometry coverage, @Metadata(title = "Distance tol") double tolerance,
			@Metadata(title = "Weight") double weight) {
		Geometry[] cov = toGeometryArray(coverage);
		CoverageSimplifier simplifier = new CoverageSimplifier(cov);
	...
	public static Geometry simplifyTolerances(Geometry coverage,
			@Metadata(title = "Tolerances (comma-sep)") String tolerancesCSV) {
		Geometry[] cov = toGeometryArray(coverage);
	...
	public static Geometry validatePolygonWithGaps(Geometry geom, Geometry adjacentPolys,
			@Metadata(title = "Gap width") double gapWidth) {
		return CoveragePolygonValidator.validate(geom, toGeometryArray(adjacentPolys), gapWidth);
	}

	public static Geometry validateWithGaps(Geometry geom, @Metadata(title = "Gap width") double gapWidth) {
