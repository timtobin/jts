...
/**
 * A {@link ResultMatcher} which compares the results of buffer operations for equality, up to the
 * given tolerance. All other operations are delagated to the standard {@link EqualityResultMatcher}
 * algorithm.
 *
 * @author mbdavis
 */
public class BufferResultMatcher implements ResultMatcher {
  ...
  /**
   * The minimum distance tolerance which will be used. This is required because densified vertices
   * do no lie precisely on their parent segment.
   */
  private static final double MIN_DISTANCE_TOLERANCE = 1.0e-8;

  public boolean isBufferResultMatch(
      Geometry actualBuffer, Geometry expectedBuffer, double distance) {
    if (actualBuffer.isEmpty() && expectedBuffer.isEmpty()) return true;

    /**
     * MD - need some more checks here - symDiffArea won't catch very small holes ("tears") near the
     * edge of computed buffers (which can happen in current version of JTS (1.8)). This can
     * probably be handled by testing that every point of the actual buffer is at least a certain
     * distance away from the geometry boundary.
     */
    if (!isSymDiffAreaInTolerance(actualBuffer, expectedBuffer)) return false;
  ...
  public boolean isBoundaryHausdorffDistanceInTolerance(
    ...
    if (expectedDistanceTol < MIN_DISTANCE_TOLERANCE) expectedDistanceTol = MIN_DISTANCE_TOLERANCE;
