...
public class PreparedGeometryTeeOperation extends TeeGeometryOperation {
  ...
  protected void runTeeOp(String opName, Geometry geometry, Object[] args) {
    ...
    if (!geometry.isValid()) throw new IllegalStateException("Input geometry A is not valid");
    if (!g2.isValid()) throw new IllegalStateException("Input geometry B is not valid");
  ...
  private void checkContainsProperly(PreparedGeometry pg, Geometry g2) {
    ...
    if (pgResult != expected)
      throw new IllegalStateException(
          "PreparedGeometry.containsProperly result does not match expected");
