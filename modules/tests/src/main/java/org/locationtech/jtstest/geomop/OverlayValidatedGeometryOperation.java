...
/**
 * A {@link GeometryOperation} which validates the result of overlay operations. If an invalid
 * result is found, an exception is thrown (this is the most convenient and noticeable way of
 * flagging the problem when using the TestRunner). All other Geometry methods are executed
 * normally.
 *
 * <p>In order to eliminate the need to specify the precise result of an overlay, this class forces
 * the final return value to be <tt>GEOMETRYCOLLECTION EMPTY</tt>.
 *
...
public class OverlayValidatedGeometryOperation implements GeometryOperation {
  ...
  public Result invokeValidatedOverlayOp(int opCode, Geometry g0, Object[] args) throws Exception {
    ...
    /**
     * Return an empty GeometryCollection as the result. This allows the test case to avoid
     * specifying an exact result
     */
    if (returnEmptyGC) {
  ...
  private void validate(int opCode, Geometry g0, Geometry g1, Geometry result) {
    ...
    if (!validator.isValid(opCode)) {
      ...
      String msg =
          "Operation result is invalid [OverlayResultValidator] ( "
              + WKTWriter.toPoint(invalidLoc)
              + " )";
      reportError(msg);
    }
  }
