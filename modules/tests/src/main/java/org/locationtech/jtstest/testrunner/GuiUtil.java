...
public class GuiUtil {
	...
	public static void center(Component componentToMove, Component componentToCenterOn) {
		...
		componentToMove.setLocation(
				...
				componentToCenterOn.getY() + ((componentToCenterOnSize.height - componentToMove.getHeight()) / 2));
	}
	...
	/**
	 * Changes the tooltip text of each component in the Container to be multiline
	 * HTML. Modifies all descendants (children, grandchildren, etc.).
	 */
	...
	/**
	 * Runs r in the event dispatch thread, which may be the current thread. Waits
	 * for r to finish before returning.
	 */
