...
public class Parameters {
	...
	/**
	 * Sets the command-line arguments. reqdKeys may be set to null if there are no
	 * required command-line arguments. Same with optionalKeys.
	 */
	...
	private Parameters() {
		...
		for (Object key : requiredKeys) {
			String requiredKey = (String) key;
			if (!hashtable.containsKey(requiredKey))
				throw new IllegalArgumentException("Required command-line argument is missing: " + requiredKey);
		}
	}
	...
	/**
	 * Returns the value of the specified parameter, or null if there is no such
