...
/**
 * Parameters passed to a main method (also known as "command-line arguments"). Arguments are
 * recognized only if they are of the form -key or -key:value
 *
 * @version 1.7
 */
public class Parameters {
  ...
  /**
   * Sets the command-line arguments. reqdKeys may be set to null if there are no required
   * command-line arguments. Same with optionalKeys.
   */
  public static void setParameters(String[] args, String[] reqdKeys, String[] optionalKeys) {
    arguments = Arrays.asList(args);
    if (reqdKeys != null) requiredKeys = StringUtil.toLowerCase(Arrays.asList(reqdKeys));
    if (reqdKeys != null) allowedKeys.addAll(StringUtil.toLowerCase(Arrays.asList(reqdKeys)));
