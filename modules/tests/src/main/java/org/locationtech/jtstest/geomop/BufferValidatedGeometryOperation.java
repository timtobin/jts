 ...
 * {@link Geometry} <tt>buffer()</tt> method. If an invalid result is found, an
 * exception is thrown (this is the most convenient and noticeable way of
 * flagging the problem when using the TestRunner). All other Geometry methods
...
public class BufferValidatedGeometryOperation implements GeometryOperation {
	...
	private void validate(Geometry geom, Geometry buffer) {
		...
		// need special check for negative buffers which disappear. Somehow need to find
		// maximum inner
		// circle - via skeleton?
	}
}
