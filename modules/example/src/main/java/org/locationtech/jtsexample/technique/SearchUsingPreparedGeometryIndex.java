...
/**
 * Demonstrates use of {@link PreparedGeometry}s in a spatial index to optimize spatial search.
 *
 * <p>The example creates a grid of circular polygons, packed into the 1 x 1 square. This set of
 * polygons is spatially indexed as PreparedGeometrys in an STRtree index. A series of random points
 * in the square is generated, and the index is used to determine whether each point intersects any
 * circles. The fraction of points which intersect will approximate the fraction of area covered by
 * the circles.
 *
 * @version 1.12
 */
...
/**
 * A spatial index which indexes {@link PreparedGeometry}s created from a set of {@link Geometry}s.
 * This can be used for efficient testing for intersection with a series of target geomtries.
 *
 * @author Martin Davis
 */
